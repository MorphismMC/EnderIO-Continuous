...
public class EnderIOEndergy implements IEnderIOAddon {
    ...
    public static void onPlayerDeath(PlayerDropsEvent evt) {
        ...
        Log.warn("PlayerDropsEvent: gamerule keepInventory is " +
                evt.getEntityPlayer().world.getGameRules().getBoolean(KEEP_INVENTORY));
        for (String rule : evt.getEntityPlayer().world.getGameRules().getRules()) {
            Log.warn("PlayerDropsEvent: gamerule '" + rule + "' is " +
                    evt.getEntityPlayer().world.getGameRules().getBoolean(rule) + "/" +
                    evt.getEntityPlayer().world.getGameRules().getInt(rule) + "/" +
                    evt.getEntityPlayer().world.getGameRules().getString(rule));
        }
    }
}
