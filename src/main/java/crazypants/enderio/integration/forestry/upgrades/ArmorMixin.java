...
public abstract class ArmorMixin extends Item implements IArmorApiarist, IArmorNaturalist, IDarkSteelItem {
    ...
    public boolean canSeePollination(@Nonnull EntityPlayer player, @Nonnull ItemStack armor, boolean doSee) {
        return isForSlot(EntityEquipmentSlot.HEAD) &&
                NaturalistEyeUpgrade.INSTANCE.hasUpgrade(player.getItemStackFromSlot(EntityEquipmentSlot.HEAD));
    }

    @Override
    public boolean protectEntity(@Nonnull EntityLivingBase entity, @Nonnull ItemStack armor, @Nullable String cause,
                                 boolean doProtect) {
        return ApiaristArmorUpgrade.HELMET.hasUpgrade(armor) || ApiaristArmorUpgrade.CHEST.hasUpgrade(armor) ||
                ApiaristArmorUpgrade.BOOTS.hasUpgrade(armor) || ApiaristArmorUpgrade.LEGS.hasUpgrade(armor);
    }
}
