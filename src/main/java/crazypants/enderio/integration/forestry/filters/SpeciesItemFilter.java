...
public class SpeciesItemFilter implements IInventory, IItemFilter.WithGhostSlots {
                ...
                switch (speciesMode) {
                    case BOTH:
                        if (primarySpeciesUids[slot].equals(primarySpeciesUid) &&
                                secondarySpeciesUids[slot].equals(secondarySpeciesUid)) {
                            return true;
                        }
                        break;
                    case PRIMARY:
    ...
    @Override
    public void createGhostSlots(@Nonnull NNList<GhostSlot> slots, final int xOffset, final int yOffset,
                                 @Nullable Runnable cb) {
        int index = 0;
        int numRows = 2;
    ...
    public boolean isDefault() {
        return !isValid() && isBlacklist == DEFAULT_BLACKLIST && speciesMode == DEFAULT_SPECIES_MODE &&
