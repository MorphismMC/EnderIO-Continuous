...
public class TicRegistration {
    ...
    private static void registerAlloyRecipe(Pair<Things, NNList<Things>> alloy) {
        ...
        for (int i = 0; i < input.size(); i++) {
            if ((fluids[i] = getFluidForItems(
                    NullHelper.notnull(input.get(i), "missing input item stack in alloy recipe"))) == null ||
        ...
        try {
            ...
            throw new RuntimeException("Error while registering alloy recipe '" + debug + " => " +
                    toString(fluidResult) + "' with Tinkers", e);
        }
    ...
    private static void tryBasinAloying(@Nonnull Things result, NNList<Things> inputs) {
        if (!result.isValid() || result.getItemStack().getCount() != 1 ||
                !(result.getItemStack().getItem() instanceof ItemBlock) || inputs.size() != 2) {
            return;
        }
        ...
        if (a == null &&
                !(input0.getItemStack().getCount() == 1 && input0.getItemStack().getItem() instanceof ItemBlock)) {
            return;
        }
        if (b == null &&
                !(input1.getItemStack().getCount() == 1 && input1.getItemStack().getItem() instanceof ItemBlock)) {
            return;
        }
    ...
    public static void registerBasinCasting() {
        for (BasinQueue basin : TiCQueues.getBasinQueue()) {
            ...
            if (basin.getFluid() == null) {
                Log.warn("Item used in basin cast recipe '" + toString(basin.getFluidItem()) +
                        "' doesn't smelt into a fluid");
            }
            ...
            if (!basin.getCast().isEmpty()) {
                for (NNIterator<ItemStack> itr = basin.getCast().getItemStacks().fastIterator(); itr.hasNext();) {
                    ...
                    try {
                        TinkerRegistry.registerBasinCasting(basin.getOutput().getItemStack(), castStack,
                                basin.getFluid(), (int) Math.ceil(basin.getAmount()));
                    } catch (TinkerAPIException e) {
                        throw new RuntimeException("Error while registering basin casting recipe '" +
                                toString(basin.getOutput()) + ", " + toString(castStack) + ", " +
                                basin.getFluid().getName() + ", " + basin.getAmount() + "' with Tinkers", e);
                    }
                    Log.debug("Tinkers.registerBasinCasting: " + toString(basin.getOutput()) + ", " +
                            toString(castStack) + ", " + basin.getFluid().getName() + ", " + basin.getAmount());
                }
            } else {
                try {
                    TinkerRegistry.registerBasinCasting(basin.getOutput().getItemStack(), Prep.getEmpty(),
                            basin.getFluid(), (int) Math.ceil(basin.getAmount()));
                } catch (TinkerAPIException e) {
                    throw new RuntimeException("Error while registering basin casting recipe '" +
                            toString(basin.getOutput()) + ", (empty), " + basin.getFluid().getName() + ", " +
                            basin.getAmount() + "' with Tinkers", e);
                }
                Log.debug("Tinkers.registerBasinCasting: " + toString(basin.getOutput()) + ", (empty), " +
                        basin.getFluid().getName() + ", " + basin.getAmount());
            }
        }
            ...
            if (cast.getFluid() == null) {
                ...
                if (!cast.getCast().isEmpty()) {
                    for (NNIterator<ItemStack> itr = cast.getCast().getItemStacks().fastIterator(); itr.hasNext();) {
                        ItemStack castStack = itr.next();
                        try {
                            TinkerRegistry.registerTableCasting(new CastingRecipe(cast.getResult().getItemStack(),
                                    RecipeMatch.ofNBT(castStack), cast.getFluid(),
                                    (int) Math.ceil(cast.getAmount()), cast.isConsumeCast(), false));
                        } catch (TinkerAPIException e) {
                            throw new RuntimeException("Error while registering table casting recipe '" +
                                    toString(cast.getResult()) + ", " + toString(castStack) + ", " +
                                    cast.getFluid().getName() + ", " + cast.getAmount() + "' with Tinkers", e);
                        }
                        Log.debug("Tinkers.registerTableCasting: " + toString(cast.getResult()) + ", " +
                                toString(castStack) + ", " + cast.getFluid().getName() + ", " + cast.getAmount());
                    }
                } else {
                    try {
                        TinkerRegistry.registerTableCasting(
                                new CastingRecipe(cast.getResult().getItemStack(), null, cast.getFluid(),
                                        (int) Math.ceil(cast.getAmount()), cast.isConsumeCast(), false));
                    } catch (TinkerAPIException e) {
                        throw new RuntimeException("Error while registering table casting recipe '" +
                                toString(cast.getResult()) + ", (no cast), " + cast.getFluid().getName() + ", " +
                                cast.getAmount() + "' with Tinkers", e);
                    }
    ...
    public static void registerSmeltings() {
        for (SmeltQueue smelt : TiCQueues.getSmeltQueue()) { // 1st because it may provide fluids for later
            if (smelt.getFluidOutput() == null) {
                ...
                if (fluid == null) {
                    Log.warn("Item used in Smeltery recipe '" + toString(smelt.getOutput()) +
                            "' doesn't smelt into a fluid");
                } else {
            ...
            if (smelt.getFluidOutput() != null) {
                for (ItemStack in : smelt.getInput().getItemStacks()) {
                    try {
                        TinkerRegistry.registerMelting(in, smelt.getFluidOutput(),
                                (int) Math.max(1, Math.floor(smelt.getAmount())));
                    } catch (TinkerAPIException e) {
                        throw new RuntimeException(
                                "Error while registering melting recipe '" + toString(in) + ", " +
                                        smelt.getFluidOutput().getName() + ", " + smelt.getAmount() + "' with Tinkers",
                                e);
                    }
                    Log.debug("Tinkers.registerMelting: " + toString(in) + ", " + smelt.getFluidOutput().getName() +
                            ", " + smelt.getAmount());
                }
            }
        }
    ...
    private static FluidStack getFluidForItems(@Nonnull ItemStack input) {
        ...
        if (result != null) {
            ...
            Log.info("Failed to get Tinker's Construct melting recipe result for " + toString(itemStack) + " -> " +
                    toString(result));
            return null;
        }
    }
