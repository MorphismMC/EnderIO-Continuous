...
public class BlockConduitBundle extends BlockEio<TileConduitBundle>
                                implements IEioGuiHandler.WithPos, IPaintable.IBlockPaintableBlock,
                                IPaintable.IWrenchHideablePaint, IYetaAwareBlock {
    ...
    public IBlockState getExtendedState(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
                                        @Nonnull BlockPos pos) {
        IBlockStateWrapper blockStateWrapper = new BlockStateWrapperConduitBundle(state, world, pos,
    ...
    public boolean addHitEffects(@Nonnull IBlockState state, @Nonnull World world, @Nonnull RayTraceResult target,
        ...
        if (tex == null) {
            tex = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
                    .getTexture(ModObject.block_machine_base.getBlockNN().getDefaultState());
        }
        lastHitIcon = tex;
        addBlockHitEffects(world, effectRenderer, target.hitVec.x, target.hitVec.y, target.hitVec.z, target.sideHit,
                tex);
        return true;
    }

    @Override
    public boolean addLandingEffects(@Nonnull IBlockState state, @Nonnull net.minecraft.world.WorldServer world,
                                     @Nonnull BlockPos bp,
                                     @Nonnull IBlockState iblockstate, @Nonnull EntityLivingBase entity,
                                     int numberOfParticles) {
        // TODO: Should probably register a dummy state for this, but this is an easy fix for the blockTank that allows
        // testing to begin
        ...
        world.spawnParticle(EnumParticleTypes.BLOCK_DUST, bp.getX() + 0.5, bp.getY() + 1, bp.getZ() + 0.5,
                numberOfParticles, 0.0D, 0.0D, 0.0D,
        ...
        for (int j = 0; j < i; ++j) {
            for (int k = 0; k < i; ++k) {
                for (int l = 0; l < i; ++l) {
                    ...
                    ParticleDigging fx = (ParticleDigging) new ParticleDigging.Factory().createParticle(-1, world, d0,
                            d1, d2, d0 - pos.getX() - 0.5D,
    ...
    @SideOnly(Side.CLIENT)
    private void addBlockHitEffects(@Nonnull World world, @Nonnull ParticleManager effectRenderer, double xCoord,
        ...
        ParticleDigging digFX = (ParticleDigging) Minecraft.getMinecraft().effectRenderer.spawnEffectParticle(
                EnumParticleTypes.BLOCK_CRACK.getParticleID(), d0, d1,
                d2, 0, 0, 0, 0);
        if (digFX != null) {
            ...
            if (bundle != null) {
                ...
                if (conduit != null) {
                    ...
                    ret = new ItemStack(ModObject.itemConduitFacade.getItemNN(), 1,
                            EnumFacadeType.getMetaFromType(bundle.getFacadeType()));
    ...
    public void getExtraDrops(@Nonnull NonNullList<ItemStack> drops, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
        ...
        if (te.hasFacade()) {
            ItemStack stack = new ItemStack(ModObject.itemConduitFacade.getItemNN(), 1,
                    EnumFacadeType.getMetaFromType(te.getFacadeType()));
    ...
    @SideOnly(Side.CLIENT)
    public int getMixedBrightnessForFacade(@Nonnull IBlockState bs, @Nonnull IBlockAccess worldIn,
                                           @Nonnull BlockPos pos, @Nonnull IBlockState facade) {
        if (facade.getBlock() instanceof BlockSlab) {
            // TODO: this is wrong for glass and glowing slabs, isn't it? find a way to do them right---once we got some
            // from somewhere
            ...
            return getNeightbourBrightness(worldIn, pos,
                    facade.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP ? EnumFacing.UP : EnumFacing.DOWN);
    ...
    public boolean removedByPlayer(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        if (YetaUtil.isSolidFacadeRendered(te, player)) {
            breakBlock = false;
            ItemStack fac = new ItemStack(ModObject.itemConduitFacade.getItemNN(), 1,
                    EnumFacadeType.getMetaFromType(te.getFacadeType()));
    ...
    public void onBlockClicked(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player) {
        ...
        if (!world.isRemote || !player.isSneaking() || equipped.isEmpty() ||
                !ToolUtil.isToolEquipped(player, EnumHand.MAIN_HAND)) {
            return;
        }
    ...
    private boolean handleWrenchClick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player,
        ...
        if (tool != null) {
            if (tool.canUse(hand, player, pos)) {
                if (!world.isRemote) {
                    ...
                    if (!PermissionAPI.hasPermission(player.getGameProfile(), permissionNodeWrenching,
                            new BlockPosContext(player, pos, bs, null))) {
    ...
    public AxisAlignedBB getSelectedBoundingBox(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos) {
        ...
        return new AxisAlignedBB(pos.getX() + minBB.minX, pos.getY() + minBB.minY, pos.getZ() + minBB.minZ,
                pos.getX() + minBB.maxX, pos.getY() + minBB.maxY,
                pos.getZ() + minBB.maxZ);
    }
    ...
    public @Nonnull List<RaytraceResult> doRayTraceAll(@Nonnull World world, @Nonnull BlockPos pos,
    ...
    private @Nullable RaytraceResult doRayTrace(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull Vec3d origin,
                                                @Nonnull Vec3d direction,
                                                EntityPlayer entityPlayer) {
        List<RaytraceResult> allHits = doRayTraceAll(world.getBlockState(pos), world, pos, origin, direction,
                entityPlayer);
        return RaytraceResult.getClosestHit(origin, allHits);
    }

    protected @Nonnull NNList<RaytraceResult> doRayTraceAll(@Nonnull IBlockState bs, @Nonnull World world,
        ...
        if (YetaUtil.isSolidFacadeRendered(bundle, player)) {
            ...
            if (hitPos != null) {
                hits.add(new RaytraceResult(new CollidableComponent(null, BoundingBox.UNIT_CUBE, hitPos.sideHit, null),
                        hitPos));
            }
        } else {
            ...
            if (bundle.getConduits().isEmpty() && !YetaUtil.isFacadeHidden(bundle, player)) {
                ...
                if (hitPos != null) {
                    hits.add(new RaytraceResult(
                            new CollidableComponent(null, BoundingBox.UNIT_CUBE, hitPos.sideHit, null), hitPos));
                }
            }
        }
