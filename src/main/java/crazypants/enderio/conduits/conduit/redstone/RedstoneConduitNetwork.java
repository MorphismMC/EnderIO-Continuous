...
public class RedstoneConduitNetwork extends AbstractConduitNetwork<IRedstoneConduit, IRedstoneConduit> {
    ...
    public void addConduit(@Nonnull IRedstoneConduit con) {
        ...
        baseId += 6;
        updateInputsFromConduit(con, true); // all call paths to here come from updateNetwork() which already notifies
                                            // all neighbors
    }
            ...
            for (EnumFacing dir : con.getExternalConnections()) {
                ...
                if (world.isBlockLoaded(bc2)) {
                    ...
                    if (bs.isBlockNormalCube() &&
                            !neighborNotifyEvent(world, bc2, bs, EnumSet.allOf(EnumFacing.class))) {
    ...
    private boolean neighborNotifyEvent(World world, @Nonnull BlockPos pos, @Nullable IBlockState state,
                                        EnumSet<EnumFacing> dirs) {
        return ForgeEventFactory
                .onNeighborNotify(world, pos, state == null ? world.getBlockState(pos) : state, dirs, false)
                .isCanceled();
    }
