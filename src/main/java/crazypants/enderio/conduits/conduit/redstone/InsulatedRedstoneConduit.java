...
public class InsulatedRedstoneConduit extends AbstractConduit
    ...
    private final NNSidedObject<NNPair<ItemStack, IInputSignalFilter>> inputFilters = new NNSidedObject<NNPair<ItemStack, IInputSignalFilter>>(
            NNPair.of(Prep.getEmpty(), DefaultInputSignalFilter.instance));
    private final NNSidedObject<NNPair<ItemStack, IOutputSignalFilter>> outputFilters = new NNSidedObject<NNPair<ItemStack, IOutputSignalFilter>>(
            NNPair.of(Prep.getEmpty(), DefaultOutputSignalFilter.instance));

    private Map<EnumFacing, ConnectionMode> forcedConnections = new EnumMap<EnumFacing, ConnectionMode>(
            EnumFacing.class);
    ...
    public boolean canConnectToExternal(@Nonnull EnumFacing direction, boolean ignoreConnectionState) {
        ...
        if (forcedConnection == ConnectionMode.DISABLED) {
            return false;
        } else if (forcedConnection == ConnectionMode.IN_OUT || forcedConnection == ConnectionMode.OUTPUT ||
                forcedConnection == ConnectionMode.INPUT) {
                    return true;
                }
    ...
    public void setFilterStack(int filterIndex, EnumFacing side, @Nonnull ItemStack stack) {
        if (filterIndex == getOutputFilterIndex()) {
            inputFilters.set(side, NNPair.of(stack, NullHelper.first((IInputSignalFilter) getFilterForUpgrade(stack),
                    DefaultInputSignalFilter.instance)));
        } else if (filterIndex == getInputFilterIndex()) {
            outputFilters.set(side, NNPair.of(stack, NullHelper.first((IOutputSignalFilter) getFilterForUpgrade(stack),
                    DefaultOutputSignalFilter.instance)));
        }
        onFilterChanged();
    }
    ...
    public NNList<ITextComponent> getConduitProbeInformation(@Nonnull EntityPlayer player) {
        ...
        if (getExternalConnections().isEmpty()) {
            ...
            for (EnumFacing dir : getExternalConnections()) {
                ...
                ITextComponent elem = Lang.GUI_CONDUIT_PROBE_REDSTONE_HEADING
