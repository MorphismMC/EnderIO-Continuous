...
public abstract class AbstractConduitItem extends Item implements ConduitItem, IHaveRenderers {
    ...
    public EnumActionResult onItemUse(@Nonnull EntityPlayer player, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        BlockPos placeAt = canPlaceItem(held, ConduitRegistry.getConduitModObjectNN().getBlockNN().getDefaultState(),
                player, world, pos, side);
        if (placeAt != null) {
            if (!world.isRemote) {
                    ...
                    if (bundle != null) {
                        if (bundle.addConduit(createConduit(held, player))) {
                            ...
                            player.sendStatusMessage(
                                    new TextComponentTranslation(Lang.GUI_CONDUIT_BUNDLE_FULL.getKey()), true);
                        }
                    }
                }
            }
    ...
    private BlockPos canPlaceItem(@Nonnull ItemStack held, IBlockState blockToPlace, @Nonnull EntityPlayer player,
