...
public class LiquidConduitImpl extends AbstractTankConduit {
    ...
    public int fill(EnumFacing from, FluidStack resource, boolean doFill, boolean doPush, int pushToken) {
        if (network == null) {
            Log.error(
                    "The network for this conduit was null when asked to fill. Please report this to the Ender IO github");
            return 0;
        }
        ...
        do {
            if (dir != from && canOutputToDir(dir) && !autoExtractForDir(dir)) {
                if (getConduitConnections().contains(dir)) {
                    LiquidConduit conduitCon = getFluidConduit(dir);
                    if (conduitCon != null) {
                        int toCon = ((LiquidConduitImpl) conduitCon).pushLiquid(dir.getOpposite(), toPush, doPush, token);
                        toPush.amount -= toCon;
                        pushed += toCon;
                    }
    ...
    private LiquidConduit getFluidConduit(@Nonnull EnumFacing dir) {
        TileEntity ent = getBundle().getTileEntity();
        return ConduitUtil.getConduit(ent.getWorld(), ent, dir, LiquidConduit.class);
    }
    ...
    public boolean canConnectToConduit(@Nonnull EnumFacing direction, @Nonnull Conduit con) {
        ...
        if (!(con instanceof LiquidConduitImpl)) {
            return false;
        }
        if (getFluidType() != null && ((LiquidConduitImpl) con).getFluidType() == null) {
            return false;
        }
        return LiquidConduitNetwork.areFluidsCompatable(getFluidType(), ((LiquidConduitImpl) con).getFluidType());
    }
    ...
    protected boolean canJoinNeighbour(LiquidConduit n) {
        return n instanceof LiquidConduitImpl;
    }
    ...
    protected class ConnectionLiquidConduitSide extends ConnectionLiquidSide {
        ...
        public int fill(FluidStack resource, boolean doFill) {
            if (!reenter && network != null && canFill(side, resource) && network.lockNetworkForFill()) {
                try {
                    reenter = true;
                    int res = LiquidConduitImpl.this.fill(side, resource, doFill, true, network.getNextPushToken());
    ...
    public Class<? extends Conduit> getCollidableType() {
        return LiquidConduitImpl.class;
    }
}
