...
public class LiquidConduitRenderer extends DefaultConduitRenderer implements IResourceManagerReloadListener {
    ...
    protected void renderTransmissionDynamic(@Nonnull Conduit conduit, @Nonnull ConduitTexture tex,
        ...
        if (component.isDirectional()) {
            ...
            // TODO: HL: I commented this out because component.getDirection() (the second to last parameter) is always
            // null in
            // this else branch and drawDynamicSection() with isTransmission=true (last parameter) would NPE on it. (Not
            // a
            // mistake in the component.dir encapsulation, this was that way before.)
            // drawDynamicSection(component.bound, tex.getMinU(), tex.getMaxU(), tex.getMinV(), tex.getMaxV(), color,
            // component.getDir(), true);
        }
    }
    ...
    public static void renderFluidOutline(@Nonnull CollidableComponent component, @Nonnull FluidStack fluidStack,
        ...
        if (fluid != null) {
            for (CachableRenderStatement elem : computeFluidOutlineToCache(component, fluid, scaleFactor,
                    outlineWidth)) {
                elem.execute();
            }
        }
    ...
    public static List<CachableRenderStatement> computeFluidOutlineToCache(@Nonnull CollidableComponent component,
                                                                           @Nonnull Fluid fluid, double scaleFactor,
                                                                           float outlineWidth) {
        ...
        for (NNIterator<EnumFacing> itr = NNList.FACING.fastIterator(); itr.hasNext();) {
            ...
            if (face != componentDirection && face != componentDirection.getOpposite()) {
                List<Vertex> corners = bbb.getCornersWithUvForFace(face, texture.getMinU(), texture.getMaxU(),
                        texture.getMinV(), texture.getMaxV());
                for (Vertex corner : corners) {
                    data.add(new CachableRenderStatement.AddVertexWithUV(corner.x(), corner.y(), corner.z(),
                            corner.uv.x, corner.uv.y, colorv));
                }
            }
        }
        return data;
