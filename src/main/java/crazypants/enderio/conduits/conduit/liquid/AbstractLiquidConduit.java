...
public abstract class AbstractLiquidConduit extends AbstractConduit implements LiquidConduit {
    ...
    protected final EnumMap<EnumFacing, RedstoneControlMode> extractionModes = new EnumMap<EnumFacing, RedstoneControlMode>(
            EnumFacing.class);
    protected final EnumMap<EnumFacing, DyeColor> extractionColors = new EnumMap<EnumFacing, DyeColor>(
            EnumFacing.class);
    ...
    /**
     * Checks if the given direction is configured for input ({@link #canExtractFromDir(EnumFacing)} and its redstone
     * control allows it to operate.
     * <p>
    ...
    protected boolean autoExtractForDir(@Nonnull EnumFacing dir) {
        return canExtractFromDir(dir) && ConduitUtil.isRedstoneControlModeMet(this, getExtractionRedstoneMode(dir),
                getExtractionSignalColor(dir), dir);
    }
    ...
    /**
     * Checks if this conduit has any external connection it can pull stuff from. This does not check for redstone
     * control, only for existence of a connection and
     * the mode that is set.
     * 
     * @return <code>true</code> if there are any external connections with {@link ConnectionMode#acceptsInput()}
     *         <code>== true</code>
     */
    ...
    /**
     * While locking on the level of a conduit is not perfect, it will prevent loops from being infinite. There still
     * are some ways to build loops that loop for a
     * long time (the more connection points between 2 networks...), but a slow down is much better than a stack
     * overflow...
     */
