...
public class EnderLiquidConduitNetwork extends AbstractConduitNetwork<LiquidConduit, EnderLiquidConduit> {
    ...
    private int tryTransfer(@Nonnull EnderLiquidConduit con, @Nonnull EnumFacing conDir, @Nonnull NetworkTank from,
                            FluidStack toDrain, int limit) {
        ...
        if (amountFilled > drained.amount) {
            Log.warn(
                    "EnderLiquidConduit at " + con.getBundle().getLocation() + " in dimension " +
                            con.getBundle().getBundleworld().provider.getDimensionType().getId() +
                            ": Inserted fluid volume (" + amountFilled + "mB) is more than we tried to insert (" +
                            drained.amount + "mB).");
        } else if (amountFilled < drained.amount) {
            Log.warn(
                    "EnderLiquidConduit at " + con.getBundle().getLocation() + " in dimension " +
                            con.getBundle().getBundleworld().provider.getDimensionType().getId() +
                            ": Inserted fluid volume (" + amountFilled +
            ...
            if (putBack < toPutBack.amount) {
                Log.warn("EnderLiquidConduit at " + con.getBundle().getLocation() + " in dimension " +
                        con.getBundle().getBundleworld().provider.getDimensionType().getId() +
                        ": In addition, putting back " + toPutBack.amount +
                        "mB into the source tank failed, leading to " + (toPutBack.amount - putBack) +
                        "mB being voided.");
            }
        }
        return amountFilled;
    ...
    public int fillFrom(@Nonnull EnderLiquidConduit con, @Nonnull EnumFacing conDir, FluidStack resource,
                        boolean doFill) {
    ...
    public int fillFrom(@Nonnull NetworkTank tank, FluidStack resource, boolean doFill) {
        ...
        /**
         * Some targets change dependent on other targets (e.g. 2 conduit connections to the same tank). Simulation will
         * differ from execution in this case. To
         * prevent simulation from reporting higher amounts that execution can really move, we stop simulation after the
         * first target. This limits throughput, but
         * prevents errors (including voiding fluids and backspill into blocks with separate input and output tanks). It
         * also makes sure that higher priority
        ...
        try {
            ...
            resource.amount = Math.min(resource.amount,
                    (int) (ConduitConfig.fluid_tier3_maxIO.get() * getExtractSpeedMultiplier(tank)));
            int filled = 0;
            int remaining = resource.amount;

            for (NetworkTank target : iteratorForTank) {
                if ((!target.equals(tank) || tank.selfFeed) && target.acceptsOuput && target.isValid() &&
                        target.inputColor == tank.outputColor &&
                        matchedFilter(resource, target.con, target.conDir, false)) {
                    int vol = doFill ? target.externalTank.fill(resource.copy()) :
                            target.externalTank.offer(resource.copy());
                    remaining -= vol;
                    filled += vol;
    ...
    private boolean matchedFilter(FluidStack drained, @Nonnull EnderLiquidConduit con, @Nonnull EnumFacing conDir,
                                  boolean isInput) {
        if (drained == null) {
            return false;
        }
