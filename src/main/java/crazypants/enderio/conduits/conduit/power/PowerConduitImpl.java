...
public class PowerConduitImpl extends AbstractConduit implements PowerConduit {
    ...
    static {
        ...
        for (String pf : POSTFIX) {
            ICONS.put(ICON_KEY + pf,
                    new crazypants.enderio.conduits.render.ConduitTexture(TextureRegistry.registerTexture(ICON_KEY), crazypants.enderio.conduits.render.ConduitTexture.arm(i)));
            ICONS.put(ICON_CORE_KEY + pf, new crazypants.enderio.conduits.render.ConduitTexture(TextureRegistry.registerTexture("blocks/conduit_core_0"),
                    crazypants.enderio.conduits.render.ConduitTexture.core(i++)));
        }
    }
    ...
    private @Nonnull PowerConduitData subtype = PowerConduitData.Registry.fromID(0);

    protected final EnumMap<EnumFacing, RedstoneControlMode> rsModes = new EnumMap<EnumFacing, RedstoneControlMode>(
            EnumFacing.class);
    ...
    public PowerConduitImpl() {}

    public PowerConduitImpl(@Nonnull PowerConduitData subtype) {
    ...
    public NNList<ITextComponent> getConduitProbeInformation(@Nonnull EntityPlayer player) {
        ...
        elem = Lang.GUI_CONDUIT_PROBE_POWER_TRACKED_2
                .toChatServer(LangPower.sRF(getEnergyStored(), getMaxEnergyStored()));
        ...
        if (ctracker != null) {
            ...
            elem = Lang.GUI_CONDUIT_PROBE_POWER_TRACKED_4
                    .toChatServer(LangPower.sRFt(ctracker.getAverageRfTickRecieved()));
            elem.getStyle().setColor(TextFormatting.BLUE);
            result.add(elem);
        }
        ...
        if (pm != null) {
            elem = Lang.GUI_CONDUIT_PROBE_POWER_NETWORK_2
                    .toChatServer(LangPower.sRF(pm.getPowerInConduits(), pm.getMaxPowerInConduits()));
            elem.getStyle().setColor(TextFormatting.DARK_BLUE);
            result.add(elem);
            elem = Lang.GUI_CONDUIT_PROBE_POWER_NETWORK_3
                    .toChatServer(LangPower.sRF(pm.getPowerInCapacitorBanks(), pm.getMaxPowerInCapacitorBanks()));
            elem.getStyle().setColor(TextFormatting.BLUE);
            result.add(elem);
            elem = Lang.GUI_CONDUIT_PROBE_POWER_NETWORK_4
                    .toChatServer(LangPower.sRF(pm.getPowerInReceptors(), pm.getMaxPowerInReceptors()));
        ...
        if (ntracker != null) {
            ...
            elem.getStyle().setColor(TextFormatting.BLUE);
            result.add(elem);
            elem = Lang.GUI_CONDUIT_PROBE_POWER_NETWORK_6
                    .toChatServer(LangPower.sRFt(ntracker.getAverageRfTickRecieved()));
            elem.getStyle().setColor(TextFormatting.BLUE);
            result.add(elem);
        }
    ...
    public void writeToNBT(@Nonnull NBTTagCompound data) {
        ...
        data.setShort("subtype", (short) subtype.getID());
        data.setInteger("energyStoredRF", energyStoredRF);

        for (Entry<EnumFacing, RedstoneControlMode> entry : rsModes.entrySet()) {
            if (entry.getValue() != null) {
                short ord = (short) entry.getValue().ordinal();
                data.setShort("pRsMode." + entry.getKey().name(), ord);
            }
        }
        ...
        for (Entry<EnumFacing, DyeColor> entry : rsColors.entrySet()) {
            if (entry.getValue() != null) {
                short ord = (short) entry.getValue().ordinal();
                data.setShort("pRsCol." + entry.getKey().name(), ord);
            }
        }
    }
    ...
    public void readFromNBT(@Nonnull NBTTagCompound data) {
        ...
        subtype = PowerConduitData.Registry.fromID(data.getShort("subtype"));

        if (data.hasKey("energyStored")) {
            data.setInteger("energyStoredRF", (int) (data.getFloat("energyStored") * 10));

        }
        setEnergyStored(data.getInteger("energyStoredRF"));
    ...
    public static int getMaxEnergyIO(PowerConduitData subtype) {
    ...
    public boolean canConnectToConduit(@Nonnull EnumFacing direction, @Nonnull Conduit conduit) {
        ...
        if (!(conduit instanceof PowerConduit)) {
            return false;
        }
        PowerConduit pc = (PowerConduit) conduit;
    ...
    public IPowerInterface getExternalPowerReceptor(@Nonnull EnumFacing direction) {
        ...
        if (test instanceof ConduitBundle) {
            return null;
        }
    ...
    public Collection<CollidableComponent> createCollidables(@Nonnull CacheKey key) {
        ...
        CollidableComponent cc = new CollidableComponent(PowerConduit.class, bb, key_dir, COLOR_CONTROLLER_ID);
    ...
    public class ConnectionSide implements IEnergyStorage {
        ...
        public int receiveEnergy(int maxReceive, boolean simulate) {
            if (isReceive()) {
                int energyFinal = PowerConduitImpl.this.receiveEnergy(maxReceive, simulate);
                if (bundle != null) {
        ...
        public int extractEnergy(int maxExtract, boolean simulate) {
            if (isExtract())
                return PowerConduitImpl.this.extractEnergy(maxExtract, simulate);
            return 0;
        }
        ...
        public int getEnergyStored() {
            if (isEnabled())
                return PowerConduitImpl.this.getEnergyStored();
            return 0;
        }
        ...
        public int getMaxEnergyStored() {
            if (isEnabled())
                return PowerConduitImpl.this.getMaxEnergyStored();
            return 0;
        }

        @Override
        public boolean canExtract() {
            return isExtract() && PowerConduitImpl.this.canExtract() && !recievedRfThisTick();
        }

        @Override
        public boolean canReceive() {
            return isReceive() && PowerConduitImpl.this.canReceive();
        }
    }
