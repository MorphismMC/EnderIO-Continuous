...
public class NetworkPowerManager {
    ...
    private @Nonnull ListIterator<ReceptorEntry> receptorIterator = NullHelper.notnullJ(receptors.listIterator(),
            "List.listIterator()");
    ...
    private void checkReceptors() {
        ...
        for (ReceptorEntry rec : network.getPowerReceptors()) {
            ...
            if (powerInterface != null) {
                ...
                // we can ignore that connection here, but the conduit should also update and remove its external
                // connection
    ...
    private class CapBankSupply {
        ...
        void init() {
            ...
            for (ReceptorEntry rec : storageReceptors) {
                final IPowerInterface powerInterface = rec.getPowerInterface();
                if (powerInterface == null) {
                    // This should be impossible for connections to capBanks. They send proper block updates when they
                    // decide to no longer have an energy capability ;)
                    continue;
                }
                ...
                if (cb.isNetworkControlledIo(rec.direction.getOpposite())) {
                    ...
                    enteries.add(
                            new CapBankSupplyEntry(cb, (int) canGet, (int) canFillLocal, rec.emmiter, rec.direction));
                }
