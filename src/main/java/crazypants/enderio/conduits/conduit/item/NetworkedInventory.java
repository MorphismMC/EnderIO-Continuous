...
public class NetworkedInventory {
    ...
    public void onTick() {
        ...
        if (tickDeficit <= 0) {
            // Transfer successful but the transferItems() didn't set a new tickDeficit, sleep for a second before
            // checking again.
            tickDeficit = ConduitConfig.sleepBetweenTries.get();
        }
    }
                    ...
                    if (filter.isLimited()) {
                        ...
                        if (count <= 0) { // doesn't pass filter
                            ...
                            if (stackInSlot.getCount() <= count) { // there's less than the limit in there
                                continue; // skip slot
                            } else if (stackInSlot.getCount() - item.getCount() < count) { // we are trying to extract
                                                                                           // more than allowed
    ...
    private boolean doTransfer(@Nonnull IItemHandler inventory, @Nonnull ItemStack extractedItem, int slot) {
        ...
        if (Prep.isInvalid(extracted) || extracted.getCount() != numInserted ||
                extracted.getItem() != extractedItem.getItem()) {
            if (extracted.getCount() < numInserted &&
                    (extracted.getCount() == 0 || extracted.getItem() == extractedItem.getItem())) {
                Log.warn("NetworkedInventory.itemExtracted: Inserted " + numInserted + " " +
                        extractedItem.getDisplayName() + " but only removed " + extracted.getCount() + " " +
                        extracted.getDisplayName() + " from " + inventory + " at " + location + ". This means that " +
                        (numInserted - extracted.getCount()) + " items were just duped by " + inventory + "!");
            } else {
                Log.warn("NetworkedInventory.itemExtracted: Inserted " + numInserted + " " +
                        extractedItem.getDisplayName() + " but only removed " + extracted.getCount() + " " +
                        extracted.getDisplayName() + " from " + inventory + " at " + location);
            }
        }
    ...
    private int insertIntoTargets(@Nonnull ItemStack toInsert) {
        ...
        // when true, a sticky filter has claimed this item and so only sticky outputs are allowed to handle it. sticky
        // outputs are first in the target
        ...
        for (Target target : getTargetIterator()) {
            ...
            if (target.stickyInput || !matchedStickyOutput) {
                ...
                // item has been claimed by a sticky output but there are no sticky outputs left in targets, so we can
                // stop checking
                break;
            }
        }
    ...
    private void calculateDistances(@Nonnull List<Target> targets, @Nonnull Map<BlockPos, Integer> visited,
                                    @Nonnull List<BlockPos> steps, int distance) {
        if (steps.isEmpty()) {
            return;
        }
        ...
        for (Target target : targets) {
            if (target != null && target.inv != null) {
                if (target.inv.getConDir() == dir &&
                        target.inv.getCon().getBundle().getLocation().equals(con1.getBundle().getLocation())) {
                    return target;
                }
            }
        }
