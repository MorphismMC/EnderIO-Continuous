        ...
        if (dir != null) {
            if (neighborPositions.containsKey(dir)) {
                ...
                mc.player.rotationPitch = MathHelper
                        .wrapDegrees((float) (-(MathHelper.atan2(d1, d3) * (180D / Math.PI))));
                mc.player.rotationYaw = MathHelper
                        .wrapDegrees((float) (MathHelper.atan2(d2, d0) * (180D / Math.PI)) - 90.0F);
    ...
    protected void findBlockDataForDirection(@Nonnull EnumFacing direction) {
        ...
        if (!world.isAirBlock(blockPos)) {
            ...
            if (b != ConduitRegistry.getConduitModObjectNN().getBlock()) {
                try {
                    ItemStack pickBlock = b.getPickBlock(bs,
                            new RayTraceResult(new Vec3d(0, 0, 0), direction.getOpposite(), blockPos), world, blockPos,
        ...
        for (EnumFacing dir : EnumFacing.VALUES) {
            if (dir != null) {
                ...
                if (neighborPositions.containsKey(dir)) {
                    ...
                    b = new GuiButton(dir.ordinal(), p.x, p.y, BUTTON_WIDTH, BUTTON_HEIGHT,
                            (stacks.containsKey(dir) ? "  " : "") + dir.toString());
                    buttonList.add(b);
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        for (EnumFacing dir : EnumFacing.VALUES) {
            if (stacks.containsKey(dir)) {
                ...
                drawString(fontRenderer, blockName, scale * (p.x + BUTTON_WIDTH / 2 - textWidth / 2), scale * p.y,
        ...
        if (Minecraft.getMinecraft().player.getName().contains("direwolf20") &&
                ((EnderIO.proxy.getTickCount() / 16) & 1) == 1) {
