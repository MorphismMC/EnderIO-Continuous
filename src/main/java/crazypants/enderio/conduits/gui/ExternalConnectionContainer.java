...
public class ExternalConnectionContainer extends ContainerEnderCap<InventoryUpgrades, TileConduitBundle>
    ...
    @Override
    public void createGhostSlots(@Nonnull NNList<GhostSlot> ghostSlots, @Nonnull NNList<ItemStack> filters,
                                 @Nonnull NNList<ItemStack> upgrades) {
        createGhostSlots(ghostSlots, filters, filters, upgrades);
    }

    @Override
    public void createGhostSlots(@Nonnull NNList<GhostSlot> ghostSlots, @Nonnull NNList<ItemStack> filtersIn,
                                 @Nonnull NNList<ItemStack> filtersOut,
                                 @Nonnull NNList<ItemStack> upgrades) {
    ...
    public void setInOutSlotsVisible(boolean filterVisible, boolean upgradeVisible, @Nonnull IConduit conduit) {
        ...
        if (filterVisible || upgradeVisible) {
            ...
            if (hasFilterHolder) {
                getItemHandler().setFilterHolder(
                        conduit.getInternalCapability(CapabilityFilterHolder.FILTER_HOLDER_CAPABILITY, dir));
            }
            if (hasUpgradeHolder) {
                getItemHandler().setUpgradeHolder(
                        conduit.getInternalCapability(CapabilityUpgradeHolder.UPGRADE_HOLDER_CAPABILITY, dir));
            }
        }
    ...
    public IMessage doOpenFilterGui(int filterIndex) {
        if (currentCon.hasInternalCapability(CapabilityFilterHolder.FILTER_HOLDER_CAPABILITY, dir)) {
            ...
            if (filterHolder != null) {
                filterHolder.getFilter(filterIndex, param1).openGui(player,
                        filterHolder.getFilterStack(filterIndex, EnumFacing.byIndex(param1)),
                        getTileEntityNN().getBundleworld(),
