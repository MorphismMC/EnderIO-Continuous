...
public class ConduitRenderMapper implements IRenderMapper.IBlockRenderMapper.IRenderLayerAware.IPaintAware {
    ...
    public List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
        ...
        if (tileEntity instanceof TileConduitBundle) {
            ...
            if (paintSource != null &&
                    (paintSource.isOpaqueCube() || ((TileConduitBundle) tileEntity).getFacadeType().isTransparent()) &&
                    !state.getYetaDisplayMode().isHideFacades()) {
                return null;
            }
            List<BakedQuad> quads = ConduitBundleRenderManager.instance.getConduitBundleRenderer()
                    .getGeneralQuads(state, blockLayer);
