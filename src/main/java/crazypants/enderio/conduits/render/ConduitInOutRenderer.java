...
public class ConduitInOutRenderer {
    ...
    // background
    public static final @Nonnull TextureSupplier ICON_BG = TextureRegistry
            .registerTexture("blocks/item_conduit_io_connector");
    // centered in arrow
    public static final @Nonnull TextureSupplier ICON_IN = TextureRegistry.registerTexture("blocks/item_conduit_input");
    // centered out arrow
    public static final @Nonnull TextureSupplier ICON_OUT = TextureRegistry
            .registerTexture("blocks/item_conduit_output");
    // offset in arrow
    public static final @Nonnull TextureSupplier ICON_INOUT_IN = TextureRegistry
            .registerTexture("blocks/item_conduit_in_out_in");
    // offset out arrow
    ...
    public static void renderIO(@Nonnull ConduitBundle bundle, @Nonnull ConduitClient conduit,
        ...
        if (mode.acceptsInput()) {
            if (mode.acceptsOutput()) {
                ...
                geometry.addModeConnectorQuads(dir, offset, ICON_IN.get(TextureAtlasSprite.class),
                        ColorUtil.toFloat4(inChannel.getColor()),
                        quads);
            }
        } else if (mode.acceptsOutput()) {
            geometry.addModeConnectorQuads(dir, offset, ICON_OUT.get(TextureAtlasSprite.class),
                    ColorUtil.toFloat4(outChannel.getColor()),
                    quads);
        }
    }
            ...
            if (extractionRedstoneMode != RedstoneControlMode.IGNORE &&
                ...
                for (EnumFacing face : EnumFacing.VALUES) {
                    if (face != null && face != dir) {
                        BakedQuadBuilder.addBakedQuadForFace(quads, bound, White.INSTANCE, face, ConduitTexture.FULL,
                                false, false, ColorUtil.toFloat4(cInt));
                    }
                }
            }
        }
