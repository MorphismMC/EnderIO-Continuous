...
public class BlockStateWrapperConduitBundle extends BlockStateWrapperBase {
    ...
    private final static Cache<ConduitCacheKey, QuadCollector> cache = CacheBuilder.newBuilder().maximumSize(500)
    ...
    public BlockStateWrapperConduitBundle(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
                                          @Nonnull BlockPos pos, IBlockRenderMapper renderMapper) {
    ...
    public static class ConduitCacheKey {
        ...
        @Override
        public String toString() {
            return "ConduitCacheKey [idx=" + idx + ", hashCode=" + hashCode + ", hashCodes=" +
                    Arrays.toString(hashCodes) + "]";
        }
    }
}
