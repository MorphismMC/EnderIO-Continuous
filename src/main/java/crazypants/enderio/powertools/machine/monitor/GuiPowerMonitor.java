...
public class GuiPowerMonitor extends GuiMachineBase<TilePowerMonitor> implements ContainerPowerMonitorProxy {
    ...
    private GuiToolTip tooltipConduitStorage, tooltipCapacitorBankStorage, tooltipMachineBuffers, tooltipAverageOutput,
            tooltipAverageInput;
    ...
    public GuiPowerMonitor(@Nonnull InventoryPlayer par1InventoryPlayer, @Nonnull TilePowerMonitor te) {
        ...
        addToolTip(tooltipConduitStorage = new GuiToolTip(new Rectangle(0, 0, 0, 0),
                Lang.GUI_POWER_MONITOR_CONDUIT_STORAGE.get()));
        addToolTip(tooltipCapacitorBankStorage = new GuiToolTip(new Rectangle(0, 0, 0, 0),
                Lang.GUI_POWER_MONITOR_CAPBANK_STORAGE.get()));
        addToolTip(tooltipMachineBuffers = new GuiToolTip(new Rectangle(0, 0, 0, 0),
                Lang.GUI_POWER_MONITOR_MACHINE_BUFFER.get()));
        addToolTip(tooltipAverageOutput = new GuiToolTip(new Rectangle(0, 0, 0, 0),
                Lang.GUI_POWER_MONITOR_AVERAGE_OUTPUT.get()));
        addToolTip(tooltipAverageInput = new GuiToolTip(new Rectangle(0, 0, 0, 0),
                Lang.GUI_POWER_MONITOR_AVERAGE_INPUT.get()));
            ...
            case CONTROL:
                ...
                if (engineControlStart_value == null) {
                    engineControlEnabled
                            .setSelected(engineControlEnabled_value = getTileEntity().isEngineControlEnabled());
                    engineControlStart
                            .setText(engineControlStart_value = "" + (int) (getTileEntity().getStartLevel() * 100));
                    engineControlStop
                            .setText(engineControlStop_value = "" + (int) (getTileEntity().getStopLevel() * 100));
                }

                if (engineControlEnabled_value != engineControlEnabled.isSelected() ||
                        !engineControlStart_value.equals(engineControlStart.getText()) ||
                        !engineControlStop_value.equals(engineControlStop.getText())) {
                    doSetConfig(engineControlEnabled.isSelected(), getInt(engineControlStart) / 100f,
    ...
    private void drawStats(int sx, int sy) {
        ...
        if (statData.maxPowerInCapBanks > 0) {
            BlockItemCapBank.setStoredEnergyForItem(capBankStack,
                    (int) (((BlockItemCapBank) capBankStack.getItem()).getMaxEnergyStored(capBankStack) *
        ...
        tooltipCapacitorBankStorage
                .setBounds(new Rectangle(TEXT_MARGIN_LEFT, TEXT_MARGIN_TOP + LINE_Y_OFFSET, TEXT_WIDTH, 16));
        ...
        tooltipMachineBuffers
                .setBounds(new Rectangle(TEXT_MARGIN_LEFT, TEXT_MARGIN_TOP + 3 * LINE_Y_OFFSET, TEXT_WIDTH, 16));
        RenderHelper.disableStandardItemLighting();

        bindGuiTexture(1);
        drawTexturedModalRect(x, y + 2 * LINE_Y_OFFSET, 180, 31, 16, 16);
        tooltipAverageOutput
                .setBounds(new Rectangle(TEXT_MARGIN_LEFT, TEXT_MARGIN_TOP + 2 * LINE_Y_OFFSET, TEXT_WIDTH / 2, 16));

        drawTexturedModalRect(x + TEXT_WIDTH / 2, y + 2 * LINE_Y_OFFSET, 196, 31, 16, 16);
        tooltipAverageInput.setBounds(new Rectangle(TEXT_MARGIN_LEFT + TEXT_WIDTH / 2,
                TEXT_MARGIN_TOP + 2 * LINE_Y_OFFSET, TEXT_WIDTH / 2, 16));
