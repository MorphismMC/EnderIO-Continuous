...
public class DynaTextureProviderPMon extends DynaTextureProvider {
    ...
    protected static final @Nonnull ResourceLocation pmon_screen = new ResourceLocation(EnderIO.DOMAIN,
            "textures/blocks/block_pm_on_screen.png");
    protected static final @Nonnull int[] pmon_screen_data = new int[TEXSIZE * TEXSIZE];
    protected static final @Nonnull ResourceLocation pmon_color = new ResourceLocation(EnderIO.DOMAIN,
            "textures/blocks/block_pm_on_color.png");
    protected static final @Nonnull int[] pmon_color_data = new int[TEXSIZE * TEXSIZE];
    protected static final @Nonnull ResourceLocation pmon_fallback = new ResourceLocation(EnderIO.DOMAIN,
            "textures/blocks/block_pm_on.png");
        ...
        for (int x = 0; x < TEXSIZE; x++) {
            for (int y = 0; y < TEXSIZE; y++) {
                imageData[y * TEXSIZE + x] = (x > 27 || TEXSIZE - y > minmax[1][x] * 23 / 63 + 5 ||
                        TEXSIZE - y < minmax[0][x] * 23 / 63 + 5 ? pmon_screen_data : pmon_color_data)[y * TEXSIZE + x];
            }
        }
    }
