...
public class TESRPowerMonitor extends ManagedTESR<TilePowerMonitor> {
    ...
    private static BoundingBox bb2 = new BoundingBox(0 * px, 0 * px, 16.00f * px, 16 * px, 16 * px, 16.00f * px); // screen,
                                                                                                                  // painted
    ...
    @Override
    protected void renderTileEntity(@Nonnull TilePowerMonitor te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
        ...
        if (isPainted) {
            ...
            helper.renderSingleFace(tes, EnumFacing.SOUTH, 0 * px, 14 * px, 0 * px, 14 * px, xform,
                    Helper.stdBrightness, false);
        } else {
            helper.setupVertices(bb1, xform);
            helper.renderSingleFace(tes, EnumFacing.SOUTH, 1 * px, 15 * px, 1 * px, 15 * px, xform,
                    Helper.stdBrightness, false);
        }
    ...
    private static class Helper {
        ...
        void addVecWithUV(BufferBuilder tes, Vector3d vec, double u, double v, float cm, EnumFacing normal) {
            tes.pos(vec.x, vec.y, vec.z).tex(u, v).color(cm, cm, cm, 1)
                    .normal(normal.getDirectionVec().getX(), normal.getDirectionVec().getY(),
                            normal.getDirectionVec().getZ())
                    .endVertex();
        }

        void renderSingleFace(BufferBuilder tes, EnumFacing face, float minU, float maxU, float minV, float maxV,
                              VertexTransform xForm, float[] brightnessPerSide,
                              boolean inside) {
