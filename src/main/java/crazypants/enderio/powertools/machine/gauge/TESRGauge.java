...
public class TESRGauge extends ManagedTESR<TileGauge> {
    ...
    @Override
    protected void renderTileEntity(@Nonnull TileGauge te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
        ...
        for (Entry<EnumFacing, Float> entry : te.data.entrySet()) {
            ...
            if (face != null && value != null) {
                FillGaugeBakery bakery = new FillGaugeBakery(world,
                        te.getPos().offset(NullHelper.first(face.getOpposite(), EnumFacing.DOWN)), face,
        ...
        if (!renderedSomething) {
            ...
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
                FillGaugeBakery bakery = new FillGaugeBakery(world, te.getPos().offset(face.getOpposite()), face,
                        BlockGauge.gaugeIcon.get(TextureAtlasSprite.class),
                        ratio);
                if (bakery.canRender()) {
                    GlStateManager.pushMatrix();
                    GlStateManager.translate(-face.getXOffset(), -face.getYOffset(), -face.getZOffset());
