...
public class TileCapBank extends TileEntityEio
                         implements ILegacyPoweredTile.Receiver, IIoConfigurable, IPowerStorage,
                         IPaintable.IPaintableTileEntity, IPowerBarData {
    ...
    public void validateDisplayTypes() {
        ...
        for (Entry<EnumFacing, InfoDisplayType> entry : faceDisplayTypes.entrySet()) {
            IBlockState bs = world
                    .getBlockState(getPos().offset(NullHelper.notnullJ(entry.getKey(), "EnumMap.getKey()")));
    ...
    public void setMaxInput(int maxInput) {
        ...
        if (DiagnosticsConfig.debugTraceCapLimitsExtremelyDetailed.get()) {
            StringBuilder sb = new StringBuilder("CapBank ").append(this).append(" input changed from ")
                    .append(this.maxInput).append(" to ").append(maxInput);
    ...
    public void setMaxOutput(int maxOutput) {
        ...
        if (DiagnosticsConfig.debugTraceCapLimitsExtremelyDetailed.get()) {
            StringBuilder sb = new StringBuilder("CapBank ").append(this).append(" output changed from ")
                    .append(this.maxOutput).append(" to ").append(maxOutput);
            for (StackTraceElement elem : new Exception("Stackstrace").getStackTrace()) {
                sb.append(" at ").append(elem);
            }
            Log.warn(sb);
