...
public class IoDisplay implements IInfoRenderer {
    ...
    public void render(@Nonnull TileCapBank cb, @Nonnull EnumFacing dir, float partialTick) {
        ...
        GlStateManager.translate((dir.getXOffset() * 1.02f) / 2 + 0.5f + right.getXOffset() * (info.width - 1) * 0.5f,
                ...
                (dir.getZOffset() * 1.02f) / 2 + 0.5f + right.getZOffset() * (info.width - 1) * 0.5f);
    ...
    private float drawText(HeadingText heading, @Nonnull String text, float offset, float scale, float size,
                           FontRenderer fr) {
