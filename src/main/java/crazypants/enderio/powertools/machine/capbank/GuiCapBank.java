...
public class GuiCapBank extends GuiContainerBaseEIO<TileCapBank> {
    ...
    public GuiCapBank(Entity player, InventoryPlayer playerInv, @Nonnull TileCapBank te,
                      @Nonnull ContainerCapBank container) {
        ...
        final Predicate<String> inputValidator = s -> s == null || s.isEmpty() || (!s.startsWith("0") &&
                NullHelper.first(LangPower.parsePower(s), (Integer) Integer.MAX_VALUE) <= network.getMaxIO());
        ...
        powerBar.addTooltip(Op.REPLACE, What.ALL, //
                ...
                () -> TextFormatting.WHITE + LangPower.format(network.getMaxEnergyStoredL()) + " " +
                        TextFormatting.GRAY + LangPower.RF(), //
                () -> crazypants.enderio.base.lang.Lang.POWER_PERTICK
                        .get(getIOColor() + LangPower.format(Math.round(network.getAverageChangePerTick())) +
                                TextFormatting.GRAY) //
        );
    }
