...
public class FillGaugeBakery {
    ...
    public FillGaugeBakery(IBlockAccess world, BlockPos pos, @Nonnull EnumFacing face,
    ...
    private void mkQuads() {
        ...
        BoundingBox back = new BoundingBox(6 * px, (connectDown ? 0 : 2) * px, full_out, 10 * px,
                (connectUp ? 16 : 14) * px, bit_in);
        buffer.add(back, EnumFacing.SOUTH, 4.01 * px, 7.99 * px, (connectDown ? 0 : 2) * px, (connectUp ? 16 : 14) * px,
                tex, null);
        ...
        BoundingBox bg = new BoundingBox(6.5 * px, (connectDown ? 0 : 2.5) * px, quarter_out, 9.5 * px,
                (connectUp ? 16 : 13.5) * px, bit_in);
        buffer.add(bg, EnumFacing.NORTH, 12.5 * px, 15.5 * px, (connectDown ? 0 : 2.5) * px,
                (connectUp ? 16 : 13.5) * px, tex, null);
        buffer.transform(rot);

        if (localFillLevel > 0.001) {
            BoundingBox fg = new BoundingBox(6.5 * px, (connectDown ? 0 : 2.99) * px, half_out, 9.5 * px,
                    localFillLevel * px, bit_in);
            litBuffer = new HalfBakedList();
            litBuffer.add(fg, EnumFacing.NORTH, 8.55 * px, 11.45 * px, (connectDown ? 0 : 2.99) * px,
                    localFillLevel * px, tex, null);
    ...
    private void countNeighbors() {
        if (world != null && pos != null) {
            ...
            while (true) {
                ...
                if (!(tileEntity instanceof TileCapBank) ||
                        ((TileCapBank) tileEntity).getDisplayType(face) != InfoDisplayType.LEVEL_BAR) {
                    break;
                }
                height++;
                connectUp = true;
