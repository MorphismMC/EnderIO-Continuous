...
public class CapBankBlockRenderMapper extends ConnectedBlockRenderMapper {
    ...
    protected List<IBlockState> renderBody(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
        ...
        if (tileEntity instanceof TileCapBank && state.getBlock() instanceof BlockCapBank) {
            NNList.FACING.apply(new Callback<EnumFacing>() {
                ...
                public void apply(@Nonnull EnumFacing face) {
                    ...
                    states.add(ModObject.block_machine_io.getBlockNN().getDefaultState().withProperty(IOMode.IO,
                            IOMode.get(face, iOMode)));
                }
            });
        } else {
            states.add(state.getState().withProperty(RENDER, EnumMergingBlockRenderMode.sides)
    ...
    protected boolean isSameKind(@Nonnull IBlockState state, @Nonnull IBlockState other) {
        ...
        return myKind.isMultiblock() && state.getBlock() == other.getBlock() &&
                myKind == other.getValue(CapBankType.KIND);
    }
