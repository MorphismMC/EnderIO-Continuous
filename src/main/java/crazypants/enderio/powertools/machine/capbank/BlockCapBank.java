...
public class BlockCapBank extends BlockEio<TileCapBank>
                          implements IEioGuiHandler.WithPos, IAdvancedTooltipProvider, ISmartRenderAwareBlock,
                          IHaveTESR, ICustomSubItems, IPaintable.ISolidBlockPaintableBlock {
    ...
    protected BlockCapBank(@Nonnull IModObject modObject) {
        ...
        setDefaultState(getBlockState().getBaseState()
                .withProperty(EnumMergingBlockRenderMode.RENDER, EnumMergingBlockRenderMode.AUTO)
    ...
    public void getSubBlocks(@Nonnull CreativeTabs p_149666_2_, @Nonnull NonNullList<ItemStack> list) {
        for (CapBankType type : CapBankType.types()) {
            if (type.isCreative()) {
                list.add(BlockItemCapBank.createItemStackWithPower(CapBankType.getMetaFromType(type),
                        type.getMaxEnergyStored() / 2));
            } else {
                list.add(BlockItemCapBank.createItemStackWithPower(CapBankType.getMetaFromType(type), 0));
                list.add(BlockItemCapBank.createItemStackWithPower(CapBankType.getMetaFromType(type),
                        type.getMaxEnergyStored()));
            }
        }
    }
    ...
    public void addBasicEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
                                @Nonnull List<String> list, boolean flag) {
        list.add(LangPower.RF(BlockItemCapBank.getStoredEnergyForItem(itemstack),
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
                                    ...
                                    @Nonnull EnumHand hand, @Nonnull EnumFacing faceHit, float hitX, float hitY,
                                    float hitZ) {
    ...
    @SideOnly(Side.CLIENT)
    @Deprecated
    public boolean shouldSideBeRendered(@Nonnull IBlockState bs, @Nonnull IBlockAccess par1IBlockAccess,
    ...
    protected boolean setDisplayToVerticalFillBar(TileCapBank cb, TileCapBank capBank) {
        ...
        if (capBank != null) {
            for (EnumFacing dir : EnumFacing.VALUES) {
                if (dir.getYOffset() == 0 && capBank.getDisplayType(dir) == InfoDisplayType.LEVEL_BAR &&
