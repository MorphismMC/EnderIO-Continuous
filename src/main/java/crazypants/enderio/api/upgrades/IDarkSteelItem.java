...
/**
 * This interface for {@link Item}s marks them as being eligible for {@link IDarkSteelUpgrade}s. Ender IO will also
 * handle repairing them if
...
public interface IDarkSteelItem extends INotDestroyedInItemFrames {
    ...
    /**
     * @return The number of {@link #isItemForRepair(ItemStack)} items it takes to repair this item fully. Usually as
     *         many as are used in the crafting recipe.
     */
     ...
     * Used by upgrades to determine if they can be applied to an item.
     * <p>
     * Note: Items that return true here should also do so for {@link #isForSlot(EntityEquipmentSlot)} with
     * {@link EntityEquipmentSlot#MAINHAND}.
     */
     ...
     * <p>
     * Used by some upgrades to determine if they can be applied to an item. Only upgrades that need the item to have
     * supporting code will call this, e.g. the
     ...
     * <p>
     * Note that the given ItemStack will be empty when determining which upgrade slots will be shown in the upgrade
     * GUI. It will be filled when checkig if the
