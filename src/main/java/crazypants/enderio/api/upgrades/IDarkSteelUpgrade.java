 ...
 * <p>
 * The upgrade needs to be registered in the
 * {@link net.minecraftforge.event.RegistryEvent.Register}&lt;IDarkSteelUpgrade&gt; event.
 * <p>
 * Upgrades themselves are stateless. They determine if they are present on an item or not. By themselves, upgrades can
 * not do very much---they are ticked and
 * optionally can render on the player. Further functionality needs to be implemented on an event handler that checks
 * for the upgrade's existence.
 * <p>
...
public interface IDarkSteelUpgrade extends IForgeRegistryEntry<IDarkSteelUpgrade>, Localizable {
    ...
    /**
     * @return A {@link Pair}<code>&lt;{@link String}, {@link Integer}&gt;</code> that is used to sort upgrades for
     *         display in JEI and the Creative Menu. The
     *         sorting is done on the concatenation of both. When determining if two upgrades should be grouped
     *         together, the String is compared and the upgrades
    ...
    /**
     * Checks if this upgrade can be applied to the given item. This is called by the anvil crafting for the "left"
     * item.
     * <p>
    ...
    /**
     * Checks if the other upgrade can be removed or if it is a prerequisite for this upgrade. This is called to build
     * the JEI recipes, so it must mirror
     ...
     */
    default boolean canOtherBeRemoved(@Nonnull ItemStack stack, @Nonnull IDarkSteelItem item,
    ...
    /**
     * See {@link PlayerTickEvent}. Called when the given item is equipped in any {@link EntityEquipmentSlot} and has
     * this upgrade.
     * 
     * @param stack
    ...
    /**
     * Called via server when another player activates the SFX for this upgrade. That activation has to send a
     * {@link PacketDarkSteelSFX} to the server if it
    ...
    /**
     * See {@link Item#getAttributeModifiers(EntityEquipmentSlot, ItemStack)}. This allows upgrades to also add
     * attribute modifieres to the item.
     * 
     * @param slot
     ...
     */
    default void addAttributeModifiers(@Nonnull EntityEquipmentSlot slot, @Nonnull ItemStack stack,
                                       @Nonnull Multimap<String, AttributeModifier> map) {}

    /**
     * Returns a list of rules that determine if the upgrade can be applied to an item. The list should be stable and
     * all rules must return the same result on
     * server and client.
     * <p>
    ...
    /**
     * If an upgrade provides an inventory (or wants to drop extras), it should return it here so it can be emptied when
