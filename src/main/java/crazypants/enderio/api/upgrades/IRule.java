...
public interface IRule {
    ...
    /**
     * Marks the rule as one that only checks the type of the item, not its current state. Only those rules are used
     * when the GUI has to determine which slots to
     * show.
     *
     */
    ...
    /**
     * Marks the rule as one checking for the existence of another upgrade. Those will be shown in the upgrade item
     * tooltip under the "depends on" header.
     * <p>
    ...
    interface Prerequisite extends IRule {
        ...
        default IRule.CheckResult check(@Nonnull ItemStack stack, @Nonnull IDarkSteelItem item) {
            return getPrerequisite().hasUpgrade(stack, item) ? CheckResult.PASS : new CheckResult( //
    ...
    /**
     * Marks the rule as one checking the general type of item (weapon, tool, ...). Those will be shown in the upgrade
     * item tooltip under the "can be applied to"
     * header.
     * <p>
     * Note that the tooltip is fetched without querying check() first, as there is no item to check against when
     * showing the tooltip for the upgrade item.
     * <p>
