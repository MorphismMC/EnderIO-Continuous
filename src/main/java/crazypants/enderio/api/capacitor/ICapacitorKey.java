...
public interface ICapacitorKey extends IForgeRegistryEntry<ICapacitorKey> {
     ...
     * <p>
     * The capacitor level is a 1, 2 and 3 for the basic capacitors. Custom ones may have any non-zero, positive level.
     * The scalers are expected to only map to
    ...
    /**
     * See {@link ICapacitorKey#get(ICapacitorData)}, but this method will return the value as a float. Depending on the
     * scaler and capacitor level, this may make
     * a difference.
     */
    ...
    /*
     * These methods are used by the xml configuration system only. For keys that are not configured that way, make the
     * setters complain and the validate do
     * nothing.
     */
