...
public interface IModObject extends IProducer, IForgeRegistryEntry<IModObject> {
    ...
    boolean openClientGui(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer entityPlayer,
                          @Nullable EnumFacing side, int param);

    boolean openClientGui(@Nonnull World world, @Nonnull EntityPlayer entityPlayer, int a, int b, int c);

    /**
     * Interface to be implemented on blocks that are created from modObjects. It will be called at the right time to
     * create and register the blockItem. Note that
