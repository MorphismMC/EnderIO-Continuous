 ...
 * <p>
 * The handler's {@link #matches(ItemStack)} method will be called on two different occasions. First to check if an item
 * can be inserted into the fertilizer
 * slot (on both client and server), second to select the correct handler when trying to apply the fertilizer (server
 * only).
 * <p>
 * The {@link #apply(ItemStack, EntityPlayer, World, BlockPos)} method will then be called to execute applying the
 * fertilizer. It is responsible for checking if
 * the target block can be fertilized and then doing the actual fertilizing. If it wants to play sounds or spawn FX
 * (green particles), is has to do so, too.
 * (Note that the purple Farming Station particles will be added by the Farming Station.) It then has to return the
 * changed itemStack.
 * <p>
 * If the fertilizer item is some kind of container item (e.g. a bucket, or an RF powered item) and got useless for
 * fertilizing after being used, you can put it
...
public interface IFertilizer extends IForgeRegistryEntry<IFertilizer> {
     ...
     * 
     * @param stack
     *               The itemStack that should be used. May be modified, but changes are ignored if the action was
     *               successful. (Please don't modify it otherwise.)
     * @param player
     *               A suitable FakePlayer. Extra results from applying the fertilizer can be put into its inventory.
     *               Those will go into the farm's output.
     * @param world
     ...
     */
    IFertilizerResult apply(@Nonnull ItemStack stack, @Nonnull EntityPlayer player, @Nonnull World world,
