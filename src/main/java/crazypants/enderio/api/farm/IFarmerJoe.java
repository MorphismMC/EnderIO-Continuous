 ...
 * <p>
 * The handler needs to be registered in the {@link net.minecraftforge.event.RegistryEvent.Register}&lt;IFarmerJoe&gt;
 * event.
 * <p>
...
public interface IFarmerJoe extends IForgeRegistryEntry<IFarmerJoe> {
     ...
     * <p>
     * This will be called to see if an item is allowed to be inserted into one of the seed slots of the Farming
     * Station.
     * 
     * @param stack
     ...
     * <p>
     * The Handler is expected to check if there's a matching seed in the correct seed slot, then to till the land and
     * plant it.
     * 
     * @param farm
     ...
     * @return ACTION if this handler wants to handle this location. CLAIM if it didn't actually do something, just
     *         wants no other handler to mess with this
     *         location. NEXT to give the next handler a try.
     */
    default IFarmerJoe.Result tryPrepareBlock(@Nonnull IFarmer farm, @Nonnull BlockPos pos,
     ...
     * <p>
     * The Handler is expected to check if there's a matching plant. If there is,
     * {@link #canHarvest(IFarmer, BlockPos, IBlockState)} will be called next.
     * <p>
     ...
     * @param state
     *              The blockstate at that location.
     * @return True if this handler wants to handle this location. Doesn't mean that it actually will harvest something,
     *         just that no other handler will get the
     *         chance to do so.
     */
     ...
     * <p>
     * The Handler is expected to check if the plant is ready for harvest, and if it is, to harvest it. If possible, the
     * plant should also be replanted directly.
     * 
     * @param farm
     *              The call back to the Farming Station.
     * @param pos
