...
public class SolarUpgrade extends AbstractUpgrade implements IHasPlayerRenderer {
    ...
    public static final @Nonnull NNList<SolarUpgrade> INSTANCES = new NNList<>( //
            new SolarUpgrade(SolarType.SIMPLE), new SolarUpgrade(SolarType.NORMAL),
            new SolarUpgrade(SolarType.ADVANCED), new SolarUpgrade(SolarType.VIBRANT));
    ...
    public SolarUpgrade(@Nonnull SolarType type) {
        super(EnderIOMachines.MODID, UPGRADE_NAME, ISolarType.getMetaFromType(type),
                NAME + ISolarType.getMetaFromType(type), type::getUpgradeLevelCost);
        this.type = type;
    }
    ...
    public List<IRule> getRules() {
        return new NNList<>(Rules.forSlot(EntityEquipmentSlot.HEAD), EnergyUpgrade.HAS_ANY,
                Rules.withLevels(getLevel(), INSTANCES),
    ...
    public void onPlayerTick(@Nonnull ItemStack helm, @Nonnull IDarkSteelItem item, @Nonnull EntityPlayer player) {
        ...
        if (!player.world
                .canBlockSeeSky(new BlockPos(MathHelper.floor(player.posX),
                        MathHelper.floor(player.posY + player.eyeHeight + .25), MathHelper.floor(player.posZ)))) {
            return;
        }
        ...
        if (toAdd != 0) {
            ...
            for (int i = 0; i < 4 && toAdd > 0; i++) {
                ...
                if (cap != null &&
                        (SolarConfig.helmetChargeOthers.get() || EnergyUpgradeManager.loadFromItem(stack) != null)) {
