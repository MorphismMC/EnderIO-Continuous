...
public final class LavaGenConfig {
    ...
    public static final IValue<Integer> heatLossActive = F.make("heatLossActive", 3, //
            "The amount of heat lost per tick when actively cooled with a cold fluid block. Note that only one neighbor is checked per tick.")
            .setRange(0, 64).sync();

    public static final IValue<Integer> maxHeatFactor = F.make("maxHeatFactor", 8, //
            "The maximum heat that can be reached, as factor of the time it takes to process one bucket of lava.")
            .setRange(1, 64).sync();
    ...
    public static final IValue<Integer> outputAmount = F.make("outputAmount", 1000, //
            "The amount of lava (in mB) needed to generate one block of cobble/stone/obsidian.").setRange(1, 64000)
            .sync();

    public static final IValue<Boolean> cobbleEnabled = F.make("outputCobbleEnabled", true, //
            "When enabled, cooled down lava will turn into cobble. The type of output depends on the type of cooling.")
            .sync();

    public static final IValue<Boolean> stoneEnabled = F.make("outputStoneEnabled", true, //
            "When enabled, cooled down lava will turn into stone. The type of output depends on the type of cooling.")
            .sync();

    public static final IValue<Boolean> obsidianEnabled = F.make("outputObsidianEnabled", true, //
            "When enabled, cooled down lava will turn into obsidian. The type of output depends on the type of cooling.")
            .sync();
}
