...
public final class FarmConfig {
    ...
    public static final IValue<Integer> farmBonemealDelaySuccess = F.make("farmBonemealDelaySuccess", 16, //
            "The delay (in blocks) between a successful bonemealing and the next try.").setRange(0, Integer.MAX_VALUE)
            .sync();

    public static final IValue<Integer> farmBonemealDelayFail = F.make("farmBonemealDelayFail", 4, //
            "The delay (in blocks) between an unsuccessful bonemealing and the next try.")
            .setRange(0, Integer.MAX_VALUE).sync();
    ...
    public static final IValue<Integer> farmHarvestEnergyUse = F.make("farmHarvestEnergyUse", 500, //
            "The amount of energy harvesting a block with anything else but an axe uses.")
            .setRange(0, Integer.MAX_VALUE).sync();
    ...
    public static final IValue<Boolean> enableCarefulCare = F.make("enableCarefulCare", true, //
            "If enabled, the farming area will receive some additional growth ticks. Disabling this can improve performance on budy servers")
            .sync();

    public static final IValue<Integer> waterTankSize = F.make("waterTankSize", 2000, //
            "The size of the water tank in mB. Setting this to 0 disables the tank and forces waterPerFarmland/waterCarefulCare to 0.")
            .setRange(0, 64000).sync();
    ...
    public static final IValue<Float> rainWaterChance = F.make("rainWaterChance", 1f, //
            "The chance that rain will flow into the tank. Set to 0 to disable. (Note: Vanilla Cauldron=0.05)")
            .setRange(0, 1).sync();

    public static final IValue<Integer> rainWaterAmount = F.make("rainWaterAmount", 100, //
            "The amount of water in mB that flows into the tankl when it rains. (Note: Vanilla Cauldron=333)")
            .setRange(0, 1000).sync();
