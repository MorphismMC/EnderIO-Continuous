...
public final class SolarConfig {
    ...
    public static final NNList<IValue<Integer>> upgradeGen = new NNList<>(//
            F.make("darkSteelSolar0Gen", 10, //
                    "Energy generated per SECOND by the Simple Solar upgrade. Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar1Gen", 40, //
                    "Energy generated per SECOND by the Solar I upgrade. Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar2Gen", 80, //
                    "Energy generated per SECOND by the Solar II upgrade. Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar3Gen", 160, //
                    "Energy generated per SECOND by the Solar III upgrade. Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar4Gen", 160 * 6, //
                    "Energy generated per SECOND by the Solar IV upgrade (Endergy). Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar5Gen", 160 * 6 * 6, //
                    "Energy generated per SECOND by the Solar V upgrade (Endergy). Split between all equipped DS armors.")
                    .setMin(1).sync(),
            F.make("darkSteelSolar6Gen", 160 * 6 * 6 * 6, //
                    "Energy generated per SECOND by the Solar VI upgrade (Endergy). Split between all equipped DS armors.")
                    .setMin(1).sync());
    ...
    public static final IValue<Boolean> solarPoweredBySunshine = F.make("solarPoweredBySunshine", true, //
            "When enabled Photovoltaic Panels accept Liquid Sunshine directly above them as source of sunlight.")
            .sync();
}
