...
public class SagMillRecipeCategory extends BlankRecipeCategory<SagRecipe> {
    ...
    public static void register() {
        RecipeWrapperIRecipe.setLevelData(SagRecipe.class, MachinesPlugin.iGuiHelper, 173 - xOff, yOff + 1,
                "textures/blocks/block_simple_sagmill_front.png",
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(new ItemStack(block_sag_mill.getBlockNN()),
                SagMillRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(new ItemStack(block_simple_sag_mill.getBlockNN()),
                SagMillRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(new ItemStack(block_enhanced_sag_mill.getBlockNN()),
                SagMillRecipeCategory.UID);
        ...
        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(
                new SagMillRecipeTransferHandler(MachinesPlugin.iModRegistry, SagMillRecipeCategory.UID,
                        FIRST_RECIPE_SLOT, NUM_RECIPE_SLOT, FIRST_INVENTORY_SLOT, NUM_INVENTORY_SLOT),
                SagMillRecipeCategory.UID);
        MachinesPlugin.iModRegistry.getRecipeTransferRegistry()
                .addRecipeTransferHandler(
                        new SimpleSagMillRecipeTransferHandler(MachinesPlugin.iModRegistry, SagMillRecipeCategory.UID,
                                FIRST_RECIPE_SLOT,
    ...
    public SagMillRecipeCategory(IGuiHelper guiHelper) {
        ...
        background = MachinesPlugin.iGuiHelper.drawableBuilder(backgroundLocation, xOff - 1, yOff, 109, 78)
                .addPadding(0, 0, 10, 30).build();
    ...
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull SagRecipe recipeWrapper,
        ...
        guiItemStacks.addTooltipCallback(new ITooltipCallback<ItemStack>() {
            ...
            @Override
            public void onTooltip(int slotIndex, boolean input, @Nonnull ItemStack ingredient,
                ...
                switch (slotIndex) {
                    ...
                    case 4:
                        if (slotIndex <= recipeWrapper.getRecipe().getOutputs().length) {
                            ...
                            if (chance > 0 && chance < 1) {
                                ...
                                String line = TextFormatting.GRAY +
                                        MessageFormat.format(Lang.JEI_SAGMILL_CHANCE.get(), chanceInt);
                    ...
                    case 5:
                        if (ballsTT.shouldHandleItem(ingredient)) {
                            ballsTT.addEntries(ingredient, tooltip,
                                    recipeWrapper.getBonusType().doMultiply() ? null : Lang.JEI_SAGMILL_NO_MAINS.get());
                        }
                        return;
                    default:
                        return;
