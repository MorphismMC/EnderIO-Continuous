...
public class EnchanterRecipeCategory extends BlankRecipeCategory<EnchanterRecipeCategory.EnchanterRecipeWrapper> {
    ...
    public static class EnchanterRecipeWrapper extends RecipeWrapperIMachineRecipe<EnchanterRecipe> {
        ...
        private static final @Nonnull ResourceLocation XP_ORB_TEXTURE = new ResourceLocation(
        ...
        private EnchanterRecipeWrapper(@Nonnull EnchanterRecipe rec, int level, @Nonnull NNList<ItemStack> bookInputs,
                                       @Nonnull NNList<ItemStack> lapisInputs,
        ...
        public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
            ...
            minecraft.fontRenderer.drawStringWithShadow("  " + Lang.GUI_VANILLA_REPAIR_COST.get(xpCost), 9, 33,
                    8453920);
            super.drawInfo(minecraft, recipeWidth, recipeHeight, mouseX, mouseY);
        }

        private @Nonnull NNList<MachineRecipeInput> getMachineInputs() {
            return new NNList<>(new MachineRecipeInput(0, bookInputs.get(0)),
                    new MachineRecipeInput(1, itemInputs.get(0)),
        ...
        public void getIngredients(@Nonnull IIngredients ingredients) {
            ingredients.setInputLists(VanillaTypes.ITEM,
                    new NNList<List<ItemStack>>(bookInputs, itemInputs, lapisInputs));
    ...
    public static void register() {
        ...
        RecipeWrapperBase.setLevelData(EnchanterRecipeWrapper.class, MachinesPlugin.iGuiHelper, 139 - xOff,
                66 - yOff - 5, null, null);
        ...
        MachinesPlugin.iModRegistry.addRecipeCatalyst(new ItemStack(block_enchanter.getBlockNN()),
                EnchanterRecipeCategory.UID);

        MachinesPlugin.iModRegistry.addRecipes(NullHelper.notnullJ(
                MachineRecipeRegistry.instance.getRecipesForMachine(MachineRecipeRegistry.ENCHANTER).values().stream()
                        .map(EnchanterRecipeWrapper::create).flatMap(Collection::stream).collect(Collectors.toList()),
                "Stream::collect"), UID);

        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(ContainerEnchanter.class,
                EnchanterRecipeCategory.UID, FIRST_RECIPE_SLOT, NUM_RECIPE_SLOT,
    ...
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout,
                          @Nonnull EnchanterRecipeCategory.EnchanterRecipeWrapper recipeWrapper,
