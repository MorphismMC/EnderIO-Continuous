...
public class ZombieGeneratorRecipeCategory extends
                                           BlankRecipeCategory<ZombieGeneratorRecipeCategory.ZombieGeneratorRecipeWrapper> {
    ...
    public static class ZombieGeneratorRecipeWrapper extends BlankRecipeWrapper {
        ...
        public void getIngredients(@Nonnull IIngredients ingredients) {
            ingredients.setInput(FluidStack.class,
                    new FluidStack(Fluids.NUTRIENT_DISTILLATION.getFluid(),
                            Math.round(ZombieGenConfig.minimumTankLevel.get() * tankCapacity)));
            ingredients.setOutputs(EnergyIngredient.class,
                    new NNList<>(
                            new EnergyIngredient(Math.round(CapacitorKey.ZOMBIE_POWER_GEN.getDefaultFloat()), true),
                            new EnergyIngredient(
                                    Math.round(CapacitorKey.ZOMBIE_POWER_GEN.get(DefaultCapacitorData.ENDER_CAPACITOR)),
                                    true)));
        }
        ...
        public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
            ...
            sw = fr.getStringWidth(txt);
            fr.drawStringWithShadow(txt, recipeWidth / 2 - sw / 2, 4 + 57 + fr.FONT_HEIGHT / 2,
    ...
    public static void register() {
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_zombie_generator.getBlockNN(), 1, 0),
                ZombieGeneratorRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_franken_zombie_generator.getBlockNN(), 1, 0),
                ZombieGeneratorRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeClickArea(GuiZombieGenerator.class, 155, 42, 16, 16,
                ZombieGeneratorRecipeCategory.UID);
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull ZombieGeneratorRecipeWrapper recipeWrapper,
        ...
        guiFluidStacks.addTooltipCallback(new ITooltipCallback<FluidStack>() {
            ...
            @Override
            public void onTooltip(int slotIndex, boolean input, @Nonnull FluidStack ingredient,
                                  @Nonnull List<String> tooltip) {
                if (slotIndex != 0)
                    return;
                tooltip.add(Lang.GUI_ZOMBGEN_MINREQ
                        .get(LangFluid.MB(Math.round(ZombieGenConfig.minimumTankLevel.get() * tankCapacity))));
            }
        });
