...
public class PainterRegistryPlugin implements IRecipeRegistryPlugin {
    ...
    private static final class WrapperSorter implements Comparator<PainterRecipeWrapper> {
        ...
        public int compare(PainterRecipeWrapper o1, PainterRecipeWrapper o2) {
            int compare = Integer.compare(Item.getIdFromItem(o1.target.getItem()),
                    Item.getIdFromItem(o2.target.getItem()));
            if (compare == 0) {
                compare = Integer.compare(Item.getIdFromItem(o1.paints.get(0).getItem()),
                        Item.getIdFromItem(o2.paints.get(0).getItem()));
            }
            return compare;
        }
    }
    ...
    private boolean is(ItemStack stack0, ItemStack stack1) {
        return stack0.getItem() == stack1.getItem() &&
                (!stack0.getHasSubtypes() || stack0.getItemDamage() == OreDictionary.WILDCARD_VALUE ||
    ...
    PainterRegistryPlugin(@Nonnull IModRegistry registry) {
        ...
        Map<String, ? extends IMachineRecipe> recipes = MachineRecipeRegistry.instance
                .getRecipesForMachine(MachineRecipeRegistry.PAINTER);

        ITEM:
        for (ItemStack itemStack : validItems) {
            if (itemStack != null) {
    ...
    @Override
    public @Nonnull <T extends IRecipeWrapper, V> List<T> getRecipeWrappers(@Nonnull IRecipeCategory<T> recipeCategory,
                        ...
                        for (IMachineRecipe rec : recipes.values()) {
                            if (rec instanceof AbstractPainterTemplate<?>) {
                                ...
                                if (recipe.isValidTarget(target)) {
                                    PainterRecipeWrapper wrapper = new PainterRecipeWrapper(recipe, target,
                                            new ArrayList<ItemStack>(), new ArrayList<ItemStack>());
                            ...
                            if (rec instanceof AbstractPainterTemplate<?>) {
                                ...
                                for (ItemStack target : VALID_TARGETS) {
                                    if (!is(target, paint) && target != null && recipe.isRecipe(paint, target)) {
                                        list2.add(new PainterRecipeWrapper(recipe, target, paint,
                                                recipe.getCompletedResult(paint, target)));
                                    }
                                }
                            }
                        }
                        ...
                        for (IMachineRecipe rec : recipes.values()) {
                            if (rec instanceof AbstractPainterTemplate<?>) {
                                ...
                                if (recipe.isRecipe(paint, target)) {
                                    list1.add(new PainterRecipeWrapper(recipe, target, paint,
                                            recipe.getCompletedResult(paint, target)));
                                }
                            }
                        }
                ...
                for (ItemStack target : VALID_TARGETS) {
                    if (target != null && recipe.isValidTarget(target)) {
                        for (ItemStack paint : SHORT_PAINTS) {
                            if (!is(target, paint) && paint != null && recipe.isRecipe(paint, target)) {
                                list.add(new PainterRecipeWrapper(recipe, target, paint,
                                        recipe.getCompletedResult(paint, target)));
                            }
                        }
                    }
