...
public class SagMillGuiItemStackGroup implements IGuiItemStackGroup {
    ...
    public @Nonnull Map<Integer, ? extends IGuiIngredient<ItemStack>> getGuiIngredients() {
        final Map<Integer, ? extends IGuiIngredient<ItemStack>> guiIngredients = new HashMap<>(
                itemStacks.getGuiIngredients());
        guiIngredients.remove(5);
        return guiIngredients;
    }

    @Override
    public void init(int slotIndex, boolean input, @Nonnull IIngredientRenderer<ItemStack> ingredientRenderer,
                     int xPosition, int yPosition, int width,
