...
public class WeatherObeliskRecipeCategory extends
                                          BlankRecipeCategory<WeatherObeliskRecipeCategory.WeatherObeliskRecipeWrapper> {
    ...
    public static class WeatherObeliskRecipeWrapper extends BlankRecipeWrapper {
        ...
        public WeatherObeliskRecipeWrapper(@Nonnull ItemStack itemInput, @Nonnull FluidStack fluidInput, int energy,
    ...
    public static void register() {
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_weather_obelisk.getBlockNN()), WeatherObeliskRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeClickArea(GuiWeatherObelisk.class, 155, 42, 16, 16,
                WeatherObeliskRecipeCategory.UID);
        ...
        result.add(new WeatherObeliskRecipeWrapper(new ItemStack(Items.FIREWORKS),
                new FluidStack(TileWeatherObelisk.WeatherTask.CLEAR.getRequiredFluidAmountType(),
                        TileWeatherObelisk.WeatherTask.CLEAR.getRequiredFluidAmount()),
                TileWeatherObelisk.WeatherTask.CLEAR.getRequiredFluidAmount() /
                        CapacitorKey.WEATHER_POWER_FLUID_USE.getDefault() * CapacitorKey.WEATHER_POWER_USE.getDefault(),
                "weather_sun", MachinesPlugin.iGuiHelper));

        result.add(new WeatherObeliskRecipeWrapper(new ItemStack(Items.FIREWORKS),
                new FluidStack(TileWeatherObelisk.WeatherTask.RAIN.getRequiredFluidAmountType(),
                        TileWeatherObelisk.WeatherTask.RAIN.getRequiredFluidAmount()),
                TileWeatherObelisk.WeatherTask.RAIN.getRequiredFluidAmount() /
                        CapacitorKey.WEATHER_POWER_FLUID_USE.getDefault() * CapacitorKey.WEATHER_POWER_USE.getDefault(),
                "weather_rain", MachinesPlugin.iGuiHelper));

        result.add(new WeatherObeliskRecipeWrapper(new ItemStack(Items.FIREWORKS),
                new FluidStack(TileWeatherObelisk.WeatherTask.STORM.getRequiredFluidAmountType(),
                        TileWeatherObelisk.WeatherTask.STORM.getRequiredFluidAmount()),
                TileWeatherObelisk.WeatherTask.STORM.getRequiredFluidAmount() /
                        CapacitorKey.WEATHER_POWER_FLUID_USE.getDefault() * CapacitorKey.WEATHER_POWER_USE.getDefault(),
        ...
        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(ContainerWeatherObelisk.class,
                WeatherObeliskRecipeCategory.UID, 0, 1, 1, 4 * 9);

        Log.info(
                String.format("WeatherObeliskRecipeCategory: Added %d weather changing recipes to JEI in %.3f seconds.",
                        result.size(), (end - start) / 1000000000d));
    }
    ...
    public WeatherObeliskRecipeCategory(IGuiHelper guiHelper) {
        ...
        flame = guiHelper.createAnimatedDrawable(guiHelper.createDrawable(backgroundLocation, 176, 0, 14, 33), 200,
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull WeatherObeliskRecipeWrapper recipeWrapper,
