...
public class VatRecipeCategory extends BlankRecipeCategory<VatRecipeCategory.VatRecipeWrapper> {
        ...
        public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
            ...
            if (currentLayout != null) {
                ...
                if (in0stack != null && fluidIn != null) {
                    final RecipeMatch match = ((VatRecipe) recipe).matchRecipe(fluidIn, in0stack,
                            in1Stack != null ? in1Stack : Prep.getEmpty());
                    if (match != null) {
                        fluidStacks.set(2, match.in);
                        fluidStacks.set(3, match.out);
                        minecraft.fontRenderer.drawString("x" + match.r0.getMulitplier(), 54 - xOff, 31 - yOff,
                                0x808080);
                        minecraft.fontRenderer.drawString("x" + match.r1.getMulitplier(), 104 - xOff, 31 - yOff,
                                0x808080);
                    }
                }
            }
    ...
    public static void register() {
        ...
        RecipeWrapperIRecipe.setLevelData(VatRecipeWrapper.class, MachinesPlugin.iGuiHelper, -1, 73 - yOff - 5, null,
                null);
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(new ItemStack(block_vat.getBlockNN()),
                VatRecipeCategory.UID);

        MachinesPlugin.iModRegistry.addRecipes(VatRecipeManager.getInstance().getRecipes(), UID);

        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(ContainerVat.class,
                VatRecipeCategory.UID, FIRST_RECIPE_SLOT, NUM_RECIPE_SLOT,
    ...
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout,
                          @Nonnull VatRecipeCategory.VatRecipeWrapper recipeWrapper,
        ...
        fluidStacks.init(2, true, inTankBounds.x, inTankBounds.y, inTankBounds.width, inTankBounds.height, 8000, false,
                null);
        fluidStacks.init(3, false, outTankBounds.x, outTankBounds.y, outTankBounds.width, outTankBounds.height, 8000,
                false, null);
