...
public class AlloyRecipeCategory extends BlankRecipeCategory<AlloyRecipeCategory.AlloyRecipeWrapper> {
    ...
    public static void register() {
        ...
        if (!PersonalConfig.enableAlloySmelterAlloyingJEIRecipes.get() &&
                !PersonalConfig.enableAlloySmelterFurnaceJEIRecipes.get()) {
            return;
        }

        MachinesPlugin.iModRegistry.addRecipeCategories(new AlloyRecipeCategory(MachinesPlugin.iGuiHelper));

        setModes(MachinesPlugin.iModRegistry, AlloySmelterConfig.profileSimpleAlloy.get(), GuiAlloySmelter.Simple.class,
                ContainerAlloySmelter.Simple.class,
                block_simple_alloy_smelter.getBlockNN(), 1);
        setModes(MachinesPlugin.iModRegistry, AlloySmelterConfig.profileSimpleFurnace.get(),
                GuiAlloySmelter.Furnace.class, ContainerAlloySmelter.Furnace.class,
                block_simple_furnace.getBlockNN(), 1);
        setModes(MachinesPlugin.iModRegistry, AlloySmelterConfig.profileNormal.get(), GuiAlloySmelter.Normal.class,
                ContainerAlloySmelter.Normal.class,
                block_alloy_smelter.getBlockNN(), 0);
        setModes(MachinesPlugin.iModRegistry, AlloySmelterConfig.profileEnhancedAlloy.get(),
                GuiAlloySmelter.Enhanced.class, ContainerAlloySmelter.Enhanced.class,
                block_enhanced_alloy_smelter.getBlockNN(), 0);

        long start = System.nanoTime();

        RecipeWrapperIRecipe.setLevelData(AlloyRecipeWrapper.class, MachinesPlugin.iGuiHelper, 137 - xOff, yOff + 1,
                "textures/blocks/block_alloy_smelter_simple_front.png",
        ...
        Log.info(String.format("AlloyRecipeCategory: Added %d alloy smelter recipes to JEI in %.3f seconds.",
                result.size(), (end - start) / 1000000000d));
    }

    private static void setModes(IModRegistry registry, @Nonnull AlloySmelterConfig.Profile profile,
                                 @Nonnull Class<? extends GuiContainer> gui,
        ...
        for (int i = 0; i < UIDS.length; i++) {
            registry.getRecipeTransferRegistry().addRecipeTransferHandler(container, NullHelper.first(UIDS[i]),
                    FIRST_RECIPE_SLOT, NUM_RECIPE_SLOT,
    ...
    public AlloyRecipeCategory(IGuiHelper guiHelper) {
        ...
        background = MachinesPlugin.iGuiHelper.drawableBuilder(backgroundLocation, xOff, yOff, 82, 78)
                .addPadding(0, 0, 0, 30).build();
    ...
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout,
                          @Nonnull AlloyRecipeCategory.AlloyRecipeWrapper recipeWrapper,
