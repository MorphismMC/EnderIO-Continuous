...
public class StirlingRecipeCategory extends BlankRecipeCategory<StirlingRecipeCategory.StirlingRecipeWrapper> {
    ...
    public static class StirlingRecipeWrapper extends BlankRecipeWrapper {
        ...
        private StirlingRecipeWrapper(@Nonnull NNList<ItemStack> solidFuel, @Nonnull IGuiHelper guiHelper) {
            ...
            if (!simpleFuel(solidFuel.get(0))) {
                ResourceLocation stirlingFrontLocation = new ResourceLocation(EnderIO.DOMAIN,
                        "textures/blocks/block_stirling_gen_simple_front_off.png");
        ...
        public void getIngredients(@Nonnull IIngredients ingredients) {
            ...
            ICapacitorKey minKey = simpleFuel(solidFuel.get(0)) ? CapacitorKey.SIMPLE_STIRLING_POWER_GEN :
                    CapacitorKey.STIRLING_POWER_GEN;

            double minEnergyProducedPerTick = minKey.getFloat(DefaultCapacitorData.BASIC_CAPACITOR);
            double maxEnergyProducedPerTick = CapacitorKey.STIRLING_POWER_GEN
                    .getFloat(DefaultCapacitorData.ENDER_CAPACITOR);

            double minEnergyProduced = minEnergyProducedPerTick *
                    TileStirlingGenerator.getBurnTime(solidFuel.get(0), minKey, DefaultCapacitorData.BASIC_CAPACITOR);
            double maxEnergyProduced = maxEnergyProducedPerTick * TileStirlingGenerator.getBurnTime(solidFuel.get(0),
                    CapacitorKey.STIRLING_POWER_GEN, DefaultCapacitorData.ENDER_CAPACITOR);
        ...
        public @Nonnull List<String> getTooltipStrings(int mouseX, int mouseY) {
            if (stirlingFront != null && mouseX >= (121 - xOff) && mouseX <= (121 - xOff + 32) && mouseY >= 32 - yOff &&
                    mouseY <= 32 - yOff + 32) {
    ...
    public static void register() {
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_stirling_generator.getBlockNN(), 1, 0), StirlingRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_simple_stirling_generator.getBlockNN(), 1, 0),
                StirlingRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeClickArea(GuiStirlingGenerator.class, 155, 42, 16, 16,
                StirlingRecipeCategory.UID);
        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(
                ContainerStirlingGenerator.Normal.class, StirlingRecipeCategory.UID, 0, 1, 2, 4 * 9);
        MachinesPlugin.iModRegistry.getRecipeTransferRegistry().addRecipeTransferHandler(
                ContainerStirlingGenerator.Simple.class, StirlingRecipeCategory.UID, 0, 1, 1, 4 * 9);
        ...
        Log.info(String.format(
                "StirlingRecipeCategory: Added %d stirling generator recipes for %d solid fuels to JEI in %.3f seconds.",
                recipeList.size(),
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull StirlingRecipeWrapper recipeWrapper,
