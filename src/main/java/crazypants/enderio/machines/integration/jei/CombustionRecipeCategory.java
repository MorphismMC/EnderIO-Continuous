...
public class CombustionRecipeCategory extends BlankRecipeCategory<CombustionRecipeCategory.CombustionRecipeWrapper> {
    ...
    public static class CombustionRecipeWrapper extends BlankRecipeWrapper {
        ...
        private CombustionRecipeWrapper(FluidStack fluidCoolant, FluidStack fluidFuel, CombustionMath mathMin,
                                        CombustionMath mathMax) {
        ...
        public void getIngredients(@Nonnull IIngredients ingredients) {
            ...
            ingredients.setOutputs(EnergyIngredient.class,
                    new NNList<>(new EnergyIngredient(mathMin.getEnergyPerTick(), true),
                            new EnergyIngredient(mathMax.getEnergyPerTick(), true)));
        }
    ...
    public static void register() {
        ...
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_combustion_generator.getBlockNN(), 1, 0),
                CombustionRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeCategoryCraftingItem(
                new ItemStack(MachineObject.block_enhanced_combustion_generator.getBlockNN(), 1, 0),
                CombustionRecipeCategory.UID);
        MachinesPlugin.iModRegistry.addRecipeClickArea(GuiCombustionGenerator.class, 155, 42, 16, 16,
                CombustionRecipeCategory.UID);
        ...
        for (Fluid fluid1 : fluids.values()) {
            ...
            if (coolant != null) {
                for (Fluid fluid2 : fluids.values()) {
                    ...
                    if (fuel != null) {
                        CombustionMath mathMin = new CombustionMath(coolant, fuel,
                                CapacitorKey.COMBUSTION_POWER_GEN.getFloat(DefaultCapacitorData.BASIC_CAPACITOR),
                                CapacitorKey.COMBUSTION_POWER_EFFICIENCY.get(DefaultCapacitorData.BASIC_CAPACITOR));
                        CombustionMath mathmax = new CombustionMath(coolant, fuel,
                                CapacitorKey.COMBUSTION_POWER_GEN.getFloat(DefaultCapacitorData.ENDER_CAPACITOR),
                                CapacitorKey.ENHANCED_COMBUSTION_POWER_EFFICIENCY
                                        .get(DefaultCapacitorData.ENDER_CAPACITOR));
                        result.add(new CombustionRecipeWrapper(new FluidStack(fluid1, 1000),
                                new FluidStack(fluid2, 1000), mathMin, mathmax));
                    }
                }
            }
        }
        ...
        Log.info(
                String.format("CombustionRecipeCategory: Added %d combustion generator recipes to JEI in %.3f seconds.",
