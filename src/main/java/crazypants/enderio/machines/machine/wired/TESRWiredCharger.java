...
public class TESRWiredCharger<T extends TileWiredCharger> extends ManagedTESR<T> {
    ...
    protected void renderItemStack(@Nonnull EnumFacing facing, @Nonnull BlockPos pos, @Nonnull ItemStack floatingItem,
                                   @Nonnull World world, float tick) {
        ...
        int j = i % 65536;
        int k = (int) (180 + (60 *
                MathHelper.sin((float) ((EnderIO.proxy.getTickCount() * 0.145D + (tick * 0.145D) + rand.nextDouble()) %
                        (Math.PI * 2)))));
    ...
    public void doRender(@Nonnull World world, @Nonnull ItemStack itemstack, double rot) {
        ...
        GlStateManager.pushMatrix();
        IBakedModel ibakedmodel = Minecraft.getMinecraft().getRenderItem().getItemModelWithOverrides(itemstack, world,
                null);
