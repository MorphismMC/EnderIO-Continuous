...
public abstract class ContainerWiredCharger extends AbstractMachineContainer<TileWiredCharger> {
    ...
    @Nonnull
    public static ContainerWiredCharger create(@Nonnull InventoryPlayer playerInv, @Nonnull TileWiredCharger te,
                                               int baubleSlots) {
    ...
    public void addGhostslots(NNList<GhostSlot> ghostSlots) {
        ...
        ghostSlots.removeAllByClass(GhostBackgroundItemSlot.class); // JEI will cause initGui to be re-run after closing
                                                                    // the recipe view, causing duplicate ghost
                                                                    // slots
    ...
    public static NNList<Triple<ItemStack, ItemStack, Integer>> getValidPair(List<ItemStack> validItems) {
        ...
        for (ItemStack stack : validItems) {
            try {
                ...
                Log.LOGGER.error("An itemstack (" + stack +
                        ") threw an exception during energy detection. This is a bug in that mod!", e);
            }
        }
        return result;
    }
    ...
    private boolean mergeItemStackIntoArmor(EntityPlayer entityPlayer, ItemStack origStack, int slotIndex) {
        if (origStack == null ||
                EntityLiving.getSlotForItemStack(origStack).getSlotType() != EntityEquipmentSlot.Type.ARMOR) {
            return false;
        }
