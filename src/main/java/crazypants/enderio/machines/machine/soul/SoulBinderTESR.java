...
public class SoulBinderTESR extends ManagedTESR<TileSoulBinder> {
    ...
    @Override
    protected void renderTileEntity(@Nonnull TileSoulBinder te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
        ...
        renderBlockModel(te.getWorld(), te.getPos(),
                blockState.withProperty(EnumRenderMode.RENDER, renderMode.rotate(te.getFacing())), true,
    ...
    public static void renderBlockModel(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
                                        boolean translateToOrigin, boolean relight) {
        ...
        for (BlockRenderLayer layer : BlockRenderLayer.values()) {
            if (state.getBlock().canRenderInLayer(state, layer)) {
                ...
                blockrendererdispatcher.getBlockModelRenderer().renderModel(worldWrapper, ibakedmodel, state, pos, wr,
                        false);
            }
        }
    ...
    /**
     * Wraps the world so that all sides of a block have the same light value. Needed when rotating the model around the
     * z axis. Otherwise quads facing a wall
     * would be black.
     * <p>
     * We cheat a bit here by pretending the block is surrounded by air blocks. That's so much easier than to compute 6
     * virtual blocks from all surrounding
     * blocks.
     *
     */
    private static class WorldWrapper extends IBlockAccessWrapper {
        ...
        public WorldWrapper(@Nonnull World world, @Nonnull BlockPos pos) {
            ...
            maxSky = maxBlk = 0;
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
                maxSky = Math.max(maxSky,
                        world.getLightFromNeighborsFor(EnumSkyBlock.SKY, pos.offset(face).up().offset(face.rotateY())));
                maxBlk = Math.max(maxBlk, world.getLightFromNeighborsFor(EnumSkyBlock.BLOCK,
                        pos.offset(face).up().offset(face.rotateY())));
            }
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
            ...
            maxSky = maxBlk = 0;
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
                maxSky = Math.max(maxSky, world.getLightFromNeighborsFor(EnumSkyBlock.SKY,
                        pos.offset(face).down().offset(face.rotateY())));
                maxBlk = Math.max(maxBlk, world.getLightFromNeighborsFor(EnumSkyBlock.BLOCK,
                        pos.offset(face).down().offset(face.rotateY())));
            }
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
            ...
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
                maxSky = Math.max(maxSky,
                        world.getLightFromNeighborsFor(EnumSkyBlock.SKY, pos.offset(face).offset(face.rotateY())));
                maxBlk = Math.max(maxBlk,
                        world.getLightFromNeighborsFor(EnumSkyBlock.BLOCK, pos.offset(face).offset(face.rotateY())));
            }
            for (EnumFacing face : EnumFacing.Plane.HORIZONTAL) {
