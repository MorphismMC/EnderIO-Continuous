...
public class ChannelTab implements ITabPanel {
    ...
    public ChannelTab(@Nonnull GuiTransceiver guiTransceiver, @Nonnull ChannelType type) {
        ...
        privateButton = new ToggleButton(parent, PRIVATE_BUTTON_ID, 118, 12, IconEIO.LOCK_UNLOCKED,
                IconEIO.LOCK_LOCKED);
    ...
    public void render(float partialTick, int mouseX, int mouseY) {
        ...
        int x = left + 59;
        int y = top + 36;
        parent.drawCenteredString(parent.getFontRenderer(), Lang.GUI_TRANS_AVAILABLE.get(), x, y,
                ColorUtil.getRGB(Color.white));

        x = left + 199;
        parent.drawCenteredString(parent.getFontRenderer(), Lang.GUI_TRANS_SEND.get(), x, y,
                ColorUtil.getRGB(Color.white));

        y += 56;
        parent.drawCenteredString(parent.getFontRenderer(), Lang.GUI_TRANS_RECEIVE.get(), x, y,
                ColorUtil.getRGB(Color.white));
    }
