...
public enum TransceiverRegistry {
    ...
    public @Nonnull IFluidTankProperties[] getTankInfoForChannels(@Nullable TileTransceiver tileTransceiver,
                                                                  @Nonnull Set<Channel> channelsIn) {
                    ...
                    if (trans != from) {
                        ...
                        if (val > 0) {
                            if (doFill && TranceiverConfig.bucketEnergyCost.get() > 0) {
                                int powerUsed = (int) Math.max(1,
                                        TranceiverConfig.bucketEnergyCost.get() * val / 1000d);
    ...
    public void sendItem(@Nonnull TileTransceiver from, @Nonnull Set<Channel> channelsIn, int slot,
    ...
    @Nonnull
    private ItemStack sendItem(@Nonnull TileTransceiver from, int slot, @Nonnull ItemStack contents,
                               @Nonnull TileTransceiver to) {
