...
public class TileAttractor extends AbstractMobObeliskEntity {
    ...
    private void cleanTrackedEntities() {
        ...
        while (iterator.hasNext()) {
            ...
            if (next.getKey().isDead || !canAttract(next.getKey())) {
                ...
                PacketSpawnParticles.create(next.getKey(), EnumParticleTypes.PORTAL, EnumParticleTypes.PORTAL,
    ...
    private void collectEntities() {
        for (EntityLiving entity : world.getEntitiesWithinAABB(EntityLiving.class, getBounds())) {
            if (!entity.isDead && !tracking.containsKey(entity) && canAttract(entity) &&
                    PermissionAPI.hasPermission(getOwner().getAsGameProfile(), BlockAttractor.permissionAttracting,
                            new TargetContext(getTarget(), entity))) {
        ...
        if (!FAILED_MOBS.contains(key)) {
            for (IMobAttractionHandler handler : AttractionHandlers.instance.getRegistry()) {
                ...
                if (state == State.CAN_ATTRACT) {
                    ...
                    PacketSpawnParticles.create(ent, EnumParticleTypes.HEART, EnumParticleTypes.PORTAL,
                            EnumParticleTypes.PORTAL, EnumParticleTypes.PORTAL,
            ...
            EntityPlayerMP player = FMLServerHandler.instance().getServer().getPlayerList()
                    .getPlayerByUUID(getOwner().getAsGameProfile().getId());
            if (player != null) {
    ...
    private class Target extends FakePlayerEIO {
        ...
        public Target(World world) {
            super(world, getLocation(), new GameProfile(null,
                    MachineObject.block_attractor_obelisk.getUnlocalisedName() + ":" + getLocation()));
            posY += 1;
        }
    }
