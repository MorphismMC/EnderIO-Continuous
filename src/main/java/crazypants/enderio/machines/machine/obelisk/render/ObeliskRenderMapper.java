...
public class ObeliskRenderMapper extends MachineRenderMapper {
    ...
    public List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
        ...
        boolean isActive = tileEntity instanceof AbstractMachineEntity ?
                ((AbstractMachineEntity) tileEntity).isActive() : true;

        quadCollector.addQuads(null, BlockRenderLayer.CUTOUT,
                ObeliskBakery.bake(
                        isActive ? ObeliskRenderManager.getActiveTextures() : ObeliskRenderManager.getTextures()));
