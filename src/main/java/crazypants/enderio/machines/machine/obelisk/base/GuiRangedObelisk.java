...
public class GuiRangedObelisk extends GuiInventoryMachineBase<AbstractRangedObeliskEntity> {
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (!getTileEntity().canWork()) {
            drawCenteredString(fontRenderer, Lang.GUI_OBELISK_NO_VIALS.get(), width / 2 + 9, sy + 68,
                    ColorUtil.getRGB(Color.red));
        } else {
            int range = (int) getTileEntity().getRange();
            drawCenteredString(fontRenderer, Lang.GUI_RANGE.get(range), width / 2 + 9, sy + 68,
