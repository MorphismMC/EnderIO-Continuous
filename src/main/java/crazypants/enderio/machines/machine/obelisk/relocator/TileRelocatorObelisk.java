    ...
    protected void processTasks(boolean redstoneCheck) {
        if (!relocationQueue.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (mob == null || mob.isDead || world.getEntityByID(mob.getEntityId()) == null ||
                        mob.ticksExisted > 2 * 60 * 20 || relocationQueue.size() > 100) {
                    iterator.remove();
                    ...
                    if (targetBB.intersects(mobbb)) {
                        ...
                        boolean spaceClear = world.checkNoEntityCollision(bb, mob) &&
                                world.getCollisionBoxes(mob, bb).isEmpty() && (world.containsAnyLiquid(bb) ==
                                        mob.isCreatureType(EnumCreatureType.WATER_CREATURE, false));

                        if (spaceClear) {
                            PacketSpawnParticles.create(mob, EnumParticleTypes.PORTAL, EnumParticleTypes.PORTAL,
    ...
    public void setShowRange(boolean showRange) {
        ...
        if (showingRange) {
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new RangeParticle<AbstractRangedObeliskEntity>(this, color) {
