...
public class TileExperienceObelisk extends AbstractInventoryMachineEntity implements IHaveExperience, ITankAccess {
    ...
    private final @Nonnull ExperienceContainer xpCont = new ExperienceContainer(
            XpUtil.getExperienceForLevelL(XPObeliskConfig.maxLevels.get()));
    ...
    protected boolean doPush(@Nullable EnumFacing dir) {
        ...
        if (dir != null && xpCont.getFluidAmountL() > 0) {
            if (FluidWrapper.transfer(xpCont, world, getPos().offset(dir), dir.getOpposite(),
                    ExperienceConfig.maxIO.get()) > 0) {
                setTanksDirty();
                return true;
            }
