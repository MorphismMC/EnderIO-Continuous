...
public class ContainerExperienceObelisk extends Container {
    ...
    /*
     * Note that creative adds/removed levels relative to the obelisk while survival works relative to the player's
     * level.
     */
    ...
    public void doAddXP(int levels) {
        try {
            if (player.capabilities.isCreativeMode) {
                ...
                long diffxp = XpUtil.getExperienceForLevelL(containerLevel) -
                        XpUtil.getExperienceForLevelL(targetLevel);
    ...
    public void doDrainXP(int levels) {
        if (player.capabilities.isCreativeMode) {
            int containerLevel = inv.getContainer().getExperienceLevel();
            int targetLevel = Math.min(containerLevel + levels,
                    XpUtil.getLevelForExperience(inv.getContainer().getMaximumExperience()));
    ...
    // TODO: HL: Find out why this is unused before deleting it completely. I do not like finding unused code like this,
    // it smells like a bug that it's not
    // called...
