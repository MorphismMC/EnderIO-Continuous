...
public class TileWeatherObelisk extends AbstractPowerConsumerEntity
                                implements IProgressTile, ITankAccess.IExtendedTankAccess {
    ...
    public float getProgress() {
        return isActive() ? world.isRemote ? progress : (float) fluidUsed / getActiveTask().getRequiredFluidAmount() :
                0;
    }
    ...
    private void doLoadingParticles() {
        if (progress < 0.9f) {
            ...
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new EntityFluidLoadingFX(world, pos1.getX() + offset + correction, yi,
                            pos1.getZ() + offset + correction, xf, yf, zf, c));
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new EntityFluidLoadingFX(world, pos1.getX() + (1 - offset) + correction, yi,
                            pos1.getZ() + offset + correction, xf, yf, zf, c));
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new EntityFluidLoadingFX(world, pos1.getX() + (1 - offset) + correction, yi,
                            pos1.getZ() + (1 - offset) + correction, xf, yf, zf, c));
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new EntityFluidLoadingFX(world, pos1.getX() + offset + correction, yi,
                            pos1.getZ() + (1 - offset) + correction, xf, yf, zf, c));
        } else if (!playedFuse) {
            world.playSound(pos.getX(), pos.getY(), pos.getZ(), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1,
                    1, true);
            playedFuse = true;
        }
    }
        ...
        if (!redstoneCheck) {
            ...
            if (isActive()) {
                if (getEnergyStored() > getPowerUsePerTick() && !inputTank.isEmpty()) {
                    ...
                    fluidUsed += inputTank
                            .removeFluidAmount(Math.min(CapacitorKey.WEATHER_POWER_FLUID_USE.get(getCapacitorData()),
                                    getActiveTask().getRequiredFluidAmount() - fluidUsed));
                }
    ...
    public boolean canStartTask(WeatherTask task) {
        return task != null && getActiveTask() == null && !WeatherTask.worldIsState(task, world) &&
                inputTank.getFluidAmount() >= task.getRequiredFluidAmount() &&
                task == WeatherTask.fromFluid(inputTank.getFluidNN().getFluid()) &&
                (world.isRemote || Prep.isValid(getStackInSlot(0)));
    }
