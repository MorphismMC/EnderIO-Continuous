...
public abstract class TileSagMill extends AbstractPoweredTaskEntity implements IPaintable.IPaintableTileEntity {
    ...
    public static class Simple extends TileSagMill {
        ...
        public Simple() {
            super(new SlotDefinition(2, 4, 0), SIMPLE_SAG_MILL_POWER_INTAKE, SIMPLE_SAG_MILL_POWER_BUFFER,
                    SIMPLE_SAG_MILL_POWER_USE);
    ...
    public static class Enhanced extends TileSagMill {
        ...
        public Enhanced() {
            super(new SlotDefinition(2, 4), ENHANCED_SAG_MILL_POWER_INTAKE, ENHANCED_SAG_MILL_POWER_BUFFER,
                    ENHANCED_SAG_MILL_POWER_USE);
        ...
        protected boolean shouldDoubleTick(@Nonnull IPoweredTask task, int usedEnergy) {
            double chance = getCapacitorData().getUnscaledValue(ENHANCED_SAG_MILL_DOUBLE_OP_CHANCE) *
    ...
    protected TileSagMill(@Nonnull SlotDefinition slotDefinition, @Nonnull ICapacitorKey maxEnergyRecieved,
    ...
    @Override
    public boolean isMachineItemValidForSlot(int i, @Nonnull ItemStack itemstack) {
        return Prep.isValid(itemstack) && SagMillRecipeManager.getInstance().isValidInput(getMachineLevel(),
                new MachineRecipeInput(i, itemstack));
    }

    public int getBallDurationScaled(int scale) {
        return MathHelper
                .clamp((grindingBallDurabilityMax > 0) ?
                        (int) (scale * (1 - ((float) grindingBallDurabilityUsed / (float) grindingBallDurabilityMax))) :
                        0, 0, scale);
    }
    ...
    protected IPoweredTask createTask(@Nonnull IMachineRecipe nextRecipe, long nextSeed) {
        ...
        if (grindingBall != null && nextRecipe.getBonusType(getRecipeInputs()).doChances()) {
            res = new PoweredTask(nextRecipe, nextSeed, grindingBall.getGrindingMultiplier(),
                    grindingBall.getChanceMultiplier(), getRecipeInputs());
