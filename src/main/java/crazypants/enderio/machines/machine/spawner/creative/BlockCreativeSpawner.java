...
public class BlockCreativeSpawner extends AbstractCapabilityMachineBlock<TileCreativeSpawner>
    ...
    public void init(@Nonnull IModObject modObject, @Nonnull FMLInitializationEvent event) {
        ...
        permissionNodeWrenching = PermissionAPI.registerNode(
                EnderIO.DOMAIN + ".wrench.break." + modObject.getUnlocalisedName(), DefaultPermissionLevel.OP,
        ...
        permissionNodeIOWrenching = PermissionAPI.registerNode(
                EnderIO.DOMAIN + ".wrench.iomode." + modObject.getUnlocalisedName(), DefaultPermissionLevel.OP,
    ...
    @Override
    public @Nonnull ContainerEnderCap<EnderInventory, TileCreativeSpawner> getServerGuiElement(@Nonnull EntityPlayer player,
                                                                                               @Nonnull World world,
                                                                                               @Nonnull BlockPos pos,
                                                                                               @Nullable EnumFacing facing,
                                                                                               int param1,
                                                                                               @Nonnull TileCreativeSpawner te) {
        return param1 == 0 ? new ContainerCreativeSpawner(player.inventory, te) :
                new ContainerCreativeSpawnerUser(player.inventory, te);
    }
    ...
    public @Nullable GuiScreen getClientGuiElement(@Nonnull EntityPlayer player, @Nonnull World world,
        ...
        return new GuiCreativeSpawner(player.inventory, te,
                getServerGuiElement(player, world, pos, facing, param1, te));
    }
