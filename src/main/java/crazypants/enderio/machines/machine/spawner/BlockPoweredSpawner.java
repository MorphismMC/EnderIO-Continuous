...
public class BlockPoweredSpawner extends AbstractPoweredTaskBlock<TilePoweredSpawner>
                                 implements IAdvancedTooltipProvider, IPaintable.INonSolidBlockPaintableBlock,
    ...
    public static BlockPoweredSpawner create(@Nonnull IModObject modObject) {
        MachineRecipeRegistry.instance.registerRecipe(MachineObject.block_powered_spawner.getUnlocalisedName(),
                new DummyRecipe());
    ...
    protected BlockPoweredSpawner(@Nonnull IModObject modObject) {
        ...
        try {
            fieldpersistenceRequired = ReflectionHelper.findField(EntityLiving.class, "persistenceRequired",
                    "field_82179_bU");
        } catch (Exception e) {
    ...
    public void handleAnvilEvent(AnvilUpdateEvent evt) {
        if (Prep.isInvalid(evt.getLeft()) || evt.getLeft().getCount() != 1 ||
                evt.getLeft().getItem() != block_powered_spawner.getItemNN()) {
            return;
        }
        if (Prep.isInvalid(evt.getRight()) || evt.getRight().getCount() != 1 ||
                evt.getRight().getItem() != ModObject.itemBrokenSpawner.getItemNN()) {
            return;
        }
    ...
    public void addCommonEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
        ...
        if (mob != null) {
            ...
            list.add(Lang.TOOLTIP_SPAWNER_COST.get(LangPower.FLOAT_NF
                    .format(EntityDataRegistry.getInstance().getCostMultiplierFor(mob.getEntityName()))));
        } else {
    ...
    public PoweredBlockItem createBlockItem(@Nonnull IModObject modObject) {
        return modObject.apply(new PoweredBlockItem(this) {
            ...
            @Override
            public @Nonnull EnumActionResult onItemUse(@Nonnull EntityPlayer player, @Nonnull World worldIn,
