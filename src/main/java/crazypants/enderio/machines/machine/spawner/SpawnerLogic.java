...
public class SpawnerLogic {
    ...
    public boolean isAreaClear(int spawnRangeXZ, int spawnRangeY, int amount) {
        if (amount > 0) {
            ...
            if (capturedMob != null) {
                return isAreaClear(spawner.getSpawnerWorld(), capturedMob.getEntityClass(), spawnRangeXZ, spawnRangeY,
                        amount);
            }
        }
        return true;
    ...
    private boolean isAreaClear(World world, Class<? extends Entity> entityClass, int spawnRangeXZ, int spawnRangeY,
        ...
        if (amount > 0 && entityClass != null) {
            if (world.getEntitiesWithinAABB(entityClass,
                    new BoundingBox(spawner.getSpawnerPos()).expand(spawnRangeXZ, spawnRangeY, spawnRangeXZ),
    ...
    Entity createEntity(DifficultyInstance difficulty, boolean forceAlive) {
        ...
        if (forceAlive && SpawnerConfig.poweredSpawnerMaxPlayerDistance.get() <= 0 &&
                SpawnerConfig.poweredSpawnerDespawnTimeSeconds.get() > 0 && ent instanceof EntityLiving) {
            ent.getEntityData().setLong(BlockPoweredSpawner.KEY_SPAWNED_BY_POWERED_SPAWNER,
                    spawner.getSpawnerWorld().getTotalWorldTime());
