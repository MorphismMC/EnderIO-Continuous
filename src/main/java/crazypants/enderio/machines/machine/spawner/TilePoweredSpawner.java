...
public class TilePoweredSpawner extends AbstractPoweredTaskEntity
    ...
    public int getPowerUsePerTick() {
        ...
        return (int) (super.getPowerUsePerTick() *
                (entityName == null ? 0 : EntityDataRegistry.getInstance().getCostMultiplierFor(entityName)));
    }

    @Override
    public int getMaxEnergyRecieved(EnumFacing dir) {
        final ResourceLocation entityName = getEntityName();
        return (int) (super.getMaxEnergyRecieved(dir) *
                (entityName == null ? 0 : EntityDataRegistry.getInstance().getCostMultiplierFor(entityName)));
    }
    ...
    protected IMachineRecipe canStartNextTask(long nextSeed) {
        ...
        if (isSpawnMode) {
            if (SpawnerConfig.poweredSpawnerMaxPlayerDistance.get() > 0) {
                BlockPos p = getPos();
                if (world.getClosestPlayer(p.getX() + 0.5, p.getX() + 0.5, p.getX() + 0.5,
                        SpawnerConfig.poweredSpawnerMaxPlayerDistance.get(), false) == null) {
    ...
    protected IPoweredTask createTask(@Nonnull IMachineRecipe nextRecipe, long nextSeed) {
        ...
        if (isSpawnMode) {
            ticksDelay = SpawnerConfig.poweredSpawnerMinDelayTicks.get() +
                    (int) Math.round((SpawnerConfig.poweredSpawnerMaxDelayTicks.get() -
                            SpawnerConfig.poweredSpawnerMinDelayTicks.get()) * Math.random());
        } else {
            ticksDelay = SpawnerConfig.poweredSpawnerMaxDelayTicks.get() -
                    ((SpawnerConfig.poweredSpawnerMaxDelayTicks.get() -
                            SpawnerConfig.poweredSpawnerMinDelayTicks.get()) / 2);
        }
