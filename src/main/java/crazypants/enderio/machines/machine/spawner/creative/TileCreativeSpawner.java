...
public class TileCreativeSpawner extends AbstractCapabilityMachineEntity
                                 implements IPaintable.IPaintableTileEntity, ISpawnerCallback,
    ...
    protected void processTasks(boolean redstoneCheck) {
        ...
        if (slot.isEmpty()) {
            ...
            if (SpawnerConfig.homeZoneEnabled.get() && !logic.isAreaClear(SpawnerConfig.homeZoneSize.get(),
                    SpawnerConfig.homeZoneSize.get(), SpawnerConfig.homeZoneLimit.get())) {
                return;
            }
        } else {
