...
public class PoweredSpawnerSpecialRenderer extends ManagedTESR<TilePoweredSpawner> {
    ...
    @Override
    protected void renderTileEntity(@Nonnull TilePoweredSpawner te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
    ...
    private void renderMob(TilePoweredSpawner te, float partialTicks) {
        ...
        if (entity != null) {
            ...
            GlStateManager.rotate(
                    (float) (te.getPrevMobRotation() + (te.getMobRotation() - te.getPrevMobRotation()) * partialTicks) *
                            10.0F,
                    0.0F, 1.0F, 0.0F);
            GlStateManager.translate(0.0F, -0.2F, 0.0F);
            GlStateManager.rotate(-30.0F, 1.0F, 0.0F, 0.0F);
            GlStateManager.scale(f, f, f);
            Minecraft.getMinecraft().getRenderManager().renderEntity(entity, 0.0D, 0.0D, 0.0D, 0.0F, partialTicks,
                    false);
