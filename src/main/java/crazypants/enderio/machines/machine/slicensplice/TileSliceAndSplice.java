...
public class TileSliceAndSplice extends AbstractPoweredTaskEntity implements IPaintable.IPaintableTileEntity {
    ...
    private @Nonnull EntityLivingBase getFakePlayer() {
        if (fakePlayer == null) {
            fakePlayer = FakePlayerFactory.getMinecraft(
                    FMLCommonHandler.instance().getMinecraftServerInstance().getWorld(world.provider.getDimension()));
        }
        return fakePlayer;
    }
    ...
    public boolean isMachineItemValidForSlot(int slot, @Nonnull ItemStack itemstack) {
        ...
        if (SliceAndSpliceRecipeManager.getInstance().isValidInput(getMachineLevel(),
            ...
            RECIPES:
            for (IManyToOneRecipe oneRecipe : SliceAndSpliceRecipeManager.getInstance()
