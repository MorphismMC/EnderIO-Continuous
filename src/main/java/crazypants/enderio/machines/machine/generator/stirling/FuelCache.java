 ...
 * <p>
 * Creating a list of all items and then getting the burn time for all of them can take a very long time---it involves
 * many items and sub-items and throws an
 * event for every single one. This cache makes sure that that is only done one.
 * <p>
 * {@link #initialize(Collection)} can take a list of all items that has already been prepared by e.g. JEI. When it is
 * not used, the first access to
...
public class FuelCache {
    ...
    /**
     * 
     * @return A list of all items that have a burn time. (Or at least had one the first time the cache was initialized.
     *         Mods are free to randomly change the burn
