...
@Storable
public class TileStirlingGenerator extends AbstractGeneratorEntity
                                   implements IProgressTile, IPaintable.IPaintableTileEntity {
    ...
    public static class Simple extends TileStirlingGenerator {
        ...
        public boolean isMachineItemValidForSlot(int i, @Nonnull ItemStack itemstack) {
            return super.isMachineItemValidForSlot(i, itemstack) &&
                    Prep.isInvalid(itemstack.getItem().getContainerItem(itemstack));
        }
    }
    ...
    protected TileStirlingGenerator(@Nonnull SlotDefinition slotDefinition, @Nonnull ICapacitorKey maxEnergyStored,
        ...
        addICap(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
                ICap.facedOnly(facingIn -> new LegacyStirlingWrapper(this, facingIn)));
    }
    ...
    public static int getBurnTime(@Nonnull ItemStack item, @Nonnull ICapacitorKey maxUsage,
                                  @Nonnull ICapacitorData data) {
        float base = (getBurnTimeGeneric(item) / (maxUsage.get(data) / maxUsage.getDefaultFloat())) *
                getBurnEfficiency(data);
                ...
                if (burnTime > 0) {
                    ...
                    if (Prep.isValid(containedItem)) {
                        if (Prep.isInvalid(fuelStack)) {
                            ...
                            world.spawnEntity(new EntityItem(world, pos.getX() + .5, pos.getY() + .5, pos.getZ() + .5,
                                    containedItem));
                        }
                    }
                    markDirty();
