...
public class ContainerLavaGenerator<T extends TileLavaGenerator>
                                   extends ContainerEnderCap<EnderInventory, TileLavaGenerator> {
    ...
    protected void addSlots() {
        ...
        addSlotToContainer(
                new EnderSlot(Type.OUTPUT, getItemHandler(), TileLavaGenerator.OUTPUT_OBS, 134, 54 - 18 - 18));
    }
    ...
    public void createGhostSlots(List<GhostSlot> slots) {
        ...
        if (slot != null) {
            slots.add(new GhostBackgroundItemSlot(
                    new ItemStack(cobbleEnabled.get() ? Blocks.COBBLESTONE : Blocks.BARRIER), slot));
        }
        slot = inventorySlots.get(2);
        if (slot != null) {
            slots.add(new GhostBackgroundItemSlot(new ItemStack(stoneEnabled.get() ? Blocks.STONE : Blocks.BARRIER),
                    slot));
        }
        slot = inventorySlots.get(3);
        if (slot != null) {
            slots.add(new GhostBackgroundItemSlot(
                    new ItemStack(obsidianEnabled.get() ? Blocks.OBSIDIAN : Blocks.BARRIER), slot));
        }
    }
}
