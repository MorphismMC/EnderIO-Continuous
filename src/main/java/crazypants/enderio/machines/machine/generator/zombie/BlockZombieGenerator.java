...
public class BlockZombieGenerator<T extends TileZombieGenerator> extends AbstractGeneratorBlock<T>
                                 implements IHaveTESR {

    public static final @Nonnull TextureSupplier textureHead1 = TextureRegistry
            .registerTexture("blocks/zombie_gen_head");
    public static final @Nonnull TextureSupplier textureHead2 = TextureRegistry
            .registerTexture("blocks/zombie_gen_head2");
    public static final @Nonnull TextureSupplier textureHeadEnder1 = TextureRegistry
            .registerTexture("blocks/ender_gen_head");
    public static final @Nonnull TextureSupplier textureHeadEnder2 = TextureRegistry
            .registerTexture("blocks/ender_gen_head2");

    private static final double px = 1d / 16d;
    public static final @Nonnull AxisAlignedBB AABB = new AxisAlignedBB(2 * px, 0 * px, 2 * px, 14 * px, 16 * px,
            14 * px);
    ...
    private static class BlockFrankNZombieGenerator extends
                                                    BlockZombieGenerator<TileZombieGenerator.TileFrankenZombieGenerator> {
            ...
            if (te != null && te.isActive()) {
                if (PersonalConfig.machineParticlesEnabled.get()) {
                    for (int i = 0; i < 2; i++) {
                        ...
                        BubbleFX fx = new BubbleFX(world, pos.getX() + xOffset, pos.getY() + yOffset,
                                pos.getZ() + zOffset, 0, 0.5, 0);
                        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                    }
                }

                if (ClientConfig.machineSoundsEnabled.get()) {
                    SoundHelper.playSound(world, pos, SoundHelper.BLOCK_TOP, SoundRegistry.ZOMBIE_BUBBLE,
                            ClientConfig.machineSoundVolume.get() * 0.045f,
    ...
    @SideOnly(Side.CLIENT)
    public void bindTileEntitySpecialRenderer() {
        ClientRegistry.bindTileEntitySpecialRenderer(TileZombieGenerator.class,
                new ZombieGeneratorRenderer(MachineObject.block_zombie_generator.getBlockNN()));
    }
}
