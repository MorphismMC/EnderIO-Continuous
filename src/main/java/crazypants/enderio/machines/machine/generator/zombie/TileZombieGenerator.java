...
public class TileZombieGenerator extends AbstractGeneratorEntity
                                 implements ITankAccess.IExtendedTankAccess, IHasNutrientTank {

    public static class TileFrankenZombieGenerator extends TileZombieGenerator {

        public TileFrankenZombieGenerator() {
            super(new SlotDefinition(0, 0, 1), CapacitorKey.FRANK_N_ZOMBIE_POWER_BUFFER,
                    CapacitorKey.FRANK_N_ZOMBIE_POWER_GEN);
    ...
    protected TileZombieGenerator(@Nonnull SlotDefinition slotDef, @Nonnull ICapacitorKey buffer,
                                  @Nonnull ICapacitorKey gen) {
    ...
    private boolean generateEnergy() {
        ...
        if (inPause && getEnergyStored() >= (getMaxEnergyStored() - (getPowerUsePerTick() * 200)) &&
