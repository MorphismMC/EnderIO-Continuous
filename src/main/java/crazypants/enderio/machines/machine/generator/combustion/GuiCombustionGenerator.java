...
public class GuiCombustionGenerator<T extends TileCombustionGenerator> extends GuiInventoryMachineBase<T> {
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        fr.drawStringWithShadow(txt, guiLeft + xSize / 2 - sw / 2, guiTop + fr.FONT_HEIGHT / 2 + 3,
        ...
        if (!coolantTank.isEmpty()) {
            ...
            if (fluidAmount > CENTER) {
                RenderUtil.renderGuiTank(fluid, coolantTank.getCapacity() - CENTER, fluidAmount - CENTER, x, y, zLevel,
                        15, 47);
            }
            ...
            if (gen.isActive()) {
                ...
                fr.drawStringWithShadow(txt, x - sw / 2 + 7, y + fr.FONT_HEIGHT / 2 + 47,
                        ColorUtil.getRGB(Color.WHITE));
            }

            // center coolant chamber
            RenderUtil.renderGuiTank(fluid, CENTER, Math.min(fluidAmount, CENTER), guiLeft + 72, guiTop + 28, zLevel,
                    33, 33);
        ...
        if (!fuelTank.isEmpty()) {
            ...
            if (fluidAmount > CENTER) {
                RenderUtil.renderGuiTank(fluid, fuelTank.getCapacity() - CENTER, fluidAmount - CENTER, x, y, zLevel, 15,
                        47);
            }
            ...
            // center tank
            RenderUtil.renderGuiTank(fluid, CENTER, Math.min(fluidAmount, CENTER), guiLeft + 81, guiTop + 38, zLevel,
                    14, 14);
        }
