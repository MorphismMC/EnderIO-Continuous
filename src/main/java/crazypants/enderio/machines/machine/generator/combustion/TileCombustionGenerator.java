...
public class TileCombustionGenerator extends AbstractGeneratorEntity
    ...
    public static class Enhanced extends TileCombustionGenerator {
        ...
        public Enhanced() {
            super(new SlotDefinition(0, 0, 1), ENHANCED_COMBUSTION_POWER_SEND, ENHANCED_COMBUSTION_POWER_BUFFER,
                    ENHANCED_COMBUSTION_POWER_GEN);
    ...
    protected TileCombustionGenerator(@Nonnull SlotDefinition slotDefinition, @Nonnull ICapacitorKey maxEnergySent,
        ...
        coolantTank = new CoolantTank(
                Math.round(CombustionGenConfig.combGenTankSize.get() * getEfficiencyMultiplier()));
    ...
    private boolean generateEnergy() {
        ...
        // Check: We are full?
        if ((ticksRemaingCoolant <= 0 && getCoolantTank().isEmpty()) ||
                (ticksRemaingFuel <= 0 && getFuelTank().isEmpty()) || getEnergyStored() >= getMaxEnergyStored()) {
            return false;
        }
    ...
    CombustionMath getMath() {
        ...
        return new CombustionMath(curCoolant, curFuel, maxEnergyUsed.getFloat(getCapacitorData()),
