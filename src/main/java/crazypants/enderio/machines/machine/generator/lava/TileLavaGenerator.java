...
@Storable
public class TileLavaGenerator extends AbstractCapabilityGeneratorEntity
                               implements IPaintable.IPaintableTileEntity, ITankAccess.IExtendedTankAccess {
    ...
    @Override
    public void onNeighborBlockChange(@Nonnull IBlockState state, @Nonnull World worldIn, @Nonnull BlockPos posIn,
        ...
        if (redstoneCheck && !getEnergy().isFull() && getEnergy().hasCapacitor() && !isOutputFull()) {
            ...
            if (burnTime <= 0 && !tank.isEmpty()) {
                ...
                burnTime = getLavaBurntime() / Fluid.BUCKET_VOLUME /
                        CapacitorKey.LAVAGEN_POWER_FLUID_USE.get(getCapacitorData());
            ...
            if (world.isBlockLoaded(pos2)) {
                ...
                if (targetFluidHandler != null) {
                    ...
                    if (fluidStack != null && fluidStack.amount >= 0 &&
                            fluidStack.getFluid().getTemperature(fluidStack) < heatInKelvin) {
                        float factor = fluidStack.amount / 1000f;
                        heat = Math.max(0, heat - LavaGenConfig.heatLossActive.get());
                        if (fluidStack.getFluid() == FluidRegistry.WATER) {
                            if (heatInKelvin > C2K(100) &&
                                    world.rand.nextFloat() < LavaGenConfig.activeCoolingEvaporatesWater.get()) {
                                targetFluidHandler.drain(fluidStack.amount, true);
                                world.playSound(null, pos2.getX() + .5f, pos2.getY() + .5f, pos2.getZ() + .5f,
                                ...
                                for (int k = 0; k < 8; ++k) {
                                    effect.add(pos2.getX() + world.rand.nextDouble(),
                                            pos2.getY() + world.rand.nextDouble(),
                                            pos2.getZ() + world.rand.nextDouble(), 1,
                                            EnumParticleTypes.SMOKE_LARGE);
                                }
                                effect.send(world, pos2);
                                stonePoints += MathHelper.floor(LavaGenConfig.heatLossActive.get() * factor); // water,
                                                                                                              // consumed
                            } else {
                                cobblePoints += MathHelper.floor(LavaGenConfig.heatLossActive.get() * factor); // water,
                                                                                                               // not
                                                                                                               // consumed
                            }
                        } else if (fluidStack.getFluid().getTemperature(fluidStack) < C2K(0)) {
                            obsidianPoints += MathHelper.floor(LavaGenConfig.heatLossActive.get() * factor); // other
                                                                                                             // fluid,
                                                                                                             // very
                                                                                                             // cold
                        } else {
                            cobblePoints += MathHelper.floor(LavaGenConfig.heatLossActive.get() * factor); // other
                                                                                                           // fluid,
                                                                                                           // luke warm
                        }
                    }
                ...
                } else if (block instanceof BlockFrostedIce || block instanceof BlockPackedIce ||
                            ...
                            if (world.rand.nextFloat() < LavaGenConfig.activeCoolingLiquefiesIce.get()) {
                                if (world.provider.doesWaterVaporize()) {
                                    ...
                                    world.playSound(null, pos2.getX() + .5f, pos2.getY() + .5f, pos2.getZ() + .5f,
                                            ...
                                            2.6F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);
                                    for (int k = 0; k < 8; ++k) {
                                        PacketSpawnParticles.create(world, pos2.getX() + world.rand.nextDouble(),
                                                pos2.getY() + world.rand.nextDouble(),
                                                pos2.getZ() + world.rand.nextDouble(), 1,
                                                EnumParticleTypes.SMOKE_LARGE);
                                    }
                                    ...
                                    for (int k = 0; k < 4; ++k) {
                                        PacketSpawnParticles.create(world, pos2.getX() + world.rand.nextDouble(),
                                                pos2.getY() + 1, pos2.getZ() + world.rand.nextDouble(), 1,
                                                EnumParticleTypes.SMOKE_LARGE);
                                    }
                                    Blocks.WATER.neighborChanged(Blocks.WATER.getDefaultState(), world, pos2, block,
                                            pos2);
                                }
                            }
                        }
    ...
    private int getLavaBurntime() {
        return LavaGenConfig.useVanillaBurnTime.get() ? 20000 :
                TileEntityFurnace.getItemBurnTime(new ItemStack(Items.LAVA_BUCKET));
    }
    ...
    private boolean mergeOutput(@Nonnull String slotName, @Nonnull ItemStack stack) {
        ...
        if (Prep.isInvalid(oldStack)) {
            ...
            oldStack
                    .grow(stack.splitStack(
                            Math.min(Math.min(oldStack.getMaxStackSize(), slot.getMaxStackSize()) - oldStack.getCount(),
                                    stack.getCount()))
                            .getCount());
            slot.set(oldStack);
