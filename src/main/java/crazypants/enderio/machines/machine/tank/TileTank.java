...
@Storable
public class TileTank extends AbstractInventoryMachineEntity
    ...
    public boolean isMachineItemValidForSlot(int i, @Nonnull ItemStack item) {
        ...
        if (i == 0) {
            ...
            return voidMode == VoidMode.ALWAYS ||
                    (voidMode == VoidMode.NEVER ? false : !FluidUtil.isFluidContainer(item));
        }
        return false;
    }
    ...
    /*
     * Limit sending of client updates because a group of tanks pushing into each other can severely kill the client's
     * fps.
     */
    ...
    public void updateLight() {
        final FluidStack fluid = tank.getFluid();
        int thisFluidLuminosity = fluid == null || fluid.getFluid() == null || tank.isEmpty() ? 0 :
                fluid.getFluid().getLuminosity(fluid);
    ...
    private void voidItems() {
        ...
        if (Prep.isValid(stack) && canVoidItems()) {
            if (!tank.isFull() && tank.hasFluid(FluidRegistry.LAVA) &&
                    TankConfig.smeltTrashIntoLava.get().smelt(stack)) {
                tank.addFluidAmount((int) MathHelper.clamp(world.rand.nextGaussian() * .75 + 3.5, 1, 10)); // 49% for 3,
                                                                                                           // 22%: for 2
                                                                                                           // and 4,
                                                                                                           // 2.2% for 1
                                                                                                           // and 5
            ...
            SoundHelper.playSound(world, pos, SoundHelper.BLOCK_CENTER, SoundRegistry.ITEM_BURN, 0.05F,
    ...
    private boolean mendItem() {
        ...
        int damageMendable = Math.min(xpToDurability(XpUtil.liquidToExperience(tank.getFluidAmount())),
                getStackInSlot(input).getItemDamage());
        if (damageMendable < 1) {
            return false;
        }
