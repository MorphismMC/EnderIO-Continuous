...
public class ReservoirRenderer extends ManagedTESR<TileReservoirBase> {
    ...
    @Override
    protected void renderTileEntity(@Nonnull TileReservoirBase te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
    ...
    @Override
    public void renderTileEntityFast(@Nonnull TileReservoirBase te, double x, double y, double z, float partialTicks,
    ...
    private BoundingBox mkFace(@Nonnull boolean[][][] merge, @Nonnull EnumFacing face, float fullness, float fullness2,
                               boolean gas) {
        // TODO: implement gas
    ...
    protected NNList<RenderFace> computeGeometry(@Nonnull SmartTank tank, @Nonnull World world, @Nonnull BlockPos pos) {
            ...
            for (int dy = 0; dy < 3; dy++) {
                for (int dz = 0; dz < 3; dz++) {
                    ...
                    if (!merge[dx][dy][dz] && world.getBlockState(pos2).getBlock() == block) {
                        final TileReservoirBase reservoir = BlockReservoirBase.getAnyTileEntity(world, pos2,
                                TileReservoirBase.class);
                        if (reservoir != null) {
        ...
        final float r = ((color >> 16) & 0xFF) / 255f, g = ((color >> 8) & 0xFF) / 255f, b = (color & 0xFF) / 255f,
    ...
    private static final class RenderFacesFast implements Callback<ReservoirRenderer.RenderFace> {
        ...
        public void apply(@Nonnull RenderFace e) {
            for (Vertex v : e.bb.getCornersWithUvForFace(e.face, e.minU, e.maxU, e.minV, e.maxV)) {
                ...
                buffer.pos(v.x(), v.y(), v.z()).color(e.r, e.g, e.b, e.a).tex(v.u(), v.v()).lightmap(e.light1, e.light2)
                        .endVertex();
            }
        }
    }
    ...
    private static final class RenderFaces implements Callback<ReservoirRenderer.RenderFace> {
        ...
        public void apply(@Nonnull RenderFace e) {
            for (Vertex v : e.bb.getCornersWithUvForFace(e.face, e.minU, e.maxU, e.minV, e.maxV)) {
                ...
                buffer.pos(v.x(), v.y(), v.z()).tex(v.u(), v.v()).color(e.r, e.g, e.b, e.a)
    ...
    private static final class RenderFace {
        ...
        protected RenderFace(@Nonnull BoundingBox bb, @Nonnull EnumFacing face, float minU, float maxU, float minV,
                             float maxV, float r, float g, float b, float a,
