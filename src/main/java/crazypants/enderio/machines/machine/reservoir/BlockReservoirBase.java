...
public abstract class BlockReservoirBase extends BlockEio<TileReservoirBase>
                                         implements IResourceTooltipProvider, ISmartRenderAwareBlock, IHaveTESR {
    ...
    private static class BlockOmniReservoir extends BlockReservoirBase {
        ...
        public void bindTileEntitySpecialRenderer() {
            ClientRegistry.bindTileEntitySpecialRenderer(TileReservoirBase.TileOmniReservoir.class,
                    new ReservoirRenderer(this));
        }
    ...
    protected BlockReservoirBase(@Nonnull IModObject modObject) {
        ...
        setDefaultState(getBlockState().getBaseState().withProperty(EnumMergingBlockRenderMode.RENDER,
                EnumMergingBlockRenderMode.AUTO));
    ...
    protected void init() {
        super.init();
        SmartModelAttacher.register(this, EnumMergingBlockRenderMode.RENDER, EnumMergingBlockRenderMode.DEFAULTS,
                EnumMergingBlockRenderMode.AUTO);
    }
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
        ...
        if (!entityPlayer.isSneaking()) {
            ...
            if (tank != null) {
                ...
                if (tank.getTank().getAvailableSpace() >= Fluid.BUCKET_VOLUME &&
                        FluidUtil.fillInternalTankFromPlayerHandItem(world, pos, entityPlayer, hand, tank)) {
                    return true;
                }
                if (!tank.getTank().isFull() && FluidUtil.fillInternalTankFromPlayerHandItem(world, pos, entityPlayer,
                        hand, new ReservoirTankWrapper(tank, world, pos, allowFluidVoiding()))) {
                    return true;
                }
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public boolean shouldSideBeRendered(@Nonnull IBlockState bs, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                                        @Nonnull EnumFacing side) {
        return !(world.getBlockState(pos.offset(side)).getBlock() == this) && !world.getBlockState(pos.offset(side))
                .doesSideBlockRendering(world, pos.offset(side), side.getOpposite());
    }
