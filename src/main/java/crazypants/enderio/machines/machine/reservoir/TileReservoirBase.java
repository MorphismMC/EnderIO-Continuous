...
public abstract class TileReservoirBase extends TileEntityEio implements ITankAccess.IExtendedTankAccess {
    ...
    public static class TileReservoir extends TileReservoirBase {
        ...
        protected void doInfiniteSource() {
            ...
            } else if (canRefill && !tank.isFull() && shouldDoWorkThisTick(WORK_TICKS * 2, -1) &&
                    tank.getFluid() != null) {
    ...
    protected void doPush(int ticks) {
        if (!tank.isEmpty()) {
            for (NNIterator<EnumFacing> itr = NNList.FACING.fastIterator(); itr.hasNext() && !tank.isEmpty();) {
                ...
                if (world.getBlockState(neighbor).getBlock() != blockType &&
                        FluidWrapper.transfer(tank, world, neighbor, dir.getOpposite(), IO_MB_TICK * ticks) > 0) {
                    setTanksDirty();
                }
            }
        }
    ...
    protected void doLeak() {
        ...
        if (doLeak(down)) {
            for (NNIterator<EnumFacing> itr = NNList.FACING_HORIZONTAL.fastIterator(); itr.hasNext() &&
                    !tank.isEmpty(); doLeak(down.offset(itr.next()))) {}
        }
    }
