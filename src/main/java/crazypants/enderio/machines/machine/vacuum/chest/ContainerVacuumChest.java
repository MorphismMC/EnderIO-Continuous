...
public class ContainerVacuumChest extends ContainerEnderCap<EnderInventory, TileVacuumChest>
    ...
    protected void addSlots() {
        ...
        int x = 8;
        int y = 18;
        for (EnderSlot slot : EnderSlot.create(getItemHandler(), EnderInventory.Type.OUTPUT, x, y,
                TileVacuumChest.ITEM_COLS, TileVacuumChest.ITEM_ROWS)) {
            if (slot != null) {
    ...
    public void createGhostSlots(List<GhostSlot> slots) {
        slots.add(new GhostBackgroundItemSlot(BasicFilterTypes.filterUpgradeBasic.getBasicFilterStack(),
                NullHelper.notnull(filterSlot, "slot awol")));
    }
