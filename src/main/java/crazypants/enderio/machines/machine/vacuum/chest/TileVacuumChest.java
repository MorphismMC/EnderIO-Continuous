...
public class TileVacuumChest extends AbstractCapabilityMachineEntity
                             implements IRedstoneModeControlable, IPaintable.IPaintableTileEntity, IRanged,
        TileFilterContainer, IHasFillLevel {

    private static PredicateItemStack PREDICATE_FILTER = new PredicateItemStack() {

        @Override
        public boolean doApply(@Nonnull ItemStack input) {
            return input.getItem() instanceof ItemFilterItemUpgrade;
        }
    };
    ...
    public TileVacuumChest() {
        ...
        getInventory().add(EnderInventory.Type.UPGRADE, "filter",
                new InventorySlot(PREDICATE_FILTER, null, CALLBACK_FILTER, 1));
                ...
                for (int chunkY = minChunkYClamped; chunkY <= maxChunkYClamped; ++chunkY) {
                    for (Entity entity : entityLists[chunkY]) {
                        if (!entity.isDead && (entity instanceof EntityItem) &&
                                ...
                                (filter == null || filter.doesItemPassFilter(null, ((EntityItem) entity).getItem())) &&
                                MagnetUtil.shouldAttract(getPos(), entity, true)) {
    ...
    @Override
    public Filter getFilter(int filterIndex, int param1) {
        return filter;
    }
