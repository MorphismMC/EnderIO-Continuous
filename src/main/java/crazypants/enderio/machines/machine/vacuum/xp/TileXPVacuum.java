...
@Storable
public class TileXPVacuum extends TileEntityEio
                          implements Predicate<EntityXPOrb>, IPaintable.IPaintableTileEntity, IRanged, ITankAccess {

    private static final int IO_MB_TICK = 10000; // no need to slow down the vacuum any more than necessary, let the
                                                 // limit be the piping
    ...
    private final @Nonnull Callback<EnumFacing> push_callback = new Callback<EnumFacing>() {
        ...
        public void apply(@Nonnull EnumFacing dir) {
            if (xpCon.getFluidAmount() > 0 &&
                    FluidWrapper.transfer(xpCon, world, getPos().offset(dir), dir.getOpposite(), IO_MB_TICK) > 0) {
                setTanksDirty();
            }
        }
    };
