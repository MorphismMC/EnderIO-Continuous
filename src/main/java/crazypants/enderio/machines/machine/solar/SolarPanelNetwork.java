...
public class SolarPanelNetwork implements ISolarPanelNetwork {
                ...
                if (panel != null && world.isBlockLoaded(panel)) {
                    ...
                    if (tileEntity instanceof TileSolarPanel && !tileEntity.isInvalid() && tileEntity.hasWorld()) {
                        if (((TileSolarPanel) tileEntity).network == this) {
                            for (NNIterator<EnumFacing> facings = NNList.FACING_HORIZONTAL.fastIterator(); facings
                                    .hasNext();) {
    ...
    /**
     * If different kinds of panels can not joint together, this is the amount of energy any panel of this network can
     * produce per tick.
     * 
            ...
            if (tick > nextCollectTick) {
                ...
                for (BlockPos panel : panels) {
                    if (panel != null && world.isBlockLoaded(panel)) {
                        ...
                        energyAvailablePerTick += MathHelper.floor(energyMaxPerTickPerPanel *
                                TileSolarPanel.calculateLocalLightRatio(world, panel, lightRatio));
                    }
                }
            }
    ...
    public NNList<ITextComponent> getConduitProbeInformation(@Nonnull EntityPlayer player, @Nullable EnumFacing side) {
        ...
        result.add(
                Lang.PROBE_SOLAR_PROD.toChatServer(panels.size(), getEnergyAvailablePerTick(), getEnergyMaxPerTick()));
        result.add(text);
        for (BlockPos panel : panels) {
