...
public class TileSolarPanel extends TileEntityEio implements ILegacyPoweredTile, IHasConduitProbeData {
    ...
    public TileSolarPanel() {
        addICap(CapabilityEnergy.ENERGY,
                facing -> facing == EnumFacing.DOWN ? InternalGeneratorTileWrapper.get(this, facing) : null);
    }
    ...
    public void doUpdate() {
        ...
        IPowerInterface receptor = PowerHandlerUtil
                .getPowerInterface(world.getTileEntity(getPos().offset(EnumFacing.DOWN)), EnumFacing.UP);
    ...
    static boolean isArtificiallyPowered(@Nonnull World world, BlockPos pos) {
        ...
        return SolarConfig.solarPoweredBySunshine.get() &&
                world.getBlockState(pos.up()).getBlock() == Fluids.LIQUID_SUNSHINE.getBlock();
    }
    ...
    static float calculateLocalLightRatio(@Nonnull World world, BlockPos pos, float baseRatio) {
        ...
        if (ratio < 1 && SolarConfig.solarPoweredBySunshine.get()) {
            ...
            if (blockState.getBlock() == Fluids.LIQUID_SUNSHINE.getBlock()) {
                ...
                float value = ((BlockFluidEio.LiquidSunshine) Fluids.LIQUID_SUNSHINE.getBlock())
                        .getScaledLevel(blockState, world, pos.up());
