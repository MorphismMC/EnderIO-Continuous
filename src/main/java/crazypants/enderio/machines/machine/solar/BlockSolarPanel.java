...
public class BlockSolarPanel extends BlockEio<TileSolarPanel>
                             implements IResourceTooltipProvider, ISmartRenderAwareBlock, ICustomSubItems {
    ...
    public void getSubBlocks(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> list) {
        ISolarType.KIND.getOrderedValues().stream()
                .forEach(solarType -> list.add(new ItemStack(BlockSolarPanel.this, 1,
                        ISolarType.getMetaFromType(NullHelper.notnull(solarType, "solarType")))));
    }
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public void randomDisplayTick(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
                                  @Nonnull Random rand) {
        if (PersonalConfig.machineParticlesEnabled.get() && state.getValue(SolarType.KIND).hasParticles() &&
                TileSolarPanel.isPowered(world, pos) &&
                TileSolarPanel.calculateLocalLightRatio(world, pos, TileSolarPanel.calculateLightRatio(world)) / 3 >
                        rand.nextFloat()) {
