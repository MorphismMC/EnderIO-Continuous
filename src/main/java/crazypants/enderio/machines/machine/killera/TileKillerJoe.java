...
@Storable
public class TileKillerJoe extends AbstractInventoryMachineEntity
                           implements ITankAccess.IExtendedTankAccess, IHasNutrientTank, IRanged {
    ...
    public TileKillerJoe() {
        ...
        addICap(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
                ICap.facedOnly(facingIn -> new LegacyKillerJoeWrapper(this, facingIn)));
    }

    // These weapons are tested to work and render correctly in the Killer Joe. That's why this is not in the config
    // file.
    public static final Things WEAPONS = new Things("tconstruct:hatchet", "tconstruct:mattock", "tconstruct:hammer",
            "tconstruct:lumberaxe", "tconstruct:scythe",
            "tconstruct:broadsword", "tconstruct:longsword", "tconstruct:rapier", "tconstruct:frypan",
            "tconstruct:cleaver", "minecraft:stick")
                    // for the ghost slot:
                    .add(Items.WOODEN_SWORD).add(Items.STONE_SWORD).add(Items.IRON_SWORD).add(Items.GOLDEN_SWORD)
                    .add(Items.DIAMOND_SWORD)
                    .add(ModObject.itemDarkSteelSword.getItemNN()).add(Items.WOODEN_AXE).add(Items.IRON_AXE)
    ...
    public boolean isMachineItemValidForSlot(int i, @Nonnull ItemStack itemstack) {
        return (itemstack.getItem() instanceof ItemSword || itemstack.getItem() instanceof ItemAxe ||
                WEAPONS.contains(itemstack)) && !TicProxy.isBroken(itemstack);
    }
    ...
    protected void processTasks(boolean redstoneCheck) {
        ...
        if (atackera.getTicksSinceLastSwing() < atackera.getCooldownPeriod()) {
            // System.out.println("Cannot attack yet, it's only been " + atackera.getTicksSinceLastSwing() + " tick(s)
            // but the cooldown is "
            // + atackera.getCooldownPeriod() + " ticks. The attack would be limited to " +
            // attackera.getCooledAttackStrength(0.5F) + " effectiveness.");
            return;
        }
        // System.out.println("Can attack now, it's been " + atackera.getTicksSinceLastSwing() + " tick(s) and the
        // cooldown is " + atackera.getCooldownPeriod()
        ...
        for (EntityLivingBase ent : entsInBounds) {
            if (!ent.isDead && ent.deathTime <= 0 && !ent.isEntityInvulnerable(DamageSource.GENERIC) &&
                ...
                if (!PermissionAPI.hasPermission(getOwner().getAsGameProfile(), BlockKillerJoe.permissionAttacking,
                        new TargetContext(atackera, ent))) {
                    continue;
                }
                try {
                    if (togglePvp) {
    ...
    private boolean canJoeSee(EntityLivingBase ent) {
        ...
        for (EnumFacing facing1 : new EnumFacing[] { facing, facing.rotateY(), facing.rotateYCCW() }) {
            if (this.world.rayTraceBlocks(new Vec3d(getPos().getX() + faceMidPoints[facing1.ordinal()][0],
                    getPos().getY() + faceMidPoints[facing1.ordinal()][1],
