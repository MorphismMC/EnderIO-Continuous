...
public class BlockKillerJoe extends AbstractInventoryMachineBlock<TileKillerJoe> implements IHaveTESR {
    ...
    public void init(@Nonnull IModObject mo, @Nonnull FMLInitializationEvent event) {
        super.init(mo, event);
        permissionAttacking = PermissionAPI.registerNode(
                EnderIO.DOMAIN + ".attack." + mo.getRegistryName().getPath().toLowerCase(Locale.ENGLISH),
                DefaultPermissionLevel.ALL,
                "Permission for the block " + mo.getUnlocalisedName() + " of Ender IO to attack entities." +
