...
public class KillerJoeRenderMapper extends MachineRenderMapper
    ...
    @Nonnull
    public static final KillerJoeRenderMapper killerJoe = new KillerJoeRenderMapper(BlockKillerJoe.textureHead1,
            BlockKillerJoe.textureHead2);

    @Nonnull
    public static final KillerJoeRenderMapper zombieGen = new KillerJoeRenderMapper(BlockZombieGenerator.textureHead1,
            BlockZombieGenerator.textureHead2);

    @Nonnull
    public static final KillerJoeRenderMapper enderGen = new KillerJoeRenderMapper(
            BlockZombieGenerator.textureHeadEnder1, BlockZombieGenerator.textureHeadEnder2);
    ...
    public List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
        ...
        if (blockLayer == BlockRenderLayer.TRANSLUCENT) {
            return Collections.singletonList(
                    state.getBlock().getDefaultState().withProperty(EnumRenderMode.RENDER, EnumRenderMode.FRONT_ON));
        } else if (blockLayer == BlockRenderLayer.SOLID) {
            quadCollector.addQuads(null, BlockRenderLayer.SOLID, renderHead(state));
            return Collections.singletonList(
                    state.getBlock().getDefaultState().withProperty(EnumRenderMode.RENDER, EnumRenderMode.FRONT));
        }
        return null;
    }
    ...
    private List<BakedQuad> renderHead(@Nullable IBlockStateWrapper state) {
        ...
        for (double angle : ROTS) {
            ...
            for (EnumFacing face : EnumFacing.values()) {
                buffer.add(bb1, face, (face.ordinal() + 1) * px, (face.ordinal() + 2) * px, 9 * px, 10 * px, tex2,
                        null);
                buffer.add(bb2, face, (face.ordinal() + 1) * px, (face.ordinal() + 2) * px, 10 * px, 11 * px, tex2,
                        null);
                buffer.add(bb3, face, (face.ordinal() + 1) * px, (face.ordinal() + 2) * px, 11 * px, 12 * px, tex2,
                        null);
            }
    ...
    public List<Pair<IBlockState, ItemStack>> mapItemRender(@Nonnull Block block, @Nonnull ItemStack stack,
        ...
        states.add(Pair.of(
                block.getStateFromMeta(stack.getMetadata()).withProperty(EnumRenderMode.RENDER, EnumRenderMode.FRONT),
                stack));
        if (!stack.hasTagCompound()) {
    ...
    public ItemQuadCollector mapItemDynamicOverlayRender(@Nonnull Block block, @Nonnull ItemStack stack) {
        if (stack.hasTagCompound()) {
            ...
            result.addBlockState(block.getStateFromMeta(stack.getMetadata()).withProperty(EnumRenderMode.RENDER,
                    EnumRenderMode.FRONT_ON), stack);
