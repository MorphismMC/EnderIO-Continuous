...
public class KillerJoeRenderer extends ManagedTESR<TileKillerJoe> {
    ...
    @Override
    protected void renderTileEntity(@Nonnull TileKillerJoe te, @Nonnull IBlockState blockState, float partialTicks,
                                    int destroyStage) {
        if (MinecraftForgeClient.getRenderPass() == 0) {
            renderSword(te.getFacing(), te.getWeapon(), te.getSwingProgress(partialTicks),
                    Minecraft.getMinecraft().player.getPrimaryHand() == EnumHandSide.LEFT);
    ...
    private void renderSword(@Nonnull EnumFacing facing, @Nonnull ItemStack sword, float swingProgress,
                             boolean leftHand) {
