...
public class FarmLogic implements IFarmer {
        ...
        public boolean apply(@Nonnull FarmSlots slot) {
            if (slot.isValid(owner, stack)) {
                ...
                if (Prep.isInvalid(slotStack)) {
                    ...
                    int addable = Math.max(0,
                            Math.min(slotStack.getMaxStackSize(), slot.getInventoryStackLimit(owner)) -
                                    slotStack.getCount());
    ...
    private static final @Nonnull GameProfile FARMER_PROFILE = new GameProfile(
            UUID.fromString("c1ddfd7f-120a-4437-8b64-38660d3ec62d"), "[EioFarmer]");
    ...
    public int isLowOnSaplings(@Nonnull BlockPos pos) {
        return 90 * (FarmConfig.farmSaplingReserveAmount.get() - getSeedTypeInSuppliesFor(pos).getCount()) /
                FarmConfig.farmSaplingReserveAmount.get();
    }
        ...
        if (Prep.isValid(toolStack) && getWorld().rand.nextFloat() <= FarmConfig.farmToolDamageChance.get()) {
            ...
            switch (tool(tool)) {
                ...
                case HOE:
                    ...
                    toolStack.getItem().onItemUse(farmerJoe, getWorld(), pos, EnumHand.MAIN_HAND, EnumFacing.UP, 0.5f,
                            0.5f, 0.5f);
