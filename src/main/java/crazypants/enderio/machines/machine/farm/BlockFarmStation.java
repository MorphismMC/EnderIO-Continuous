...
public class BlockFarmStation extends AbstractPoweredTaskBlock<TileFarmStation>
    ...
    public void init(@Nonnull IModObject mo, @Nonnull FMLInitializationEvent event) {
        super.init(mo, event);
        permissionFarming = PermissionAPI.registerNode(
                EnderIO.DOMAIN + ".farm." + getTranslationKey().toLowerCase(Locale.ENGLISH), DefaultPermissionLevel.ALL,
                "Permission for the block " + getTranslationKey() +
                        " of Ender IO to farm land. This includes tilling, planting, harvesting and fertilizing." +
    ...
    public void fillWithRain(@Nonnull World worldIn, @Nonnull BlockPos pos) {
        if (worldIn.rand.nextFloat() < FarmConfig.rainWaterChance.get() && worldIn.getBiomeProvider()
                .getTemperatureAtHeight(worldIn.getBiome(pos).getTemperature(pos), pos.getY()) >= 0.15F) {
