...
public class TileFarmStation extends AbstractPoweredTaskEntity implements IPaintable.IPaintableTileEntity, IRanged,
    ...
    public boolean isMachineItemValidForSlot(int i, @Nonnull ItemStack stack) {
        ...
        if (i <= maxToolSlot) {
            ...
            // TODO: When switching to EnderInventory, make it so that the GUI allows putting things into empty locked
            // slots
            return isSlotLocked(i) ? ItemUtil.areStacksEqual(stack, getStackInSlot(i)) :
                    Commune.instance.canPlant(stack);
        } else {
            return false;
        }
    }
        ...
        while (i-- > 0) {
            ...
            if (!farmingPos.equals(getPos()) && world.isBlockLoaded(farmingPos)) {
                if (PermissionAPI.hasPermission(getOwner().getAsGameProfile(), BlockFarmStation.permissionFarming,
                        new BlockPosContext(getFarmer().getFakePlayer(), farmingPos, world.getBlockState(farmingPos),
                                null))) {
                    return farmingPos;
                }
            }
    ...
    private boolean isOutputFull() {
        ...
        for (FarmSlots slot : FarmSlots.OUTPUTS) {
            ...
            if (Prep.isInvalid(curStack) ||
                    (!FarmConfig.farmStopOnNoOutputSlots.get() && curStack.getCount() < curStack.getMaxStackSize())) {
                return false;
            }
        }
        return true;
                ...
                while (boost > 0) {
                    ...
                    if ((boost >= 1 || random.nextFloat() < boost) && world.isBlockLoaded(boostPos)) {
                        ...
                        if (block.getTickRandomly()) {
                            ...
                            // much too noisy:
                            // PacketHandler.sendToAllAround(new PacketFarmAction(Collections.singletonList(boostPos)),
                            // this);
                        }
                    }
                    boost--;
    ...
    public int getInventoryStackLimit(int slot) {
        ...
        if (slot >= minSupSlot && slot <= maxSupSlot) {
            return getCapacitorData() == DefaultCapacitorData.NONE ? 0 :
                    Math.min(FARM_STACK_LIMIT.get(getCapacitorData()), 64);
        }
        return 64;
    }
