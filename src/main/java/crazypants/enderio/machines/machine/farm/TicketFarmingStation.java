...
public class TicketFarmingStation extends SimpleTicket<Vec3d> {
    ...
    public void prepare() {
        if (getMasterManager() == null && !farm.getWorld().isRemote) {
            final int maxSize = (int) (FARM_BASE_SIZE.getFloat(DefaultCapacitorData.LVL10) +
                    FARM_BONUS_SIZE.getFloat(DefaultCapacitorData.LVL10));
            final PlanarBlockIterator blockIterator = new PlanarBlockIterator(farm.getPos(), Orientation.HORIZONTAL,
                    maxSize);
