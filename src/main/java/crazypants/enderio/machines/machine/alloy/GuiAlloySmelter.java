...
public abstract class GuiAlloySmelter<T extends TileAlloySmelter> extends GuiInventoryMachineBase<T>
                                     implements ContainerAlloySmelterProxy {

    // Multiple classes so JEI can see the difference
    @SuppressWarnings("unchecked")
    public static @Nonnull <E extends TileAlloySmelter> GuiAlloySmelter<E> create(@Nonnull InventoryPlayer playerInv,
                                                                                  @Nonnull E te) {
    ...
    public static class Enhanced extends GuiAlloySmelter<TileAlloySmelter> {
        ...
        public Enhanced(@Nonnull InventoryPlayer par1InventoryPlayer,
                        @Nonnull TileAlloySmelter.Enhanced furnaceInventory) {
    ...
    public static class Furnace extends GuiAlloySmelter<TileAlloySmelter.Furnace> {
        ...
        public Furnace(@Nonnull InventoryPlayer par1InventoryPlayer,
                       @Nonnull TileAlloySmelter.Furnace furnaceInventory) {
            super(par1InventoryPlayer, furnaceInventory);
        }
    ...
    public GuiAlloySmelter(@Nonnull InventoryPlayer par1InventoryPlayer, @Nonnull T furnaceInventory) {
        super(furnaceInventory, ContainerAlloySmelter.create(par1InventoryPlayer, furnaceInventory),
                OperatingProfile.getAllGuiTextures());

        this.mode = furnaceInventory.getOperatingProfile();

        vanillaFurnaceTooltip = new GuiToolTip(new Rectangle(xSize - 5 - BUTTON_SIZE, 62, BUTTON_SIZE, BUTTON_SIZE),
                (String[]) null);
