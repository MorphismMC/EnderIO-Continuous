...
public class TileAlloySmelter extends AbstractPoweredTaskEntity implements IPaintable.IPaintableTileEntity {
    ...
    public static class Simple extends TileAlloySmelter {
        ...
        public Simple() {
            super(AlloySmelterConfig.profileSimpleAlloy.get().get(), new SlotDefinition(3, 1, 0),
                    SIMPLE_ALLOY_SMELTER_POWER_INTAKE,
    ...
    public static class Furnace extends TileAlloySmelter {
        ...
        public Furnace() {
            super(AlloySmelterConfig.profileSimpleFurnace.get().get(), new SlotDefinition(3, 1, 0),
                    SIMPLE_ALLOY_SMELTER_POWER_INTAKE,
                    SIMPLE_ALLOY_SMELTER_POWER_BUFFER, SIMPLE_ALLOY_SMELTER_POWER_USE);
    ...
    public static class Enhanced extends TileAlloySmelter {
        ...
        public Enhanced() {
            super(AlloySmelterConfig.profileEnhancedAlloy.get().get(), new SlotDefinition(3, 1, 1),
                    ENHANCED_ALLOY_SMELTER_POWER_INTAKE,
        ...
        protected boolean shouldDoubleTick(@Nonnull IPoweredTask task, int usedEnergy) {
            double chance = getCapacitorData().getUnscaledValue(ENHANCED_ALLOY_SMELTER_DOUBLE_OP_CHANCE) *
                    (usedEnergy / task.getRequiredEnergy());
    ...
    public TileAlloySmelter() {
        this(AlloySmelterConfig.profileNormal.get().get(), new SlotDefinition(3, 1), ALLOY_SMELTER_POWER_INTAKE,
                ALLOY_SMELTER_POWER_BUFFER,
                ALLOY_SMELTER_POWER_USE);
    }

    protected TileAlloySmelter(@Nonnull OperatingProfile operatingProfile, @Nonnull SlotDefinition slotDefinition,
                               @Nonnull ICapacitorKey maxEnergyRecieved,
    ...
    protected IMachineRecipe canStartNextTask(long nextSeed) {
        ...
        if (getMode().doAlloyRecipes() &&
                AlloyRecipeManager.getInstance().getRecipeForInputs(getMachineLevel(), getRecipeInputs()) != null) {
            nextRecipe = AlloyRecipeManager.getInstance();
        }
        if (nextRecipe == null && getMode().doFurnaceRecipes() &&
                VanillaSmeltingRecipe.getInstance().isRecipe(getMachineLevel(), getRecipeInputs())) {
    ...
    public boolean isMachineItemValidForSlot(int slot, @Nonnull ItemStack itemstack) {
        ...
        // We will assume anything that is in a slot is valid, so just return if the new input can be stacked with the
        // current one
