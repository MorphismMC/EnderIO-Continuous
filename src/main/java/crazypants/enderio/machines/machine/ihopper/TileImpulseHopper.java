...
public class TileImpulseHopper extends AbstractCapabilityMachineEntity {
    ...
    public TileImpulseHopper() {
        super(CapacitorKey.IMPULSE_HOPPER_POWER_INTAKE, CapacitorKey.IMPULSE_HOPPER_POWER_BUFFER,
                CapacitorKey.IMPULSE_HOPPER_POWER_USE);
        ...
        for (int i = 0; i < SLOTS; i++) {
            ...
            getInventory().add(Type.OUTPUT, OUTPUT_SLOT + i,
                    new InventorySlot(Filters.ALWAYS_FALSE, Filters.ALWAYS_TRUE));
        }
    ...
    public boolean checkInputSlot(int slot) {
        ...
        return !ghostStack.isEmpty() && !inputStack.isEmpty() && ItemUtil.areStacksEqual(ghostStack, inputStack) &&
    ...
    protected void processTasks(boolean redstoneCheck) {
        if (shouldDoWorkThisTick() && redstoneCheck) {
            if (getEnergy().useEnergy()) {
                ...
                if (!doSomething ||
                        getEnergy().getMaxUsage(CapacitorKey.IMPULSE_HOPPER_POWER_USE_PER_ITEM) * neededPower >
