...
public class ContainerCrafter<T extends TileCrafter> extends ContainerEnderCap<EnderInventory, TileCrafter> {
    ...
    @Nonnull
    public static <E extends TileCrafter> ContainerCrafter<E> create(@Nonnull InventoryPlayer playerInv,
                                                                     @Nonnull E te) {
    ...
    protected void addSlots() {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                ...
                int y = topY + row * 18;
                addSlotToContainer(
                        new EnderSlot(getTileEntityNN().getInventory().getView(Type.INPUT), "INPUT" + index, x, y));
                index++;
            }
        }
