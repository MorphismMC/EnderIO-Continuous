...
public class TileCrafter extends AbstractCapabilityMachineEntity implements IPaintable.IPaintableTileEntity {
    ...
    public static class Simple extends TileCrafter {
        ...
        public Simple() {
            super(CapacitorKey.SIMPLE_CRAFTER_POWER_INTAKE, CapacitorKey.SIMPLE_CRAFTER_POWER_BUFFER,
                    CapacitorKey.SIMPLE_CRAFTER_POWER_USE);
            getInventory().getSlot(EnergyLogic.CAPSLOT)
                    .set(new ItemStack(ModObject.itemBasicCapacitor.getItemNN(), 1,
    ...
    public TileCrafter(@Nonnull ICapacitorKey maxEnergyRecieved, @Nonnull ICapacitorKey maxEnergyStored,
    ...
    protected void processTasks(boolean redstoneCheck) {
        ...
        if (isOutputQueued()) {
            ...
            } else if (redstoneCheck && getEnergy().canUseEnergy(CapacitorKey.CRAFTER_POWER_CRAFT) &&
                    canMergeOutput() && canCraft() && craftRecipe()) {
                        ticksSinceLastCraft = 0;
    ...
    private @Nonnull FakePlayerEIO getFakePlayer() {
        return playerInst != null ? playerInst :
                (playerInst = new FakePlayerEIO(world, getLocation(), DUMMY_PROFILE).setOwner(getOwner()));
    }
