...
public class TileLightNode extends TileEntityEio {
    ...
    public void onNeighbourChanged() {
        parents.apply((NNList.Callback<BlockPos>) parent -> FuncUtil.doIf(
                BlockEnder.getAnyTileEntitySafe(world, parent, TileElectricLight.class),
    ...
    protected void checkSelf() {
        if (!world.isRemote) {
            for (NNIterator<BlockPos> iterator = parents.iterator(); iterator.hasNext();) {
                ...
                if (world.isBlockLoaded(parent) &&
                        world.getBlockState(parent).getBlock() != block_electric_light.getBlock()) {
                    iterator.remove();
                }
            }
            if (parents.isEmpty()) {
