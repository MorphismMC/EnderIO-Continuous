...
public class TileElectricLight extends TileEntityEio implements ILegacyPoweredTile.Receiver {
    ...
    public @Nonnull LightType getLightType() {
        return lighttype != null ? lighttype :
                (lighttype = world.getBlockState(getPos()).getValue(BlockElectricLight.TYPE));
    }
    ...
    private boolean isTransparent(Vector3d offset) {
        return world.getBlockLightOpacity(new BlockPos(getPos().getX() + (int) offset.x,
                getPos().getY() + (int) offset.y, getPos().getZ() + (int) offset.z)) == 0;
    }

    private boolean isAir(Vector3d offset) {
        return world.isAirBlock(new BlockPos(getPos().getX() + (int) offset.x, getPos().getY() + (int) offset.y,
                getPos().getZ() + (int) offset.z));
    }
