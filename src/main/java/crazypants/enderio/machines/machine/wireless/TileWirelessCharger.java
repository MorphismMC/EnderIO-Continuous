...
@Storable
public class TileWirelessCharger extends TileEntityEio implements ILegacyPoweredTile.Receiver, IWirelessCharger,
                                 IPaintable.IPaintableTileEntity, IRanged {
    ...
    private static boolean shouldChargeWirelessly(ItemStack stack) {
        return !(stack.getItem() instanceof IWirelessCharger.ExcludedItem) ||
    ...
    public BoundingBox getRange() throws SelfDestructionException {
        if (this.isInvalid()) {
            Log.error("TileEntity " + this + " at " + pos +
                    " is invalid but was not invalidated. This should not be possible!");
            return bb;
        }
