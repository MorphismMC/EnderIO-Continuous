...
public class BlockEnhancedWirelessCharger extends BlockNormalWirelessCharger
                                          implements IResourceTooltipProvider, IHaveRenderers {
    ...
    private BlockEnhancedWirelessCharger(@Nonnull IModObject modObject) {
        super(modObject);
        setDefaultState(getBlockState().getBaseState().withProperty(PUP, E.DISH).withProperty(PEAST, E.DISH)
                .withProperty(PWEST, E.DISH)
    ...
    private boolean canConnect(@Nonnull IBlockAccess world, @Nonnull BlockPos pos, @Nonnull EnumFacing dir) {
        ...
        return other.getBlock() == MachineObject.block_wireless_charger_extension.getBlockNN() &&
                other.getValue(BlockAntenna.BASE) == dir.getOpposite();
    }
    ...
    protected @Nonnull BoundingBox getChargingStrength(@Nonnull IBlockState state, @Nonnull BlockPos pos) {
        ...
        int x0 = pos.getX() - r, y0 = pos.getY() - r, z0 = pos.getZ() - r, x1 = pos.getX() + 1 + r,
                y1 = pos.getY() + 1 + r, z1 = pos.getZ() + 1 + r;
