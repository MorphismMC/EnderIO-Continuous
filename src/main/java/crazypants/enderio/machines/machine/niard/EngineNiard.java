...
public class EngineNiard {
    ...
    public boolean work() {
        ...
        for (int i = 0; i < radiusItr.size(); i++) {
            ...
            if (!seen.isEmpty()) {
                ...
                owner.getWorld().getBlockState(base.offset(downflowDirection)).neighborChanged(owner.getWorld(),
                        base.offset(downflowDirection),
        ...
        if (radius >= 0) {
            for (int i = 0; i < radiusItr.size(); i++) {
                ...
                if (isInWorld(next) && canPlace(next)) {
                    ...
                    remaining -= i1;
                    final EntityXPOrb xpOrb = new EntityXPOrb(owner.getWorld(), next.getX() + 0.5D, next.getY() + 0.7D,
                            next.getZ() + 0.5D, i1);
                    xpOrb.motionX /= 4d;
                    xpOrb.motionY = 0;
                    xpOrb.motionZ /= 4d;
    ...
    private void setSourceBlock(@Nonnull BlockPos bc) {
        ...
        // First clean up, we may target locations destroyBlockOnFluidPlacement doesn't clean, so call it manually and
        // then clear the pos hard. That's with drops
        // but without sound.
    ...
    private void setVerticalBlock(@Nonnull BlockPos bc, boolean blockUpdate) {
        ...
        switch (type) {
            case CLASSIC:
                metaToSet = block.getDefaultState()
                        .withProperty(NullHelper.notnullF(BlockFluidClassic.LEVEL, "BlockFluidClassic.LEVEL"), 1);
                break;
            case FINITE:
                return;
            case VANILLA:
