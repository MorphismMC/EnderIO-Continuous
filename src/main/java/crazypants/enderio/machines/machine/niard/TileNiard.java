...
public class TileNiard extends AbstractCapabilityMachineEntity implements ITankAccess.IExtendedTankAccess, IRanged {
    ...
    protected final @Nonnull SmartTankFluidHandler smartTankFluidHandler = new SmartTankFluidMachineHandler(this,
            inputTank);
    ...
    public TileNiard() {
        ...
        getInventory().add(Type.INPUT, SLOT.INPUT,
                new InventorySlot(item -> item != null && FluidUtil.getFluidTypeFromItem(item) != null, -1));
    ...
    protected boolean canTick(boolean redstoneChecksPassed) {
        if (redstoneChecksPassed) {
            if (!inputTank.isFull() && !isOutputQueued() && !getInventory().getSlot(SLOT.INPUT).isEmpty() &&
            ...
            return getEnergy().useEnergy() && sleep == 0 && !inputTank.isEmpty() &&
                    getEnergy().canUseEnergy(CapacitorKey.NIARD_POWER_WORK);
        }
        return false;
    }
        ...
        if (shouldDoWorkThisTick(CapacitorKey.NIARD_DELAY.get(getCapacitorData()))) {
            if (inputTank.getFluidNN().getFluid() == Fluids.XP_JUICE.getFluid()) {
                doWorkXP();
                    ...
                    engine.setFluid(inputTank.getFluidNN().getFluid()).work()) {
    ...
    public void setShowRange(boolean showRange) {
        ...
        if (showingRange) {
            ...
            if (fluid != null) {
                ...
                if (color != 0xFFFFFFFF) {
                    final float r = ((color >> 16) & 0xFF) / 255f, g = ((color >> 8) & 0xFF) / 255f,
    ...
    public BoundingBox getBounds() {
        ...
        if (fluid != null && fluid.getFluid().getDensity() <= 0) {
            return new BoundingBox(pos.south().east().up(), BlockCoord.withY(pos, 256)).expand(getRange(), 0,
                    getRange());
        } else {
