...
public class ContainerTravelAccessable extends ContainerEnder<IInventory>
    ...
    public ContainerTravelAccessable(@Nonnull InventoryPlayer playerInv,
                                     final @Nonnull ITravelAccessable travelAccessable, @Nonnull World world,
                                     int guiOffset) {
            ...
            if (isAuth) {
                if (ta.getAccessMode() == AccessMode.PROTECTED && ta instanceof TileEntityBase) {
                    PacketHandler.INSTANCE
                            .sendToServer(PacketPassword.setPassword((TileEntityBase) ta, getSlot(), stack));
                }
            } else {
