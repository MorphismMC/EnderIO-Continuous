...
public class TelePadRenderMapper implements IRenderMapper.IBlockRenderMapper.IRenderLayerAware,
        ...
        if (state.getBlock() == block_tele_pad.getBlock()) {
            if (blockLayer == BlockRenderLayer.SOLID) {
                ...
                if (type == BlockType.SINGLE) {
                    return Collections
                            .singletonList(state.getState().withProperty(EnumRenderMode.RENDER, SINGLE_MODEL));
