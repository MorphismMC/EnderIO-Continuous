...
public class TeleportEntityRenderHandler {
    ...
    /*
     * gl flags when this is called: GL_ALPHA_TEST GL_COLOR_MATERIAL GL_CULL_FACE GL_DEPTH_TEST GL_DITHER GL_FOG
     * GL_LIGHTING GL_TEXTURE_2D
     */
    ...
    public static void onEntityRender(RenderLivingEvent.Pre<EntityLivingBase> event) {
        if (TelePadConfig.telepadShrinkEffect.get()) {
            ...
            if (entityData.getBoolean(TileTelePad.TELEPORTING_KEY)) {
                isTarget = true;
                // shrink the entity for longer than the teleport lasts so it doesn't pop big again at the source
                // location
            ...
            if (isTarget) {
                final float progress = entityData.hasKey(TileTelePad.PROGRESS_KEY) ?
                        entityData.getFloat(TileTelePad.PROGRESS_KEY) : 1;
                GlStateManager.pushMatrix();
                GlStateManager.translate(event.getX(), event.getY(), event.getZ());
