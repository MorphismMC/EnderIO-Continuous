...
public class GuiTelePad extends GuiContainerBaseEIO<TileTelePad> {
    ...
    public GuiTelePad(@Nonnull InventoryPlayer playerInv, final @Nonnull TileTelePad te) {
        ...
        travelSettingsButton = new IconButton(this, ID_TRAVEL_SETTINGS_BUTTON, settingsBX, settingsBY,
                IconEIO.GEAR_LIGHT);
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        for (int i = 0; i < text.length; i++) {
            ...
            fnt.drawString(NullHelper.first(text[i]),
                    f.x - (fnt.getStringWidth(NullHelper.first(text[i], "")) / 2) - 10,
        ...
        if (e != null) {
            ...
            fnt.drawString(name, sx + xSize / 2 - fnt.getStringWidth(name) / 2, sy + progressY + fnt.FONT_HEIGHT + 6,
                    0x000000);
        } else if (getOwner().wasBlocked()) {
            String s = Lang.GUI_TELEPAD_ERROR_BLOCKED.get();
            fnt.drawString(s, sx + xSize / 2 - fnt.getStringWidth(s) / 2, sy + progressY + fnt.FONT_HEIGHT + 6,
                    0xAA0000);
        }
    ...
    protected void actionPerformed(@Nonnull GuiButton button) throws IOException {
        ...
        if (button.id == ID_TELEPORT_BUTTON) {
            ...
            PacketHandler.INSTANCE
                    .sendToServer(new PacketOpenServerGui(getOwner(), BlockTelePad.GUI_ID_TELEPAD_TRAVEL));
        }
    }
}
