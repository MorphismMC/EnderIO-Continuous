...
public class TelePadSpecialRenderer extends TravelEntitySpecialRenderer<TileTelePad> {
    ...
    public TelePadSpecialRenderer() {
        ...
        blade = block_tele_pad.getBlockNN().getDefaultState().withProperty(EnumRenderMode.RENDER,
                EnumRenderMode.FRONT_WEST);
        lights = block_tele_pad.getBlockNN().getDefaultState().withProperty(EnumRenderMode.RENDER,
                EnumRenderMode.FRONT_ON_WEST);
        glass = block_tele_pad.getBlockNN().getDefaultState().withProperty(EnumRenderMode.RENDER,
                EnumRenderMode.FRONT_ON);
    }
    ...
    @Override
    public void renderTileEntity(@Nonnull TileTelePad te, @Nonnull IBlockState blockState, float partialTicks,
                                 int destroyStage) {
        if (MinecraftForgeClient.getRenderPass() == 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                GlStateManager.rotate(
                        te.bladeRots[i] + rand.nextInt(360) + (te.spinSpeed * partialTicks * ((i * 2) + 20)), 0, 1, 0);
    ...
    public void render(@Nonnull TileEntity tileEntity, final @Nonnull IBlockState state) {
        ...
        blockrendererdispatcher.getBlockModelRenderer().renderModel(tileEntity.getWorld(), ibakedmodel, state, pos,
                vertexBuffer, false);
