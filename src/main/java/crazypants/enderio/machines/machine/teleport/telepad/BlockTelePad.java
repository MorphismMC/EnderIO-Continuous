...
public class BlockTelePad extends BlockTravelAnchor<TileTelePad>
                          implements IPaintable.ISolidBlockPaintableBlock, ITEProxy {
    ...
    @Nonnull
    public static final PropertyEnum<BlockType> BLOCK_TYPE = PropertyEnum.<BlockType>create("blocktype",
            BlockType.class);
    ...
    protected void initDefaultState() {
        setDefaultState(getBlockState().getBaseState().withProperty(EnumRenderMode.RENDER, EnumRenderMode.AUTO)
                .withProperty(BLOCK_TYPE, BlockType.SINGLE));
    }
    ...
    public @Nonnull IBlockState getStateFromMeta(int meta) {
        return getDefaultState().withProperty(BLOCK_TYPE, BlockType.getType(meta)).withProperty(EnumRenderMode.RENDER,
                EnumRenderMode.AUTO);
    }
    ...
    /*
     * This makes us "air" for purposes of lighting. Otherwise our model would be much too dark, as it is always
     * surrounded be 8 TelePad blocks.
     */
    @Override
    ...
    @Deprecated
    @Override
    public @Nonnull AxisAlignedBB getSelectedBoundingBox(@Nonnull IBlockState bs, @Nonnull World world,
                                                         @Nonnull BlockPos pos) {
        if (bs.getBlock() == this) {
            BlockType bt = bs.getValue(BLOCK_TYPE);
            if (bt != BlockType.SINGLE) {
                BlockPos masterLoc = bt.getLocationOfMaster(pos);
                AxisAlignedBB res = new AxisAlignedBB(masterLoc.south().south().west(),
                        masterLoc.north().east().east().up());
                return res;
            }
        }
    ...
    public void neighborChanged(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
                                @Nonnull Block changedTo, @Nonnull BlockPos fromPos) {
    ...
    public @Nullable Container getServerGuiElement(@Nonnull EntityPlayer player, @Nonnull World world,
                                                   @Nonnull BlockPos pos, @Nullable EnumFacing facing, int ID,
    ...
    private void updateMultiBlock(World world, BlockPos masterPos, BlockPos ignorePos, boolean form) {
        for (BlockType type : BlockType.values()) {
            if (type != BlockType.SINGLE) {
                ...
                BlockPos targetPos = new BlockPos(masterPos.getX() + offset.getX(), masterPos.getY() + offset.getY(),
                        masterPos.getZ() + offset.getZ());
    ...
    private BlockPos getMasterPosForNewMB(@Nonnull World worldIn, @Nonnull BlockPos swCorner,
                                          @Nonnull BlockPos ignorePos) {
