...
public class GuiDialingDevice extends GuiContainerBaseEIO<TileDialingDevice> implements ContainerDialingDeviceProxy {
    ...
    public GuiDialingDevice(@Nonnull InventoryPlayer playerInv, @Nonnull TileDialingDevice te,
        ...
        addToolTip(new GuiToolTip(new Rectangle(x, y, w, h), "") {
            ...
            protected void updateText() {
                text.clear();
                TelepadTarget el = targetList.getElementAt(getLastMouseX() + getGuiLeft(),
                        getLastMouseY() + getGuiTop());
                if (el != null) {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTick, int mouseX, int mouseY) {
        ...
        if (getOwner().getEnergy().getEnergyStored() <
                getOwner().getEnergy().getMaxUsage(CapacitorKey.DIALING_DEVICE_POWER_USE_TELEPORT)) {
