...
public class TileDialingDevice extends AbstractCapabilityMachineEntity implements IRanged {
    ...
    public TileDialingDevice() {
        super(CapacitorKey.DIALING_DEVICE_POWER_INTAKE, CapacitorKey.DIALING_DEVICE_POWER_BUFFER,
                CapacitorKey.DIALING_DEVICE_POWER_USE);
    ...
    public @Nullable TileTelePad findTelepad() {
        for (BlockPos check : new CubicBlockIterator(getBounds())) {
            TileTelePad result = BlockEnder.getAnyTileEntitySafe(getWorld(), NullHelper.first(check, pos),
                    TileTelePad.class);
            if (result != null) {
    ...
    public BoundingBox getBounds() {
        ...
        BlockPos checkMax = pos.offset(forward, (range * 2 - 1) + 1).offset(side, (range * 2 - 1) - range).offset(up,
                (range * 2 - 1) - range);
