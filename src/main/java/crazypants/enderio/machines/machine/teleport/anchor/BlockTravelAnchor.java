...
public class BlockTravelAnchor<T extends TileTravelAnchor> extends AbstractMachineBlock<T>
                              implements IPaintable.IBlockPaintableBlock, IPaintable.IWrenchHideablePaint,
                              IDefaultRenderers, IHaveTESR {
    ...
    public boolean removedByPlayer(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        if (te != null) {
            if (te.getOwner().equals(UserIdent.create(entityPlayer.getGameProfile())) ||
                    (te.getAccessMode() == ITravelAccessable.AccessMode.PUBLIC) ||
                ...
                if (!world.isRemote) {
                    entityPlayer.sendStatusMessage(
                            Lang.GUI_AUTH_ERROR_HARVEST.toChatServer(te.getOwner().getPlayerName()), true);
                }
            }
        }
        return false;
    ...
    public static void sendPrivateStatusMessage(@Nonnull World world, @Nonnull EntityPlayer player,
                                                @Nonnull UserIdent owner) {
    ...
    public @Nullable GuiScreen getClientGuiElement(@Nonnull EntityPlayer player, @Nonnull World world,
                                                   ...
                                                   @Nonnull T te) {
        if (GUI_ID_TRAVEL_ACCESSABLE == ID) {
            return new GuiTravelAccessable<T>(player.inventory,
                    new ContainerTravelAccessable(player.inventory, te, world, ID), te, world);
        } else {
    ...
    public void bindTileEntitySpecialRenderer() {
        ClientRegistry.bindTileEntitySpecialRenderer(TileTravelAnchor.class,
                new TravelEntitySpecialRenderer<TileTravelAnchor>());
    }
}
