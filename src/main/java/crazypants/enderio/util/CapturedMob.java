...
public final class CapturedMob {
    ...
    public static @Nullable CapturedMob create(@Nullable Entity entity) {
        if (!(entity instanceof EntityLivingBase) || !entity.isEntityAlive() || entity.world.isRemote ||
    ...
    public @Nonnull ItemStack toStack(@Nonnull Item item, int meta, int amount) {
        ...
        if (item == itemSoulVial.getItemNN() && customName == null && PIG.equals(entityId) && Math.random() < 0.01) {
            NullHelper
                    .notnullM(stack.getTagCompound(),
                            "getTagCompound() doesn't produce value that was set with setTagCompound()")
                    .setString(CUSTOM_NAME_KEY,
    ...
    public static boolean isBlacklisted(@Nonnull Entity entity) {
        ...
        return entityId == null || isBlacklistedBoss(entityId, entity) ||
                EntityDataRegistry.getInstance().isBlackListedForSoulVial(entityId);
    }
    ...
    public @Nullable Entity doSpawn(@Nullable World world, @Nullable BlockPos pos, @Nullable EnumFacing side,
                                    boolean clone) {
        ...
        entity.setPositionAndRotation(spawnX, spawnY, spawnZ, MathHelper.wrapDegrees(world.rand.nextFloat() * 360.0F),
                0);
        if (entity instanceof EntityLiving) {
        ...
        if (theSide != EnumFacing.DOWN) {
            // we are aligned with out feet on the ground, so let the block below push us up if it is higher than 1
            // block
            for (AxisAlignedBB blockBB : world.getCollisionBoxes((Entity) null,
    ...
    public @Nullable Entity getEntity(@Nullable World world, @Nullable BlockPos pos,
        ...
        if (entityNbt_nullchecked != null && (clone || EntityDataRegistry.getInstance().needsCloning(entityId))) {
            ...
            if (!clone && entity != null) {
                // The caller doesn't expect a clone, but we return one. Give it a unique/new ID to avoid problems with
                // duplicate entities.
                entity.setUniqueId(MathHelper.getRandomUUID(world.rand));
            }
            return entity;
        ...
        if (entity instanceof EntityLiving) {
            ...
            if (difficulty != null) {
                if (pos == null || !ForgeEventFactory.doSpecialSpawn((EntityLiving) entity, world, pos.getX(),
                        pos.getY(), pos.getZ(), null)) {
    ...
    /*
     * Note: The Ender Dragon cannot be spawned as expected. All of its logic (moving, fighting, being hit, ...) is a
     * special manager class, which is very
