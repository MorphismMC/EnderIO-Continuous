...
public class ConduitRefinedStorageNode implements INetworkNode, INetworkNodeVisitor {
        ...
        if (network != null && handler != null) {
            ...
            if (outputFilter instanceof IFluidFilter) {
                ...
                if (!exportFilter.isEmpty()) {
                    do {
                        stack = exportFilter
                                .getFluidStackAt(exportFilterSlot.get(dir) >= exportFilter.getSlotCount() ?
                                        exportFilterSlot.set(dir, 0) : exportFilterSlot.get(dir));
            ...
            if (inputFilter instanceof IFluidFilter) {
                ...
                if (!all) {
                    do {
                        stack = importFilter.getFluidStackAt(importFilterSlot >= importFilter.getSlotCount() ?
                                importFilterSlot = 0 : importFilterSlot);
            ...
            if (outputFilter instanceof IItemFilter) {
                ...
                if (!exportFilter.isEmpty()) {
                    ...
                    if (!prototype.isEmpty()) {
                        ...
                        ItemStack available = network.extractItem(prototype,
                                Math.min(prototype.getMaxStackSize(), itemsPerTick), compare, Action.SIMULATE);
                        if (available == null || Prep.isInvalid(available)) {
                            if (isCraftingUpgrade(ItemFunctionUpgrade.getFunctionUpgrade(upgrade))) {
                                available = ItemHandlerHelper.copyStackWithSize(prototype,
                                        getInsertLimit(exportFilter, handler, prototype, Integer.MAX_VALUE));
                                int insertable = available.getCount() -
                                        ItemTools.insertItemStacked(handler, available, true).getCount();
                                if (insertable > 0 &&
                                        network.getCraftingManager().getTask(craftingTask.get(dir)) == null) {
                                    craftingTask.set(dir,
                                            FuncUtil.runIf(network.getCraftingManager().request(prototype, insertable),
                                                    ICraftingTask::getId));
                                }
                            }
                        } else {
                            available = ItemHandlerHelper.copyStackWithSize(available,
                                    getInsertLimit(exportFilter, handler, available, itemsPerTick));
                            if (Prep.isValid(available)) {
                                ...
                                if (remains.getCount() < available.getCount()) { // meaning something was inserted
                                    available = network.extractItem(prototype,
                                            available.getCount() - remains.getCount(), compare, Action.PERFORM);
                                    if (available != null && Prep.isValid(available)) {
                                        ...
                                        if (Prep.isValid(remains)) {
                                            ...
                                            if (remains != null && Prep.isValid(remains)) {
                                                ...
                                                ItemUtil.spawnItemInWorldWithRandomMotion(getWorld(), remains,
                                                        getPos());
                                            }
                                        }
                                    }
            ...
            if (inputFilter instanceof IItemFilter) {
                ...
                if (Prep.isValid(existing)) {
                    ...
                    if (maxCountThatPassesFilter > 0) {
                        ItemStack extractable = handler.extractItem(currentImportSlot.get(dir),
                                maxCountThatPassesFilter, true);
                        if (Prep.isValid(extractable)) {
                            ItemStack remains = network.insertItem(extractable, maxCountThatPassesFilter,
                                    Action.SIMULATE);
                            ...
                            if (insertable > 0) {
                                ...
                                if (remains != null && Prep.isValid(remains)) {
                                    ...
                                    // not trying to put stuff back, this is not an issue in some random mod we won't be
                                    // able to get hold of
    ...
    private boolean isCraftingUpgrade(@Nullable FunctionUpgrade functionUpgrade) {
        return functionUpgrade == FunctionUpgrade.RS_CRAFTING_UPGRADE ||
                functionUpgrade == FunctionUpgrade.RS_CRAFTING_SPEED_UPGRADE ||
     ...
     * <p>
     * That is the case if there's a another RS conduit that is not detached, or if there's a (potential) external
     * connection that's not disabled.
     * <p>
     * Note that disabled conduit connections look the same as disabled external connections, so this case doesn't need
