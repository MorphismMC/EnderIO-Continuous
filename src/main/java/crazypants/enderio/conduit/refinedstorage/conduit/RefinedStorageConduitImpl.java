...
public class RefinedStorageConduitImpl extends AbstractConduit implements RefinedStorageConduit {
    ...
    protected final EnumMap<EnumFacing, Filter> outputFilters = new EnumMap<>(EnumFacing.class);
    protected final EnumMap<EnumFacing, Filter> inputFilters = new EnumMap<>(EnumFacing.class);
    ...
    public RefinedStorageConduitImpl() {
    ...
    public boolean hasInternalCapability(@Nonnull Capability<?> capability, @Nullable EnumFacing facing) {
        if (capability == CapabilityUpgradeHolder.UPGRADE_HOLDER_CAPABILITY ||
                capability == CapabilityFilterHolder.FILTER_HOLDER_CAPABILITY) {
            return true;
        }
        return false;
    }
    ...
    public void writeToNBT(@Nonnull NBTTagCompound data) {
        ...
        for (Entry<EnumFacing, Filter> entry : inputFilters.entrySet()) {
            if (entry.getValue() != null) {
        ...
        for (Entry<EnumFacing, Filter> entry : outputFilters.entrySet()) {
            if (entry.getValue() != null) {
                Filter f = entry.getValue();
                if (!isDefault(f)) {
                    NBTTagCompound itemRoot = new NBTTagCompound();
        ...
        for (Entry<EnumFacing, ItemStack> entry : inputFilterUpgrades.entrySet()) {
            if (entry.getValue() != null) {
                ItemStack up = entry.getValue();
                Filter filter = getInputFilter(entry.getKey());
        ...
        for (Entry<EnumFacing, ItemStack> entry : outputFilterUpgrades.entrySet()) {
            if (entry.getValue() != null) {
                ItemStack up = entry.getValue();
                Filter filter = getOutputFilter(entry.getKey());
                FilterRegistry.writeFilterToStack(filter, up);
    ...
    private boolean isDefault(Filter f) {
    ...
    public void readFromNBT(@Nonnull NBTTagCompound data) {
        ...
        for (EnumFacing dir : EnumFacing.VALUES) {
            ...
            if (data.hasKey(key)) {
                ...
                Filter filter = FilterRegistry.loadFilterFromNbt(filterTag);
    ...
    // Filter Capability
    @Override
    public Filter getFilter(int filterId, int param1) {
    ...
    @Override
    public void setFilter(int filterId, EnumFacing side, @Nonnull Filter filter) {
        if (filterId == getInputFilterIndex()) {
    ...
    @Override
    public void setInputFilter(@Nonnull EnumFacing dir, @Nonnull Filter filter) {
        inputFilters.put(dir, filter);
        setClientStateDirty();
    }

    @Override
    public void setOutputFilter(@Nonnull EnumFacing dir, @Nonnull Filter filter) {
        outputFilters.put(dir, filter);
        setClientStateDirty();
    }

    @Override
    public Filter getInputFilter(@Nonnull EnumFacing dir) {
        return inputFilters.get(dir);
    }

    @Override
    public Filter getOutputFilter(@Nonnull EnumFacing dir) {
    ...
    public boolean isFilterUpgradeAccepted(@Nonnull ItemStack stack, boolean isInput) {
        return stack.getItem() instanceof ItemFilterFluidUpgrade || stack.getItem() instanceof ItemBasicItemFilter;
    }
}
