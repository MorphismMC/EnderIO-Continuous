...
public class Registry {
    ...
    public static void registerRegistry(@Nonnull RegistryEvent.NewRegistry event) {
        REGISTRY = new RegistryBuilder<IFarmerJoe>().setName(new ResourceLocation(EnderIO.DOMAIN, "farmers"))
                .setType(IFarmerJoe.class)
    ...
    public static <T extends Object> T foreach(@Nonnull Callback<T> callback) {
        for (ResourceLocation farmer : Registry.REGISTRY.getSlaveMap(Registry.PRIOLIST, RegistryCallbacks.PrioMap.class)
                .values()) {
