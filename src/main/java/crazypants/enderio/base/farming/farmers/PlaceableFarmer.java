...
public class PlaceableFarmer extends AbstractFarmerJoe {
    ...
    /**
     * Configures the farmer to till the ground instead of checking for usable soil. This means the {@link #DIRT} is
     * ignored!
     */
    ...
    protected IFarmerJoe.Result plant(@Nonnull IFarmer farm, @Nonnull BlockPos bc, @Nonnull IBlockState state) {
        if (Prep.isValid(farm.takeSeedFromSupplies(bc, true)) &&
                farm.checkAction(FarmingAction.PLANT, FarmingTool.HOE)) {
            final ItemStack seedStack = farm.takeSeedFromSupplies(bc, false);
            EntityPlayerMP joe = farm.startUsingItem(seedStack);
            EnumActionResult res = seedStack.getItem().onItemUse(joe, joe.world, bc.down(), EnumHand.MAIN_HAND,
