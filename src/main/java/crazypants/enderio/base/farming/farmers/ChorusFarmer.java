...
public class ChorusFarmer extends AbstractFarmerJoe {
        ...
        if (canPlant(seed) && isValidPlantingSpot(world, pos) && player.canPlayerEdit(pos, EnumFacing.UP, seed) &&
            ...
            if (placeBlockAt(seed, player, world, pos, iblockstate1)) {
                if (Prep.isValid(farm.takeSeedFromSupplies(pos))) {
                    SoundType soundtype = world.getBlockState(pos).getBlock().getSoundType(world.getBlockState(pos),
                            world, pos, player);
    ...
    private boolean placeBlockAt(@Nonnull ItemStack stack, @Nonnull EntityPlayer player, @Nonnull World world,
    ...
    public IHarvestResult harvestBlock(@Nonnull IFarmer farm, @Nonnull BlockPos bc, @Nonnull IBlockState state) {
        ...
        while (!walker.stemsToHarvest.isEmpty() && farm.checkAction(FarmingAction.HARVEST, FarmingTool.AXE)) {
            // Note: While stems can be broken by any tool, I decided to use the axe. This way a chorus farm can be used
            // without hoe and the various auto-smelt
    ...
    private void doHarvest(@Nonnull final IFarmer farm, @Nonnull final World world, @Nonnull IBlockState blockState,
                           @Nonnull final BlockPos pos, int fortune,
        ...
        final float chance = ForgeEventFactory.fireBlockHarvesting(drops, world, pos, blockState, fortune, 1f, false,
                joe);
    ...
    private static final @Nonnull NNList<EnumFacing> GROW_DIRECTIONS = new NNList<>(EnumFacing.UP, EnumFacing.NORTH,
            EnumFacing.SOUTH, EnumFacing.WEST,
            EnumFacing.EAST);

    private static class ChorusWalker {

        // Yes, there are some edge cases remaining where a chorus plant is broken improperly. But this is good enough
        // and can deal with all properly grown plants.
