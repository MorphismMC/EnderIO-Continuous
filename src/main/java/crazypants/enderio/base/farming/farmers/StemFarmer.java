...
public class StemFarmer extends CustomSeedFarmer {
    ...
    @Override
    public IHarvestResult harvestBlock(@Nonnull final IFarmer farm, @Nonnull final BlockPos pos,
        ...
        do {
            ...
            if (hasHoe && plantedBlock == harvestState.getBlock()) {
                ...
                float chance = ForgeEventFactory.fireBlockHarvesting(drops, joe.world, harvestCoord, state, fortune,
                        1.0F, false, joe);
    ...
    protected boolean plantFromInventory(@Nonnull IFarmer farm, @Nonnull BlockPos bc) {
        ...
        if (canPlant(farm, world, bc) && Prep.isValid(farm.takeSeedFromSupplies(seeds, bc, true)) &&
                plant(farm, world, bc)) {
