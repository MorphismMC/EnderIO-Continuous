...
public class TreeFarmer extends AbstractFarmerJoe {
    ...
    protected boolean plant(@Nonnull IFarmer farm, @Nonnull World world, @Nonnull BlockPos bc,
        ...
        if (canPlant(world, bc, sapling) && farm.checkAction(FarmingAction.PLANT, FarmingTool.HOE)) {
            ...
            final IBlockState state = Block.getBlockFromItem(item)
                    .getStateFromMeta(item.getMetadata(sapling.getMetadata()));
    ...
    public IHarvestResult harvestBlock(@Nonnull IFarmer farm, @Nonnull BlockPos bc, @Nonnull IBlockState state) {
        ...
        final IHarvestingTarget target = new FarmHarvestingTarget(this, farm, FarmingConfig.treeHarvestRadius.get(),
                FarmingConfig.treeHarvestHeight.get());
     ...
     * @param harvestPos
     *                   The {@link BlockPos} to get the drops for. It <em>must</em> be part of the
     *                   {@link HarvestResult}'s harvested blocks list!
     */
    boolean harvestSingleBlock(@Nonnull IFarmer farm, final @Nonnull World world, final @Nonnull IHarvestResult result,
        ...
        if (blk instanceof IShearable && hasShears &&
                ((shearCount / result.getHarvestedBlocks().size() + noShearingPercentage) < 100)) {
            ...
            chance = ForgeEventFactory.fireBlockHarvesting(drops, joe.world, harvestPos, state, fortune, chance, false,
                    joe);
    ...
    protected void tryReplanting(@Nonnull IFarmer farm, @Nonnull World world, @Nonnull BlockPos bc,
                                 @Nonnull HarvestResult res) {
