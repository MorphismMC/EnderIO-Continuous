...
public final class FarmersRegistry {
    ...
    public static final @Nonnull Things slotItemsHoeTools = new Things(TOOL_HOE).add(Items.WOODEN_HOE)
            .add(Items.STONE_HOE).add(Items.IRON_HOE)
            .add(Items.GOLDEN_HOE).add(Items.DIAMOND_HOE).add(ModObject.itemDarkSteelHand);
    public static final @Nonnull Things slotItemsAxeTools = new Things().add(Items.WOODEN_AXE).add(Items.STONE_AXE)
            .add(Items.IRON_AXE).add(Items.GOLDEN_AXE)
            .add(Items.DIAMOND_AXE).add(ModObject.itemDarkSteelAxe);
    public static final @Nonnull Things slotItemsExtraTools = new Things(TOOL_SHEARS).add(Items.SHEARS)
            .add(ModObject.itemDarkSteelShears).add(TOOL_TREETAP);
    public static final @Nonnull Things slotItemsSeeds = new Things().add(SAPLINGS).add(Items.WHEAT_SEEDS)
            .add(Items.CARROT).add(Items.POTATO)
            .add(Blocks.RED_MUSHROOM).add(Blocks.BROWN_MUSHROOM).add(Items.NETHER_WART).add(Blocks.SAPLING)
            .add(Items.REEDS).add(Items.MELON_SEEDS)
            .add(Items.PUMPKIN_SEEDS);
    public static final @Nonnull Things slotItemsProduce = new Things().add(WOODS).add(new ItemStack(Blocks.LOG, 1, 0))
            .add(Blocks.WHEAT)
    ...
    public static void registerFarmers(@Nonnull RegistryEvent.Register<IFarmerJoe> event) {
        event.getRegistry().register(
                new FlowerPicker(FLOWERS).setPriority(EventPriority.LOW).setRegistryName(EnderIO.DOMAIN, "flowers"));
        event.getRegistry()
                .register(new StemFarmer(Blocks.REEDS, new ItemStack(Items.REEDS)).setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "reeds"));
        event.getRegistry()
                .register(new StemFarmer(Blocks.CACTUS, new ItemStack(Blocks.CACTUS)).setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "cactus"));
        event.getRegistry().register(new TreeFarmer(SAPLINGS, WOODS).setPriority(EventPriority.LOW)
                .setRegistryName(EnderIO.DOMAIN, "trees"));
        // mushrooms only grow bigger on certain blocks. so plant them on those and harvest them on others
        event.getRegistry().register(new SmallMushroomFarmer().setPriority(EventPriority.LOW)
                .setRegistryName(EnderIO.DOMAIN, "small_mushrooms"));
        event.getRegistry()
                .register(new BigMushroomFarmer(true, Blocks.RED_MUSHROOM, Blocks.RED_MUSHROOM_BLOCK)
                        .setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "red_mushrooms"));
        event.getRegistry()
                .register(new BigMushroomFarmer(true, Blocks.BROWN_MUSHROOM, Blocks.BROWN_MUSHROOM_BLOCK)
                        .setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "brown_mushrooms"));
        // special case of plantables to get spacing correct
        event.getRegistry()
                .register(new MelonFarmer(Blocks.MELON_STEM, Blocks.MELON_BLOCK, new ItemStack(Items.MELON_SEEDS))
                        .setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "melons"));
        event.getRegistry()
                .register(new MelonFarmer(Blocks.PUMPKIN_STEM, Blocks.PUMPKIN, new ItemStack(Items.PUMPKIN_SEEDS))
                        .setPriority(EventPriority.LOW)
                        .setRegistryName(EnderIO.DOMAIN, "pumpkins"));
        // 'BlockNetherWart' is not an IGrowable
        event.getRegistry()
                .register(new CustomSeedFarmer(Blocks.NETHER_WART, 3, new ItemStack(Items.NETHER_WART))
                        .setRequiresTilling(false)
                        .setPriority(EventPriority.LOW).setRegistryName(EnderIO.DOMAIN, "netherwart"));
        // Cocoa is odd
        event.getRegistry()
                .register(new CocoaFarmer().setPriority(EventPriority.LOW).setRegistryName(EnderIO.DOMAIN, "cocoa"));
        // Chorus plant is even odder
        event.getRegistry()
                .register(new ChorusFarmer().setPriority(EventPriority.LOW).setRegistryName(EnderIO.DOMAIN, "chorus"));

        // Handles all 'vanilla' style crops
        event.getRegistry().register(
                new PlantableFarmer().setPriority(EventPriority.LOWEST).setRegistryName(EnderIO.DOMAIN, "default"));
    }

    public static void addPickable(@Nonnull RegistryEvent.Register<IFarmerJoe> event, @Nonnull String mod,
                                   @Nonnull String blockName, @Nonnull String itemName) {
        Block cropBlock = findBlock(mod, blockName);
        Item seedItem = findItem(mod, itemName);
        if (cropBlock != null && seedItem != null) {
            event.getRegistry()
                    .register(new PickableFarmer(cropBlock, new ItemStack(seedItem)).setRegistryName(mod, blockName));
        }
    }

    public static CustomSeedFarmer addSeed(@Nonnull RegistryEvent.Register<IFarmerJoe> event, @Nonnull String mod,
