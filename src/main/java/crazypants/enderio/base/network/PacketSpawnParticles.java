...
public class PacketSpawnParticles implements IMessage {
    ...
    public static void create(Entity position, int count, EnumParticleTypes... particles) {
        new PacketSpawnParticles()
                .add(position.posX, position.posY + position.height * 0.8, position.posZ, count, particles)
                .send(position.world,
    ...
    public static void create(World world, BlockPos position, int count, EnumParticleTypes... particles) {
        new PacketSpawnParticles()
                .add(position.getX() + .5, position.getY() + .5, position.getZ() + .5, count, particles)
                .send(world, position);
    }

    public static void create(World world, double posX, double posY, double posZ, int count,
