 ...
 * <li>If needed, add a new DataType value and handling code to GuiPacket (for things that cannot be transmitted as int
 * or long).
 * <li>In the GUI, call the appropriate send() method. Chose a unique (per container) message ID.
 * <li>In the container, implement networkExec() and switch on the message ID. You can get the parameters with the same
 * index numbers as the send() method put
 * them in. Be careful to match the data types used or you'll get either 0 values.
 * <li>Done.
 * </ul>
 * 
 * The GuiPacket's handler will make sure that calls are only delivered to the correct container. If that container
 * isn't open, the message will be silently
 * ignored.
 * <p>
...
public class GuiPacket implements IMessage {
    ...
    private static final DataType[][] PATTERN = { {}, { DataType.INT }, { DataType.STRING },
            { DataType.INT, DataType.INT, DataType.LONG },
    ...
    private boolean checkAccess(int idx, DataType type) {
        if (idx >= PATTERN[pattern].length) {
            Log.warn("Invalid network packet received (" + guiID + "/" + msgID + "/" + pattern + "): idx" + idx +
                    " high");
            return false;
        }
        ...
        if ((type == DataType.INT && ints == null) || (type == DataType.LONG && longs == null) ||
                (type == DataType.STRING && strings == null)) {
            Log.warn("Invalid network packet received (" + guiID + "/" + msgID + "/" + pattern + "): idx" + idx +
                    " no " + type + " data");
            return false;
        }
    ...
    public static class Handler implements IMessageHandler<GuiPacket, IMessage> {
        ...
        public IMessage onMessage(GuiPacket message, MessageContext ctx) {
            ...
            if (openContainer instanceof IRemoteExec.IContainer &&
                    ((IRemoteExec.IContainer) openContainer).getGuiID() == message.guiID && message.guiID >= 0) {
                Log.debug("Exec (" + message.guiID + "/" + message.msgID + "/" + message.pattern + "): cont=" +
                        openContainer);
                return ((IRemoteExec.IContainer) openContainer).networkExec(message.msgID, message);
            } else {
                Log.debug("Invalid network packet received (" + message.guiID + "/" + message.msgID + "/" +
                        message.pattern + "): cont=" + openContainer + " id=" +
                        ((openContainer instanceof IRemoteExec.IContainer) ?
                                ((IRemoteExec.IContainer) openContainer).getGuiID() : "n/a"));
                return null;
            }
        }
    }
    ...
    public @Nonnull <E extends Enum<?>> E getEnum(int idx, Class<E> clazz) {
        ...
        return NullHelper.notnullJ(enumConstants[MathHelper.clamp(getInt(idx), 0, enumConstants.length - 1)],
