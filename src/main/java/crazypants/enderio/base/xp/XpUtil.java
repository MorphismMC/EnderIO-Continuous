 ...
 * 
 * @see <a href=
 *      "https://github.com/OpenMods/OpenModsLib/blob/1.7.2/src/main/java/openmods/utils/EnchantmentUtils.java">OpenMods</a>
 *
 */
public class XpUtil {
    ...
    /*
     * The level has 3 ranges with different formula. For the highest range, we can use a neat reverse formula but that
     * one would ignore the lower levels. So we
    ...
    /**
     * Variant of {@link #getLevelForExperience(long)} for unit tests only. This checks its result and if it is wrong,
     * will output debug info t0 stdout and thrown
     ...
     * <p>
     * Updated to use sqrt() impl that work over the full range of long. Ran through an extensive test, now the result
     * is good.
     */
    @SuppressWarnings("null")
    private static int getLevelFromExp(long exp) {
        if (exp > Long.MAX_VALUE / 72) {
            return BigIntegerMath.sqrt(BigInteger.valueOf(exp).multiply(B72).subtract(B54215), RoundingMode.DOWN)
                    .add(B325).divide(B18).intValueExact();
        }
    ...
    /**
     * Gets the total amount of XP a player has, calculating it from their level and XP bar. Note that the player object
     * has the field
     * {@link EntityPlayer#experienceTotal} that should already contain this number. There's just one issue with
     * that---Minecraft uses level+exp more often than
     * total and there's a math error. For example, adding 91 XP points gives 91 points in experienceTotal (that's
     * exactly 7 levels), but 6 levels and 99.999994%
    ...
    public static int getPlayerXP(@Nonnull EntityPlayer player) throws TooManyXPLevelsException {
        try {
            return player.capabilities.isCreativeMode ? Integer.MAX_VALUE / 2 : Math.addExact(
                    getExperienceForLevel(player.experienceLevel), (int) (player.experience * player.xpBarCap()));
    ...
    public static long getPlayerXPL(@Nonnull EntityPlayer player) {
        return player.capabilities.isCreativeMode ? Integer.MAX_VALUE / 2 : Math.addExact(
                getExperienceForLevelL(player.experienceLevel), (long) (player.experience * player.xpBarCap()));
    }

    /**
     * Adds the given experience to the player, throwing an exception if the total experience of the player would
     * overflow an {@code int}. In this case the player
     * will not be changed.
     *
     * @param player
     *               the player
