...
public class ExperienceContainer extends FluidTank {
    ...
    public void drainPlayerXpToReachContainerLevel(@Nonnull EntityPlayer player,
                                                   int level) throws TooManyXPLevelsException {
        if (level >= 0 && level <= XpUtil.getMaxLevelsStorable()) {
            ...
            Log.info("Invalid Call to drainPlayerXpToReachContainerLevel(), target level of ", level,
                    " is out of range.");
        }
    }

    public void drainPlayerXpToReachPlayerLevel(@Nonnull EntityPlayer player,
                                                int level) throws TooManyXPLevelsException {
        if (level >= 0 && level <= XpUtil.getMaxLevelsStorable()) {
            long targetXP = XpUtil.getExperienceForLevelL(level);
    ...
    public FluidStack drain(EnumFacing from, int maxDrain, boolean doDrain) {
        ...
        final int fluidToExtract = MathUtil.limit(XpUtil.experienceToLiquid(xpAskedToExtract)); // limit to fluidstack
                                                                                                // int
    ...
    public boolean canFillFluidType(FluidStack resource) {
        return canFill() && resource != null && resource.getFluid() != null &&
                FluidUtil.areFluidsTheSame(resource.getFluid(), XP.get());
    }
