...
public class EnderIOCrashCallable implements ICrashCallable {
            ...
            if (modContainer.getModId() != null && modContainer.getModId().startsWith("EnderIOAPI")) {
                if (!EnderIOAPIProps.VERSION.equals(apiVersionString)) {
                    result.add(" * Another mod is shipping a version of our API that doesn't match our version (" +
                            apiVersionString + " from " + modContainer.getSource().getName() +
                            "). That may not actually work.");
                } else if (modContainer.getSource().getName() != null &&
                        (!modContainer.getSource().getName().startsWith("EnderIO") &&
                                !modContainer.getSource().getName().startsWith("enderio") &&
                                !modContainer.getSource().getName().equals("bin"))) {
                                    result.add(" * Our API got loaded from " + modContainer.getSource().getName() +
                                            ". That's unexpected.");
                                }
            }
        }
    ...
    public String call() throws Exception {
        ...
        // msg += " " + string + "\n";
        // }
        msg += "\tAuthlib is : " + whereFrom(com.mojang.authlib.minecraft.MinecraftProfileTexture.class)
                .replace("jar:", "").replace("file:", "").replaceFirst("![^!]+$", "") + "\n";
    ...
    public static List<String> teslaDiagnostics() {
        ...
        apiDiagnostics(result, "Tesla", "net.darkhax.tesla.api.implementation.", "BaseTeslaContainer",
                "BaseTeslaContainerProvider", "InfiniteTeslaConsumer",
        ...
        for (String clazz : clazzes) {
            try {
                ...
                if (Log.LOGGER.isDebugEnabled()) {
                    // e.printStackTrace();
                    Log.debug(
                            "Hey, you wanted diagnostics output? Guess what, you're not getting it. Too many people reported it as bug. Deal with it.");
                }
            }
        }
    }
