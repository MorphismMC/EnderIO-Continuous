...
public class InternalRecieverTileWrapper extends InternalPoweredTileWrapper {
    ...
    public static @Nullable IEnergyStorage get(@Nonnull ILegacyPoweredTile.Receiver tile, @Nullable EnumFacing facing) {
        if (facing != null && tile.canConnectEnergy(facing)) {
            EnumMap<EnumFacing, IEnergyStorage> sideMap = CACHE.get().computeIfAbsent(tile,
                    theTile -> new EnumMap<>(EnumFacing.class));
            IEnergyStorage energyStorage = sideMap.get(facing);
            if (energyStorage == null) { // no computeIfAbsent() here as it would need to create a new Function every
                                         // time to capture the tile
    ...
    private static @Nonnull IEnergyStorage getInternal(@Nonnull ILegacyPoweredTile.Receiver tile,
    ...
    public static class ValidatingRecieverTileCapabilityProvider extends InternalRecieverTileWrapper {
        ...
        public ValidatingRecieverTileCapabilityProvider(@Nonnull ILegacyPoweredTile.Receiver tile,
                                                        @Nonnull EnumFacing facing) {
            super(tile, facing);
        }
        ...
        public int receiveEnergy(int maxReceive, boolean simulate) {
            ...
            final int receivedEnergy = super.receiveEnergy(Math.max(0, Math.min(maxReceive, maxEnergyRecieved - recv)),
                    simulate);
            if (!simulate) {
                if (recv == 0) { // first call each tick is allowed to push anything
                    recv += receivedEnergy;
                } else {
                    recv += maxReceive; // we didn't take that much, but they did try to push that much, so count what
                                        // they wanted to push to us
                    if (recv > maxEnergyRecieved) {
                        if (cooldown <= 0) {
                            final BlockPos pos = tile.getLocation().offset(from);
                            ((TileEntity) tile).getWorld().createExplosion(null, pos.getX() + .5, pos.getY() + .5,
                                    pos.getZ() + .5, 1f, (recv / 2 > maxEnergyRecieved));
                            cooldown = 30;
                        } else {
                            cooldown--;
                        }
    ...
    public static class LimitingRecieverTileCapabilityProvider extends InternalRecieverTileWrapper {
        ...
        public LimitingRecieverTileCapabilityProvider(@Nonnull ILegacyPoweredTile.Receiver tile,
                                                      @Nonnull EnumFacing facing) {
            super(tile, facing);
        }
