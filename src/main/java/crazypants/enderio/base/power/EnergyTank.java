...
public class EnergyTank extends NullEnergyTank implements IEnergyTank {
    ...
    public EnergyTank(@Nonnull ICapacitorKey maxEnergyRecieved, @Nonnull ICapacitorKey maxEnergyStored,
                      @Nonnull ICapacitorKey maxEnergyUsed) {
        this(null, maxEnergyRecieved, maxEnergyStored, maxEnergyUsed);
    }

    public EnergyTank(TileEntity owner, @Nonnull ICapacitorKey maxEnergyRecieved,
    ...
    public EnergyTank(TileEntity owner) {
        this(owner, CapacitorKey.LEGACY_ENERGY_INTAKE, CapacitorKey.LEGACY_ENERGY_BUFFER,
    ...
    @Override
    public int receiveEnergy(int maxReceive, boolean simulate) {
        int result = Math.max(0,
                Math.min(getMaxEnergyStored() - getEnergyStored(), Math.min(getMaxEnergyRecieved(), maxReceive)));
