...
public class ModObjectRegistry {
    ...
    public static void registerRegistry(@Nonnull RegistryEvent.NewRegistry event) {
        REGISTRY = new RegistryBuilder<IModObject>().setName(NAME).setType(IModObject.class).setIDRange(0, 0x00FFFFFF)
                .create();
    }
    ...
    public static void registerRegistry(@Nonnull EnderIOLifecycleEvent.PreInit event) {
        MinecraftForge.EVENT_BUS
                .post(new RegisterModObject(NAME, NullHelper.notnullF(REGISTRY, "RegistryBuilder.create()")));
    }
    ...
    private static void checkUseNeighborBrightness(IModObject mo, Block block) {
        if (Log.isInDev() && !block.getUseNeighborBrightness(block.getDefaultState())) {
            ...
            if (flag1 || flag2 || flag4 || flag5) {
                Log.error("Block " + mo.getRegistryName() +
                        " doesn't set useNeighborBrightness but it seems it should because: " +
                        (flag1 ? "stairs " : "") + (flag2 ? "slab " : "") + (flag4 ? "translucent " : "") +
                        (flag5 ? "lightOpacity " : ""));
            }
        }
    }
    ...
    public static void registerAddonBlocks(@Nonnull RegistryEvent.Register<Block> event) {
        for (ModContainer modContainer : Loader.instance().getModList()) {
            ...
            if (mod instanceof IEnderIOAddon) {
                ((IEnderIOAddon) mod).injectBlocks(
                        NullHelper.notnullF(event.getRegistry(), "RegistryEvent.Register<Block>.getRegistry()"));
            }
        }
    }
            ...
            if (item != null) {
                for (CreativeTabs tab : item.getCreativeTabs()) {
                    ...
                    for (ItemStack itemStack : list) {
                        ...
                        if (oreIDs.length == 0) {
                            System.out.println("  <item name=\"" + itemStack.getItem().getRegistryName() + ":" +
                                    itemStack.getItemDamage() + "\" show=\"false\" />");
                        } else {
                            System.out.println("  <item name=\"" + itemStack.getItem().getRegistryName() + ":" +
                                    itemStack.getItemDamage() + "\" show=\"false\" /><!-- " +
                        ...
                        for (int i : oreIDs) {
                            System.out.println(
                                    "    <item name=\"" + OreDictionary.getOreName(i) + "\" show=\"false\" />");
                        }
                    }
                }
            }
    ...
    public static @Nonnull Block getBlockNN(@Nonnull IModObject forModObject) {
        return NullHelper.notnull(getBlock(forModObject),
                "missing block for modObject " + forModObject.getRegistryName());
    }
    ...
    public static @Nonnull Item getItemNN(@Nonnull IModObject forModObject) {
        return NullHelper.notnull(getItem(forModObject),
                "missing item for modObject " + forModObject.getRegistryName());
    }
