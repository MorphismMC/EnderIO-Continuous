...
public class PaintUtil {
    ...
    public static boolean isValid(@Nonnull ItemStack paintSource, Block target) {
        ...
        if (Prep.isValid(paintSource)) {
            ...
            IBlockState bs = Block$getBlockFromItem_stack$getItem___$getStateFromMeta_stack$getMetadata___(paintSource,
                    block);
            ...
            if (!shouldHaveModel(block)) {
                ...
                // Weird error case: Our pressure plates are always painted, so they cannot be used as paint sources
                // (e.g. to unpaint them)
        ...
        if (paintSource == null || paintSource.getBlock() == Blocks.AIR) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Failed to write blockstate to nbt. blockstate=" + paintSource + " registry name=" + s +
    ...
    // Note: Config-based white-/blacklisting is done by PaintSourceValidator and checked as part of the input slot
    // validation of the Painter
        ...
        if (itemIn != null) {
            if (itemIn instanceof ItemBlock) {
                ...
                if (NullHelper.untrust(block) == null) {
                    Log.warn(
                            "ItemBlock " + itemIn + " returned null from getBlock(). This is a major bug in the mod '" +
                                    block2Modname(itemIn) + "'.");
                } else if (NullHelper.untrust(Block.REGISTRY.getNameForObject(block)) == null) {
                    throw new RuntimeException(
                            "ItemBlock " + itemIn +
                                    " returned an unregistered block from getBlock(). This is a major bug in the mod '" +
                                    block2Modname(itemIn) + "'.");
                }
                return block;
            }
            ...
            if (fluidStack != null && fluidStack.getFluid() != null) {
                ...
                if (block != null && NullHelper.untrust(Block.REGISTRY.getNameForObject(block)) == null) {
                    throw new RuntimeException("Fluid " + fluidStack.getFluid() + " (" +
                            fluidStack.getFluid().getName() + ", " + fluidStack.getFluid().getClass() + ") from item " +
                            itemIn +
    ...
    public static Block getBlockFromItem(@Nonnull ItemStack itemStack) {
        if (Prep.isValid(itemStack)) {
            if (itemStack.getItem() instanceof ItemBlock) {
                ...
                if (NullHelper.untrust(block) == null) {
                    Log.warn("ItemBlock " + itemStack +
                            " returned null from getBlock(). This is a major bug in the mod '" +
                            block2Modname(itemStack.getItem()) + "'.");
                } else if (NullHelper.untrust(Block.REGISTRY.getNameForObject(block)) == null) {
                    throw new RuntimeException("ItemBlock " + itemStack +
                            " returned an unregistered block from getBlock(). This is a major bug in the mod '" +
                            block2Modname(itemStack.getItem()) + "'.");
                }
                return block;
    ...
    public static IBlockState Block$getBlockFromItem_stack$getItem___$getStateFromMeta_stack$getMetadata___(ItemStack paintSource,
                                                                                                            Block paintBlock) {
        ...
        try {
            ...
            throw new RuntimeException(
                    String.format(
                            "Block %s (%s) belonging to item %s failed to convert its item damage into a blockstate. This is a bug in the mod '%s'.",
                            paintBlock,
        ...
        if (NullHelper.untrust(stateFromMeta) == null) {
            throw new RuntimeException(String.format(
                    "Block %s (%s) belonging to item %s returned null from getStateFromMeta(). This is a major bug in the mod '%s'.",
            ...
            throw new RuntimeException(String.format(
                    ...
                    paintBlock, paintBlock.getClass(), paintSource, stateFromMeta, stateFromMeta.getBlock(),
    ...
    /**
     * Registers blocks that can be painted but do not implement IPaintable themselves. Used for blocks that are not
     * Ender IO blocks, e.g. vanilla fences.
     */
    ...
    /**
     * Registers items that can be painted but are not items for blocks implement IPaintable themselves. Used for items
     * that do not have a block , e.g. Dark Steel
     * Helmets and Facades.
     */
    ...
    public static boolean isPaintable(@Nonnull ItemStack stack) {
        ...
        return isPainted(stack) || Block.getBlockFromItem(stack.getItem()) instanceof IPaintable ||
                PaintUtil.paintables.contains(stack);
    }
