...
public class HandlerSoulBound {
     ...
     * 
     * We go early, so we can get our items before other mods put them into some grave. Also remove them from the list
     * so they won't get duped. If the inventory
    ...
    /*
     * Do a second (late) pass. If any mod has added items to the list in the meantime, this gives us a chance to save
     * them, too. If some gravestone mod has
     ...
     * onPlayerDeath() above placed items in it.
     * 
     * Note: Without other death-modifying mods, the content of the old inventory would always fit into the new one
     * (both being empty but for soulbound items in
     * the old one) and the old one would be discarded just after this method. But better play it safe and assume that
     * an overflow is possible and that another
    ...
    public static void onPlayerClone(PlayerEvent.Clone evt) {
        ...
        if (evt.getOriginal() == evt.getEntityPlayer() ||
            ...
            Log.warn("Player " + evt.getEntityPlayer().getName() +
                    " just died and respawned in their old body. Did someone fire a PlayerEvent.Clone(death=true) " +
    ...
    /*
     * Do a second (late) pass and try to preserve any remaining items by spawning them into the world. They might end
     * up nowhere, but if we do nothing they will
    ...
    public static void onPlayerCloneLast(PlayerEvent.Clone evt) {
        ...
        for (int i = 0; i < evt.getOriginal().inventory.armorInventory.size(); i++) {
            ...
            if (isSoulBound(item)) {
                if (addToPlayerInventory(evt.getEntityPlayer(), item) ||
                        tryToSpawnItemInWorld(evt.getOriginal(), item)) {
                    evt.getOriginal().inventory.armorInventory.set(i, Prep.getEmpty());
                }
            }
        }
    ...
    private static boolean isSoulBound(@Nonnull ItemStack item) {
        return EnchantmentHelper.getEnchantmentLevel(NullHelper.notnullF(SOULBOUND, "Soulbound went unbound"), item) >
                0;
    }
