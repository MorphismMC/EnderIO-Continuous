...
public final class EnchantmentRepellent extends EnchantmentBase {
    ...
    private EnchantmentRepellent() {
        super(NAME, EnchantmentConfig.repellentRarity, EnumEnchantmentType.ARMOR,
                new EntityEquipmentSlot[] { EntityEquipmentSlot.CHEST, EntityEquipmentSlot.LEGS,
                        EntityEquipmentSlot.FEET },
                EnchantmentConfig.repellentEnabled);
    }

    @Override
    public int getMinEnchantability(int enchantmentLevel) {
        return EnchantmentConfig.repellentMinEnchantabilityBase.get() +
                EnchantmentConfig.repellentMinEnchantabilityPerLevel.get() * enchantmentLevel;
    }

    @Override
    public int getMaxEnchantability(int enchantmentLevel) {
        return EnchantmentConfig.repellentMaxEnchantabilityBase.get() +
                EnchantmentConfig.repellentMaxEnchantabilityPerLevel.get() * enchantmentLevel;
    }
    ...
    public void onUserHurt(@Nonnull EntityLivingBase user, @Nonnull Entity attacker, int level) {
        if (user instanceof EntityPlayer && attacker instanceof EntityLivingBase &&
                !EnchantmentHelper.getEnchantedItem(this, user).isEmpty()) {
            if (level > getMaxLevel()) {
                // No config here, this is the "out of range" error case
                for (Entity e : user.world.getEntitiesWithinAABBExcludingEntity(user,
                        user.getEntityBoundingBox().expand(level * 8, level * 4, level * 8))) {
                ...
                RandomTeleportUtil.teleportEntity(attacker.world, attacker, false,
                        attacker instanceof EntityPlayer ||
                                user.getRNG().nextFloat() < EnchantmentConfig.repellentSafeMobsChance.get(),
                        getRange(level));
            }
        }
    }
