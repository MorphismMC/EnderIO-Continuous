...
public class EnderItemOverrideList extends ItemOverrideList {
    ...
    public @Nonnull IBakedModel handleItemState(@Nonnull IBakedModel originalModel, @Nonnull ItemStack stack,
        ...
        if (block == Blocks.AIR) {
            ...
            if (block == null || block == Blocks.AIR) {
                throw new NullPointerException(
                        "Wrong parameter 'ItemStack stack', not an ItemBlock and not registered with ModObjectRegistry");
            }
        }
        ...
        if (block instanceof IBlockPaintableBlock &&
                (!(block instanceof IWrenchHideablePaint) || !YetaUtil.shouldHeldItemHideFacadesClient())) {
            ...
            if (paintSource != null && paintSource != Blocks.AIR.getDefaultState()) {
                Pair<Block, Long> cacheKey = NullHelper.notnull(
                        Pair.of((Block) null, new CacheKey().addCacheKey(paintSource).getCacheKey()), "no way");
                ...
                if (quads == null) {
                    ...
                    quads.addBlockState(
                            ModObject.block_machine_base.getBlockNN().getDefaultState().withProperty(EnumRenderPart.SUB,
                                    EnumRenderPart.PAINT_OVERLAY),
                            Prep.getEmpty());
        ...
        if (block instanceof ISmartRenderAwareBlock) {
            ...
            Pair<Block, Long> cacheKey = Pair.of(block, renderMapper
                    .getCacheKey(block, stack, new CacheKey().addCacheKey(stack.getMetadata())).getCacheKey());
            ...
            if (quads == null) {
                ...
                if (renderMapper instanceof IRenderMapper.IItemRenderMapper.IItemStateMapper) {
                    quads.addBlockStates(((IRenderMapper.IItemRenderMapper.IItemStateMapper) renderMapper)
                            .mapItemRender(block, stack, quads), stack, block);
                } else if (renderMapper instanceof IRenderMapper.IItemRenderMapper.IItemModelMapper) {
                    List<IBakedModel> bakedModels = ((IRenderMapper.IItemRenderMapper.IItemModelMapper) renderMapper)
                            .mapItemRender(block, stack);
                    if (bakedModels != null) {
            ...
            if (renderMapper instanceof IItemRenderMapper.IDynamicOverlayMapper) {
                quads = quads.combine(((IItemRenderMapper.IDynamicOverlayMapper) renderMapper)
                        .mapItemDynamicOverlayRender(block, stack));
            }
