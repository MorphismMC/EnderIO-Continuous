...
public class PaintWrangler {
    ...
    public static boolean wrangleBakedModel(@Nonnull IBlockAccess blockAccess, @Nonnull BlockPos pos,
        ...
        PaintedBlockAccessWrapper fakeWorld = null; // Mods may put this into their model, so we need to keep it around
                                                    // until we discard the model!
        try {
            if (memory.doActualState || memory.doExtendedState) {
                ...
                if (memory.doActualState) {
                    try {
                        ...
                        Log.warn("Failed to get actual state of block ", block,
                                " to use as paint. Error while rendering: ", t);
                ...
                if (memory.doExtendedState) {
                    try {
                        ...
                        Log.warn("Failed to get extended state of block ", block,
                                " to use as paint. Error while rendering: ", t);
            ...
            final IBakedModel paintModel = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
                    .getModelForState(actualPaintSource);
            if (paintModel == Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
            ...
            for (BlockRenderLayer layer : quads.getBlockLayers()) {
                if (layer == BREAKING || block.canRenderInLayer(paintSource,
                        NullHelper.notnullJ(layer, "'a == null || b(a)' gave null to b()"))) {
                    ForgeHooksClient.setRenderLayer(layer);
                    final String errors = quads.addUnfriendlybakedModel(layer, paintModel, extendedPaintSource,
                            positionRandom);
                    if (errors != null) {
