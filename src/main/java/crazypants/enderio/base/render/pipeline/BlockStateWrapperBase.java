...
public class BlockStateWrapperBase extends CacheKey implements IBlockStateWrapper {
    ...
    private final static Cache<Pair<Block, Long>, QuadCollector> cache = CacheBuilder.newBuilder().maximumSize(500)
    ...
    public BlockStateWrapperBase(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                                 IRenderMapper.IBlockRenderMapper renderMapper) {
        this.state = state;
    ...
    @Override
    public @Nonnull <T extends Comparable<T>, V extends T> IBlockState withProperty(@Nonnull IProperty<T> property,
                                                                                    @Nonnull V value) {
    ...
    public void bakeModel() {
        ...
        if (block instanceof IBlockPaintableBlock &&
                (!(block instanceof IWrenchHideablePaint) || !getYetaDisplayMode().isHideFacades())) {
            hasPaintRendered = PaintWrangler.wrangleBakedModel(world, pos,
                    ((IBlockPaintableBlock) block).getPaintSource(state, world, pos), paintQuads);
        }

        boolean fromCache = doCaching && MinecraftForgeClient.getRenderLayer() != null;
        if (!hasPaintRendered ||
                renderMapper instanceof IRenderMapper.IBlockRenderMapper.IRenderLayerAware.IPaintAware) {
            if (fromCache) {
                quads = getFromCache();
    ...
    protected void bakeBlockLayer(QuadCollector quads) {
        if (renderMapper == nullRenderMapper) {
            IBakedModel missingModel = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
    ...
    @Override
    public float getPlayerRelativeBlockHardness(@Nonnull EntityPlayer player, @Nonnull World worldIn,
    ...
    @Override
    public boolean shouldSideBeRendered(@Nonnull IBlockAccess blockAccess, @Nonnull BlockPos pos1,
    ...
    @Override
    public @Nonnull RayTraceResult collisionRayTrace(@Nonnull World worldIn, @Nonnull BlockPos pos1,
                                                     @Nonnull Vec3d start, @Nonnull Vec3d end) {
        return state.collisionRayTrace(worldIn, pos1, start, end);
    }

    @Override
    public boolean doesSideBlockRendering(@Nonnull IBlockAccess world1, @Nonnull BlockPos pos1,
    ...
    @Override
    public void neighborChanged(@Nonnull World worldIn, @Nonnull BlockPos pos1, @Nonnull Block blockIn,
    ...
    @Override
    public @Nonnull BlockFaceShape getBlockFaceShape(@Nonnull IBlockAccess worldIn, @Nonnull BlockPos pos1,
                                                     @Nonnull EnumFacing facing) {
        return state.getBlockFaceShape(worldIn, pos1, facing);
    }

    @Override
    public boolean doesSideBlockChestOpening(@Nonnull IBlockAccess world1, @Nonnull BlockPos pos1,
                                             @Nonnull EnumFacing side) {
