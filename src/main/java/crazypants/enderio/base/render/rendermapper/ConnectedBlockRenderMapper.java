...
public abstract class ConnectedBlockRenderMapper implements IRenderMapper.IBlockRenderMapper {
    ...
    protected boolean skip_top = false, skip_bottom = false, skip_side = false, skip_top_side = false,
            skip_bottom_side = false;

    @SideOnly(Side.CLIENT)
    protected abstract List<IBlockState> renderBody(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
    ...
    protected boolean isSameKind(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                                 @Nonnull BlockPos other) {
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public @Nullable List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
                                                      @Nonnull BlockPos pos,
        ...
        // For each of the 4 sides we add the top, bottom and right edge as well as the two corner between those. That
        // are all
        ...
        while (iterator.hasNext()) {
            ...
            add(states, skip_top, upper_edge, stateBordered, stateMerged, RENDER,
                    EnumMergingBlockRenderMode.get(facing, EnumFacing.UP));
            add(states, skip_bottom, lower_edge, stateBordered, stateMerged, RENDER,
                    EnumMergingBlockRenderMode.get(facing, EnumFacing.DOWN));
            add(states, skip_side, right_edge, stateBordered, stateMerged, RENDER,
                    EnumMergingBlockRenderMode.get(facing, facing.rotateYCCW()));

            // Our Corners //
            // /////////// //

            // The 3 edges that connect to the corners and belong to us. Two of them are checked above, this is the
            // third one:
            ...
            // The pathological case of having blocks over edge connecting to a common base. This adds the corner where
            // the 2 edges meet:
            ...
            boolean upper_right_corner = upper_edge || right_edge || upper_edge_around_right_corner ||
                    right_edge_above || top_facing_edge_right || facing_top_edge_right || checker_top_a ||
                    checker_top_b || checker_top_c;
            boolean lower_right_corner = lower_edge || right_edge || lower_edge_around_right_corner ||
                    right_edge_below || bottom_facing_edge_right || facing_bottom_edge_right || checker_bottom_a ||
                    checker_bottom_b || checker_bottom_c;
    ...
    protected <T extends Comparable<T>,
            V extends T> void add(@Nonnull List<IBlockState> states, boolean skip, boolean border,
                                  @Nullable IBlockState stateBordered, @Nullable IBlockState stateMerged,
                                  @Nonnull IProperty<T> property, @Nonnull V value) {
