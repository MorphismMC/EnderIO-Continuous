...
public class PowerBarOverlayRenderHelper {
     ...
     * 
     * SHOW_ON_FULL_UPGRADEABLE: Same but for items that can be upgraded to have an energy storage. This will give a
     * visual difference between unupgraded and full
     * items.
     * 
     ...
     */
    public static final @Nonnull PowerBarOverlayRenderHelper instance_upgradeable = new PowerBarOverlayRenderHelper(
            true);

    public static final @Nonnull PowerBarOverlayRenderHelper instance_upgradeable_vert = new VertPowerBarOverlayRenderHelper(
            true);
    ...
    public void render(double level, int xPosition, int yPosition, int offset, boolean shadow) {
        ...
        this.drawGrad(worldrenderer, xPosition + 2, yPosition + 13 - offset, (BAR_W + width) / 2, 1, colorBarLeft,
                colorBarRight);
    ...
    protected void drawGrad(BufferBuilder renderer, int x, int y, double width, double height, Vector4i colorL,
                            Vector4i colorR) {
    ...
    public static class FluidBarOverlayRenderHelper extends PowerBarOverlayRenderHelper {
        ...
        public boolean render(@Nonnull ItemStack stack, int xPosition, int yPosition, int barOffset,
            ...
            if (alwaysShow || shouldShowBar(maxFluid, fluidAmount)) {
                ...
                boolean up = stack.getItem().showDurabilityBar(stack);
                boolean top = stack.getCount() != 1;
                render(level, xPosition, yPosition, top ? 12 - barOffset : up ? 2 + barOffset : barOffset,
                        (barOffset & 1) == 0);
                return true;
            }
            return false;
    ...
    public static class VertPowerBarOverlayRenderHelper extends PowerBarOverlayRenderHelper {
        ...
        public void render(double level, int xPosition, int yPosition, int ignored, boolean ignored2) {
            ...
            // Note: witdh=1.4 --- we render a bit wider than correct to avoid "1 pixel bar, 2 pixel shadow" with some
            // screen scales
