...
public class SmartModelAttacher {
    ...
    private static class RegistrationHolder<T extends Comparable<T>, V extends T> {
        ...
        protected RegistrationHolder(@Nonnull Block block, IProperty<T> property, V defaultsValue, V autoValue,
                                     boolean itemOnly) {
            this.block = block;
    ...
    /**
     * Register a block that does not have one of our special rendering properties. All its blockstates will be rendered
     * by our smart model, so the render mapper
    ...
    public static <T extends Comparable<T>, V extends T> void register(@Nonnull Block block, IProperty<T> property,
                                                                       V defaultsValue, V autoValue) {
        register(block, property, defaultsValue, autoValue, false);
    }

    private static <T extends Comparable<T>, V extends T> void register(@Nonnull Block block, IProperty<T> property,
                                                                        V defaultsValue, V autoValue,
                                                                        boolean itemOnly) {
    ...
    /**
     * Registers the default ModelResourceLocation for the items of all blocks that have registered for
     * MachineSmartModel-based rendering.
     * <p>
     * For items that have subtypes, all subtypes are registered. All subtypes are registered to the same model, as the
                ...
                if (item instanceof IHaveRenderers || block instanceof IHaveRenderers) {
                    ...
                    if (item instanceof ICustomSubItems || block instanceof ICustomSubItems) {
                        throw new RuntimeException(
                                block.getClass() + " implements both IHaveRenderers and ICustomSubItems");
                    }
                    ...
                    if (item instanceof ICustomSubItems || block instanceof ICustomSubItems) {
                        throw new RuntimeException(
                                block.getClass() + " implements both IDefaultRenderers and ICustomSubItems");
                    }
                    ...
                    final @Nonnull ResourceLocation registryName = item instanceof ICustomItemResourceLocation ?
                            ...
                            NullHelper.notnullF(item.getRegistryName(),
                                    "Item.getItemFromBlock() returned an unregistered item");
                    ...
                    if (item.getHasSubtypes()) {
                        ...
                        if (item instanceof ICustomSubItems) {
                            ...
                            throw new RuntimeException(
                                    block.getClass() + " has subitems but it does not implement ICustomSubItems");
                        }
    ...
    public static void registerColoredBlocksAndItems(EnderIOLifecycleEvent.Init.Post event) {
        ...
        for (RegistrationHolder<?, ?> holder : blocks) {
            ...
            if (block instanceof IPaintable || block instanceof ITintedBlock || block instanceof ITintedItem ||
                    item instanceof ITintedItem) {
                blocklist.add(block);
                if (item != Items.AIR) {
                    itemlist.add(item);
                }
    ...
    public static void bakeModels(@Nonnull ModelBakeEvent event) {
        for (RegistrationHolder holder : blocks) {
            Block block = holder.block;
            Map<IBlockState, ModelResourceLocation> locations = event.getModelManager().getBlockModelShapes()
            ...
            if (holder_property != null && block.getDefaultState().getPropertyKeys().contains(holder_property) &&
                ...
                if (defaultBakedModel == null) {
                    Log.warn("ModelBakeEvent: cannot register smart model over null model for " + holder.block +
                            ". See model errors below.");
                    continue;
                }
                RelayingBakedModel model = new RelayingBakedModel(defaultBakedModel);

                ModelResourceLocation itemMrl = new ModelResourceLocation(
                        defaultMrl.getNamespace() + ":" + defaultMrl.getPath() + "#inventory");
                event.getModelRegistry().putObject(itemMrl, model);

                if (defaultBakedModel == missingModel) {
                    // This is normal on the first pass. We register our synthetic models above anyway to avoid model
                    // loading errors.
                    continue;
                }

                for (Entry<IBlockState, ModelResourceLocation> entry : locations.entrySet()) {
                    final ModelResourceLocation entryMrl = NullHelper.notnullF(entry.getValue(),
                            "BlockModelShapes contains null keys");
                ...
                if (!holder.itemOnly) {
                    for (ModelResourceLocation mrl0 : locations.values()) {
                        final ModelResourceLocation mrl = NullHelper.notnullF(mrl0,
                                "BlockModelShapes contains null keys");
                        ...
                        if (model == null || model == missingModel) {
                            ...
                            event.getModelRegistry().putObject(mrl,
                                    new RelayingBakedModel(NullHelper.first(model, defaultBakedModel)));
                        }
                    }
                }
    ...
    private static String debugOutput(IRegistry<ModelResourceLocation, IBakedModel> modelRegistry,
                                      ResourceLocation defaultMrl, IBakedModel missingModel) {
        ...
        if (modelRegistry instanceof RegistrySimple) {
            ...
            for (Object key : rg.getKeys()) {
                if (key != null && modelRegistry.getObject((ModelResourceLocation) key) != missingModel &&
