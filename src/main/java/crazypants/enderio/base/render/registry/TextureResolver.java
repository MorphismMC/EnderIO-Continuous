...
public final class TextureResolver {
    ...
    public static void onIconLoad(TextureStitchEvent.Pre event) {
        for (Entry<ResourceLocation, Object> entry : TextureRegistry.getSprites().entrySet()) {
            ...
            entry.setValue(
                    event.getMap().registerSprite(NullHelper.notnull(entry.getKey(), "internal data corruption")));
        }
        TextureRegistry.lock(key -> {
            // this fallback should never be triggered as all sprites get assigned a texture and the registry is locked
            // here
