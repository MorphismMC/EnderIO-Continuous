...
public class HalfBakedQuad {
    ...
    public HalfBakedQuad(@Nonnull BoundingBox bb, @Nonnull EnumFacing face, float umin, float umax, float vmin,
                         float vmax, @Nullable TextureAtlasSprite tex,
    ...
    public void bake(@Nonnull List<BakedQuad> quads) {
        ...
        UnpackedBakedQuad.Builder builder = new UnpackedBakedQuad.Builder(DefaultVertexFormats.ITEM); // this one has
                                                                                                      // normals
    ...
    public void render(@Nonnull BufferBuilder tes) {
        for (Vertex v : corners) {
            tes.pos(v.x(), v.y(), v.z()).tex(tex.getInterpolatedU(v.u() * 16), tex.getInterpolatedV(v.v() * 16))
    ...
    public static class HalfBakedList extends AbstractList<HalfBakedQuad> {
        ...
        public void add(@Nonnull BoundingBox bb, @Nonnull EnumFacing face, float umin, float umax, float vmin,
                        float vmax, TextureAtlasSprite tex, Vector4f color) {
            store.add(new HalfBakedQuad(bb, face, umin, umax, vmin, vmax, tex, color));
        }

        public void add(@Nonnull BoundingBox bb, @Nonnull EnumFacing face, double umin, double umax, double vmin,
                        double vmax, TextureAtlasSprite tex,
                        Vector4f color) {
        ...
        public void add(@Nonnull BoundingBox bb, @Nonnull EnumFacing face, double umin, double umax, double vmin,
            ...
            if (upsidedown) {
                store.add(new HalfBakedQuad(bb, face, (float) umin, (float) umax, (float) vmax, (float) vmin, tex,
                        color));
            } else {
                store.add(new HalfBakedQuad(bb, face, (float) umin, (float) umax, (float) vmin, (float) vmax, tex,
                        color));
            }
        }
