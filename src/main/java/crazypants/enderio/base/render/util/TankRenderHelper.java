...
public class TankRenderHelper {
     ...
     */
    public static HalfBakedList mkTank(SmartTank tank, double xzBorder, double miny, double maxy,
        ...
        if (tank != null) {
            ...
            if (fluid != null && ratio > 0) {
                ...
                Vector4f vecC = new Vector4f((color >> 16 & 0xFF) / 255d, (color >> 8 & 0xFF) / 255d,
                        (color & 0xFF) / 255d, 1);
                TextureAtlasSprite sprite = still == null ? null :
                        Minecraft.getMinecraft().getTextureMapBlocks().getTextureExtry(still.toString());
                if (sprite == null) {
                ...
                BoundingBox bb = gas ?
                        new BoundingBox(xzBorder * px, (maxy - (maxy - miny) * ratio) * px, xzBorder * px,
                                (16 - xzBorder) * px, maxy * px, (16 - xzBorder) * px) :
                        new BoundingBox(xzBorder * px, miny * px, xzBorder * px, (16 - xzBorder) * px,
                                ((maxy - miny) * ratio + miny) * px, (16 - xzBorder) * px);
