...
public class QuadCollector {
    ...
    private static Integer mkKey(EnumFacing side, BlockRenderLayer pass) {
        return (side == null ? 0 : side.ordinal() + 1) * (BlockRenderLayer.values().length + 1) +
                (pass == BREAKING ? 0 : pass.ordinal() + 1);
    }
    ...
    /**
     * Adds the baked model(s) of the given block states to the quad lists for the given block layer. The models are
     * expected to behave. The block layer will be
    ...
    /**
     * Adds a baked model that is may blow up to the quad lists for the given block layer. The block layer will NOT be
     * set when the model is asked for its quads.
     * <p>
     * Any errors from the model will be returned.
     */
    public @Nullable String addUnfriendlybakedModel(BlockRenderLayer pass, IBakedModel model, IBlockState state,
                                                    long rand) {
        if (model == null) {
            return null;
        }
    ...
    /**
     * Adds a baked model that is expected to behave to the quad lists for the given block layer. The block layer will
     * be set when the model is asked for its
     * quads.
     */
    public void addFriendlybakedModel(BlockRenderLayer pass, IBakedModel model, @Nullable IBlockState state,
                                      long rand) {
        if (model != null) {
    ...
    private static final class BlockRenderLayerIterable implements Iterable<BlockRenderLayer> {
        ...
        private static final class BlockRenderLayerIterator implements Iterator<BlockRenderLayer> {
            ...
            private final static BlockRenderLayer[] LAYERS = Arrays.copyOf(BlockRenderLayer.values(),
                    BlockRenderLayer.values().length + 1);
    ...
    /**
     * Returns an Iterable for all valid BlockRenderLayers (including null). Be aware that the Iterator returned by the
     * Iterable does no checks on proper use at
