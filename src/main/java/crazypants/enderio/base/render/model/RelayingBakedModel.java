...
public class RelayingBakedModel implements IBakedModel {
    ...
    private @Nonnull IBakedModel getDefaults() {
        if (defaults == null) {
            try {
                defaults = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getModelManager()
                        .getMissingModel();
            } catch (Throwable t) {
    ...
    public @Nonnull List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand) {
        ...
        if (state instanceof BlockStateWrapperBase) {
            ...
            if (model != null) {
                crazypants.enderio.util.Profiler.instance.stop(start,
                        state.getBlock().getLocalizedName() + " (relayed)");
