...
public class RotatingSmartItemModel implements IBakedModel {
    ...
    public RotatingSmartItemModel(@Nonnull IBakedModel parent,
                                  @Nonnull Function<ItemCameraTransforms.TransformType, Vector4d> rots) {
        this.parent = parent;
        this.rots = rots;
    }
    ...
    public @Nonnull Pair<? extends IBakedModel, Matrix4f> handlePerspective(@Nonnull ItemCameraTransforms.TransformType cameraTransformType) {
        ...
        double r = (EnderIO.proxy.getTickCount() % 360) +
                (Minecraft.getMinecraft().isGamePaused() ? 0 : Minecraft.getMinecraft().getRenderPartialTicks());
        ...
        TRSRTransformation transformNew = new TRSRTransformation(transformOrig.getTranslation(), leftRot,
                transformOrig.getScale(), transformOrig.getRightRot());
