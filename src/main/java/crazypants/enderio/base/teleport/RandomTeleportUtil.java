...
public class RandomTeleportUtil {
    ...
    public static void teleportSpawnItem(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull ItemStack stack) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (isClear(world, entity, targetX, targetY, targetZ) &&
                    doTeleport(world, entity, targetX, targetY, targetZ)) {
    ...
    public static void teleportEntity(@Nonnull World world, @Nonnull Entity entity, boolean isItem,
                                      boolean dropToGround, float range) {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (dropToGround) {
                ...
                while (targetY >= 2f && !(hasGround(world, targetX, targetY, targetZ) &&
                        isClear(world, entity, targetX, targetY, targetZ))) {
                    targetY -= 1f;
                }
            }
            if (targetY >= 2f && isClear(world, entity, targetX, targetY, targetZ) &&
                    doTeleport(world, entity, targetX, targetY, targetZ)) {
                final SoundRegistry sound = isItem ? SoundRegistry.TRAVEL_SOURCE_ITEM :
                        SoundRegistry.TRAVEL_SOURCE_BLOCK;
    ...
    private static boolean isClear(@Nonnull World world, @Nonnull Entity entity, double targetX, double targetY,
                                   double targetZ) {
    ...
    private static boolean doTeleport(@Nonnull World world, @Nonnull Entity entity, double targetX, double targetY,
                                      double targetZ) {
    ...
    private static boolean doTeleport(@Nonnull World world, @Nonnull EntityLivingBase entity, double targetX,
        ...
        if (!MinecraftForge.EVENT_BUS.post(event)) {
            if (rand.nextFloat() < 0.15F && world.getGameRules().getBoolean("doMobSpawning") &&
                ...
                entityendermite.setLocationAndAngles(entity.posX, entity.posY, entity.posZ, entity.rotationYaw,
            ...
            if (entity instanceof EntityPlayerMP) {
                ((EntityPlayerMP) entity).connection.setPlayerLocation(event.getTargetX(), event.getTargetY(),
                        event.getTargetZ(), entity.rotationYaw,
                        entity.rotationPitch);
            } else {
