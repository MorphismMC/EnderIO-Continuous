...
public class DSUGui extends GuiContainerBaseEIO<DSUContainer> implements DSUContainerProxy {
    ...
    private final static NNList<String> TEXTURES = new NNList<>(EnderIO.DOMAIN + ":items/paint_overlay",
    ...
    private void setInitialTab() {
        if (!hasSetTab) {
            hasSetTab = true;
            EntityEquipmentSlot preferedSlot = initialTab > -1 ? EnumReader.get(EntityEquipmentSlot.class, initialTab) :
            ...
            for (UpgradeCap cap : getOwner().caps) {
                if (cap.isAvailable()) {
                    ...
                    if (found < 0 ||
                            (cap.getSlotSelector().isSlot() && cap.getSlotSelector().getSlot() == preferedSlot)) {
                        found = cap.getSlotSelector().getTabOrder();
                    }
                }
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (getOwner().activeTab.isAnvil()) {
            ...
            for (Slot inventorySlot : getOwner().inventorySlots) {
                if (inventorySlot instanceof DSUContainer.AutoSlot) {
                    ...
                    if (slot.isEnabled()) {
                        hasAnySlots = true;
                        if (slot.isHead()) {
                            drawTexturedModalRect(guiLeft + slot.getX() - 1 - 6, guiTop + slot.getY() - 1, 200, 18, 6,
                                    18);
                        }
                        if (slot.isLocked()) {
            ...
            if (hasAnySlots) {
                ...
                String str = (getOwner().activeTab.isItem() ? Lang.GUI_DSU_NOT_INSERTED : Lang.GUI_DSU_NOT_INSTALLED)
                        .get();
                int y = 0;
                for (String sub : fontRenderer.listFormattedStringToWidth(str, xSize)) {
                    if (sub != null) {
                        int stringWidth = fontRenderer.getStringWidth(sub);
                        fontRenderer.drawString(sub, guiLeft + xSize / 2 - stringWidth / 2,
                                guiTop + (ySize - 86) / 2 + y, 4210752);
                        y += 9;
                    }
                }
            }
    ...
    private void drawTabs() {
        ...
        for (UpgradeCap cap : getOwner().caps) {
            if (cap.getSlotSelector().isItem()) {
                Rectangle tabarea = renderStdTab(guiLeft, guiTop, cap.getSlotSelector().getTabOrder(),
                ...
                renderStdTab(guiLeft, guiTop, cap.getSlotSelector().getTabOrder(),
                        cap.getSlotSelector().getItem(mc.player),
                        cap.getSlotSelector() == getOwner().activeTab);
            } else if (cap.getSlotSelector().isSlot()) {
                renderStdTab(guiLeft, guiTop, cap.getSlotSelector().getTabOrder(),
                        new AtlasWidgetIcon(mc.getTextureMapBlocks()
                                .getAtlasSprite(TEXTURES.get(cap.getSlotSelector().getSlot().ordinal()))),
                        cap.getSlotSelector() == getOwner().activeTab);
            }
        }
    }
