...
public class DSUContainer extends ContainerEnderCap<EIOCombinedInvWrapper<UpgradeCap>, TileEntity> {
        ...
        public boolean drawGhostSlotToolTip(@Nonnull GuiContainerBase gui, int mouseX, int mouseY) {
            if (gui.mc.player.inventory.getItemStack().isEmpty()) {
                ...
                if (slot.isBlocked()) {
                    ...
                    if (!reason.isEmpty()) {
                        text.add("");
                        text.addAll(reason.stream().peek(itc -> itc.getStyle().setColor(TextFormatting.DARK_RED))
                                .map(ITextComponent::getFormattedText)
                                .collect(Collectors.toList()));
                    }
                }
                GuiUtils.drawHoveringText(Prep.getEmpty(), text, mouseX, mouseY, gui.width, gui.height, -1,
                        gui.getFontRenderer());
                return true;
            }
            return false;
    ...
    public static @Nullable DSUContainer create(EntityPlayer player, World world, BlockPos pos,
    ...
    @Override // opened for sub container
    protected void clearContainer(@Nonnull EntityPlayer playerIn, @Nonnull World worldIn,
                                  @Nonnull IInventory inventoryIn) {
