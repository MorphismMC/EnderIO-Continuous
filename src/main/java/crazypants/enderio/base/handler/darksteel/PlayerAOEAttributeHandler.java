...
public class PlayerAOEAttributeHandler {
    ...
    public static final @Nonnull IAttribute AOE_XZ = new RangedAttribute(null, "enderio.aoe.xz", 0.0D, 0.0D, 16)
            .setShouldWatch(true);
    public static final @Nonnull IAttribute AOE_Y = new RangedAttribute(null, "enderio.aoe.y", 0.0D, 0.0D, 16)
            .setShouldWatch(true);
    public static final @Nonnull IAttribute AOE_XYZ = new RangedAttribute(null, "enderio.aoe.xyz", 0.0D, 0.0D, 16)
            .setShouldWatch(true);

    @SubscribeEvent
    public static void handleConstruct(@Nonnull net.minecraftforge.event.entity.EntityEvent.EntityConstructing event) {
        /*
         * This event is fired in the constructor of Entity. The attribute map we add values to belongs to EntityLiving.
         * So this should not actually work, as
         * EntityLiving has not yet been constructed fully when we are called. But luckily for us, EntityLiving does not
         * initialize the attribute map field with
         * anything. So our value gets put in too early but is then not overwritten. However, I doubt the JVM specs
         * actually allow this loophole. So this may stop
         * working at any time.
         * 
         * And when it does, there will be three warnings about ignored unknown SharedMonsterAttributes when a player is
         * loaded from the server's save file. Which
    ...
    public static boolean hasAOE(@Nonnull EntityPlayer player) {
        return StateController.isActive(player, ExplosiveUpgrade.INSTANCE) &&
                (player.getEntityAttribute(AOE_XZ).getAttributeValue() > 0 ||
                        player.getEntityAttribute(AOE_Y).getAttributeValue() > 0 ||
                        player.getEntityAttribute(AOE_XYZ).getAttributeValue() > 0);
    }

    public static @Nonnull AxisAlignedBB expandBBbyAOE(@Nonnull EntityPlayer player, @Nonnull AxisAlignedBB bb,
                                                       RayTraceResult rtr) {
            ...
            if (player != null && !player.isSneaking() && hasAOE(player)) {
                ...
                if (world.getWorldBorder().contains(blockpos)) {
                    ...
                    if (iblockstate.getMaterial() != Material.AIR) {
                        ...
                        final AxisAlignedBB bb = expandBBbyAOE(player, origin, movingObjectPositionIn)
                                .grow(0.0020000000949949026D).offset(-d3, -d4, -d5);

                        drawSelectionBoundingBox(origin.grow(0.0020000000949949026D).offset(-d3, -d4, -d5), bb, 0.25F,
                                0.0F, 0.0F, 0.4F);
                    }
                }
            }
    ...
    public static @Nullable RayTraceResult rayTrace(@Nonnull EntityPlayer player) {
        return ((ItemDarkSteelPickaxe) ModObject.itemDarkSteelPickaxe.getItemNN()).rayTrace(player.world, player,
                false);
    }
    ...
    private static class BBIterator extends CubicBlockIterator {
        ...
        public BBIterator(@Nonnull BlockPos origin, @Nonnull BoundingBox bb) {
            super(origin, (int) bb.minX, (int) bb.minY, (int) bb.minZ, (int) bb.maxX - 1, (int) bb.maxY - 1,
                    (int) bb.maxZ - 1);
        }
    ...
    public static Iterator<BlockPos> getAOE(@Nonnull BlockPos origin, @Nonnull EntityPlayer player) {
        ...
        if (movingObjectPositionIn != null && movingObjectPositionIn.typeOfHit == RayTraceResult.Type.BLOCK) {
            if (origin.equals(movingObjectPositionIn.getBlockPos())) {
                return new BBIterator(origin,
                        new BoundingBox(expandBBbyAOE(player, new AxisAlignedBB(origin), movingObjectPositionIn)));
            }
        }
     ...
     */
    @SideOnly(Side.CLIENT)
    public static void drawSelectionBoundingBox(@Nonnull AxisAlignedBB origin, @Nonnull AxisAlignedBB box, float red,
                                                float green, float blue, float alpha) {
        final double minXO = origin.minX, minYO = origin.minY, minZO = origin.minZ, maxXO = origin.maxX,
                maxYO = origin.maxY, maxZO = origin.maxZ;
        final double minX = box.minX, minY = box.minY, minZ = box.minZ, maxX = box.maxX, maxY = box.maxY,
                maxZ = box.maxZ;
