...
public class AnvilSubGui implements IInventoryChangedListener {
    ...
    /**
     * How far to shift the anvil GUI down from its original position. It's smaller then our GUI so it'd look funny if
     * it stayed at the top.
     */
    private static final int ANVIL_Y_OFFSET = AnvilSubContainer.ANVIL_Y_OFFSET;
    /**
     * How many pixels to remove at the left, right and top of the anvil texture. This should just cut off the GUI
     * border.
     */
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (anvil.anvil.getMaximumCost() > 0) {
            ...
            if (anvil.anvil.getMaximumCost() >= AnvilSubContainer.getMaxCost() &&
                    !parent.mc.player.capabilities.isCreativeMode) {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        int i = (parent.width - parent.getXSize()) / 2;
        int j = (parent.height - parent.getYSize()) / 2;
        parent.drawTexturedModalRect(i + ANVIL_BORDER, j + ANVIL_Y_OFFSET + ANVIL_BORDER, ANVIL_BORDER, ANVIL_BORDER,
                parent.getXSize() - 2 * ANVIL_BORDER,
                ANVIL_HEIGHT - ANVIL_BORDER);
        parent.drawTexturedModalRect(i + 59, j + 20 + ANVIL_Y_OFFSET, 0,
                166 /* ysize */ + (anvil.getSlot(0).getHasStack() ? 0 : 16), 110, 16);
