...
public class DarkSteelTooltipManager {
    ...
    public static void addCommonTooltipEntries(@Nonnull ItemStack itemstack, EntityPlayer entityplayer,
        ...
        if (itemstack.getItem() instanceof IDarkSteelItem) {
            for (IDarkSteelUpgrade upgrade : UpgradeRegistry.getUpgrades()) {
                if (upgrade instanceof IAdvancedTooltipProvider &&
                        upgrade.hasUpgrade(itemstack, (IDarkSteelItem) itemstack.getItem())) {
    ...
    public static void addBasicTooltipEntries(@Nonnull ItemStack itemstack, EntityPlayer entityplayer,
                                              @Nonnull List<String> list, boolean flag) {
    ...
    public static void addAdvancedTooltipEntries(@Nonnull ItemStack itemstack, EntityPlayer entityplayer,
                                                 @Nonnull List<String> list, boolean flag) {
        ...
        if (itemstack.getItem() instanceof IDarkSteelItem && !skipUpgradeTooltips) {
            ...
            if (!applyableUpgrades.isEmpty()) {
                ...
                for (IDarkSteelUpgrade up : applyableUpgrades) {
                    list.add(Lang.DARK_STEEL_LEVELS2.get(TextFormatting.DARK_AQUA, TextFormatting.ITALIC,
                            up.getDisplayName(), 1));
                }
            }
        }
