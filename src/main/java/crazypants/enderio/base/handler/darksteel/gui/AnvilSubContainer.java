...
public class AnvilSubContainer {
    ...
    class OutputSlot extends HidingSlot {
        ...
        public boolean canTakeStack(@Nonnull EntityPlayer playerIn) {
            return (playerIn.capabilities.isCreativeMode || playerIn.experienceLevel >= getMaximumCost()) &&
                    getMaximumCost() > 0 && getHasStack();
        }
    ...
    public void updateRepairOutput() {
        ...
        if (itemstack.isEmpty()) {
            ...
            if (!itemstack2.isEmpty()) {
                ...
                applyingBook = itemstack2.getItem() == Items.ENCHANTED_BOOK &&
                        !ItemEnchantedBook.getEnchantments(itemstack2).isEmpty();

                if (outputStack.isItemStackDamageable() &&
                        outputStack.getItem().getIsRepairable(itemstack, itemstack2)) {
                    ...
                    for (materialCost = 0; itemDamageToRepair > 0 &&
                            materialCost < itemstack2.getCount(); ++materialCost) {
                    ...
                    if (!applyingBook &&
                            (outputStack.getItem() != itemstack2.getItem() || !outputStack.isItemStackDamageable())) {
                        abortCalculatingOutput();
                        return;
                    }
                    ...
                    for (Enchantment enchantment : bookEnchants.keySet()) {
                        if (enchantment != null) {
                            int existingLevel = existingEnchantments.containsKey(enchantment) ?
                                    existingEnchantments.get(enchantment).intValue() : 0;
                            ...
                            for (Enchantment existingEnchantment : existingEnchantments.keySet()) {
                                if (existingEnchantment != null && existingEnchantment != enchantment &&
                                        !enchantment.isCompatibleWith(existingEnchantment)) {
                                    canApply = false;
                                    ++cost;
                                }
                            }
    ...
    private int handleAnvilDamage(ItemStack stack) {
        ...
        float breakChance = ForgeHooks.onAnvilRepair(player, stack, inputSlots.getStackInSlot(0),
        ...
        if (player.getRNG().nextFloat() < (breakChance * damageChance)) {
            ...
            if (l > 2) {
                if (iblockstate.getBlock() == ModObject.blockDarkSteelAnvil.getBlockNN()) {
                    parent.target
                            .write(ModObject.blockBrokenAnvil.getBlockNN().getDefaultState()
                                    .withProperty(BlockAnvil.FACING, iblockstate.getValue(BlockAnvil.FACING)));
                } else {
