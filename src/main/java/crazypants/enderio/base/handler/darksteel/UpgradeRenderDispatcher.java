...
public class UpgradeRenderDispatcher implements LayerRenderer<AbstractClientPlayer> {
    ...
    @Override
    public void doRenderLayer(@Nonnull AbstractClientPlayer player, float limbSwing, float limbSwingAmount,
        ...
        NNList.of(EntityEquipmentSlot.class).apply(new Callback<EntityEquipmentSlot>() {
            ...
            public void apply(@Nonnull EntityEquipmentSlot slot) {
                ...
                if (item.getItem() instanceof IDarkSteelItem) {
                    for (IDarkSteelUpgrade upgrade : UpgradeRegistry.getUpgrades()) {
                        if (upgrade instanceof IHasPlayerRenderer && upgrade.hasUpgrade(item)) {
                            ((IHasPlayerRenderer) upgrade).getRender(player).doRenderLayer(renderPlayer, slot, item,
                                    player, limbSwing, limbSwingAmount, partialTicks,
                                    ageInTicks,
                ...
                if (item.getItem() instanceof IHasPlayerRenderer) {
                    ((IHasPlayerRenderer) item.getItem()).getRender(player).doRenderLayer(renderPlayer, slot, item,
        ...
        if (baubles != null) {
            for (int i = 0; i < baubles.getSizeInventory(); i++) {
                ...
                if (piece.getItem() instanceof IHasPlayerRenderer) {
                    ((IHasPlayerRenderer) piece.getItem()).getRender(player).doRenderLayer(renderPlayer, null, piece,
                            player, limbSwing, limbSwingAmount,
                            partialTicks,
                            ageInTicks, netHeadYaw, headPitch, scale);
                }
