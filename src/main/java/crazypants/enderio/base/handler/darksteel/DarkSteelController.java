...
public class DarkSteelController {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == Phase.START && !event.player.isSpectator() &&
        ...
        if (distance > 3) {
            ...
            if (boots.getItem() instanceof IDarkSteelItem) {
                ...
                if (energyStored > 0) {
                    ...
                    int energyCost = (int) Math.min(energyStored,
                            Math.ceil(toMitigate * DarkSteelConfig.fallDistanceCost.get()));
                    ...
                    if (mitigated < toMitigate) {
                        // Log.debug("Mitigating fall damage partially: original=", distance, " mitigated=", mitigated,
                        // " remaining=", distance - mitigated, " power used=",
                        // energyCost);
    ...
    private static void updateStepHeight(EntityPlayer player) {
        if (!player.isSneaking() && StepAssistUpgrade.isEquipped(player) &&
                StateController.isActive(player, StepAssistUpgrade.INSTANCE)) {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (NullHelper.untrust(player) == null) {
            // Log.warn("(in TickEvent.ClientTickEvent) net.minecraft.client.Minecraft.player is marked @Nonnull but it
            // is null.");
            return;
        }
        if (NullHelper.untrust(player.movementInput) == null) {
            // Log.warn("(in TickEvent.ClientTickEvent) net.minecraft.client.entity.EntityPlayerSP.movementInput is
            // marked @Nonnull but it is null.");
            return;
        }
        ...
        if (!jumpHandled && input.jump && !DATA.get().jumpPre && !player.onGround && player.motionY < 0.0D &&
                !player.capabilities.isFlying && isElytraUpgradeEquipped(player) &&
                !StateController.isActive(player, ElytraUpgrade.INSTANCE)) {
    ...
    private static boolean doJump(@Nonnull EntityPlayerSP player) {
        ...
        int requiredPower = DarkSteelConfig.bootsJumpPowerCost.get() *
                (int) Math.pow(DATA.get().jumpCount + 1 - autoJumpOffset, 2.5);
        ...
        if (availablePower > 0 && requiredPower <= availablePower && DATA.get().jumpCount < maxJumps) {
            DATA.get().jumpCount++;
            player.motionY += 0.15 * DarkSteelConfig.darkSteelBootsJumpModifier.get() *
                    (DATA.get().jumpCount - autoJumpOffset);
    ...
    public static boolean isTopActive(@Nonnull EntityPlayer player) {
        return ItemUtil.getOrCreateNBT(player.getItemStackFromSlot(EntityEquipmentSlot.HEAD))
                .hasKey(TheOneProbeUpgrade.PROBETAG);
    }
