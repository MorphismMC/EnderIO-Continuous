...
public class UpgradeCap implements IItemHandlerModifiable {
    ...
    /**
     * Upgrades with multiple levels only have the highest applied upgrade on the item. In this case, the lower tier
     * upgrades need to be shown in the GUI
     * artificially. This method checks if an upgrade either is applied itself or is the lower level of an applied
     * higher tier.
     * <p>
     * This relies on tiered upgrades being in order.
     * <p>
     * Note that this method should not be used on the "extract" path. Only upgrades that are actually present may be
     * removed. Extraction will automatically fail
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        ...
        if (isInventorySlot(slot)) {
            ...
            if (!simulate) {
                NbtValue.DSUINV.setStack(owner.getLeft(), slot - stacks.size(),
        ...
        if (showUpgrade(slot) || !UpgradeRegistry.isUpgradeItem(upgrade, stack) ||
                !upgrade.canAddToItem(owner.getLeft(), owner.getRight())) {
            return stack;
        }
     ...
     * <p>
     * Block slots represent upgrades that cannot be applied because either their prerequisites are not met or they
     * conflict with another already applied upgrade.
     * <p>
     * Slots that already contain an upgrade are always "open".
     */
    public boolean isSlotBlocked(int slot) {
        return !isInventorySlot(slot) && !showUpgrade(slot) &&
                !stacks.get(slot).upgrade.canAddToItem(getOwner().getLeft(), getOwner().getRight());
    }
    ...
    public List<ITextComponent> getSlotBlockedReason(int slot) {
        final NNPair<ItemStack, IDarkSteelItem> owner = getOwner();
        return NullHelper
                .first(isSlotBlocked(slot) ? stacks.get(slot).upgrade.getRules().stream()
                        .map(rule -> rule.check(owner.getLeft(), owner.getRight())).filter(CheckResult::hasResult)
                        .map(CheckResult::getResult).collect(Collectors.toList()) : null, NNList.emptyList());
    }
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!existing ||
                !stacks.stream().map(holder -> holder.upgrade)
                        .filter(up -> up != upgrade && up.hasUpgrade(owner.getLeft()))
    ...
    /**
     * The game doesn't sync player inventory slots that don't have a matching slot in the currently open container. We
                ...
                for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                    if (player.inventory.getStackInSlot(i) == owner) {
                        ((EntityPlayerMP) player).connection
                                .sendPacket(new SPacketSetSlot(-2, i, player.inventory.getStackInSlot(i)));
                    }
                }
            }
        }
    ...
    protected NNPair<ItemStack, IDarkSteelItem> getOwner() {
        ...
        return NNPair.of(new ItemStack(ModObject.itemDarkSteelPickaxe.getItemNN()),
                (IDarkSteelItem) ModObject.itemDarkSteelPickaxe.getItemNN()); // dummy
    }
    ...
    public void setStackInSlot(int slot, @Nonnull ItemStack stack) {
        if (isInventorySlot(slot)) {
            ...
            NbtValue.DSUINV.setStack(getOwner().getLeft(), slot - stacks.size(),
                    ItemHandlerHelper.copyStackWithSize(stack, 1));
            syncChangesToClient();
            return;
        }
    ...
    public boolean isVisible(int slot) {
        ...
        return stacks.get(slot).upgrade.getRules().stream().filter(Rules::isStatic)
                .allMatch(Rules.makeChecker(getOwner()));
    }
            ...
            for (int slot = 0; slot < getSlots(); slot++) {
                ...
                if (Prep.isValid(stack)) {
                    if (isInventorySlot(slot)) {
                        ...
                        dropSubInventory(stack, stacks.get(slot).upgrade.getInventoryHandler(owner.getLeft()),
                                wipeOriginal);
                    } else {
