...
public class UpgradeRegistry {
    ...
    public static void registerRegistry(@Nonnull RegistryEvent.NewRegistry event) {
        REGISTRY = new RegistryBuilder<IDarkSteelUpgrade>().setName(new ResourceLocation(EnderIO.DOMAIN, "upgrades"))
                .setType(IDarkSteelUpgrade.class)
                .setIDRange(0, Integer.MAX_VALUE - 1)
                .add((AddCallback<IDarkSteelUpgrade>) (owner, stage, id, obj, oldObj) -> sortedList.clear())
    ...
    public static @Nonnull NNList<IDarkSteelUpgrade> getUpgrades() {
        if (sortedList.isEmpty()) {
            synchronized (sortedList) {
                ...
                Arrays.stream(DarkSteelConfig.disabledUpgrades.get().split("\\s*[,;]\\s*"))
                        .map(s -> s != null ? new ResourceLocation(s) : null)
    ...
    public static @Nonnull ItemStack getUpgradeItem(@Nonnull IDarkSteelUpgrade upgrade, boolean enabled) {
        return ItemUpgrades.setEnabled(((ItemUpgrades) ModObject.itemDarkSteelUpgrade.getItemNN()).withUpgrade(upgrade),
                enabled);
    }
    ...
    public static boolean isUpgradeItem(@Nonnull IDarkSteelUpgrade upgrade, @Nonnull ItemStack stack, Boolean enabled) {
        return stack.getCount() == 1 && stack.getItem() == ModObject.itemDarkSteelUpgrade.getItemNN() &&
                ItemUpgrades.getUpgrade(stack) == upgrade &&
    ...
    public static @Nullable IDarkSteelUpgrade getUpgradeFromItem(@Nonnull ItemStack stack) {
        if (stack.getCount() == 1 && stack.getItem() == ModObject.itemDarkSteelUpgrade.getItemNN() &&
