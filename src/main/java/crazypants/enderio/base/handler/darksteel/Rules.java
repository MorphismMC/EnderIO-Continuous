...
public final class Rules {
    ...
    /**
     * Creates a rule for multi-level upgrades to depend on the next lower tier upgrade or, for the lowest level, to
     * require no upgrade to be present.
     * 
     * @param level
     *                 the level of the upgrade
     * @param upgrades
     *                 dependencies for the levels starting at level <em>1</em>. (The first element (index 0) is the
     *                 dependency for level 1.)
     * @return Either a {@link Prerequisite} on the given upgrade for the given level or a <code>not(or())</code> of all
     *         given upgrades.
     */
    @SuppressWarnings("null")
    public static @Nonnull IRule withLevels(int level, IDarkSteelUpgrade... upgrades) {
        if (level == 0 || upgrades[level - 1] == null) {
            return not(or(Stream.of(upgrades).filter(upgrade -> upgrade != null).map(Rules::withPrerequisite)
                    .toArray(IRule.Prerequisite[]::new)));
        }
    ...
    public static @Nonnull IRule withLevels(int level, List<? extends IDarkSteelUpgrade> upgrades) {
        if (level == 0 || upgrades.get(level - 1) == null) {
            return not(or(upgrades.stream().filter(upgrade -> upgrade != null).map(Rules::withPrerequisite)
                    .toArray(IRule.Prerequisite[]::new)));
        }
    ...
    /**
     * Generates a rule that always succeeds but will list the given language element in the "can be applied to" list of
     * the upgrade's tooltips. You need to add
     * additional rules to actually implement the check(s).
     * <p>
     * This is a separate rule from the actual check because the tooltip list is considered an "or" list, but rules are
     * checked "and".
     * 
     * @param lang
     ...
     * 
     * @param slot
     *             And {@link EntityEquipmentSlot} to add to the tooltip. Note that only the armor values have
     *             meaningful results.
     * @return the generated rule
     */
