...
public abstract class BlockEio<T extends TileEntityEio> extends BlockEnder<T>
                              implements IModObject.LifecycleInit, IModObject.WithBlockItem {
    ...
    public void init(@Nonnull IModObject modObject, @Nonnull FMLInitializationEvent event) {
        if (canBeWrenched()) {
            permissionNodeWrenching = PermissionAPI.registerNode(
                    EnderIO.DOMAIN + ".wrench.break." + modObject.getUnlocalisedName(), DefaultPermissionLevel.ALL,
                    "Permission to wrench-break the block " + modObject.getUnlocalisedName() + " of Ender IO");
            permissionNodeIOWrenching = PermissionAPI.registerNode(
                    EnderIO.DOMAIN + ".wrench.iomode." + modObject.getUnlocalisedName(), DefaultPermissionLevel.ALL,
                    "Permission to set IO mode by wrench-clicking the block " + modObject.getUnlocalisedName() +
                            " of Ender IO");
        }
    }
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
                                    ...
                                    float hitZ) {
        if (shouldWrench(world, pos, entityPlayer, side) &&
                ToolUtil.breakBlockWithTool(this, world, pos, side, entityPlayer, hand, permissionNodeWrenching)) {
            return true;
        }
        ...
        if (te instanceof AbstractMachineEntity) {
            ...
            if (tool != null && !entityPlayer.isSneaking() && tool.canUse(hand, entityPlayer, pos)) {
                if (!world.isRemote) {
                    if (!PermissionAPI.hasPermission(entityPlayer.getGameProfile(), permissionNodeIOWrenching,
                            new BlockPosContext(entityPlayer, pos, state, side))) {
    ...
    public boolean shouldWrench(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer entityPlayer,
     ...
     *         {@link #shouldWrench(World, BlockPos, EntityPlayer, EnumFacing)} <strong>must never</strong> return
     *         <code>true</code>.
     */
     ...
     */
    public boolean openGui(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer entityPlayer,
