...
public class Recipe implements IRecipe {
    ...
    public Recipe(RecipeOutput output, int energyRequired, @Nonnull RecipeBonusType bonusType,
                  @Nonnull RecipeLevel recipeLevel, @Nonnull IRecipeInput... input) {
        this(input, new RecipeOutput[] { output }, energyRequired, bonusType, recipeLevel);
    }

    public Recipe(IRecipeInput input, int energyRequired, @Nonnull RecipeBonusType bonusType,
                  @Nonnull RecipeLevel recipeLevel, @Nonnull RecipeOutput... output) {
        this(new IRecipeInput[] { input }, output, energyRequired, bonusType, recipeLevel);
    }

    public Recipe(@Nonnull IRecipeInput[] input, @Nonnull RecipeOutput[] output, int energyRequired,
                  @Nonnull RecipeBonusType bonusType,
    ...
    public boolean isInputForRecipe(NNList<MachineRecipeInput> machineInputs) {
        ...
        for (MachineRecipeInput realInput : machineInputs) {
            if (realInput != null && (realInput.fluid != null || Prep.isValid(realInput.item)) &&
                    !isAnyInput(realInput)) {
                return false;
            }
        }
    ...
    private boolean isAnyInput(@Nonnull MachineRecipeInput realInput) {
        for (IRecipeInput recipeInput : inputs) {
            if (recipeInput != null &&
                    ((recipeInput.isInput(realInput.item)) || recipeInput.isInput(realInput.fluid))) {
                return true;
            }
        }
        return false;
    ...
    public String toString() {
        return "Recipe [input=" + Arrays.toString(inputs) + ", output=" + Arrays.toString(outputs) +
