...
public class BasicPainterTemplate<T extends Block & IPaintable> extends AbstractPainterTemplate<T> {
    ...
    public BasicPainterTemplate(boolean allowEasyConversion, @Nullable T resultBlock,
                                @Nonnull Block... validTargetBlocks) {
    ...
    public boolean isRecipe(@Nonnull ItemStack paintSource, @Nonnull ItemStack target) {
        return isValidTarget(target) && Prep.isValid(paintSource) &&
                PaintUtil.isValid(paintSource, getTargetBlock(target));
    }
    ...
    @Override
    public @Nonnull ResultStack[] produceCompletedResult(@Nonnull ItemStack paintSource,
                                                         @Nonnull ItemStack inputStack) {
        ...
        IBlockState paintState = PaintUtil
                .Block$getBlockFromItem_stack$getItem___$getStateFromMeta_stack$getMetadata___(paintSource, paintBlock);
        if (paintState == null) {
            return new ResultStack[0];
        }
        ...
        if (Prep.isInvalid(outputStack)) {
            ...
            if (outputBlock == Block.getBlockFromItem(inputStack.getItem()) && inputStack.hasTagCompound()) {
                outputStack.setTagCompound(NullHelper
                        .notnullM(inputStack.getTagCompound(), ITEM_STACK_GET_TAG_COMPOUND_AFTER_HAS_TAG_COMPOUND)
                        .copy());
            }
            ...
            outputStack.setTagCompound(NullHelper
                    .notnullM(inputStack.getTagCompound(), ITEM_STACK_GET_TAG_COMPOUND_AFTER_HAS_TAG_COMPOUND).copy());
    ...
    public @Nonnull ItemStack isUnpaintingOp(@Nonnull ItemStack paintSource, @Nonnull ItemStack target) {
        ...
        // The paint source is the paintable block we produce with this recipe. We know it must be unpainted, as paint
        // sources that are painted are rejected. So we
        // user wants the input item but without its paint. The input item must be able to exist in the world unpainted
        // because it does so as paint source. So we
        // copy the input item without its paint information.
        if (paintBlock == resultBlock) {
            return mkItemStack(target, targetBlock);
        }

        // The paint source and the target are the same item, but maybe with different meta. This means that we can
        // simplify the painting by doing an item
        ...
        // The target is paintable, so let's check if the paint source is what was used to create it. If yes, then we
        // unpaint it into it's original form.
