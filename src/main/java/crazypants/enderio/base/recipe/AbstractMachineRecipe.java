...
public abstract class AbstractMachineRecipe implements IMachineRecipe {
    ...
    public abstract IRecipe getRecipeForInputs(@Nonnull RecipeLevel machineLevel,
    ...
    protected boolean consume(@Nonnull IRecipeInput required, @Nonnull MachineRecipeInput available,
                              @Nonnull List<MachineRecipeInput> consumedInputs) {
    ...
    @Override
    public @Nonnull ResultStack[] getCompletedResult(long nextSeed, float chanceMultiplier,
                                                     @Nonnull NNList<MachineRecipeInput> inputs) {
        if (inputs.size() <= 0) {
            return EMPTY_RESULT;
        }
