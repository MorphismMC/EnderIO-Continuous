...
public final class EntityDataRegistry {
    ...
    private static class Entry {
        ...
        Entry(@Nullable Object identity, @Nonnull Predicate<ResourceLocation> selector, double cost,
              boolean blacklistedSpawning, boolean blacklistedSoulvial,
    ...
    public double getCostMultiplierFor(@Nonnull ResourceLocation entity) {
        return entries.stream().filter(elem -> elem.selector.test(entity) && elem.cost > 0).map(elem -> elem.cost)
                .reduce(Double::max)
    ...
    public boolean isBlackListedForSpawning(@Nonnull ResourceLocation entity) {
        return entries.stream().filter(elem -> elem.selector.test(entity)).map(elem -> elem.blacklistedSpawning)
    ...
    public boolean isBlackListedForSoulVial(@Nonnull ResourceLocation entity) {
        return entries.stream().filter(elem -> elem.selector.test(entity)).map(elem -> elem.blacklistedSoulvial)
                .reduce(Boolean::logicalOr)
    ...
    public boolean needsCloning(@Nonnull ResourceLocation entity) {
        return entries.stream().filter(elem -> elem.selector.test(entity)).map(elem -> elem.needsCloning)
                .reduce(Boolean::logicalOr).orElse(defaultNeedsCloning);
    }
    ...
    public void setDefaults(double costMultiplier, boolean blacklistedSpawning, boolean blacklistedSoulvial,
                            boolean needsCloning) {
    ...
    /**
     * Adds or replaces a blacklist entry. If an entry for the same ResourceLocation already exists it is replaced.
    ...
    /**
     * Adds or replaces a blacklist entry. If an identity object is given, this tries to replace an existing entry with
     * the same identity object. Otherwise a new
    ...
    /**
     * Adds or replaces a cost entry. If an entry for the same ResourceLocation already exists it is replaced. Otherwise
    ...
    /**
     * Adds or replaces a cost entry. If an identity object is given, this tries to replace an existing entry with the
     * same identity object. Otherwise a new entry
     ...
     *                       The cost multiplier to be recorded. Must not be 0 or negative.
     */
    public void addEntityCost(@Nullable Object identity, @Nonnull Predicate<ResourceLocation> entityFilter,
    ...
    /**
     * Adds or replaces an entry. If an identity object is given, this tries to replace an existing entry with the same
     * identity object. Otherwise a new entry
     * that cannot be replaced is created always.
     * 
     * @param identity
     *                            An optional identity object that is needed to make entries replaceable.
     ...
     */
    public void addEntityData(@Nullable Object identity, @Nonnull Predicate<ResourceLocation> entityFilter,
                              double costMultiplier, boolean blacklistedSpawning,
                              boolean blacklistedSoulvial, boolean needsCloning) {
        Entry entry = identity != null ? getByIdentity(identity) : null;
        if (entry != null) {
            entries.remove(entry);
        }
        entries.add(new Entry(identity, entityFilter, costMultiplier, blacklistedSpawning, blacklistedSoulvial,
                needsCloning));
    }
}
