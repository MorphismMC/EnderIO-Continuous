...
public class AlloyRecipeManager extends AbstractMachineRecipe {
    ...
    public void addRecipe(boolean prohibitDupes, @Nonnull NNList<IRecipeInput> input, @Nonnull ItemStack output,
                          int energyCost, float xpChance,
                          @Nonnull RecipeLevel recipeLevel) {
        Recipe recipe = new Recipe(new RecipeOutput(output, 1, xpChance), energyCost, RecipeBonusType.NONE, recipeLevel,
                input.toArray(new IRecipeInput[0]));
    ...
    private void addDedupedRecipe(@Nonnull Recipe recipe) {
        ...
        for (ItemStack stack0 : list0) {
            IRecipeInput rinp0 = new RecipeInput(stack0, stack0.getItemDamage() != OreDictionary.WILDCARD_VALUE,
                    recipe.getInputs()[0].getMulitplier(),
                    recipe.getInputs()[1].getSlotNumber());
            for (ItemStack stack1 : list1) {
                IRecipeInput rinp1 = new RecipeInput(stack1, stack1.getItemDamage() != OreDictionary.WILDCARD_VALUE,
                        recipe.getInputs()[1].getMulitplier(),
                        recipe.getInputs()[1].getSlotNumber());
                for (ItemStack stack2 : list2) {
                    ...
                    if (t.isValid() && !seen.contains(t)) {
                        ...
                        addRecipe(
                                new BasicManyToOneRecipe(new Recipe(recipeOutput, recipe.getEnergyRequired(),
                                        recipe.getBonusType(), recipe.getRecipeLevel(),
                                        new NNList<>().addIf(rinp0).addIf(rinp1)
                                                .addIf(recipe.getInputs().length >= 3 ? new RecipeInput(stack2,
                                                        stack2.getItemDamage() != OreDictionary.WILDCARD_VALUE,
                                                        recipe.getInputs()[2].getMulitplier(),
                                                        recipe.getInputs()[2].getSlotNumber()) : null)
        ...
        if (seen.isEmpty()) {
            Log.warn("Splitting alloying recipe for ", output,
                    " into sub-recipes yielded nothing. Something may be wrong with this recipe.");
        } else {
            Log.debug("Split alloying recipe for ", output, " into ", seen.size(), " sub-recipes");
        }
        // display recipe for JEI only (AlloyRecipeWrapper will explode this into a single display recipe with rotating
        // inputs)
    ...
    private void addSyntheticRecipe(@Nonnull Recipe recipe) {
        ...
        addRecipe(new BasicManyToOneRecipe(
                new Recipe(out3, er * 3, bns, lvl, new IRecipeInput[] { in.copy(), in.copy(), in.copy() }))
                        .setSynthetic());
        addRecipe(new BasicManyToOneRecipe(
                new Recipe(out3, er * 3, bns, lvl, new IRecipeInput[] { in.copy(), in2.copy() })).setSynthetic());
        addRecipe(new BasicManyToOneRecipe(
                new Recipe(out3, er * 3, bns, lvl, new IRecipeInput[] { in2.copy(), in.copy() })).setSynthetic());
        addRecipe(new BasicManyToOneRecipe(
                new Recipe(out2, er * 2, bns, lvl, new IRecipeInput[] { in.copy(), in.copy() })).setSynthetic());
        addRecipe(new BasicManyToOneRecipe(new Recipe(out3, er * 3, bns, lvl, new IRecipeInput[] { in3.copy() }))
                .setSynthetic());
        addRecipe(new BasicManyToOneRecipe(new Recipe(out2, er * 2, bns, lvl, new IRecipeInput[] { in2.copy() }))
                .setSynthetic());
        addRecipe(new BasicManyToOneRecipe(recipe));
    ...
    private boolean needsSynthetics(Recipe recipe) {
        return RecipeConfig.createSyntheticRecipes.get() //
                && recipe.getInputs().length == 1 &&
                recipe.getInputs()[0].getStackSize() <= (recipe.getInputs()[0].getInput().getMaxStackSize() / 3) &&
                recipe.getOutputs()[0].getOutput().getCount() <=
                        (recipe.getOutputs()[0].getOutput().getMaxStackSize() / 3);
    }
    ...
    private static void addRecipeToLookup(@Nonnull TriItemLookup<IManyToOneRecipe> lookup,
                                          @Nonnull IManyToOneRecipe recipe) {
    ...
    private static class Tuple {
        ...
        boolean isValid() {
            ...
            return Prep.isValid(stack0) && Prep.isValid(stack1) && !eq(stack0, stack1) && !eq(stack0, stack2) &&
                    !eq(stack2, stack1);
        }
    ...
    // only called when a player takes the output from the slot manually. Doesn't matter if it's a bit slow. Not nice
    // that it has a good chance of producing wrong
    ...
    /**
     * Rebuilds the internal recipe lookup tree. This should be called when the {@link Things} used in the recipes have
     * changed so the item lists are re-read.
     */
    public int rebuild() {
        int count = 0;
