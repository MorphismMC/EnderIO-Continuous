...
public class AnvilCapacitorRecipe {
    ...
    public static void handleAnvilEvent(AnvilUpdateEvent evt) {
        ...
        if (!RecipeConfig.enableLootCapCombining.get() || Prep.isInvalid(left) || Prep.isInvalid(right) ||
                left.getItem() != ModObject.itemBasicCapacitor.getItemNN() || left.getItem() != right.getItem() ||
                left.getMetadata() < 3 || right.getMetadata() < 3 ||
                (RecipeConfig.requireSameType.get() && left.getMetadata() != right.getMetadata()) ||
        ...
        for (WeightedUpgradeImpl wai : WeightedUpgrade.getWeightedupgrades()) {
            ...
            if (data.containsKey(wa)) {
                ...
                Log.debug("Combining " + wa + " " + data.get(wa).getKey() + " and " + data.get(wa).getValue() + " to " +
                        combine);
            }
        }
        ...
        for (Entry<WeightedUpgrade, Float> entry : result.entrySet()) {
            stack = CapacitorHelper.addCapData(stack, entry.getKey().setType, entry.getKey().capacitorKey,
                    entry.getValue());
