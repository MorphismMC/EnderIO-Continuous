...
public class EntityLootHelper {
    ...
    /**
     * Drops loot like {@link EntityLiving#dropLoot(boolean, int, DamageSource)} but using the looting enchantment for
     * the luck value. This allows us to use the
     * loot pool's bonus rolls instead of having to add a function to each single loot table entry. That way we can
     * match the old behavior of dropping additional
    ...
    static public void dropLoot(@Nonnull EntityLiving entity, ResourceLocation lootTable,
        ...
        if (resourcelocation != null) {
            ...
            if (trueSource instanceof EntityPlayer) {
                builder = builder.withPlayer((EntityPlayer) trueSource)
                        .withLuck(ForgeHooks.getLootingLevel(entity, trueSource, source));
            }

            long deathLootTableSeed = getDeathLootTableSeed(entity);
            for (ItemStack itemstack : loottable.generateLootForPools(
                    deathLootTableSeed == 0L ? rand : new Random(deathLootTableSeed), builder.build())) {
                if (itemstack != null) {
