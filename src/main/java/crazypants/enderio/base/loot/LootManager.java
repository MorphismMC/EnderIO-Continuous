...
public class LootManager {
    ...
    // Note: Testing code is on the capacitor item. Right-click a chest in creative mode to fill it with loot. Edit the
    // code to select which loot table to use.
    ...
    // to re-create the json tables from the code below, set PersonalConfig.lootGeneration to DEVELOPMENT then run the
    // game with
    // https://minecraft.curseforge.com/projects/loottweaker and dump the tables with "/mt loottables all". Open the
    // generated json files and remove the vanilla
    ...
    private static void injectTables(@Nonnull LootTableLoadEvent evt) {
        if (MC_TABLES.contains(evt.getName())) {
            LootPool lp = new LootPool(new LootEntry[0], NO_CONDITIONS, new RandomValueRange(1, 3),
                    new RandomValueRange(0, 0), EnderIO.MOD_NAME);
    ...
    public static void onLootTableLoad(@Nonnull LootTableLoadEvent evt) {
        ...
        if (evt.getName().equals(LootTableList.CHESTS_SIMPLE_DUNGEON)) {
            ...
            if (polarBear != null) {
                lp.addEntry(
                        new LootEntryItem(ModObject.itemSoulVial.getItemNN(), 1, 1,
            ...
            lp.addEntry(new LootEntryItem(bucket.getItem(), 1, 1,
                    new LootFunction[] { setCount(1, 1), setMetadata(bucket.getMetadata()), setNBT(bucket) },
                    new LootCondition[] { new RandomChance(.05F) },
                    bucket.getItem().getTranslationKey() + ":" + bucket.getMetadata()));
            ...
            if (shulker != null) {
                lp.addEntry(
                        new LootEntryItem(ModObject.itemSoulVial.getItemNN(), 1, 1,
                                new LootFunction[] { setCount(1, 1), new SetNBT(NO_CONDITIONS, shulker.toNbt(null)) },
        ...
        if (table.isFrozen()) {
            throw new RuntimeException(
                    "Some other mod (a list of suspects is printed in the log file) put a frozen loot table into the load event for loot table '" +
                            evt.getName() +
                            "'. This is a bug in that other mod. Ender IO is the victim here. Don't blame the victim!");
        }
     ...
     */
    private @Nonnull static LootEntry createLootEntry(@Nonnull Item item, int meta, int minStackSize, int maxStackSize,
                                                      float chance) {
        ...
        if (item.isDamageable()) {
            return new LootEntryItem(item, 1, 1,
                    new LootFunction[] { setCount(minStackSize, maxStackSize), setDamage(item, meta), setEnergy() },
                    chanceCond,
                    registryName.toString() + ":" + meta);
        } else {
            return new LootEntryItem(item, 1, 1,
                    new LootFunction[] { setCount(minStackSize, maxStackSize), setMetadata(meta) }, chanceCond,
                    registryName.toString() + ":" + meta);
        }
    }

    private @Nonnull static LootEntry createLootEntry(@Nonnull ItemStack stack, int minStackSize, int maxStackSize,
                                                      float chance) {
        LootCondition[] chanceCond = new LootCondition[] { new RandomChance(chance) };
        final ResourceLocation registryName = NullHelper.notnull(stack.getItem().getRegistryName(),
                ERROR_UNREGISTERED_ITEM);
        return new LootEntryItem(stack.getItem(), 1, 1,
                new LootFunction[] { setCount(minStackSize, maxStackSize), setMetadata(stack.getMetadata()) },
                chanceCond,
    ...
    private @Nonnull static LootEntry createDarkSteelLootEntry(@Nonnull Item item, int minSize, int maxSize,
                                                               float chance) {
        return createDarkSteelLootEntry(item, 0, minSize, maxSize, chance);
    }

    private @Nonnull static LootEntry createDarkSteelLootEntry(@Nonnull Item item, int meta, int minStackSize,
                                                               int maxStackSize, float chance) {
        LootCondition[] chanceCond = new LootCondition[] { new RandomChance(chance) };
        final ResourceLocation registryName = NullHelper.notnull(item.getRegistryName(), ERROR_UNREGISTERED_ITEM);
        return new LootEntryItem(item, 1, 1,
                new LootFunction[] { setCount(minStackSize, maxStackSize), setDamage(item, meta), setUpgrades(),
                        setEnergy() },
    ...
    private @Nonnull static SetDamage setDamage(Item item, int damage) {
        return new SetDamage(NO_CONDITIONS,
                new RandomValueRange(damage > 0 ? damage : 1, damage > 0 ? damage : item.getMaxDamage()));
    }
