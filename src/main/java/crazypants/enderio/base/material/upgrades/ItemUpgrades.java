...
public final class ItemUpgrades extends Item implements IHaveRenderers, IAdvancedTooltipProvider {
    ...
    @SideOnly(Side.CLIENT)
    protected @Nonnull ModelResourceLocation getMRL(final ResourceLocation base,
                                                    final @Nonnull ResourceLocation registryName) {
    ...
    public static IDarkSteelUpgrade getUpgrade(@Nonnull ItemStack stack) {
        if (stack.getItemDamage() == 1) { // TODO 1.14: just drop the meta
            // Note: This is just so vanilla's and other mods' item/recipe handling don't see a blank plate and a
            // upgrade one as the same item
    ...
    public @Nonnull String getItemStackDisplayName(@Nonnull ItemStack stack) {
        IDarkSteelUpgrade upgrade = getUpgrade(stack);
        if (upgrade != null) {
            return I18n.translateToLocalFormatted(getTranslationKey() + ".with", super.getItemStackDisplayName(stack))
                    .trim();
        } else {
    ...
    public void addDetailedEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
        ...
        if (upgrade != null) {
            ...
            // This would be nice, but the code in there adds the upgrade name and has placeholder support that depends
            // on a live itemstack
            ...
            List<IDarkSteelUpgrade> dependencies = upgrade.getRules().stream()
                    .filter(rule -> rule instanceof IRule.Prerequisite)
                ...
                if (levels >= levelCost || playerIn.capabilities.isCreativeMode) {
                    if (!worldIn.isRemote) {
                        if (!playerIn.capabilities.isCreativeMode) {
                            // Note: This is much more expensive than using a tank recipe. It takes the number of levels
                            // off the top of the player's levels, whereas the tank
                            // recipe calculates the levels from 0. This is on purpose. The easy way is expensive, the
                            // hard one is cheap.
