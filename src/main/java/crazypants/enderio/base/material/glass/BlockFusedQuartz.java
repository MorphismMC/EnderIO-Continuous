...
public class BlockFusedQuartz extends BlockFusedQuartzBase<TileEntityEio> implements ITintedBlock, ICustomSubItems {
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public boolean shouldSideBeRendered(@Nonnull IBlockState blockStateIn, @Nonnull IBlockAccess world,
        ...
        if (otherState.getBlock() instanceof BlockFusedQuartz) {
            ...
            return !ourFState.getType().connectTo(otherFState.getType()) ||
                    (!BlockConfig.glassConnectToTheirColorVariants.get() &&
                            ourFState.getColor() != otherFState.getColor());
        }
        return true;
    }

    @Override
    public int getBlockTint(@Nonnull IBlockState state, @Nullable IBlockAccess worldIn, @Nullable BlockPos pos,
                            int tintIndex) {
    ...
    @Override
    @Nullable
    public float[] getBeaconColorMultiplier(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
                                            @Nonnull BlockPos beaconPos) {
        if (PersonalConfig.candyColors.get()) {
    ...
    @Override
    public @Nonnull MapColor getMapColor(@Nonnull IBlockState state, @Nonnull IBlockAccess worldIn,
                                         @Nonnull BlockPos pos) {
        return MapColor.AIR; // Same as glass
    }

    public static class BlockItemFusedQuartz extends BlockFusedQuartzBase.BlockItemFusedQuartzBase
                                             implements ITintedItem, IWithPaintName {
