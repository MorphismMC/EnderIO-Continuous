...
public class MaterialCraftingHandler {
    ...
    public static void spawnInfinityPowder(final @Nonnull WorldServer world, final @Nonnull BlockPos pos,
                                           @Nonnull ResourceLocation table) {
        @SuppressWarnings("null")
        final ResourceLocation subTable = new ResourceLocation(table.getNamespace(),
                table.getPath() + "/" + world.getBlockState(pos.down()).getBlock().getRegistryName().toString()
                        .replaceAll("[:]", "/").replaceAll("[^a-z0-9_/]", "_"));
        ...
        for (ItemStack itemstack : lootTable.generateLootForPools(RANDOM, new LootContext.Builder(world).build())) {
            if (itemstack != null && Prep.isValid(itemstack)) {
                ...
                if (InfinityConfig.makesSound.get()) {
                    world.playSound(null, pos, SoundEvents.ENTITY_FIREWORK_LARGE_BLAST, SoundCategory.BLOCKS, 1.0F,
    ...
    public static void onWorldTick(@Nonnull TickEvent.WorldTickEvent event) {
        if (!fires.isEmpty() && !event.world.getGameRules().getBoolean("doFireTick") &&
                InfinityConfig.inWorldCraftingEnabled.get() &&
