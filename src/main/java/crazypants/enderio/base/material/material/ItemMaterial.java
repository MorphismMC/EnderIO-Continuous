        ...
        NNList.of(Material.class).apply(new Callback<Material>() {
            ...
            public void apply(@Nonnull Material alloy) {
                ModelLoader.setCustomModelResourceLocation(ItemMaterial.this, Material.getMetaFromType(alloy),
                        makeMRL(modObject, alloy));
            }
        });
        if (PersonalConfig.animatedGears.get()) {
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_WOOD),
                    new Vector4d(0, 0, 1, .5));
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_STONE),
                    new Vector4d(0, 0, 1, -1));
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_IRON),
                    new Vector4d(0, 0, 1, 1.5));
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_ENERGIZED),
                    new Vector4d(0, 0, 1, -2));
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_VIBRANT),
                    new Vector4d(0, 0, 1, 3));
            ItemModelRegistry.registerRotating(ItemMaterial.makeMRL(modObject, Material.GEAR_DARKSTEEL),
                    new Vector4d(0, 0, 1, -3.5));
        }
    }
