...
public class BlockSceneRenderer {
    ...
    private void renderScene() {
        ...
        final LayerRenderer layerRenderer = new LayerRenderer(
                new Vector3d((-origin.x) + eye.x, (-origin.y) + eye.y, (-origin.z) + eye.z));
                ...
                try {
                    ...
                    switch (type) {
                        case MODEL:
                            ...
                            blockrendererdispatcher.getBlockModelRenderer().renderModel(Minecraft.getMinecraft().world,
                                    ibakedmodel, bs, pos, worldRendererIn, false);
                            break;
                        case LIQUID:
                            if (fluidForBlock != null) {
                                ...
                                final TextureAtlasSprite tex = NullHelper.first(Minecraft.getMinecraft()
                                        .getTextureMapBlocks().getTextureExtry(iconKey.toString()), tex1);
                                ...
                                for (Vertex v : bb.getCornersWithUvForFace(EnumFacing.DOWN, minU1, maxU1, minV1,
                                        maxV1)) {
                                    worldRendererIn.pos(v.x(), v.y(), v.z()).color(0.5F, 0.5F, 0.5F, 1.0F)
                                            .tex(v.u(), v.v()).lightmap(240, 240).endVertex();
                                }
                        ...
                        case INVISIBLE:
                        default:
                            blockrendererdispatcher.renderBlock(bs, pos, Minecraft.getMinecraft().world,
                                    worldRendererIn);
                            break;
                    }
