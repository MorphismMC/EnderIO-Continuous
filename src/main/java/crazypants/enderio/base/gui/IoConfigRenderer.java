...
public class IoConfigRenderer<E extends TileEntity & IIoConfigurable> {
    ...
    public static final TextureSupplier selectedFaceIcon = TextureRegistry
            .registerTexture("blocks/overlays/selected_face");
        ...
        configurables.apply(new Callback<BlockPos>() {
            ...
            public void apply(@Nonnull BlockPos pos) {
                if (!world.isAirBlock(pos)) {
                    ...
                    RayTraceResult hit = bs.collisionRayTrace(world, pos, new Vec3d(start.x, start.y, start.z),
    ...
    public void drawScreen(int par1, int par2, float partialTick, @Nonnull Rectangle vp,
                           @Nonnull Rectangle parentBounds) {
    ...
    private void renderSelection() {
        ...
        List<Vertex> corners = bb.getCornersWithUvForFace(selection.face, icon.getMinU(), icon.getMaxU(),
    ...
    private void renderOverlay(int mx, int my) {
        ...
        if (mx >= x && mx <= x + IconEIO.IO_WHATSIT.width && my >= y && my <= y + IconEIO.IO_WHATSIT.height) {
            ...
            RenderUtil.renderQuad2D(x, y, 0, IconEIO.IO_WHATSIT.width, IconEIO.IO_WHATSIT.height,
    ...
    private void doWorldRenderPass(@Nonnull Vector3d trans, @Nonnull NNList<BlockPos> blocks,
                                   final @Nonnull BlockRenderLayer layer) {
    ...
    public void renderBlock(@Nonnull IBlockState state, @Nonnull BlockPos pos, @Nonnull IBlockAccess blockAccess,
                            @Nonnull BufferBuilder worldRendererIn) {
        try {
            ...
            blockrendererdispatcher.getBlockModelRenderer().renderModel(blockAccess, ibakedmodel, state, pos,
                    worldRendererIn, false);
