...
public class GuiHelper {
    ...
    public static void init(@Nonnull EnderIOLifecycleEvent.Init.Normal event) {
        for (IModObject mo : ModObjectRegistry.getRegistry()) {
            if (mo.getBlock() instanceof IEioGuiHandler.WithServerComponent ||
                    mo.getItem() instanceof IEioGuiHandler.WithServerComponent) {
                Log.info("Registered permission ",
                        PermissionAPI.registerNode(getPermission(mo), DefaultPermissionLevel.ALL,
    ...
    public static boolean openGui(@Nonnull IModObject mo, @Nonnull World world, @Nonnull BlockPos pos,
    ...
    public static boolean openGui(@Nonnull IModObject mo, @Nonnull World world, @Nonnull EntityPlayer entityPlayer,
                                  int a, int b, int c) {
        if (!world.isRemote) {
            if (PermissionAPI.hasPermission(entityPlayer.getGameProfile(), getPermission(mo),
                    new PlayerContext(entityPlayer))) {
    ...
    public static boolean openClientGui(@Nonnull IModObject mo, @Nonnull World world, @Nonnull BlockPos pos,
                                        @Nonnull EntityPlayer entityPlayer,
                                        @Nullable EnumFacing side, int param) {
        if (world.isRemote) {
            return openGui(world, entityPlayer, getID(mo), pos, side, param, 0, 0);
    ...
    public static boolean openClientGui(@Nonnull IModObject mo, @Nonnull World world,
                                        @Nonnull EntityPlayer entityPlayer, int a, int b, int c) {
        if (world.isRemote) {
            return openGui(world, entityPlayer, getID(mo), null, null, a, b, c);
        }
        return false;
    ...
    protected static boolean openGui(@Nonnull World world, @Nonnull EntityPlayer entityPlayer, int id,
                                     @Nullable BlockPos pos, @Nullable EnumFacing facing,
    ...
    protected static @Nonnull String getPermission(@Nonnull IModObject mo) {
        if (mo.getBlock() instanceof IEioGuiHandler.WithServerComponent ||
            ...
            throw new RuntimeException(
                    "Cannot open GUI for object " + mo + " because it has no server-side GUI handler.");
        }
    }
}
