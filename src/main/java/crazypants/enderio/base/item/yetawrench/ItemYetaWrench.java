...
public class ItemYetaWrench extends Item implements ITool, IConduitControl, IAdvancedTooltipProvider, IToolWrench,
    ...
    public @Nonnull EnumActionResult onItemUseFirst(@Nonnull EntityPlayer player, @Nonnull World world,
        ...
        if (!player.isSneaking() && block.rotateBlock(world, pos, side)) {
            ret = true;
        } else if (block instanceof IBlockPaintableBlock && !player.isSneaking() &&
                !YetaUtil.shouldHeldItemHideFacades(player)) {
        ...
        // Need to catch 'shift-clicks' here and pass them on manually or an item in the off hand can eat the right
        // click
    ...
    public void registerRenderers(@Nonnull IModObject modObject) {
        ModelLoader.setCustomModelResourceLocation(this, 0,
                new ModelResourceLocation(modObject.getRegistryName(), "#inventory"));
        if (PersonalConfig.animatedYeta.get()) {
            ItemModelRegistry.registerRotating(new ModelResourceLocation(modObject.getRegistryName(), "#inventory"),
                    transformType -> {
