...
public class ItemSoulVial extends Item implements IResourceTooltipProvider, IHaveRenderers, IOverlayRenderAware,
    ...
    public final @Nonnull IBehaviorDispenseItem DISPENSER_BEHAVIOR = new BehaviorDefaultDispenseItem() {
        ...
        protected @Nonnull ItemStack dispenseStack(@Nonnull IBlockSource source, @Nonnull ItemStack stack) {
            ...
            if (capturedMob != null) {
                ...
                for (EntityLivingBase entity : source.getWorld().<EntityLivingBase>getEntitiesWithinAABB(
            ...
            if (Prep.isValid(secondaryResult)) {
                if (Prep.isInvalid(stack)) {
                    ...
                    if (!(blockTileEntity instanceof TileEntityDispenser) ||
                            ((TileEntityDispenser) blockTileEntity).addItemStack(secondaryResult) < 0) {
                        Block.spawnAsEntity(source.getWorld(), source.getBlockPos().offset(enumfacing),
                                secondaryResult);
                    }
                }
            }
    ...
    public void init(@Nonnull IModObject modObject, @Nonnull FMLInitializationEvent event) {
        permissionPickupOwned = PermissionAPI.registerNode(EnderIO.DOMAIN + ".soulvial.pickup_owned",
    ...
    public void registerRenderers(@Nonnull IModObject modObject) {
        ModelLoader.setCustomModelResourceLocation(this, 0,
                new ModelResourceLocation(modObject.getRegistryName(), "variant=empty"));
        ModelLoader.setCustomModelResourceLocation(this, 1,
                new ModelResourceLocation(modObject.getRegistryName(), "variant=full"));
    }
    ...
    public @Nonnull EnumActionResult onItemUse(@Nonnull EntityPlayer player, @Nonnull World world,
        ...
        if (!PermissionAPI.hasPermission(player.getGameProfile(), permissionPlace,
                new BlockPosContext(player, pos, null, side))) {
    ...
    @Override
    public boolean itemInteractionForEntity(@Nonnull ItemStack item, @Nonnull EntityPlayer player,
                                            @Nonnull EntityLivingBase entity, @Nonnull EnumHand hand) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            return false;
        } else if (entity instanceof IEntityOwnable && ((IEntityOwnable) entity).getOwnerId() != null &&
                !player.equals(((IEntityOwnable) entity).getOwner()) &&
                !PermissionAPI.hasPermission(player.getGameProfile(), permissionPickupOwned,
                        new TargetContext(player, entity))) {
                            player.sendMessage(Lang.SOUL_VIAL_DENIED_OWNED_PET.toChatServer());
                            return false;
                        } else
            if (!PermissionAPI.hasPermission(player.getGameProfile(), permissionPickup,
                    new TargetContext(player, entity))) {
                        player.sendMessage(Lang.SOUL_VIAL_DENIED.toChatServer());
                        return false;
                    } else
