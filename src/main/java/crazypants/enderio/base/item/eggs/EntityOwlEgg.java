...
public class EntityOwlEgg extends EntityThrowable {
    ...
    public static void onEntityRegister(Register<EntityEntry> event) {
        EntityRegistry.registerModEntity(new ResourceLocation(EnderIO.MODID, "owl_egg"), EntityOwlEgg.class,
                EnderIO.MODID + ".owl_egg", 9, EnderIO.MODID, 64, 10,
                true);
    }
    ...
    protected void onImpact(@Nonnull RayTraceResult impact) {
        ...
        if (!world.isRemote && rand.nextInt(8) == 0) {
            Entity entitychicken = EntityList.createEntityByIDFromName(new ResourceLocation("enderiozoo", "owl"),
                    world);
        ...
        for (int i = 0; i < 8; ++i) {
            world.spawnParticle(EnumParticleTypes.ITEM_CRACK, posX, posY, posZ, (rand.nextFloat() - 0.5D) * 0.08D,
                    ...
                    (rand.nextFloat() - 0.5D) * 0.08D,
                    new int[] { Item.getIdFromItem(ModObject.item_owl_egg.getItemNN()) });
        }
        if (!world.isRemote) {
            setDead();
        }
