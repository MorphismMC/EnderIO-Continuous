...
public class ItemStaffOfLevity extends Item implements IAdvancedTooltipProvider, IOverlayRenderAware, IDarkSteelItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World world, @Nonnull EntityPlayer player,
        ...
        if (player.isSneaking()) {
            ...
            if (world.isRemote && (ticksSinceActivation == 0 ||
                    ticksSinceActivation >= ItemConfig.staffOfLevityTicksBetweenActivation.get())) {
                if (!isEffectActive && hasFluid(stack)) {
                    useFluid(stack);
                    player.addPotionEffect(new PotionEffect(MobEffects.LEVITATION,
                            ItemConfig.staffOfLevityDurationSeconds.get() * 20));
                } else {
    ...
    private final class FluidCapabilityProvider implements IFluidHandlerItem {
        ...
        public int fill(FluidStack resource, boolean doFill) {
            return ItemStaffOfLevity.this.fill(container,
