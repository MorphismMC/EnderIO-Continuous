...
public class GuiLocationPrintout extends GuiScreenBase {
    ...
    public GuiLocationPrintout(@Nonnull TelepadTarget target, @Nonnull EntityPlayer player,
                               @Nonnull EntityEquipmentSlot slot) {
        this(target, player.getItemStackFromSlot(slot), slot, -1);
    }

    private GuiLocationPrintout(@Nonnull TelepadTarget target, @Nonnull ItemStack stack, EntityEquipmentSlot slot,
                                int paperSlot) {
        this.slot = slot;
        this.stack = stack;
