...
public class PacketUpdateLocationPrintout implements IMessage {
    ...
    public static class Handler implements IMessageHandler<PacketUpdateLocationPrintout, IMessage> {
        ...
        private void createPrintout(@Nonnull EntityPlayerMP player, @Nonnull TelepadTarget telepadTarget,
                                    int paperSlot) {
            ...
            if (telepadTarget.getLocation().distanceSq(new BlockPos(player)) > 160 * 160) {
                // ideally we'd want to raytrace this, but the difference between raytracing on the server and the
                // client is just too big, especially over long
                // distances...
                return;
            }
        ...
        private void updatePrintout(@Nonnull EntityPlayerMP player, @Nonnull EntityEquipmentSlot slot,
                                    @Nonnull TelepadTarget telepadTarget) {
