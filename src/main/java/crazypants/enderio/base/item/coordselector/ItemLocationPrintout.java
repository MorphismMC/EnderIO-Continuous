...
public class ItemLocationPrintout extends Item implements IEioGuiHandler {
    ...
    public @Nonnull ActionResult<ItemStack> onItemRightClick(@Nonnull World world, @Nonnull EntityPlayer player,
        ...
        if (player.isSneaking() && TelepadTarget.readFromNBT(stack) != null) {
            ModObjectRegistry.getModObjectNN(this).openClientGui(world, player, GUI_ID_LOCATION_PRINTOUT,
                    hand.ordinal(), 0);
    ...
    private @Nonnull EnumActionResult onTelepadClicked(@Nonnull ItemStack stack, @Nonnull EntityPlayer player,
                                                       @Nonnull World worldIn,
                                                       @Nonnull ICoordinateAware.SingleTarget tile,
                                                       @Nonnull TelepadTarget targ) {
    ...
    public static void sendPrivateChatMessage(@Nonnull EntityPlayer player, @Nonnull UserIdent owner) {
        if (!player.isSneaking()) {
            ChatUtil.sendNoSpam(player,
                    Lang.PRINTOUT_PRIVATE.get(TextFormatting.RED, owner.getPlayerName(), TextFormatting.WHITE));
        }
    }
    ...
    public Object getGuiElement(boolean server, @Nonnull EntityPlayer player, @Nonnull World world,
        ...
        if (server) {
            ...
            int foundPaper = -1;
            for (int paperIndex = 0; paperIndex < player.inventoryContainer.inventorySlots.size() &&
                    foundPaper < 0; paperIndex++) {
