...
public final class MagnetController {
    ...
    public static ActiveMagnet getMagnet(EntityPlayer player, boolean activeOnly) {
        ...
        for (int i = 0; i < maxSlot; i++) {
            ...
            if (ItemMagnet.isMagnet(item) &&
                    (!activeOnly || (ItemMagnet.isActive(item) && ItemMagnet.hasPower(item)))) {
        ...
        if (ItemMagnet.isMagnet(item) && (!activeOnly || (ItemMagnet.isActive(item) && ItemMagnet.hasPower(item)))) {
            return new ActiveMagnet(item,
                    player.inventory.mainInventory.size() + player.inventory.armorInventory.size());
        }
        return null;
    }
    ...
    public static void doHoover(EntityPlayer player) {
        ...
        AxisAlignedBB aabb = new AxisAlignedBB(player.posX - range, player.posY - range, player.posZ - range,
                            ...
                            if (isValidTarget) {
                                if (arraylist == null) {
                                    arraylist = new ArrayList<Entity>(
                                            ItemConfig.magnetMaxItems.get() > 0 ? ItemConfig.magnetMaxItems.get() : 20);
                                }
    ...
    public static void setMagnetActive(@Nonnull EntityPlayerMP player, @Nonnull SlotType type, int slot,
                                       boolean isActive) {
