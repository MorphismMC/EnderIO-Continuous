    ...
    public static void onBreakEvent(BlockEvent.BreakEvent evt) {
        if (evt.getState().getBlock() instanceof BlockMobSpawner) {
            if (evt.getPlayer() != null && !evt.getPlayer().capabilities.isCreativeMode &&
                    !evt.getPlayer().world.isRemote && !evt.isCanceled()) {
                TileEntity tile = evt.getPlayer().world
                        .getTileEntity(NullHelper.notnullF(evt.getPos(), "BlockEvent.BreakEvent.getPos()"));
                if (tile instanceof TileEntityMobSpawner) {
                    ...
                    if (Prep.isValid(equipped) &&
                            BrokenSpawnerConfig.brokenSpawnerToolBlacklist.get().contains(equipped)) {
                        return;
                    }
                        ...
                        if (object instanceof TileEntityMobSpawner) {
                            ...
                            if (spawner.getWorld() == evt.getWorld() && p.equals(evt.getPos())) {
                                ...
                                if (entityName != null && !isBlackListed(entityName)) {
                                    ...
                                    if (capturedMob != null) {
                                        evt.getDrops().add(
                                                capturedMob.toStack(ModObject.itemBrokenSpawner.getItemNN(), 0, 1));
                                    }
                                }
                            }
                        }
