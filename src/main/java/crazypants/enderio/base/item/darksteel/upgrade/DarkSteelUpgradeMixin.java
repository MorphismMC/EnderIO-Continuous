...
public abstract class DarkSteelUpgradeMixin extends Item implements IDarkSteelItem, IAdvancedTooltipProvider {
    ...
    public @Nullable ICapabilityProvider initCapabilities(@Nonnull ItemStack stack, @Nullable NBTTagCompound nbt) {
        return initCapabilities(stack, nbt,
                new MappedCapabilityProvider().add(CapabilityEnergy.ENERGY, new EnergyUpgradeCap(stack)));
    }

    // Attribute Modifiers

    @Override
    public @Nonnull Multimap<String, AttributeModifier> getAttributeModifiers(@Nonnull EntityEquipmentSlot slot,
    ...
    public void openUpgradeGui(@Nonnull EntityPlayer player, @Nullable EntityEquipmentSlot slot) {
        if (player.world.isRemote) {
            ...
            ModObject.blockDarkSteelAnvil.openGui(player.world, new BlockPos(0, -1, 0), player, null,
                    slot != null ? slot.ordinal() : -1);
        }
    }
}
