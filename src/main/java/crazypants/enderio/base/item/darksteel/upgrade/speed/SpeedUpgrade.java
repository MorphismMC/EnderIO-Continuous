...
public class SpeedUpgrade extends AbstractUpgrade {
    ...
    public SpeedUpgrade(int level) {
        super(UPGRADE_NAME, level, "enderio.darksteel.upgrade.speed_" + numbers[level - 1],
                DarkSteelConfig.speedUpgradeCost.get(level - 1));
    ...
    public List<IRule> getRules() {
        return new NNList<>(Rules.forSlot(EntityEquipmentSlot.LEGS), EnergyUpgrade.HAS_ANY,
                Rules.withLevels(level, null, SPEED_ONE, SPEED_TWO),
    ...
    public void addAttributeModifiers(@Nonnull EntityEquipmentSlot slot, @Nonnull ItemStack stack,
        ...
        if (slot == EntityEquipmentSlot.LEGS && !NbtValue.SPEED_DISABLED.getBoolean(stack)) {
            ...
            if (upgradeHolder != null && upgradeHolder.getEnergy() > 0) {
                AttributeModifier modifier = DarkSteelAttributeModifiers.getWalkSpeed(getLevel(),
                        upgradeHolder.getUpgrade().getLevel());
    ...
    public void onPlayerTick(@Nonnull ItemStack stack, @Nonnull IDarkSteelItem item, @Nonnull EntityPlayer player) {
        ...
        int costModifier = (player.isSprinting() ? DarkSteelConfig.darkSteelSpeedSprintEnergyCost :
                DarkSteelConfig.darkSteelSpeedWalkEnergyCost).get();
