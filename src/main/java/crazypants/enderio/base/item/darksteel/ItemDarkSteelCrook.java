...
public class ItemDarkSteelCrook extends ItemTool
    ...
    private boolean doMultiHarvest(@Nonnull EntityPlayer player, @Nonnull World world, @Nonnull BlockPos bc) {
        ...
        float chance = ForgeEventFactory.fireBlockHarvesting(drops, world, bc, bs,
        ...
        for (ItemStack stack : drops) {
            if (world.rand.nextFloat() <= chance) {
                world.spawnEntity(
                        new EntityItem(world, bc.getX() + 0.5, bc.getY() + 0.5, bc.getZ() + 0.5, stack.copy()));
                EnergyUpgradeManager.extractEnergy(player.getHeldItemMainhand(), this,
                        DarkSteelConfig.crookEnergyPerDamageMulti, false);
                usedPower = true;
            }
        }
        return usedPower;
    ...
    protected void setBlock(ItemStack stack, @Nonnull EntityPlayer player, World worldIn, @Nonnull BlockPos pos,
    ...
    public static void onHarvest(@Nonnull BlockEvent.HarvestDropsEvent event) {
        ...
        if (world == null || world.isRemote || event.getHarvester() == null || event.isSilkTouching() ||
                !isEquipped(event.getHarvester())) {
            return;
        }
        ...
        final int loops = Math.max(start,
                (powered ? DarkSteelConfig.crookExtraDropsPowered : DarkSteelConfig.crookExtraDropsUnpowered).get());
