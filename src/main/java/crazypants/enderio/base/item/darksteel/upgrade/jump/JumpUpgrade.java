...
public class JumpUpgrade extends AbstractUpgrade {
    ...
    public static final @Nonnull IRule.Prerequisite HAS_ANY = new IRule.Prerequisite() {
        ...
        public CheckResult check(@Nonnull ItemStack stack, @Nonnull IDarkSteelItem item) {
            ...
            return new CheckResult(Lang.DSU_CHECK_PREREQ_MISSING
                    .toChatServer(new TextComponentTranslation(JUMP_ONE.getUnlocalizedName() + ".name")));
        }
    ...
    public JumpUpgrade(int level) {
        super(UPGRADE_NAME, level, "enderio.darksteel.upgrade.jump_" + numbers[level - 1],
    ...
    public List<IRule> getRules() {
        return new NNList<>(Rules.forSlot(EntityEquipmentSlot.FEET), EnergyUpgrade.HAS_ANY,
                Rules.withLevels(level, null, JUMP_ONE, JUMP_TWO),
    ...
    public void doMultiplayerSFX(@Nonnull EntityPlayer player) {
        SoundHelper.playSound(player.world, player, SoundRegistry.JUMP, 1.0f,
                player.world.rand.nextFloat() * 0.5f + 0.75f);
        ...
        for (int i = rand.nextInt(10) + 5; i >= 0; i--) {
            ...
            final double velX = ((player instanceof EntityOtherPlayerMP) ? 0 : player.motionX) +
                    (rand.nextDouble() * 0.5 - 0.25);
            final double velY = ((player instanceof EntityOtherPlayerMP) ? 0 : (player.motionY / 2)) +
                    (rand.nextDouble() * -0.5);
            final double velZ = ((player instanceof EntityOtherPlayerMP) ? 0 : player.motionZ) +
                    (rand.nextDouble() * 0.5 - 0.25);
            Particle fx = Minecraft.getMinecraft().effectRenderer
                    .spawnEffectParticle(EnumParticleTypes.REDSTONE.getParticleID(), posX, posY, posZ, 1, 1, 1);
            ClientUtil.setParticleVelocity(fx, velX, velY, velZ);
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(NullHelper.notnullM(fx, "spawnEffectParticle() failed unexptedly"));
        }
    }
