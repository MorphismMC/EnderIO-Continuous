...
public class ExplosiveUpgrade extends AbstractUpgrade {
    ...
    public static final @Nonnull IRule.Prerequisite HAS_ANY = new IRule.Prerequisite() {
        ...
        public CheckResult check(@Nonnull ItemStack stack, @Nonnull IDarkSteelItem item) {
            for (ExplosiveUpgrade upgrade : new ExplosiveUpgrade[] { INSTANCE, INSTANCE2, INSTANCE3, INSTANCE4,
                    INSTANCE5 }) {
            ...
            return new CheckResult(Lang.DSU_CHECK_PREREQ_MISSING
                    .toChatServer(new TextComponentTranslation(INSTANCE.getUnlocalizedName() + ".name")));
        }
    ...
    public ExplosiveUpgrade(int level) {
        super(UPGRADE_NAME, level, "enderio.darksteel.upgrade." + UPGRADE_NAME + "." + level,
    ...
    public List<IRule> getRules() {
        return new NNList<>(Rules.callbacksFor(INSTANCE), Rules.staticCheck(item -> item.isPickaxe()),
                EnergyUpgrade.HAS_ANY,
                Rules.withLevels(variant, INSTANCE, INSTANCE2, INSTANCE3, INSTANCE4),
    ...
    static final @Nonnull NNList<AttributeModifier> SIZE = new NNList<>( //
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 0, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 1, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 2, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 3, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 4, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 5, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 6, 0),
            new AttributeModifier(UPGRADE_UUID, UPGRADE_NAME, 7, 0));

    @Override
    public void addAttributeModifiers(@Nonnull EntityEquipmentSlot slot, @Nonnull ItemStack stack,
