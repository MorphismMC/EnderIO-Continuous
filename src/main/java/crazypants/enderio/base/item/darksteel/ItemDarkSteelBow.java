...
public class ItemDarkSteelBow extends ItemBow implements IDarkSteelItem, IAdvancedTooltipProvider, IOverlayRenderAware {
    ...
    public static ItemDarkSteelBow createEndSteel(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemDarkSteelBow res = new ItemDarkSteelBow(modObject, EquipmentData.END_STEEL, DarkSteelConfig.endBowDrawSpeed,
                DarkSteelConfig.endBowForceMultipliers,
    ...
    public static ItemDarkSteelBow createDarkSteel(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemDarkSteelBow res = new ItemDarkSteelBow(modObject, EquipmentData.DARK_STEEL,
                DarkSteelConfig.darkBowDrawSpeed, DarkSteelConfig.darkBowForceMultipliers,
    ...
    protected ItemDarkSteelBow(@Nonnull IModObject modObject, @Nonnull IEquipmentData data,
                               @Nonnull NNList<IValue<Integer>> drawSpeeds,
                               @Nonnull NNList<IValue<Float>> forceMultipliers,
                               @Nonnull NNList<IValue<Float>> fovMultipliers,
                               @Nonnull NNList<IValue<Double>> damageBonus) {
    ...
    private float updatePullProperty(@Nonnull ItemStack stack, @Nullable World worldIn,
    ...
    @Override
    public void onPlayerStoppedUsing(@Nonnull ItemStack theBow, @Nonnull World worldIn,
                                     @Nonnull EntityLivingBase theShootingEntity, int timeLeft) {
        ...
        final ItemStack theArrows = getArrowsToShoot(theShooter,
                isCreativeMode || EnchantmentHelper.getEnchantmentLevel(Enchantments.INFINITY, theBow) > 0);
        if (Prep.isInvalid(theArrows)) {
            return;
        }
        final int drawTime = ForgeEventFactory.onArrowLoose(theBow, worldIn, (EntityPlayer) theShootingEntity,
                getMaxItemUseDuration(theBow) - timeLeft, true);
        if (drawTime < 0) {
            return;
        }
        ...
        final ItemArrow theArrow = (ItemArrow) (theArrows.getItem() instanceof ItemArrow ? theArrows.getItem() :
                Items.ARROW);
        ...
        if (!worldIn.isRemote) {
            ...
            entityArrow.shoot(theShooter, theShooter.rotationPitch, theShooter.rotationYaw, 0.0F,
                    drawRatio * 3.0F * getForceMultiplier(upgrade), 0.25F);
        ...
        worldIn.playSound((EntityPlayer) null, theShooter.posX, theShooter.posY, theShooter.posZ,
    ...
    public int getRequiredPower(EntityLivingBase entity, EnergyUpgradeManager upgrade, @Nonnull ItemStack stack,
                                int drawTime) {
        int powerRequired = 0;
        if (upgrade == null) {
            return powerRequired;
