...
public final class DarkShieldRenderer extends TileEntityItemStackRenderer {
     ...
     */
    private static final @Nonnull ResourceLocation RES_ITEM_GLINT = new ResourceLocation(
    ...
    public void renderByItem(@Nonnull ItemStack stack, float partialTicks) {
        ...
        if (item instanceof ItemDarkSteelShield) {
            ...
            NNPair<ResourceLocation, Cache> tex = TEX.computeIfAbsent(shieldBase,
                    base -> NNPair.of(new ResourceLocation(base + "_base_nopattern.png"),
                            new BannerTextures.Cache("D" + data.getArmorMaterial().getName(),
                                    new ResourceLocation(base + "_base.png"), "textures/entity/shield/")));

            if (stack.getSubCompound("BlockEntityTag") != null) {
                banner.setItemValues(stack, true);
                ResourceLocation resourceLocation = tex.getRight().getResourceLocation(
            ...
            if (stack.hasEffect()) {
                ...
                GlStateManager.translate((Minecraft.getSystemTime() >>> 3) % 3000L / 3000.0F, 0.0F, 0.0F); // reduce
                                                                                                           // speed
                                                                                                           // before
                                                                                                           // scaling to
                                                                                                           // avoid
                ...
                GlStateManager.pushMatrix();
                GlStateManager.scale(.8F, .8F, .8F);
                GlStateManager.translate(-((Minecraft.getSystemTime() >> 2) % 4873L / 4873.0F), 0.0F, 0.0F); // reduce
                                                                                                             // speed
                                                                                                             // before
                                                                                                             // scaling
                                                                                                             // to avoid
