...
public class ItemDarkSteelArmor extends ItemArmor implements ISpecialArmor, IAdvancedTooltipProvider, IDarkSteelItem,
                                IOverlayRenderAware, IHasPlayerRenderer,
    ...
    public static ItemDarkSteelArmor createDarkSteelHelmet(@Nonnull IModObject modObject, @Nullable Block block) {
        final ItemDarkSteelArmor helmet = new ItemDarkSteelArmor(EquipmentData.DARK_STEEL, modObject,
                EntityEquipmentSlot.HEAD, 1);
    ...
    public static ItemDarkSteelArmor createEndSteelHelmet(@Nonnull IModObject modObject, @Nullable Block block) {
        final ItemDarkSteelArmor helmet = new ItemDarkSteelArmor(EquipmentData.END_STEEL, modObject,
                EntityEquipmentSlot.HEAD, 2);
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER, new HelmetPainterTemplate(helmet));
        return helmet;
    }
    ...
    public static ItemDarkSteelArmor createStellarAlloyHelmet(@Nonnull IModObject modObject, @Nullable Block block) {
        final ItemDarkSteelArmor helmet = new ItemDarkSteelArmor(EquipmentData.STELLAR_ALLOY, modObject,
                EntityEquipmentSlot.HEAD, 2);
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER, new HelmetPainterTemplate(helmet));
        return helmet;
    }
    ...
    protected ItemDarkSteelArmor(@Nonnull IEquipmentData data, @Nonnull IModObject modObject,
                                 @Nonnull EntityEquipmentSlot armorType, @Nonnull Integer tier) {
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> par3List) {
        if (isInCreativeTab(tab)) {
            ...
            for (int i = 0; i <= 2; i++) { // upgrades may not be in the best order, so try adding them a couple of
                                           // times
                for (IDarkSteelUpgrade upgrade : UpgradeRegistry.getUpgrades()) {
                    if (!(upgrade instanceof EnergyUpgrade || upgrade instanceof GliderUpgrade ||
                            upgrade instanceof ElytraUpgrade) && upgrade.canAddToItem(is, this)) {
    ...
    @Override
    public String getArmorTexture(@Nonnull ItemStack itemStack, @Nonnull Entity entity,
                                  @Nonnull EntityEquipmentSlot slot, @Nonnull String layer) {
        if (armorType == EntityEquipmentSlot.LEGS ||
                (armorType == EntityEquipmentSlot.HEAD && !NightVisionUpgrade.INSTANCE.hasUpgrade(itemStack) &&
    ...
    public ModelBiped getArmorModel(@Nonnull EntityLivingBase entityLiving, @Nonnull ItemStack itemStack,
        ...
        if (armorType == EntityEquipmentSlot.HEAD &&
                (PaintUtil.hasPaintSource(itemStack) || itemStack.getSubCompound("DSPAINT") != null)) { // TODO 1.13
                                                                                                        // remove
                                                                                                        // DSPAINT
            // Don't render the armor model of the helmet if it is painted. The paint will be rendered by the
            // PaintedHelmetLayer.
    ...
    public @Nonnull IRenderUpgrade getRender(@Nonnull AbstractClientPlayer player) {
        return armorType == EntityEquipmentSlot.HEAD ? PaintedHelmetLayer.instance :
                IHasPlayerRenderer.super.getRender(player);
    }
    ...
    @Override
    public @Nonnull Multimap<String, AttributeModifier> getAttributeModifiers(@Nonnull EntityEquipmentSlot equipmentSlot,
        ...
        if (equipmentSlot == this.armorType) {
            ...
            multimap.put(SharedMonsterAttributes.ARMOR.getName(),
                    new AttributeModifier(ARMOR_MODIFIERS.get(equipmentSlot), "Armor modifier",
                            armorMaterial.getDamageReductionAmount(equipmentSlot), 0));
            multimap.put(SharedMonsterAttributes.ARMOR_TOUGHNESS.getName(),
                    new AttributeModifier(ARMOR_MODIFIERS.get(equipmentSlot), "Armor toughness",
                            armorMaterial.getToughness(), 0));
            // see
            // crazypants.enderio.base.item.darksteel.upgrade.DarkSteelUpgradeMixin.getAttributeModifiers(EntityEquipmentSlot,
            // ItemStack)
            for (IDarkSteelUpgrade upgrade : UpgradeRegistry.getUpgrades()) {
    ...
    public ArmorProperties getProperties(EntityLivingBase player, @Nonnull ItemStack armor, DamageSource source,
        ...
        double ratio = 0d; // percentage of damage that is removed before normal armor calculations are done. This is
                           // actually bad for armor with a high
                           // toughness...
        if (!source.isUnblockable()) {
            ArmorMaterial armorMaterial = getMaterial(armor);
            int damageReductionAmount = armorMaterial.getDamageReductionAmount(armorType) -
            ...
            if (damageReductionAmount > 0) {
                ...
                // This is just to counter the maximum effective armor (80%) vanilla enforces for normal armor
                // calculations
            }
        }
    ...
    public boolean hasUpgradeCallbacks(@Nonnull IDarkSteelUpgrade upgrade) {
        return upgrade == ElytraUpgrade.INSTANCE || upgrade == GogglesOfRevealingUpgrade.INSTANCE ||
                upgrade == ThaumaturgeRobesUpgrade.BOOTS || upgrade == ThaumaturgeRobesUpgrade.LEGS ||
                upgrade == ThaumaturgeRobesUpgrade.CHEST;
    }
    ...
    // Note: The GUI is bound to ModObject.itemDarkSteelChestplate, but that is just for technical reasons. It supports
    // any armor item with the upgrade, even if
    ...
    public Container getServerGuiElement(@Nonnull EntityPlayer player, int param1, int param2, int param3) {
        ...
        if (enc.hasSlots()) {
            // Note: StorageCap(..., size:0, ...) ignores all other parameters, so it is ok to call it with another
            // armor item
            return new StorageContainer(player.inventory, //
                    new StorageCap(NbtValue.INVENTORY, EntityEquipmentSlot.FEET, enc.get(EntityEquipmentSlot.FEET),
                            player), //
                    new StorageCap(NbtValue.INVENTORY, EntityEquipmentSlot.LEGS, enc.get(EntityEquipmentSlot.LEGS),
                            player), //
                    new StorageCap(NbtValue.INVENTORY, EntityEquipmentSlot.CHEST, enc.get(EntityEquipmentSlot.CHEST),
                            player), //
                    new StorageCap(NbtValue.INVENTORY, EntityEquipmentSlot.HEAD, enc.get(EntityEquipmentSlot.HEAD),
                            player));
        }
        return null;
    }
    ...
    @Override
    public @Nonnull ActionResult<ItemStack> onItemRightClick(@Nonnull World worldIn, @Nonnull EntityPlayer playerIn,
