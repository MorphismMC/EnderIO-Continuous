...
public class ItemInventoryCharger extends Item
    ...
    public static ItemInventoryCharger createSimple(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemInventoryCharger res = new ItemInventoryCharger(modObject,
                CapacitorKey.DARK_STEEL_CHARGER_SIMPLE_ENERGY_BUFFER,
    ...
    public static ItemInventoryCharger createBasic(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemInventoryCharger res = new ItemInventoryCharger(modObject,
                CapacitorKey.DARK_STEEL_CHARGER_BASIC_ENERGY_BUFFER,
    ...
    public static ItemInventoryCharger create(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemInventoryCharger res = new ItemInventoryCharger(modObject, CapacitorKey.DARK_STEEL_CHARGER_ENERGY_BUFFER,
                CapacitorKey.DARK_STEEL_CHARGER_ENERGY_INPUT,
    ...
    public static ItemInventoryCharger createVibrant(@Nonnull IModObject modObject, @Nullable Block block) {
        ItemInventoryCharger res = new ItemInventoryCharger(modObject,
                CapacitorKey.DARK_STEEL_CHARGER_VIBRANT_ENERGY_BUFFER,
                CapacitorKey.DARK_STEEL_CHARGER_VIBRANT_ENERGY_INPUT,
                CapacitorKey.DARK_STEEL_CHARGER_VIBRANT_ENERGY_USE, false);
        return res;
    }
    ...
    protected ItemInventoryCharger(@Nonnull IModObject modObject, @Nonnull ICapacitorKey energyStorageKey,
                                   @Nonnull ICapacitorKey energyInputKey,
    ...
    public static void onTick(PlayerTickEvent event) {
        if (event.side == Side.SERVER && event.phase == Phase.END &&
                EnderIO.proxy.getServerTickCount() % (20 * 1) == 0) {
    ...
    public void charge(@Nonnull EntityPlayer player, @Nonnull ItemStack stack, int slot) {
        if (NbtValue.ENABLED.getBoolean(stack)) {
            ...
            if (eu != null && eu.getEnergy() > 0) {
                for (int i : rangeLimited ? SlotNeighborHelper.getSlotNeighbors(slot) :
                        SlotNeighborHelper.getAllSlots()) {
        ...
        if (EnergyUpgradeManager.loadFromItem(stack) != null) {
            if (!worldIn.isRemote) {
                ...
                playerIn.sendStatusMessage(
                        (NbtValue.ENABLED.getBoolean(stack) ? Lang.INVCHARGER_ENABLED : Lang.INVCHARGER_DISABLED)
                                .toChatServer(),
                        true);
            }
            return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, stack);
