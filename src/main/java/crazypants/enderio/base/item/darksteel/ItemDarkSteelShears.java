...
public class ItemDarkSteelShears extends ItemShears
                                 implements IAdvancedTooltipProvider, IDarkSteelItem, IOverlayRenderAware {
    ...
    public boolean onBlockStartBreak(@Nonnull ItemStack itemstack, @Nonnull BlockPos pos,
        ...
        for (int dx = -DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dx <=
                DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dx++) {
            for (int dy = -DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dy <=
                    DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dy++) {
                for (int dz = -DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dz <=
                        DarkSteelConfig.shearsBlockAreaBoostWhenPowered.get(); dz++) {
                    Block block2 = player.world.getBlockState(new BlockPos(x + dx, y + dy, z + dz)).getBlock();
                    if ((block2 instanceof BlockTripWire) || (block2 instanceof IShearable && ((IShearable) block2)
                            .isShearable(itemstack, player.world, new BlockPos(x + dx, y + dy, z + dz)))) {
    ...
    /**
     * This is a copy of net.minecraft.item.ItemShears.onBlockStartBreak(ItemStack, BlockPos, EntityPlayer) with added
     * code to fire a BlockHarvestEvent.
     */
    @SuppressWarnings({ "cast", "null" })
    public boolean super_onBlockStartBreak(@Nonnull ItemStack itemstack, @Nonnull BlockPos pos,
                                           @Nonnull net.minecraft.entity.player.EntityPlayer player) {
        if (player.world.isRemote) {
            return false;
        }
        ...
        if (block instanceof net.minecraftforge.common.IShearable) {
            ...
            if (target.isShearable(itemstack, player.world, pos)) {
                ...
                float chance = ForgeEventFactory.fireBlockHarvesting(drops, player.world, pos, blockState,
                        ...
                        EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) != 0, player); // EIO
                                                                                                                 // ADD
                ...
                for (ItemStack stack : drops) {
                    if (player.world.rand.nextFloat() <= chance) { // EIO ADD
                        ...
                        net.minecraft.entity.item.EntityItem entityitem = new net.minecraft.entity.item.EntityItem(
                                player.world, (double) pos.getX() + d,
    ...
    private static final @Nonnull Predicate<Entity> selectShearable = new Predicate<Entity>() {
        ...
        public boolean apply(@Nullable Entity entity) {
            return entity instanceof IShearable && !entity.isDead &&
                    ((IShearable) entity).isShearable(Prep.getEmpty(), entity.world, entity.getPosition());
        }
    ...
    @Override
    public boolean itemInteractionForEntity(@Nonnull ItemStack itemstack, @Nonnull EntityPlayer player,
        ...
        if (entity instanceof IShearable) {
            AxisAlignedBB bb = new AxisAlignedBB(entity.posX - DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get(),
                    entity.posY - DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get(),
                    entity.posZ - DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get(),
                    entity.posX + DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get(),
                    entity.posY + DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get(),
                    entity.posZ + DarkSteelConfig.shearsEntityAreaBoostWhenPowered.get());

            List<Entity> sortedTargets = new ArrayList<Entity>(
                    entity.world.getEntitiesWithinAABB(Entity.class, bb, selectShearable));
            ...
            for (int i = 0; i < maxSheep; i++) {
                ...
                if (entity2 instanceof EntityLivingBase &&
                        super.itemInteractionForEntity(itemstack, player, (EntityLivingBase) entity2, hand)) {
                    result = true;
                }
            }
            return result;
    ...
    public void onBreakSpeedEvent(PlayerEvent.BreakSpeed evt) {
        if (evt.getOriginalSpeed() > 2.0 &&
                isEquippedAndPowered(evt.getEntityPlayer(), DarkSteelConfig.shearsPowerUsePerDamagePoint)) {
    ...
    public void addDetailedEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
        ...
        if (EnergyUpgradeManager.itemHasAnyPowerUpgrade(itemstack)) {
            ...
            list.add(Lang.SHEARS_POWERED.get(TextFormatting.WHITE,
                    DarkSteelConfig.shearsEfficiencyBoostWhenPowered.get()));
        }
