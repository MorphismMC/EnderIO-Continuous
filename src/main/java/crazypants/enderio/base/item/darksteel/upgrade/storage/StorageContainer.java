...
public class StorageContainer extends ContainerEnderCap<EIOCombinedInvWrapper<StorageCap>, TileEntity> {
    ...
    public StorageContainer(@Nonnull InventoryPlayer playerInv, @Nonnull StorageCap feet, @Nonnull StorageCap legs,
                            @Nonnull StorageCap body,
        ...
        for (int i = 0; i < getItemHandler().getSlots(); i++) {
            ...
            addSlotToContainer(new BaseSlotItemHandler(getItemHandler(), i, X0 + 18 * (x + xoff), Y0 + 18 * y) {
                ...
                public boolean isEnabled() {
                    return activeTab == ((EIOCombinedInvWrapper<StorageCap>) getItemHandler())
                            .getHandlerFromSlot(getSlotIndex()).getEquipmentSlot();
                }
    ...
    public boolean canInteractWith(@Nonnull EntityPlayer player) {
        return feet.isStillConnectedToPlayer() && legs.isStillConnectedToPlayer() && body.isStillConnectedToPlayer() &&
                head.isStillConnectedToPlayer();
    }
}
