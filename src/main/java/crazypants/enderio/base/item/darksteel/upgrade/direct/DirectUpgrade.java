...
public class DirectUpgrade extends AbstractUpgrade {
    ...
    public List<IRule> getRules() {
        return new NNList<>(Rules.forSlot(EntityEquipmentSlot.MAINHAND),
                Rules.or(Rules.staticCheck(item -> item.isBlockBreakingTool()),
                        Rules.staticCheck(item -> item.isWeapon())),
    ...
    public static void blockDropEvent(BlockEvent.HarvestDropsEvent event) {
        ...
        for (EnumHand hand : EnumHand.values()) {
            ...
            if (INSTANCE.hasAnyUpgradeVariant(stack) && EnergyUpgradeManager.getEnergyStored(stack) > 0) {
                EnergyUpgradeManager.extractEnergy(stack, doDirect(event) * DarkSteelConfig.directEnergyCost.get(),
                        false);
                return;
            }
        }
    ...
    public static void attackEntityEvent(AttackEntityEvent event) {
        if (event.getEntityPlayer() == null || event.getEntityPlayer().world.isRemote ||
                event.getEntityPlayer() instanceof FakePlayerEIO) {
            return;
        }
        ...
        if (mob.getEntityData().hasUniqueId(HIT_BY_DIRECT)) {
            ...
            if (uuid != null) {
                ...
                if (player != null) {
                    ...
                    if (INSTANCE.hasAnyUpgradeVariant(stack) && EnergyUpgradeManager.getEnergyStored(stack) > 0) {
                        EnergyUpgradeManager.extractEnergy(stack,
                                doDirect(event, player) * DarkSteelConfig.directEnergyCost.get(), false);
                    }
                }
            }
    ...
    private static Integer doDirect(LivingDropsEvent event, @Nonnull EntityPlayer player) {
        ...
        for (Iterator<EntityItem> iterator = event.getDrops().iterator(); iterator.hasNext();) {
            ...
            ItemStack remains = fakeItemPickup(player, next.getItem().copy()); // TODO use ItemUtil version after we
                                                                               // update endercore dep
    ...
    public static int doDirect(BlockEvent.HarvestDropsEvent event) {
        int count = 0;
        final EntityPlayer player = NullHelper.notnullF(event.getHarvester(),
        ...
        for (Iterator<ItemStack> iterator = event.getDrops().iterator(); iterator.hasNext();) {
            ...
            if (random.nextFloat() < event.getDropChance()) {
                ItemStack remains = fakeItemPickup(player, next.copy()); // TODO use ItemUtil version after we update
                                                                         // endercore dep
