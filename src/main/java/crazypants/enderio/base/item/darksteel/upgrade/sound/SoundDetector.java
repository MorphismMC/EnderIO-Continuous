...
public class SoundDetector {
    ...
    private static final ArrayBlockingQueue<SoundSource> soundQueue = new ArrayBlockingQueue<SoundSource>(
            MAX_PARTICLES);
    ...
    public static void onSound(PlaySoundSourceEvent evt) {
        if (isEnabled() && soundQueue.size() < MAX_PARTICLES) {
            switch (evt.getSound().getCategory()) {
                case BLOCKS:
                    soundQueue.offer(new SoundSource(evt.getSound().getXPosF(), evt.getSound().getYPosF(),
                            evt.getSound().getZPosF(), evt.getSound().getVolume(), false));
                    break;
                case AMBIENT:
                case HOSTILE:
                case NEUTRAL:
                case PLAYERS:
                    soundQueue.offer(new SoundSource(evt.getSound().getXPosF(), evt.getSound().getYPosF(),
                            evt.getSound().getZPosF(), evt.getSound().getVolume(), true));
                    break;
                default:
                    break;
            }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        while (true) {
            ...
            if (distSq > minDist &&
                    distSq <= DarkSteelConfig.soundLocatorRange.get() * DarkSteelConfig.soundLocatorRange.get()) {
                Minecraft.getMinecraft().effectRenderer
                        .addEffect(new SoundParticle(Minecraft.getMinecraft().player.world, ss));
            }
        }
    }
    ...
    public static class SoundSource {
        ...
        public SoundSource(Entity ent, float volume) {
            ...
            pos = new Vector3d(bb.minX + (bb.maxX - bb.minX) / 2, bb.minY + (bb.maxY - bb.minY) / 2,
