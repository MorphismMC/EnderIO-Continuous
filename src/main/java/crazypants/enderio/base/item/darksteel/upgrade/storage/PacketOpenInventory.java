    ...
    public static class Handler implements IMessageHandler<PacketOpenInventory, IMessage> {
        ...
        public IMessage onMessage(PacketOpenInventory message, MessageContext ctx) {
            ...
            for (EntityEquipmentSlot slot : StorageUpgrade.ARMOR) {
                param1.set(slot, StorageUpgrade.slots(slot, StorageUpgrade.INSTANCE
                        .getUpgradeVariantLevel(player.inventory.armorInventory.get(slot.getIndex()))));
            }
            if (param1.hasSlots()) {
                // Note: The GUI is bound to ModObject.itemDarkSteelChestplate, but that is just for technical reasons.
                // It supports any armor item with this upgrade
