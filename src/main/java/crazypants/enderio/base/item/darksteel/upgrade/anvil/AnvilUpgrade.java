...
public class AnvilUpgrade extends AbstractUpgrade {
    ...
    public static final @Nonnull NNList<AnvilUpgrade> INSTANCES = new NNList<>(new AnvilUpgrade(0), new AnvilUpgrade(1),
            new AnvilUpgrade(2));
    ...
    public static AnvilUpgrade getHighestEquippedUpgrade(@Nonnull EntityPlayer player) {
        ...
        for (EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
            level = Math.max(level, INSTANCES.get(0)
                    .getUpgradeVariantLevel(player.getItemStackFromSlot(NullHelper.notnullJ(slot, "Enum.values()"))));
        }
        return level < 0 ? null : INSTANCES.get(level);
    }
    ...
    public AnvilUpgrade(int level) {
        super(UPGRADE_NAME, level, "enderio.darksteel.upgrade." + UPGRADE_NAME + "." + level,
                DarkSteelConfig.anvilUpgradeCost.get(level));
    }
