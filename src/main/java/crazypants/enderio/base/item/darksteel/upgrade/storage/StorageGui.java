...
public class StorageGui extends GuiContainerBaseEIO<StorageContainer> implements StorageContainerProxy {
    ...
    /**
     * Make sure client and server agree on which tab is open initially. Prefer the chest tab as it is the biggest (with
     * default config).
     * <p>
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        for (int i = 0; i < StorageUpgrade.ARMOR.length; i++) {
            ...
            if (getOwner().getItemHandler().getHandlerFromIndex(drawTab.getIndex()).getSlots() > 0) {
                renderStdTab(sx, sy, i,
                        Minecraft.getMinecraft().player.inventory.armorInventory.get(drawTab.getIndex()),
                        drawTab == getOwner().activeTab);
            }
        }
    }
