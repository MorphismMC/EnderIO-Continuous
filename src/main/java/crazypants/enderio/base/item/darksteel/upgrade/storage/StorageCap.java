 ...
 * <p>
 * <ol>
 * <li>The size is fixed, even if the nbt has a different size. If the inventory is connected to an item, the item's
 * data will be forced to the right size. If
 ...
 * </ol>
 * 
 * Note that this relies on the calling code having determined if the stack can have an inventory and how big that
 * inventory is. The only thing that is checked
...
public class StorageCap extends ItemStackHandler {
     ...
     * 
     */
    public StorageCap(@Nonnull NbtValue NBT, @Nonnull EntityEquipmentSlot equipmentSlot, int size,
    ...
    public boolean isStillConnectedToPlayer() {
        return Prep.isInvalid(owner) == isOwnerEmpty &&
                (player == null || Prep.isInvalid(owner) || owner == player.getItemStackFromSlot(equipmentSlot));
    }
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        if (stack == owner || stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null) != null ||
                isLarge(stack) || (Prep.isValid(stack) && !validator.apply(stack))) {
            if (player != null) {
