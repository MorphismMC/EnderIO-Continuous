...
public class ItemConduitProbe extends Item implements IResourceTooltipProvider, IHideFacades, IHaveRenderers {
    ...
    @Override
    public @Nonnull EnumActionResult onItemUse(@Nonnull EntityPlayer playerIn, @Nonnull World world,
        ...
        if (playerIn.isCreative()) {
            ...
            if (tileEntity instanceof TileEntityBase) {
                playerIn.sendMessage(new TextComponentString(
                        (world.isRemote ? "CLIENT: " : "SERVER: ") + tileEntity.getUpdateTag().toString()));
                if (!world.isRemote) {
                    playerIn.sendMessage(
                            new TextComponentString("SAVE: " + tileEntity.writeToNBT(new NBTTagCompound()).toString()));
                }
            }
        }
    ...
    public void registerRenderers(@Nonnull IModObject modObject) {
        ModelLoader.setCustomModelResourceLocation(this, 0,
                new ModelResourceLocation(modObject.getRegistryName(), "variant=probe"));
        ModelLoader.setCustomModelResourceLocation(this, 1,
                new ModelResourceLocation(modObject.getRegistryName(), "variant=copy"));
    }
}
