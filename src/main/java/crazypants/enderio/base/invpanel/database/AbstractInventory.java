...
public abstract class AbstractInventory {
     ...
     * @return true if it should be scanned either because the data is stale or because it sent a notification about
     *         changes.
     */
    ...
    /**
     * Called during a scan when a change in the inventory has been detected. This will determine if the inventory sends
    ...
    private void updateSlotKey(@Nonnull IInventoryDatabaseServer db, int slot, SlotKey slotKey, IServerItemEntry key,
                               int count) {
        if (slotKey != null) {
            slotKey.remove(db);
            slotKey = null;
