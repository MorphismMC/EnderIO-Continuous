...
public final class BlockFluidEio {
    ...
    static class FireWater extends BlockFluidEnder {
        ...
        public static final @Nonnull ResourceLocation LOOT_TABLE = new ResourceLocation(EnderIO.DOMAIN,
                "infinityfluid");

        @Override
        public void randomTick(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
        ...
        public void onEntityCollision(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
            ...
            if (!world.isRemote && entity instanceof EntityLivingBase) {
                ((EntityLivingBase) entity)
                        .addPotionEffect(new PotionEffect(MobEffects.JUMP_BOOST, 150, 3, true, true));
            }
            super.onEntityCollision(world, pos, state, entity);
        }
                ...
                NNList.FACING.apply(new Callback<EnumFacing>() {
                    ...
                    public void apply(@Nonnull EnumFacing side) {
                        ...
                        if (neighbor.getBlock() instanceof BlockFire &&
                                neighbor.getBlock() != ModObject.blockColdFire.getBlock()) {
                            if (worldIn.rand.nextFloat() < .5f) {
                                ...
                                if (isSourceBlock(worldIn, pos)) {
                                    worldIn.newExplosion(null, pos.getX() + .5f, pos.getY() + .5f, pos.getZ() + .5f, 2,
                                            true, true);
                                }
                                float strength = .5f;
                                for (BlockPos explosion : explosions) {
                                    worldIn.newExplosion(null, explosion.getX() + .5f, explosion.getY() + .5f,
                                            explosion.getZ() + .5f, strength, true, true);
    ...
    static class NutrientDistillation extends BlockFluidEnder {
        ...
        public void onEntityCollision(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
            ...
            if (!world.isRemote && entity instanceof EntityPlayerMP) {
                ...
                if (time % FluidConfig.nutrientFoodBoostDelay.get() == 0 &&
    ...
    static class VaporOfLevity extends BlockFluidEnder {
        ...
        public void randomDisplayTick(@Nonnull IBlockState state, @Nonnull World worldIn, @Nonnull BlockPos pos,
                                      @Nonnull Random rand) {
            if (rand.nextFloat() < .5f) {
                ...
                if (!neighborState.isFullCube()) {
                    ...
                    worldIn.spawnParticle(EnumParticleTypes.REDSTONE, x, y, z, (col >> 16 & 255) / 255d,
                            (col >> 8 & 255) / 255d, (col & 255) / 255d);
                }
            }
        }

        /*
         * BlockFluidClassic uses both random ticks and update ticks (after a neighbor change) to flow. This means we
         * cannot decouple our snow gen completely from
         * that by using one or the other. However, we can stay out of the random tick, so we don't have to do anything
         * if there wasn't either a block update around
         * us or we scheduled a update ourselves because there's the possibility to gen something. This means we may
         * miss the corners if they are removed after we
            ...
            if (!world.isRemote && rand.nextFloat() < 0.05f) {
                ...
                if (canMakeSnow(world, neighborPos, neighborState)) {
                    world.setBlockState(neighborPos,
                            NullHelper.notnullF(
                                    ForgeEventFactory.fireFluidPlaceBlockEvent(world, neighborPos, pos,
        ...
        protected boolean canMakeIce(@Nonnull World world, final @Nonnull BlockPos neighborPos,
                                     final @Nonnull IBlockState neighborState) {
        ...
        protected boolean canMakeSnow(@Nonnull World world, final @Nonnull BlockPos neighborPos,
                                      final @Nonnull IBlockState neighborState) {
            final BlockPos belowNeighborPos = neighborPos.down();
            final Block neighborBlock = neighborState.getBlock();
            return neighborBlock != Blocks.SNOW_LAYER && !(neighborBlock instanceof IFluidBlock) &&
                    !(neighborBlock instanceof BlockLiquid) && neighborBlock.isReplaceable(world, neighborPos) &&
                    world.getBlockState(belowNeighborPos).isSideSolid(world, belowNeighborPos, EnumFacing.UP);
        }
            ...
            for (int x = -1; x <= 1; x++) {
                for (int z = -1; z <= 1; z++) {
                    if (x != 0 || z != 0) {
                        ...
                        final IBlockState neighborState = world.getBlockState(neighborPos);
                        if (canMakeSnow(world, neighborPos, neighborState) ||
                                canMakeIce(world, neighborPos, neighborState)) {
                            return true;
                        }
                    }
                }
