...
public class ItemFilterImpl implements ItemFilter.WithGhostSlots {
    ...
    public void copyFrom(@Nonnull ItemFilterImpl o) {
    ...
    public ItemFilterImpl() {
        this(BasicFilterTypes.filterUpgradeBasic);
    }

    public ItemFilterImpl(BasicFilterTypes type) {
    ...
    private FilterResult itemMatched(@Nonnull ItemStack item) {
        if (damageMode.passesFilter(item)) {
            // if there are no filter items, but a damage mode is set, the filter will let items pass that match that
            // filter mode
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if (Prep.isValid(filterStack)) {
                    if (item.getItem() == filterStack.getItem()) {
                        if (!matchMeta || !item.getHasSubtypes() ||
                                item.getItemDamage() == filterStack.getItemDamage()) {
    ...
    public void readFromNBT(@Nonnull NBTTagCompound nbtRoot) {
        ...
        damageMode = NullHelper.notnullJ(DamageMode.values()[NbtValue.FILTER_DAMAGE.getInt(nbtRoot) & 255],
                "Enum.values()");
        ...
        int numItems = isBig ? BasicFilterTypes.filterUpgradeBig.getSlots() : (isAdvanced ?
                BasicFilterTypes.filterUpgradeAdvanced.getSlots() : BasicFilterTypes.filterUpgradeBasic.getSlots());
    ...
    public boolean isDefault() {
        return !isAdvanced && !isValid() && isBlacklist == DEFAULT_BLACKLIST && matchMeta == DEFAULT_META &&
                matchNBT == DEFAULT_NBT && useOreDict == DEFAULT_ORE_DICT && sticky == DEFAULT_STICKY && !isBig;
    }
    ...
    public String toString() {
        ...
        // + sticky + ", items=" + Arrays.toString(items) + ", oreIds=" + Arrays.toString(oreIds) + ", isAdvanced=" +
        // isAdvanced + "]";
