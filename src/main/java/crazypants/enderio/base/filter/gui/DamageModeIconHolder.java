...
public enum DamageModeIconHolder implements ICycleEnum {
    ...
    public @Nonnull List<String> getTooltipLines() {
        return Collections.singletonList(
                EnderIO.lang.localize("gui.item_filter.damage.".concat(name().toLowerCase(Locale.ENGLISH))));
    }

    public @Nonnull DamageMode getMode() {
        return NullHelper.first(
                DamageMode.values()[MathHelper.clamp(this.ordinal(), 0, DamageMode.values().length - 1)],
                DamageMode.DISABLED);
    }
