...
public class CommandConfig extends CommandBase {
    ...
    public @Nonnull String getUsage(@Nonnull ICommandSender sender) {
        return "/" + getName() +
                " config set <section> <key> <value>|config get <section> <key>|config list [<section>]|config save|config help";
    }
    ...
    private void doList(MinecraftServer server, ICommandSender sender, String section) {
        if (sections.containsKey(section) && !sections.get(section).isEmpty()) {
            ...
            sender.sendMessage(
                    new TextComponentString(sections.get(section).getOrderedValues().stream()
                            .map(property -> property.getName()).collect(Collectors.joining(", "))));
        } else {
            String list = sections.keySet().stream().filter(name -> name.startsWith(section))
                    .collect(Collectors.joining(", "));
            if (list != null && !list.isEmpty()) {
                // (2) partial section name, matches the start of names. List sections
                sender.sendMessage(new TextComponentString(list));
            } else {
                list = sections.keySet().stream().filter(name -> name.contains(section))
                        .collect(Collectors.joining(", "));
                if (list != null && !list.isEmpty()) {
    ...
    private void sendKeyComments(ICommandSender sender, Property property, String section, String key, boolean isSet,
                                 boolean isValidSet) {
                    ...
                    if (sections.containsKey(args[2]) && !sections.get(args[2]).isEmpty()) {
                        if (args.length == 4) {
                            return getListOfStringsMatchingLastWord(args,
                                    sections.get(args[2]).getOrderedValues().stream()
                                            .map(property -> property.getName()).collect(Collectors.toList())
