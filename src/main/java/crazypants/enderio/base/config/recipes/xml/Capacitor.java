...
public class Capacitor extends AbstractConditional {
    ...
    public void register(@Nonnull String recipeName, @Nonnull RecipeLevel recipeLevel) {
        if (!disabled && valid && active) {
            ...
            // TODO 1.16: we need to know if this is the base value (setValue) or an override (setOverride) here.
            // Alternatively we can remove the registry's override
            // funtionality and rely on re-registering core values. That's what other registries will be doing. It's
            // just that we always need default values, unlike
