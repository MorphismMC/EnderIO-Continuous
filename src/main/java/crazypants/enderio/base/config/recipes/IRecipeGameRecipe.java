...
public interface IRecipeGameRecipe extends IRecipeConfigElement {
     ...
     * <p>
     * Please note that the parameters are used to be able to define some attributes on a relatively high level of the
     * xml. The first level that has access to the
     ...
     * <p>
     * Not all recipe types need to support this on a recipe level, this can be done by wiping whole registries on a top
     * level instead.
     * <p>
     * Note that this may be called on recipe elements that have not been registered. The element should keep a
     * reference to its registration around and not rely
    ...
    /**
     * 
     * @return the configured name if there is such a thing. Recipe elements that don't have names don't need to
     *         implement this---the elements containing them
