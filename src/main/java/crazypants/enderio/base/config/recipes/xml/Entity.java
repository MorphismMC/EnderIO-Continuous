...
public class Entity implements IRecipeConfigElement {
    ...
    public void register(String recipeName, RecipeLevel recipeLevel) {
        if (isDefault()) {
            ...
            EntityDataRegistry.getInstance().addEntityData(name, filter, getCostMultiplier(), isDisabled(),
                    !isSoulvial(), isClone());
        }
    }
        ...
        if (name.equals("*") || name.equals("*:*")) {
            ...
            if (disabled) {
                throw new InvalidRecipeConfigException(
                        "Cannot set the 'all modded bosses' entry to 'disabled' (set 'soulvial' to false instead)");
            }
            if (clone) {
            ...
            if (matcher1.matches()) {
                filter = new ResourceLocationMatcher(matcher1.group(1), null, null, null, matcher1.group(2),
                        matcher1.group(3));
            } else if (matcher2.matches()) {
                filter = new ResourceLocationMatcher(null, matcher2.group(1), matcher2.group(2), matcher2.group(3),
                        null, null);
            } else {
    ...
    private static class ResourceLocationMatcher implements Predicate<ResourceLocation> {
        ...
        ResourceLocationMatcher(@Nullable String modid, @Nullable String modidPre, @Nullable String modidPost,
                                @Nullable String id, @Nullable String pre,
