    ...
    public void register(@Nonnull String recipeName, @Nonnull RecipeLevel recipeLevel) {
        if (isValid() && isActive()) {
            if (TicProxy.isLoaded()) {
                if (cast.isPresent()) {
                    TicProxy.registerTableCast(getOutput().getThing(), cast.get().getThing(), input.get().getThing(),
                            input.get().amount, cast.get().getConsumed());
                } else {
                    TicProxy.registerTableCast(getOutput().getThing(), new Things(), input.get().getThing(),
                            input.get().amount, false);
                }
                if (recipeLevel != RecipeLevel.IGNORE) {
                    Log.warn("Ignoring recipe level " + recipeLevel + " configured for Tinkers casting recipe '" +
                            recipeName + "'---the Tinkers casting table doesn't have (or support) levels");
                }
            } else {
