...
public class Grid implements IRecipeConfigElement {
        ...
        try {
            ...
            if (items.size() < width * height) {
                throw new InvalidRecipeConfigException(
                        "Not enough items (required=" + (width * height) + ", provided=" + items.size() + ")");
            }
            if (items.size() > width * height) {
                throw new InvalidRecipeConfigException(
                        "Too many items (required=" + (width * height) + ", provided=" + items.size() + ")");
            }
            valid = true;
    ...
    public void enforceValidity() throws InvalidRecipeConfigException {
        ...
        if (!hasAtLeastOneItem) {
            throw new InvalidRecipeConfigException(
                    "Rejecting crafting recipe without any items, only empty spaces in <grid>");
        }
    }
