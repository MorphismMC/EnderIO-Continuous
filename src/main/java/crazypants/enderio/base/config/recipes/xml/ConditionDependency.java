        ...
        try {
            ...
            if (upgradeString.isPresent()) {
                if (upgradeString.get().length() > 64) {
                    throw new InvalidRecipeConfigException(String.format(
                            "The upgrade ID %s is longer than the maximum of 64 characters.", upgradeString.get()));
                }
                if (!upgradeString.get().equals(upgradeString.get().toLowerCase(Locale.ENGLISH))) {
                    throw new InvalidRecipeConfigException(
                            String.format("The upgrade ID %s must be all lowercase.", upgradeString.get()));
                }
            ...
            if (modString.isPresent()) {
                if (modString.get().length() > 64) {
                    throw new InvalidRecipeConfigException(String
                            .format("The modId %s is longer than the maximum of 64 characters.", modString.get()));
                }
                if (!modString.get().equals(modString.get().toLowerCase(Locale.ENGLISH))) {
                    throw new InvalidRecipeConfigException(
                            String.format("The modId %s must be all lowercase.", modString.get()));
                }
