...
public class StaxFactory {
    ...
    public <T extends IRecipeRoot> T readRoot(T target, String rootElement) throws XMLStreamException,
                                                                            InvalidRecipeConfigException {
        T result = null;
    ...
    public @Nonnull <
            T extends IRecipeConfigElement> T read(T target,
                                                   StartElement startElement) throws InvalidRecipeConfigException,
                                                                              XMLStreamException {
        ...
        try {
            ...
            while (attributes.hasNext()) {
                ...
                if (!target.setAttribute(this, attribute.getName().getLocalPart().toString(),
                        NullHelper.first(attribute.getValue(), ""))) {
                    throw new InvalidRecipeConfigException(
                            "Unexpected attribute '" + attribute.getName() + "' inside " + startElement.getName());
                }
            }
            ...
            if (e.getMessage().contains("[row,col]")) {
                ...
                throw new InvalidRecipeConfigException(e,
                        "at [row,col]:[" + startElement.getLocation().getLineNumber() + "," +
        ...
        while (eventReader.hasNext()) {
            ...
            try {
                if (event.isStartElement()) {
                    if (!target.setElement(this, NullHelper.first(event.asStartElement().getName().getLocalPart(), ""),
                            NullHelper.first(event.asStartElement()))) {
                        throw new InvalidRecipeConfigException("Unexpected tag '" + event.asStartElement().getName() +
                                "' inside " + startElement.getName());
                    }
                    ...
                    throw new InvalidRecipeConfigException(
                            "Unexpected '" + event.getEventType() + "' inside " + startElement.getName());
                }
            } catch (InvalidRecipeConfigException e) {
                if (e.getMessage().contains("[row,col]")) {
                    throw e;
                } else {
                    throw new InvalidRecipeConfigException(e, "at [row,col]:[" + event.getLocation().getLineNumber() +
    ...
    public void skip(StartElement startElement) throws InvalidRecipeConfigException, XMLStreamException {
        while (eventReader.hasNext()) {
            ...
            if (event.isEndElement() &&
                    event.asEndElement().getName().getLocalPart().equals(startElement.getName().getLocalPart())) {
                return;
            }
        }
        throw new InvalidRecipeConfigException("Unexpected end of document inside " + startElement.getName());
