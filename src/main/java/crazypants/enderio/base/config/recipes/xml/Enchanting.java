...
public class Enchanting extends AbstractConditional {
    ...
    public void register(@Nonnull String recipeName, @Nonnull RecipeLevel recipeLevel) {
        if (isValid() && isActive()) {
            MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.ENCHANTER,
                    new EnchanterRecipe(recipeLevel, input.get().getThing(),
                            input.get().getAmount(), enchantment.get().getEnchantment(),
