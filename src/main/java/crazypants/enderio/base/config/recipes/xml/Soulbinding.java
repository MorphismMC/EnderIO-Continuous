        ...
        if (isValid() && isActive()) {
            ...
            switch (soulHandling) {
                case ALL:
                    ...
                    break;
                case SPAWNABLE:
                    entityFilter = name -> name != null &&
                            !EntityDataRegistry.getInstance().isBlackListedForSpawning(name);
                    break;
                case MONSTERS:
                    entityFilter = name -> FuncUtil.runIfOr(FuncUtil.runIf(name, n -> EntityList.getClass(n)),
                            eclass -> IMob.class.isAssignableFrom(eclass), false);
                    break;
                case ANIMALS:
                    entityFilter = name -> FuncUtil.runIfOr(FuncUtil.runIf(name, n -> EntityList.getClass(n)),
                            eclass -> IAnimals.class.isAssignableFrom(eclass) && !IMob.class.isAssignableFrom(eclass),
                            false);
                    break;
                default:
                case LISTED:
                ...
                public void apply(@Nonnull ItemStack anInput) {
                    if (entityFilter2 != null) {
                        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.SOULBINDER, //
                                new DynamicSoulBinderRecipe(anInput, getOutput().getItemStack(), energy, levels,
                                        recipeName + i++, recipeLevel, entityFilter2, logic));
                    } else {
                        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.SOULBINDER, //
                                new BasicSoulBinderRecipe(anInput, getOutput().getItemStack(), energy, levels,
                                        recipeName + i++, recipeLevel, soulnames, logic));
                    }
                }
            });
        }
