            ...
            for (AbstractConditional recipe : ((Recipes) other).recipes) {
                if (!recipeNames.containsKey(recipe.getName())) {
                    ...
                    throw new InvalidRecipeConfigException(
                            "Duplicate recipe '" + recipe.getName() +
                                    "'. A recipe with the same name was already read from " + recipe.getSource());
                } else if (overrides == Overrides.WARN) {
                    Log.warn("Recipe '" + recipe.getName() + "' from '" + recipe.getSource() +
                            "' is being replaced by a recipe from '" + recipeNames.get(recipe.getName()).getSource() +
                            "'");
                }
            }
    ...
    protected <T extends AbstractConditional> void addRecipe(T element, StaxFactory factory, StartElement startElement)
        ...
        for (AbstractConditional existingRecipe : recipes) {
            if (existingRecipe.getName().equals(recipe.getName())) {
                throw new InvalidRecipeConfigException(
                        "Duplicate recipe '" + recipe.getName() +
                                "'. A recipe with the same name was already read from " + existingRecipe.getSource());
            }
        }
        recipes.add(recipe);
    }
