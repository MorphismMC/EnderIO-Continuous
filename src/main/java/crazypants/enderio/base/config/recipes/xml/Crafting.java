...
public class Crafting extends AbstractCrafting {
    ...
    public static ResourceLocation mkRL(String recipeName) {
        String s = recipeName.replaceAll("[^A-Za-z0-9]", "_").replaceAll("([A-Z])", "_$0").replaceAll("__+", "_")
                .replaceFirst("^_", "").replaceFirst("_$", "")
                .toLowerCase(Locale.ENGLISH);
        ModContainer activeMod = Loader.instance().activeModContainer();
        if (activeMod == null) {
            throw new RuntimeException(
                    "I really doubt Mojang would ever use our recipe handler, so something just went really wrong because " +
    ...
    private void log(String recipeType, String recipeName, ResourceLocation recipeRL, ItemStack result,
                     NNList<Ingredient> nnList) {
        Log.debug("Registering " + recipeType + " for '" + recipeName + "' as '" + recipeRL + "': " + result + ": " +
                nnList);
    }
    ...
    public void register(@Nonnull String recipeName, @Nonnull RecipeLevel recipeLevel) {
        if (valid && active) {
            ...
            if (grid.isPresent()) {
                ...
                if (upgrade) {
                    log("ShapedRecipe.Upgrade", recipeName, recipeRL, getOutput().getItemStack(),
                            grid.getIngredients());
                    registry.register(new ShapedRecipe.Upgrade(recipeRL, grid.getWidth(), grid.getHeight(),
                            grid.getIngredients(), getOutput().getThing()));
                } else {
                    log("ShapedRecipe", recipeName, recipeRL, getOutput().getItemStack(), grid.getIngredients());
                    registry.register(new ShapedRecipe(recipeRL, grid.getWidth(), grid.getHeight(),
                            grid.getIngredients(), getOutput().getThing()));
                }
                if (getOutput().hasAlternatives()) {
                    getOutput().getAlternatives().apply(new Callback<ItemStack>() {
                        ...
                        public void apply(@Nonnull ItemStack alternative) {
                            Log.debug("Providing synthetic alternative recipe to JEI for oredicted output: " +
                                    alternative + ": " + grid.getIngredients());
                            JeiAccessor.addAlternativeRecipe(new ShapedRecipes("", grid.getWidth(), grid.getHeight(),
                                    grid.getIngredients(), alternative));
                        }
                    });
                }
            } else {
                ...
                if (upgrade) {
                    log("GenericUpgradeRecipeShapeless", recipeName, recipeRL, getOutput().getItemStack(),
                            shapeless.getIngredients());
                    registry.register(
                            new ShapelessRecipe.Upgrade(recipeRL, shapeless.getIngredients(), getOutput().getThing()));
                } else {
                    log("ShapelessOreRecipe", recipeName, recipeRL, getOutput().getItemStack(),
                            shapeless.getIngredients());
                    registry.register(
                            new ShapelessRecipe(recipeRL, shapeless.getIngredients(), getOutput().getThing()));
                }
                if (getOutput().hasAlternatives()) {
                    getOutput().getAlternatives().apply(new Callback<ItemStack>() {
                        ...
                        public void apply(@Nonnull ItemStack alternative) {
                            Log.debug("Providing synthetic alternative recipe to JEI for oredicted output: " +
                                    alternative + ": " + shapeless.getIngredients());
                            JeiAccessor.addAlternativeRecipe(
                                    new ShapelessRecipes("", alternative, shapeless.getIngredients()));
                        }
                    });
                }
            }
            ...
            if (recipeLevel != RecipeLevel.IGNORE) {
                Log.warn("Ignoring recipe level " + recipeLevel + " configured for vanilla crafting recipe '" +
                        recipeName + "'---the vanilla crafting table doesn't have (or support) levels");
                // TODO: We could add the level to the crafting recipes for our Crafter. But unless we also get to show
                // that in JEI it would make thsi weird for
                // players. So I'm skipping this for now.
            }
        } else {
            Log.debug("Skipping Crafting '" + (getOutputs().isEmpty() ? "null" : getOutput().getItemStack()) +
