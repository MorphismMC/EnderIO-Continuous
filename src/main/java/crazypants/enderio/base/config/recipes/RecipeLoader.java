...
public final class RecipeLoader {
    ...
    public static void addRecipes() {
        ...
        /*
         * Note that we load the recipes in core-imc-user order but merge them in reverse order. The loading order
         * allows aliases to be added in the expected order,
    ...
    private static @Nonnull <T extends IRecipeRoot> T handleIMCRecipes(@Nonnull Class<T> target, @Nonnull T config) {
        try {
            ...
            for (Pair<String, Pair<IMCTYPE, String>> recipe : imcRecipes) {
                ...
                try {
                    T recipes = targets.containsKey(recipe.getKey()) ? targets.get(recipe.getKey()) :
                            target.newInstance();
                    ...
                    recipeError(NullHelper.first(e.getFilename(), "IMC from the mod '" + recipe.getKey() + "'"),
                            e.getMessage());
                } catch (IOException e) {
                    Log.error("IO error while parsing IMC:");
                    e.printStackTrace();
                    Log.error("IMC message:\n" + recipe.getValue().getValue());
                    recipeError("IMC from the mod '" + recipe.getKey() + "'",
                            "IO error while parsing string: " + e.getMessage());
                } catch (XMLStreamException e) {
                    Log.error("IMC has malformed XML:");
                    e.printStackTrace();
                    Log.error("IMC message:\n" + recipe.getValue().getValue());
                    recipeError("IMC from the mod '" + recipe.getKey() + "'",
                            "IMC has malformed XML: " + e.getMessage());
                }
            }
            ProgressManager.pop(bar);
            ...
            for (Entry<String, T> entry : targets.entrySet()) {
                ...
                try {
                    ...
                    Log.error("Invalid recipe while parsing IMC:");
                    e.printStackTrace();
                    recipeError(NullHelper.first(e.getFilename(), "IMC from the mod '" + entry.getKey() + "'"),
                            e.getMessage());
                }
            }
            ProgressManager.pop(bar);
    ...
    private static <T extends IRecipeRoot> T readUserFile(T target, final RecipeFactory recipeFactory, String filename,
                                                          File file) {
        try {
    ...
    private static <T extends IRecipeRoot> T readCoreFile(T target, final RecipeFactory recipeFactory,
                                                          String filename) {
        try {
