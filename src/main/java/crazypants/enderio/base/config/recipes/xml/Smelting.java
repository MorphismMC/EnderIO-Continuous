...
public class Smelting extends AbstractCrafting {
    ...
    public void register(@Nonnull String recipeName, @Nonnull RecipeLevel recipeLevel) {
        if (isValid() && isActive()) {
                ...
                input.get().getThing().getItemStacks().apply(new Callback<ItemStack>() {
                    ...
                    public void apply(@Nonnull ItemStack stack) {
                        ...
                        if (Prep.isValid(smeltingResult)) {
                            if (result.getItem() != smeltingResult.getItem() ||
                                    result.getCount() != smeltingResult.getCount()) {
                                Log.info("Cannot add smelting recipe " + recipeName + " (" + stack + " => " + result +
                                        ") because another mod already has registered a recipe " + stack + " => " +
                                        smeltingResult + ".");
                            } else {
                                Log.debug(
                                        "Smelting recipe " + recipeName + " (" + stack + " => " + result +
                                                ") is a real duplicate and will be ignored (XP may be different).");
                            }
                        } else {
                            GameRegistry.addSmelting(stack, result, exp);
                            if (recipeLevel != RecipeLevel.IGNORE) {
                                Log.warn("Ignoring recipe level " + recipeLevel +
                                        " configured for vanilla smelting recipe '" + recipeName +
            ...
            if (tinkers) {
                TicProxy.registerSmelterySmelting(input.get().getThing(), getOutput().getThing(),
                        1f / input.get().amount);
            }
        }
    }
