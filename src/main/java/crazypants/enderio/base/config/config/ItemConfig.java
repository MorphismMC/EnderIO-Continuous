...
public class ItemConfig {
    ...
    public static final IValue<Boolean> magnetAllowDeactivatedInBaublesSlot = MAG.make("allowDeactivatedInBaublesSlot",
            false, //
            "If true the magnet can be put into a Baubles slot even if switched off (requires Baubles to be installed and allowInBaublesSlot to be on).")
            .sync();
    public static final IValue<String> magnetBaublesType = MAG.make("baublesType", "AMULET", //
            ...
            "The BaublesType the magnet should be, 'AMULET', 'RING' or 'BELT' (requires Baubles to be installed and allowInBaublesSlot to be on).")
            .sync();
    ...
    public static final IValue<Integer> staffOfLevityDurationSeconds = STAFF.make("durationSeconds", 255, //
            "Duration for the levitation effect of the Staff of Levity. This is just a failsafe to prevent players from floating forever.")
            .setRange(1, 60 * 60 * 24)
            .sync();
    ...
    public static final IValue<Integer> rodOfReturnMinTicksToRecharge = ROD.make("ticksToRecharge", 100, //
            "Min number of ticks required to recharge the internal energy buffer of the Rod of Return.")
            .setRange(1, 20 * 60).sync();
    ...
    public static final IValue<Boolean> dpFree = DP.make("freeGraves", false, //
            "Determine if a Death Urn item is needed to create a grave. Don't forget to disable the item recipe when you enable this.")
            .sync();
    ...
    public static final IValue<Boolean> dpPrivate = DP.make("protectedGraves", true, //
            "Determine if a Death Urn can be looted by everone or only by the owner. Creative mode players can loot them always.")
            .sync();
}
