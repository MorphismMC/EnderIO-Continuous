...
public class ItemCapacitor extends Item implements IHaveRenderers {
        ...
        NNList.of(DefaultCapacitorData.class).apply(new Callback<DefaultCapacitorData>() {
            ...
            public void apply(@Nonnull DefaultCapacitorData alloy) {
                ModelLoader.setCustomModelResourceLocation(ItemCapacitor.this, alloy.ordinal(),
                        new ModelResourceLocation(modObject.getRegistryName(),
                                "variant=" + alloy.getUnlocalizedName()));
            }
        });
    }
    ...
    public @Nonnull String getTranslationKey(@Nonnull ItemStack stack) {
        return getTranslationKey() + "." +
                stack.getCapability(CapabilityCapacitorData.getCapNN(), null).getUnlocalizedName();
    }
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable World worldIn, @Nonnull List<String> tooltip,
        ...
        if (NbtValue.GLINT.hasTag(stack)) {
            tooltip.add(EnderIO.lang.localize("loot.capacitor.entry." + NbtValue.CAPNO.getInt(stack),
                    NbtValue.CAPNAME.getString(stack, "(!%$&�*&%*???")));
        }
    }
        ...
        ICapabilityProvider capProvider = new ICapabilityProvider() {
            ...
            public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
                if (capability == CapabilityCapacitorData.getCapNN()) {
                    return CapabilityCapacitorData.getCapNN().cast(NullHelper
                            .notnullJ(DefaultCapacitorData.values()[getMetadata(stack)], "Enum.values() has a null"));
                }
                return null;
            }
        };
    ...
    public @Nonnull EnumActionResult onItemUseFirst(@Nonnull EntityPlayer player, @Nonnull World world,
        ...
        if (te instanceof TileEntityChest) {
            ...
            LootTable loottable = world.getLootTableManager()
                    .getLootTableFromLocation(LootTableList.CHESTS_VILLAGE_BLACKSMITH);
            // LootTable loottable =
            // world.getLootTableManager().getLootTableFromLocation(LootTableList.CHESTS_SIMPLE_DUNGEON);
            // LootTable loottable =
            // world.getLootTableManager().getLootTableFromLocation(LootTableList.CHESTS_IGLOO_CHEST);
