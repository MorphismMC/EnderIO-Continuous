...
public class CapacitorKeyRegistry {
    ...
    public static void registerRegistry(@Nonnull RegistryEvent.NewRegistry event) {
        REGISTRY = new RegistryBuilder<ICapacitorKey>().setName(new ResourceLocation(EnderIO.DOMAIN, "capacitor"))
                .setType(ICapacitorKey.class)
                .setIDRange(0, Integer.MAX_VALUE - 1).create();
    }

    public static void setValue(@Nonnull ResourceLocation name, int baseValue, @Nullable Scaler scaler,
                                @Nonnull String scalerString) {
        ICapacitorKey key = REGISTRY.getValue(name);
        if (key == null) {
            throw new RuntimeException("Trying to configure non-existant key " + name);
        }
        Scaler theScaler = scaler != null ? scaler :
                NullHelper.notnull(ScalerFactory.fromString(scalerString), "Invalid scaler '", scalerString, "'");
            ...
            try {
                ...
                if (!RecipeConfig.loadCoreRecipes.get()) {
                    throw new UnconfiguredCapKeyException(
                            "Ender IO core recipe loading has been disabled in the configuration and you have failed to provide a user recipe for: " +
                                    key.getRegistryName(),
                            e);
                }
                throw e;
