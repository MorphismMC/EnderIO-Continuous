...
public interface IConduit {
    ...
    /**
     * Creates a single item for the conduit. Called when determining the Conduit's drops, when picking the block and
     * when determining if a held item is the same
     * as a placed conduit.
     *
    ...
    /**
     * see
     * {@link Block#getLightValue(net.minecraft.block.state.IBlockState, net.minecraft.world.IBlockAccess, net.minecraft.util.math.BlockPos)}
     * <p>
    ...
    /**
     * Sets the conduit's bundle. This will be called just before the conduit is placed into the list of conduits in the
     * bundle. It can be used to initialize
     * transient values, but should not access the world or other conduits of the bundle. It will also only ever be
     * called once.
     *
     * @param bundle
    ...
    /**
     * Conduit connections are connections to the same conduit type (see
     * {@link IServerConduit#canConnectToConduit(EnumFacing, IConduit)}) in neighboring conduit
    ...
    boolean onBlockActivated(@Nonnull EntityPlayer player, @Nonnull EnumHand hand, @Nonnull RaytraceResult res,
