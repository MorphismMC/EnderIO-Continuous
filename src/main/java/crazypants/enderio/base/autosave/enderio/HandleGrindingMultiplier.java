...
public class HandleGrindingMultiplier implements IHandler<IGrindingMultiplier> {
    ...
    public boolean store(@Nonnull Registry registry, @Nonnull Set<NBTAction> phase, @Nonnull NBTTagCompound nbt,
                         ...
                         @Nonnull IGrindingMultiplier object) throws IllegalArgumentException, IllegalAccessException,
    ...
    @Override
    @Nullable
    public IGrindingMultiplier read(@Nonnull Registry registry, @Nonnull Set<NBTAction> phase,
        ...
        if (nbt.hasKey(name)) {
            ...
            if (tag.hasKey(CM) && tag.hasKey(PM) && tag.hasKey(GM) && tag.hasKey(DMJ)) {
                return new GrindingMultiplierNBT(tag.getFloat(CM), tag.getFloat(PM), tag.getFloat(GM),
                        tag.getInteger(DMJ));
            }
        }
        return null;
