...
public final class ItemTools {
    ...
    public static MoveResult move(@Nonnull Limit limit, @Nonnull IBlockAccess world, @Nonnull BlockPos sourcePos,
                                  @Nonnull EnumFacing sourceFacing,
    ...
    public static MoveResult move(@Nonnull Limit limit, @Nonnull IBlockAccess world, @Nonnull TileEntity sourceTE,
                                  @Nonnull EnumFacing sourceFacing,
                                  @Nonnull BlockPos targetPos, @Nonnull EnumFacing targetFacing) {
    ...
    private static MoveResult move(@Nonnull Limit limit, @Nonnull IBlockAccess world, @Nullable TileEntity sourceTE,
                                   @Nonnull BlockPos sourcePos,
                                   @Nonnull EnumFacing sourceFacing, @Nullable TileEntity targetTE,
                                   @Nonnull BlockPos targetPos, @Nonnull EnumFacing targetFacing) {
        if (!limit.canWork()) {
                                ...
                                if (movable > 0) {
                                    ...
                                    if (Prep.isValid(removed)) {
                                        ...
                                        if (Prep.isValid(targetRejected)) {
                                            ItemStack sourceRejected = insertItemStacked(sourceHandler, targetRejected,
                                                    false);
                                            if (Prep.isValid(sourceRejected)) {
                                                EntityItem drop = new EntityItem(source.getWorld(),
                                                        sourcePos.getX() + 0.5, sourcePos.getY() + 0.5,
                                                        sourcePos.getZ() + 0.5,
                                                        sourceRejected);
    ...
    public static boolean hasFreeSpace(@Nonnull IItemHandler handler) {
        for (int i = 0; i < handler.getSlots(); i++) {
            ...
            if (Prep.isInvalid(stack) || (stack.isStackable() && stack.getCount() < stack.getMaxStackSize() &&
                    stack.getCount() < handler.getSlotLimit(i))) {
                return true;
            }
        }
        return false;
    ...
    public static @Nullable IItemHandler getExternalInventory(@Nonnull IBlockAccess world, @Nonnull BlockPos pos,
    ...
    public static @Nullable IItemHandler getExternalInventory(@Nonnull TileEntity tile, @Nonnull EnumFacing face) {
        return tile.getCapability(NullHelper.notnullF(ITEM_HANDLER_CAPABILITY, "Capability<IItemHandler> is missing"),
                face);
    }

    @Nonnull
    public static ItemStack insertItemStacked(@Nonnull IItemHandler inventory, @Nonnull ItemStack stack,
                                              boolean simulate) {
        if (Prep.isValid(stack)) {
            ...
            for (int i = 0; i < sizeInventory; i++) {
                ...
                if (ItemHandlerHelper.canItemStacksStackRelaxed(slot, stack)) {
                    ...
                    if ((simulate && stack.getCount() != origSize) || Prep.isInvalid(stack)) {
                        // stack has been completely inserted, or we are are simulating and have a partial insert. As
                        // inventories may change their acceptance rules after a
