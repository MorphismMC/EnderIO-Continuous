    ...
    public static class Handler implements IMessageHandler<PacketAddRemoveChannel, IMessage> {
        ...
        public IMessage onMessage(PacketAddRemoveChannel message, MessageContext ctx) {
            if (ctx.side == Side.SERVER) {
                if (!message.channel.getUser().equals(UserIdent.NOBODY) &&
                        !message.channel.getUser().equals(ctx.getServerHandler().player.getGameProfile())) {
                    ctx.getServerHandler().player.connection
                            .disconnect(new TextComponentString("Don't mess with other players' channels, you cheat."));
                    return null;
                }
                if (message.isAdd) {
