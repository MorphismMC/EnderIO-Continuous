...
public abstract class AbstractMachineContainer<E extends AbstractInventoryMachineEntity>
                                              extends ContainerEnder<InventoryWrapper> {
    ...
    protected void addSlots(@Nonnull InventoryPlayer playerInv) {
        ...
        if (getInv().getOwner().getSlotDefinition().getNumUpgradeSlots() == 1) {
            addSlotToContainer(
                    upgradeSlot = new Slot(getInv(), getInv().getOwner().getSlotDefinition().getMinUpgradeSlot(),
                            getUpgradeOffset().x, getUpgradeOffset().y) {
        ...
        if (updatePacket != null) {
            for (IContainerListener containerListener : listeners) {
                if (containerListener instanceof EntityPlayerMP &&
                        doesClientSeeTileEntity((EntityPlayerMP) containerListener)) {
