...
public abstract class GuiMachineBase<T extends AbstractInventoryMachineEntity> extends GuiContainerBaseEIO<T> {
    ...
    public void renderSlotHighlights(@Nonnull IoMode mode) {
        ...
        for (Slot invSlot : inventorySlots.inventorySlots) { // this is a bit hacky, we need a better way for cap-based
                                                             // machines
            if (invSlot.inventory == getTileEntity() || (inventorySlots instanceof ContainerEnder &&
                    invSlot.inventory == ((ContainerEnder<?>) inventorySlots).getInv())) {
