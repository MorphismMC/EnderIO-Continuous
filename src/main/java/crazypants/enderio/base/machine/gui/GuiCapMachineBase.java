...
public class GuiCapMachineBase<T extends AbstractCapabilityMachineEntity> extends GuiContainerBaseEIO<T> {
    ...
    protected GuiCapMachineBase(@Nonnull T machine, @Nonnull ContainerEnderCap<EnderInventory, T> par1Container,
                                @Nonnull String... guiTexture) {
    ...
    protected void renderSplitHighlight(int x, int y, int widthIn, int heightIn) {
        ...
        RenderUtil.renderQuad2D(getGuiLeft() + x, getGuiTop() + y + (heightIn / 2), 0, widthIn, heightIn / 2,
                PUSH_COLOR);
        GlStateManager.disableBlend();
    }
