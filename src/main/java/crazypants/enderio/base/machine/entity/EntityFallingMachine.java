...
public class EntityFallingMachine extends EntityFallingBlock {
    ...
    public static void onEntityRegister(Register<EntityEntry> event) {
        EntityRegistry.registerModEntity(new ResourceLocation(EnderIO.DOMAIN, "falling_machine"),
                EntityFallingMachine.class, EnderIO.DOMAIN + ".falling_machine",
    ...
    // itemstack with the nbt of the dropped machine. super only creates dumb itemstack without nbt, even if we have TE
    // nbt...
    private static final @Nonnull DataParameter<ItemStack> DROP_STACK = EntityDataManager.<ItemStack>createKey(
            EntityFallingMachine.class,
            DataSerializers.ITEM_STACK);

    // client-only. blockstate to render. The client will find this itself if the client-entity has a chance to tick
    // before the original block is removed by the
    // server-entity's tick
    protected IBlockState renderBS = null;

    public EntityFallingMachine(World worldIn) {
        // super always needs a block so update() doesn't NPE. EntityFallingBlock has a special spawning network package
        // that calls the other constructor, but we
        // don't get that luxury
        super(worldIn, 0, 0, 0, Blocks.BLUE_GLAZED_TERRACOTTA.getDefaultState());
    }

    public <T extends AbstractMachineEntity> EntityFallingMachine(@Nonnull WorldServer worldIn, @Nonnull BlockPos pos,
                                                                  @Nonnull Block block) {
        ...
        if (block instanceof AbstractMachineBlock) {
            ...
            final T te = AbstractMachineBlock.getAnyTileEntity(worldIn, pos,
                    ((AbstractMachineBlock<T>) block).getTeClass());
            if (te != null) {
                tileEntityData = te.writeToNBT(new NBTTagCompound());
            }
            @SuppressWarnings("unchecked")
            final ItemStack nbtDrop = ((AbstractMachineBlock<T>) block).getNBTDrop(worldIn, pos,
                    worldIn.getBlockState(pos), 0, te);
            if (nbtDrop != null) {
            ...
            setDropStack(block.getPickBlock(worldIn.getBlockState(pos),
                    new RayTraceResult(new Vec3d(0, 0, 0), EnumFacing.DOWN, pos), worldIn, pos,
    ...
    public void onUpdate() {
        if (world.isRemote && renderBS == null) {
            ...
            if (blockState1.equals(blockState2) && blockState1.getBlock().hasTileEntity(blockState1)) {
                // This is the best way to render---this extended blockstate contains a nice model with everything. But
                // this only works before the original block is
                // removed. Usually we are fast enough to get this information, but often enough it fails. See below for
                // 2 fallback solutions.
                renderBS = blockState1.getBlock().getExtendedState(
                        blockState1.getBlock().getActualState(world.getBlockState(getOrigin()), world, getOrigin()),
                        world,
                        getOrigin());
            }
        }
                        ...
                        if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION) {
                            ...
                            if (this.world.mayPlace(block, blockpos1, true, EnumFacing.UP, this) &&
                                ...
                                if (this.tileEntityData != null && block.hasTileEntity(fallTile)) {
                                    ...
                                    if (tileentity != null) {
                                        // HL: Original doesn't work well with AutoSave as we use multiple keys for one
                                        // value
