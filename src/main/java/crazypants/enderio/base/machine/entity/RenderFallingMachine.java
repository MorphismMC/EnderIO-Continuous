...
public class RenderFallingMachine extends Render<EntityFallingMachine> {
    ...
    @Override
    public void doRender(@Nonnull EntityFallingMachine entity, double x, double y, double z, float entityYaw,
                         float partialTicks) {
        IBlockState iblockstate = entity.getBlock();
        IBlockState extendedState = iblockstate;
        if (iblockstate instanceof IBlockStateWrapper) {
            extendedState = iblockstate;
            // only reason to copy this whole class: we need to dig out the real blockstate from the wrapper so
            // blockrendererdispatcher.getModelForState() below
            // will work
        ...
        if (extendedState != null && iblockstate != null) {
            if (iblockstate.getRenderType() == EnumBlockRenderType.MODEL) {
                ...
                if (iblockstate != world.getBlockState(new BlockPos(entity)) &&
                    ...
                    GlStateManager.translate((float) (x - blockpos.getX() - 0.5D), (float) (y - blockpos.getY()),
                    ...
                    blockrendererdispatcher.getBlockModelRenderer().renderModel(world,
                            blockrendererdispatcher.getModelForState(iblockstate), extendedState, blockpos,
