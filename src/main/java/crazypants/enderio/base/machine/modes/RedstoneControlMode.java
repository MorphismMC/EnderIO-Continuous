...
public enum RedstoneControlMode {
    ...
    /**
     * This is the highest level that will make a difference. Meaning, that if it is known that the input level is at
     * least MIN_ON_LEVEL, then there is no need to
    ...
    public static boolean isConditionMet(@Nonnull RedstoneControlMode redstoneControlMode, TileEntity te) {
        switch (redstoneControlMode) {
            ...
            case NEVER:
                return false;
            default:
                return isConditionMet(redstoneControlMode,
                        ConduitUtil.isBlockIndirectlyGettingPoweredIfLoaded(te.getWorld(), te.getPos()));
        }
    }
