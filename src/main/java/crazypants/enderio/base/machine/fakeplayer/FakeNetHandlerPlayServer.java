...
public class FakeNetHandlerPlayServer extends NetHandlerPlayServer {
    ...
    public FakeNetHandlerPlayServer(EntityPlayerMP p_i1530_3_) {
        super(FMLCommonHandler.instance().getMinecraftServerInstance(),
                new NetworkManager(EnumPacketDirection.CLIENTBOUND), p_i1530_3_);
    }
    ...
    public @Nonnull NetworkManager getNetworkManager() {
        if (warnCount++ < 10) {
            Log.warn(
                    "Someone is trying to send network packets to a fake player. This may crash and that is NOT Ender IO's fault.");
        }
    ...
    public void setPlayerLocation(double x, double y, double z, float yaw, float pitch,
                                  @Nonnull Set<EnumFlags> relativeSet) {}
