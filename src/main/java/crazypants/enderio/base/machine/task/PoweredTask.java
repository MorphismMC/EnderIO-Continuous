...
public class PoweredTask implements IPoweredTask {
    ...
    protected PoweredTask(@Nonnull IMachineRecipe recipe, float usedEnergy, long nextSeed, float outputMultiplier,
    ...
    public @Nonnull ResultStack[] getCompletedResult() {
        ...
        result.addAll(
                recipe.getCompletedResult(rand.nextLong(), getBonusType().doChances() ? chanceMultiplier : 1f, inputs));
        if (getBonusType().doMultiply()) {
            float mul = outputMultiplier - 1f;
            while (mul > 0) {
                if (rand.nextFloat() < mul) {
                    result.addAll(recipe.getCompletedResult(rand.nextLong(),
                            getBonusType().doChances() ? chanceMultiplier : 1f, inputs));
                }
                mul--;
            }
        }
    ...
    public static @Nullable IPoweredTask readFromNBT(@Nonnull NBTTagCompound nbtRoot) {
        ...
        if (recipe != null) {
            // TODO: Check if it is harmful if the recipe changed its input items in the meantime. Do we use the items
            // we got from our nbt when the task is complete?
            // If not, why do we store them?
            final PoweredTask poweredTask = new PoweredTask(recipe, usedEnergy, seed, outputMultiplier,
                    chanceMultiplier, ins);
            if (hasCustomEnergyCost) {
