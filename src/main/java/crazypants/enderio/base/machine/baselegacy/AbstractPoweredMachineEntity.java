...
@Storable
public abstract class AbstractPoweredMachineEntity extends AbstractInventoryMachineEntity
                                                   implements ILegacyPoweredTile, IPowerBarData {
    ...
    public void doUpdate() {
        ...
        if ((lastSyncPowerStored != scaledPower &&
                (lastSyncPowerStored == 0 || scaledPower == 0 || shouldDoWorkThisTick(20)))) {
    ...
    public void setEnergyStored(int stored) {
        // Don't clamp this on the client, as the client may not know about the capacitor (inventory data is only synced
        // while the GUI is open, while energy is
    ...
    public int getEnergyStoredScaled(int scale) {
        ...
        return maxEnergyStored2 == 0 ? 0 :
                VecmathUtil.clamp(Math.round(scale * ((float) storedEnergyRF / maxEnergyStored2)), 0, scale);
    }
    ...
    private void updateCapacitorFromSlot() {
        if (slotDefinition.getNumUpgradeSlots() <= 0) {
            ...
            final ICapacitorData capacitorDataFromItemStack = NullHelper
                    .first(CapacitorHelper.getCapacitorDataFromItemStack(NullHelper.first(stack, ItemStack.EMPTY)),
