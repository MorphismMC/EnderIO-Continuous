...
public abstract class AbstractGeneratorEntity extends AbstractPoweredMachineEntity implements ILegacyPoweredTile {
    ...
    protected AbstractGeneratorEntity(@Nonnull SlotDefinition slotDefinition, @Nonnull ICapacitorKey maxEnergyStored,
                                      @Nonnull ICapacitorKey maxEnergyUsed) {
        this(slotDefinition, NO_POWER, maxEnergyStored, maxEnergyUsed);
    }

    protected AbstractGeneratorEntity(@Nonnull SlotDefinition slotDefinition, @Nonnull ICapacitorKey maxEnergySent,
    ...
    /**
     * Determines how much power a generator may push out to all its neighbors per tick. This does take into account the
     * configured value and the available amount
     * of energy.
     * <p>
     * Note that this is not sided as the PowerDistributor only has a single amount parameter and I didn't want to
     * change to much when I added this method.
     * --Henry
     * 
     * @return a positive integer
     */
    public int getMaxEnergySent() {
        return Math.min(getEnergyStored(),
                maxEnergyRecieved != NO_POWER ? maxEnergyRecieved.get(getCapacitorData()) : (getPowerUsePerTick() * 2));
    }
}
