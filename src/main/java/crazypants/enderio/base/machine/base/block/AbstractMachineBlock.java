...
public abstract class AbstractMachineBlock<T extends AbstractMachineEntity> extends BlockEio<T>
                                          implements IEioGuiHandler.WithPos, IResourceTooltipProvider,
                                          ISmartRenderAwareBlock, IClearableConfiguration {
    ...
    public final @Nonnull IBlockState getExtendedState(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
                                                       @Nonnull BlockPos pos) {
    ...
    protected abstract void setBlockStateWrapperCache(@Nonnull IBlockStateWrapper blockStateWrapper,
                                                      @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                                                      @Nonnull T tileEntity);

    protected @Nonnull BlockStateWrapperBase createBlockStateWrapper(@Nonnull IBlockState state,
    ...
    public void onBlockPlaced(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
                              @Nonnull EntityLivingBase player, @Nonnull T te) {
        ...
        if (isEnhanced && block != null) {
            if (worldIn.getBlockState(pos.up()).getBlock() != block) {
                if (super.canPlaceBlockAt(worldIn, pos.up())) {
                    ...
                    // impossible error state a.k.a. someone ripped the machine apart. And what do machines that are
                    // ripped apart do? They explode. Violently.
                    // TODO: very soon: re-add
                    // worldIn.createExplosion(null, pos.getX() + .5, pos.getY() + .5, pos.getZ() + .5, 3f, true); // 3
                    // == normal Creeper
    ...
    public void randomDisplayTick(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        if (PersonalConfig.machineParticlesEnabled.get() && isActive(world, pos)) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, startX + xOffset, startY + yOffset,
    ...
    public abstract @Nullable Container getServerGuiElement(@Nonnull EntityPlayer player, @Nonnull World world,
                                                            @Nonnull BlockPos pos,
                                                            @Nullable EnumFacing facing, int param1, @Nonnull T te);

    @SideOnly(Side.CLIENT)
    public abstract @Nullable GuiScreen getClientGuiElement(@Nonnull EntityPlayer player, @Nonnull World world,
                                                            @Nonnull BlockPos pos,
                                                            @Nullable EnumFacing facing, int param1, @Nonnull T te);

    protected final @Nonnull IShape<T> mkShape(@Nonnull BlockFaceShape down, @Nonnull BlockFaceShape up,
                                               @Nonnull BlockFaceShape front,
    ...
    public boolean canPlaceBlockAt(@Nonnull World world, @Nonnull BlockPos pos) {
        return super.canPlaceBlockAt(world, pos) &&
                (!isEnhanced || (pos.getY() < 255 && super.canPlaceBlockAt(world, pos.up())));
    }
