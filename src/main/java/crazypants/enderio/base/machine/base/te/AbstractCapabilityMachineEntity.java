...
public abstract class AbstractCapabilityMachineEntity extends AbstractMachineEntity {
    ...
    protected AbstractCapabilityMachineEntity(@Nonnull ICapacitorKey maxEnergyRecieved,
    ...
    protected AbstractCapabilityMachineEntity(@Nullable EnderInventory subclassInventory,
                                              ...
                                              @Nonnull ICapacitorKey maxEnergyStored,
                                              @Nonnull ICapacitorKey maxEnergyUsed) {
        this(() -> subclassInventory, owner -> new EnergyLogic(owner,
                new EnergyTank(owner, maxEnergyRecieved, maxEnergyStored, maxEnergyUsed)));
    }
    ...
    protected AbstractCapabilityMachineEntity(@Nonnull Supplier<EnderInventory> inventorySupplier,
        ...
        addICap(ITEM_HANDLER_CAPABILITY,
                ICap.facedOnly(facingIn -> getIoMode(facingIn).canInputOrOutput() ? new Side(facingIn) : null));
    }
    ...
    protected boolean doPush(@Nullable EnumFacing dir) {
        ...
        return ItemTools.move(getPushLimit(), world, this, dir, getPos().offset(dir), dir.getOpposite()) ==
                MoveResult.MOVED;
    }
    ...
    protected boolean doPull(@Nullable EnumFacing dir) {
        ...
        return ItemTools.move(getPullLimit(), world, getPos().offset(dir), dir.getOpposite(), this, dir) ==
                MoveResult.MOVED;
    }
    ...
    protected boolean mergeOutput(@Nonnull ItemStack stack) {
        ...
        for (InventorySlot slot : outputSlots) {
            ...
            if (oldOutput.isEmpty()) {
                ...
                oldOutput.grow(
                        stack.splitStack(Math.min(oldOutput.getMaxStackSize() - oldOutput.getCount(), stack.getCount()))
                                .getCount());
                slot.set(oldOutput);
    ...
    /**
     * 
     * @return a capacitor data holder. Note that "no capacitor in the slot" also has a valid data holder. And for
     *         machines without capacitor slot, that's the
    ...
    protected void onAfterNbtRead() {
        ...
        getEnergyLogic().updateCapacitorFromSlot(); // TODO trace out if we need this or if the inventory calls
                                                    // onChange() for this
    }
