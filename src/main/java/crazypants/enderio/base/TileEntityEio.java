...
public abstract class TileEntityEio extends TileEntityBase {
    ...
    /**
     * The block is processed by {@link Block#getPickBlock(IBlockState, RayTraceResult, World, BlockPos, EntityPlayer)}
     * but not in "copy" mode. Add stuff that
     * belongs to the block's identity but not its state.
     */
    protected @Nonnull ItemStack processPickBlock(@Nonnull RayTraceResult target, @Nonnull EntityPlayer player,
