...
public class BlockInfinityFog extends BlockEio<TileEntityEio> implements IDefaultRenderers, IResourceTooltipProvider {
    ...
    @Override
    public int getWeakPower(@Nonnull IBlockState state, @Nonnull IBlockAccess blockAccess, @Nonnull BlockPos pos,
        ...
        switch (rnd.nextInt(10)) {
            ...
            case 8:
                if (rnd.nextFloat() < .025f) {
                    @SuppressWarnings("null")
                    EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(world, pos.getX() + 0.5F, pos.getY(),
                            pos.getZ() + 0.5F, null);
                    world.spawnEntity(entitytntprimed);
                    world.playSound((EntityPlayer) null, entitytntprimed.posX, entitytntprimed.posY,
                            entitytntprimed.posZ, SoundEvents.ENTITY_TNT_PRIMED,
        ...
        if (capturedMob != null) {
            NNList.SHELL.apply(new NNList.ShortCallback<BlockPos>() {
                ...
                public boolean apply(@Nonnull BlockPos e) {
                    ...
                    return spawn(pos.up()) || spawn(pos) || spawn(pos.down()) || spawn(pos.up().up()) ||
                            spawn(pos.up().down().down());
                }
        ...
        if (!worldIn.isRemote) {
            ...
            if (rand.nextFloat() < .05f && entityIn instanceof EntityLivingBase) {
                ((EntityLivingBase) entityIn)
                        .addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, 20, 0, true, true));
            }
            if (rand.nextFloat() < .05f && entityIn instanceof EntityLivingBase) {
            ...
            if (rand.nextFloat() < .33f && entityIn instanceof EntityLivingBase) {
                ((EntityLivingBase) entityIn)
                        .addPotionEffect(new PotionEffect(MobEffects.LEVITATION, 20, 0, true, true));
            }
        }
    }
    ...
    public void randomDisplayTick(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        for (int i = 0; i < count; i++) {
            ...
            float maxSize = Math.min(Math.min(Math.min(1f - offsetX, offsetX), Math.min(1f - offsetY, offsetY)),
                    Math.min(1f - offsetZ, offsetZ)) * (.5f + .5f * rnd.nextFloat()) * 2;
            float color = (i == 0 && !bs.getValue(HARMLESS) && count > 1) ? 0 : rnd.nextFloat();
            Minecraft.getMinecraft().effectRenderer
                    .addEffect(new InfinityParticle(world, pos, new Vector4f(color, color, color, 0.4f),
