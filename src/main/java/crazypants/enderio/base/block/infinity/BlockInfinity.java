...
public class BlockInfinity extends BlockFalling
                           implements IResourceTooltipProvider, IHaveRenderers, IModObject.WithBlockItem {
    ...
    @Override
    public @Nonnull BlockFaceShape getBlockFaceShape(@Nonnull IBlockAccess worldIn, @Nonnull IBlockState state,
    ...
    private static final int[] COLS = { 0x010101, 0x111111, 0x222222, 0x333333, 0xFFFFFF }; // red==0 is forced to 255
                                                                                            // by the particle
    ...
    private boolean breakup(@Nonnull IBlockState state, @Nonnull World worldIn, @Nonnull BlockPos pos,
                            float baseChance) {
        if (!worldIn.isRemote && RANDOM.nextFloat() < (state.getValue(LEVEL) + 1) * baseChance) {
            ...
            while (count > 0) {
                ...
                spawnAsEntity(worldIn, pos,
                        crazypants.enderio.base.material.material.Material.POWDER_INFINITY.getStack(num));
            }
    ...
    @Override
    public void onFallenUpon(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull Entity entityIn,
                             float fallDistance) {
    ...
    private boolean tryTouchWater(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull IBlockState state) {
        for (EnumFacing enumfacing : EnumFacing.values()) {
            if (enumfacing != EnumFacing.DOWN && enumfacing != null &&
