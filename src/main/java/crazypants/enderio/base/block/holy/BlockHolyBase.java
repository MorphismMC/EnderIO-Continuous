...
public abstract class BlockHolyBase extends BlockEio<TileEntityEio>
                                    implements IDefaultRenderers, IResourceTooltipProvider {
    ...
    protected static void makeParticle(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull Random rnd) {
        ...
        float maxSize = Math.min(Math.min(Math.min(1f - offsetX, offsetX), Math.min(1f - offsetY, offsetY)),
                Math.min(1f - offsetZ, offsetZ)) * (.5f + .5f * rnd.nextFloat()) / 4f;
        Minecraft.getMinecraft().effectRenderer
                .addEffect(new InfinityParticle(world, pos, new Vector4f(0xFD / 255f, 1, 0, 0.4f),
                        new Vector4f(offsetX, offsetY, offsetZ, maxSize)));
    }

    public BlockHolyBase(@Nonnull IModObject modObject) {
        super(modObject, new MaterialTransparent(MapColor.AIR)); // not Material.AIR because AIR doesn't get update
                                                                 // ticks
        setCreativeTab(EnderIOTab.tabEnderIOMaterials);
        // volume -1 gives effective volume of 0 when used by ItemBlock
        setSoundType(new SoundType(-1.0F, 1.0F, SoundEvents.BLOCK_CLOTH_BREAK, SoundEvents.BLOCK_CLOTH_STEP,
    ...
    @Override
    @Nullable
    public AxisAlignedBB getCollisionBoundingBox(@Nonnull IBlockState blockStateIn, @Nonnull IBlockAccess worldIn,
    ...
    @Override
    public void dropBlockAsItemWithChance(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull IBlockState state,
    ...
    @Override
    public void onBlockPlaced(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
    ...
    @Override
    public void updateTick(final @Nonnull World world, final @Nonnull BlockPos pos, final @Nonnull IBlockState state,
        ...
        if (!world.isRemote) {
            ...
            if (downQuanta > 0 && downQuanta < getMaxQuanta()) {
                ...
                if (downQuanta <= getMaxQuanta()) {
                    ...
                    setQuanta(world, pos, myQuanta, 6); // give the down block a chance to flow away before we flow
                                                        // again
                    setQuanta(world, pos.down(), downQuanta, 5);
                }
                countPerTick++;
    ...
    public void onEntityCollision(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
        ...
        if (!world.isRemote && entityIn instanceof EntityLivingBase &&
                ((EntityLivingBase) entityIn).getCreatureAttribute() == EnumCreatureAttribute.UNDEAD &&
    ...
    @SideOnly(Side.CLIENT)
    @Override
    public void randomDisplayTick(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos,
                                  @Nonnull Random random) {
        final int quanta = getQuanta(world, pos, bs) +
                (Minecraft.getMinecraft().player.getHeldItemMainhand().getItem() ==
                        ModObjectRegistry.getItem(ModObjectRegistry.getModObjectNN(this)) ? 20 : 0);
