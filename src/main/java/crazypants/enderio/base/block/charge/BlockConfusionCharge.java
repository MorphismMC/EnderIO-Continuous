...
public class BlockConfusionCharge extends BlockTNT implements ICharge, IDefaultRenderers, IModObject.WithBlockItem {
    ...
    public void explode(@Nonnull EntityPrimedCharge entity) {
        ...
        world.playSound((EntityPlayer) null, entity.posX, entity.posY, entity.posZ, SoundEvents.ENTITY_TNT_PRIMED,
                SoundCategory.BLOCKS, 3F,
                1.4f + ((world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F));

        PacketHandler.INSTANCE.sendToAllAround(new PacketExplodeEffect(entity, this), new BlockPos(entity),
                entity.world);
    }

    @Override
    public void explode(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state,
                        @Nonnull EntityLivingBase igniter) {
        if (!world.isRemote) {
            if (state.getValue(EXPLODE).booleanValue()) {
                EntityPrimedCharge entity = new EntityPrimedCharge(this, world, pos.getX() + 0.5F, pos.getY() + 0.5F,
    ...
    public void explodeEffect(@Nonnull World world, double x, double y, double z) {
        List<EntityPlayer> players = world.getEntitiesWithinAABB(EntityPlayer.class,
                getBoundsAround(x, y, z, BlockConfig.confusingChargeRange.get()));
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            ParticleSpell entityfx = (ParticleSpell) new ParticleSpell.InstantFactory().createParticle(i, world,
                    x + motionX * 0.1, y + motionY * 0.1,
