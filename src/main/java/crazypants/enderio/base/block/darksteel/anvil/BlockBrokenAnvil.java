...
public class BlockBrokenAnvil extends BlockFalling {
    ...
    protected static final @Nonnull AxisAlignedBB SHAPE = new AxisAlignedBB(1 * px, 0 * px, 1 * px, 15 * px, 4.5 * px,
            15 * px);
    ...
    public void getDrops(@Nonnull NonNullList<ItemStack> drops, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
        ...
        float min = MathHelper.clamp(Math.min(BlockConfig.dsaMinDrop.get(), BlockConfig.dsaMaxDrop.get()), 0, 1); // don't
                                                                                                                  // trust
                                                                                                                  // users
                                                                                                                  // to
        float max = MathHelper.clamp(Math.max(BlockConfig.dsaMinDrop.get(), BlockConfig.dsaMaxDrop.get()), 0, 1); // give
                                                                                                                  // us
                                                                                                                  // valid
                                                                                                                  // values
        ...
        while (blocks > 0) {
            drops.add(Alloy.DARK_STEEL
                    .getStackBlock(Math.min(blocks, Alloy.DARK_STEEL.getStackBlock().getMaxStackSize())));
            blocks -= Alloy.DARK_STEEL.getStackBlock().getMaxStackSize();
        }
        while (ingots > 0) {
            drops.add(Alloy.DARK_STEEL
                    .getStackIngot(Math.min(ingots, Alloy.DARK_STEEL.getStackIngot().getMaxStackSize())));
            ingots -= Alloy.DARK_STEEL.getStackIngot().getMaxStackSize();
        }
        while (nuggets > 0) {
            drops.add(Alloy.DARK_STEEL
                    .getStackNugget(Math.min(nuggets, Alloy.DARK_STEEL.getStackNugget().getMaxStackSize())));
