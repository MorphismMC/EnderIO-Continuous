...
public class BlockPaintedCarpet extends BlockCarpet implements ITileEntityProvider, IPaintable.ITexturePaintableBlock,
    ...
    private void init(@Nonnull IModObject modObject) {
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new BasicPainterTemplate<BlockPaintedCarpet>(this, Blocks.CARPET));
        SmartModelAttacher.registerNoProps(this);
        PaintRegistry.registerModel(MODEL_CARPET, new ResourceLocation("minecraft", "block/carpet"),
    ...
    public List<IBakedModel> mapItemRender(@Nonnull Block block, @Nonnull ItemStack stack) {
        ...
        IBakedModel model2 = PaintRegistry.getModel(IBakedModel.class, MODEL_CARPET, stdOverlay,
    ...
    @Override
    public boolean shouldSideBeRendered(@Nonnull IBlockState state, @Nonnull IBlockAccess worldIn,
                                        @Nonnull BlockPos pos, @Nonnull EnumFacing side) {
        return super.shouldSideBeRendered(state, worldIn, pos, side) && !(side.getAxis() != EnumFacing.Axis.Y &&
                worldIn.getBlockState(pos.offset(side)).getBlock() instanceof BlockCarpet);
    }
