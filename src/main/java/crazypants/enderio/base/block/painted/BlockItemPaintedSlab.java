...
public class BlockItemPaintedSlab extends ItemBlock {
    ...
    @Override
    public @Nonnull EnumActionResult onItemUse(@Nonnull EntityPlayer playerIn, @Nonnull World worldIn,
        ...
        if (!playerIn.canPlayerEdit(pos.offset(side), side, stack)) {
            ...
            return this.tryPlace(stack, worldIn, pos.offset(side)) ? EnumActionResult.SUCCESS :
                    super.onItemUse(playerIn, worldIn, pos, hand, side, hitX, hitY, hitZ);
        }
    }
    ...
    private boolean tryPlace(@Nonnull ItemStack stack, @Nonnull World worldIn, @Nonnull BlockPos pos) {
        ...
        if (iblockstate.getBlock() == this.singleSlab && singleSlab != doubleSlab) {
            ...
            if ((collisionBoundingBox == null || worldIn.checkNoEntityCollision(collisionBoundingBox)) &&
