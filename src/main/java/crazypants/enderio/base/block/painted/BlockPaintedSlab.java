...
public abstract class BlockPaintedSlab extends BlockSlab implements ITileEntityProvider,
                                       IPaintable.ITexturePaintableBlock, ISmartRenderAwareBlock,
    ...
    public static class BlockPaintedHalfSlab extends BlockPaintedSlab {
        ...
        public BlockPaintedHalfSlab(@Nonnull IModObject modObject, @Nonnull Material material,
    ...
    public static class BlockPaintedDoubleSlab extends BlockPaintedSlab {
        ...
        public BlockPaintedDoubleSlab(@Nonnull IModObject modObject, @Nonnull Material material, Block halfVariant,
                                      @Nonnull SoundType sound) {
    ...
    protected void init(@Nonnull IModObject modObject) {
        ...
        PaintRegistry.registerModel("slab_lo", new ResourceLocation("minecraft", "block/half_slab_stone"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
        PaintRegistry.registerModel("slab_hi", new ResourceLocation("minecraft", "block/upper_slab_stone"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
    }
    ...
    public void getDrops(@Nonnull NonNullList<ItemStack> drops, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
        ...
        for (ItemStack drop : drops2) {
            if (first || !isDouble()) {
                ...
                PaintUtil.setSourceBlock(NullHelper.notnullM(drop, "null stack from getDrops()"),
                        getPaintSource2(state, world, pos));
            }
        }
        drops.addAll(drops2);
    }
    ...
    @Override
    public @Nonnull ItemStack getPickBlock(@Nonnull IBlockState bs, @Nonnull RayTraceResult target,
    ...
    public void setPaintSource2(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
    ...
    public @Nonnull IBlockState getExtendedState(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
        ...
        if (isDouble()) {
            blockStateWrapper.addCacheKey(getPaintSource(state, world, pos))
                    .addCacheKey(getPaintSource2(state, world, pos));
        } else {
    ...
    public List<IBakedModel> mapItemRender(@Nonnull Block block, @Nonnull ItemStack stack) {
        ...
        IBakedModel model2 = PaintRegistry.getModel(IBakedModel.class, "slab_lo", stdOverlay,
                PaintRegistry.OVERLAY_TRANSFORMATION3);
    ...
    @Override
    public boolean doesSideBlockRendering(@Nonnull IBlockState bs, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                                          @Nonnull EnumFacing face) {
        return false;
    }

    @Override
    public boolean shouldSideBeRendered(@Nonnull IBlockState ourBlockState, @Nonnull IBlockAccess worldIn,
                                        @Nonnull BlockPos here, @Nonnull EnumFacing side) {
        ...
        if (block2 instanceof BlockPaintedSlab) {
            ...
            if (side == EnumFacing.UP) {
                ...
                IBlockState ourPaint = isDouble() ? getPaintSource2(ourBlockState, worldIn, here) :
                        getPaintSource(ourBlockState, worldIn, here);
                ...
                IBlockState otherPaint = otherBlock.isDouble() ? getPaintSource2(blockState2, worldIn, there) :
                        getPaintSource(blockState2, worldIn, there);
                return ourPaint != otherPaint;
            } else {
                if (isDouble()) {
                    if (!otherBlock.isDouble()) {
                        ...
                        return getPaintSource(ourBlockState, worldIn, here) !=
                                getPaintSource(blockState2, worldIn, there) ||
                                getPaintSource2(ourBlockState, worldIn, here) !=
                                        getPaintSource2(blockState2, worldIn, there);
                    }
                } else {
    ...
    public List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
        ...
        for (BlockSlab.EnumBlockHalf half : BlockSlab.EnumBlockHalf.values()) {
            if (isDouble() || half == state.getValue(HALF)) {
                ...
                IBlockState paintSource = isTop ? getPaintSource2(state, world, pos) :
                        getPaintSource(state, world, pos);
                if (blockLayer == null || PaintUtil.canRenderInLayer(paintSource, blockLayer)) {
                    quadCollector.addFriendlybakedModel(blockLayer,
                            PaintRegistry.getModel(IBakedModel.class, isTop ? "slab_hi" : "slab_lo", paintSource, null),
