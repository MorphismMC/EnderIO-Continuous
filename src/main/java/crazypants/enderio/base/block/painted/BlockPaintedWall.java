...
public class BlockPaintedWall extends BlockWall
                              implements ITileEntityProvider, IPaintable.ITexturePaintableBlock, ISmartRenderAwareBlock,
    ...
    public static BlockPaintedWall create(@Nonnull IModObject modObject) {
        ...
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new BasicPainterTemplate<BlockPaintedWall>(result, Blocks.COBBLESTONE_WALL));
    ...
    private void init(@Nonnull IModObject modObject) {
        ...
        PaintRegistry.registerModel("wall_post", new ResourceLocation("minecraft", "block/cobblestone_wall_post"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
        PaintRegistry.registerModel("wall_side", new ResourceLocation("minecraft", "block/cobblestone_wall_side"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
        PaintRegistry.registerModel("wall_inventory",
                new ResourceLocation("minecraft", "block/cobblestone_wall_inventory"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
    }
    ...
    // || (block instanceof IPaintable.IBlockPaintableBlock && ((IPaintable.IBlockPaintableBlock)
    // block).getPaintSource(blockState2, worldIn,
    ...
    public boolean shouldSideBeRendered(@Nonnull IBlockState bs, @Nonnull IBlockAccess worldIn, @Nonnull BlockPos pos,
        ...
        if (side.getAxis() != EnumFacing.Axis.Y) {
            ...
            if (otherBlockState.getBlock() instanceof BlockPaintedWall &&
    ...
    public @Nonnull IBlockState getExtendedState(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
        ...
        blockStateWrapper.addCacheKey(getPaintSource(state, world, pos)).addCacheKey(state.getValue(NORTH))
                .addCacheKey(state.getValue(EAST))
    ...
    private List<IBakedModel> mapRender(IBlockState state, @Nullable IBlockState paint) {
        ...
        if (state.getValue(SOUTH)) {
            result.add(
                    PaintRegistry.getModel(IBakedModel.class, "wall_side", paint, new UVLock(ModelRotation.X0_Y180)));
        }
        if (state.getValue(WEST)) {
            result.add(
                    PaintRegistry.getModel(IBakedModel.class, "wall_side", paint, new UVLock(ModelRotation.X0_Y270)));
        }
    ...
    public List<IBakedModel> mapItemRender(@Nonnull Block block, @Nonnull ItemStack stack) {
        ...
        IBakedModel model2 = PaintRegistry.getModel(IBakedModel.class, "wall_inventory", stdOverlay,
