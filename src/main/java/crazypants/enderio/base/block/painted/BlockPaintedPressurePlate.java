...
public class BlockPaintedPressurePlate extends BlockBasePressurePlate
                                       implements ITileEntityProvider, IPaintable.ITexturePaintableBlock,
                                       ISmartRenderAwareBlock, IRenderMapper.IBlockRenderMapper.IRenderLayerAware,
                                       INamedSubBlocks, IResourceTooltipProvider,
                                       IRenderMapper.IItemRenderMapper.IItemModelMapper, IModObject.WithBlockItem,
                                       ICustomSubItems {
    ...
    public static BlockPaintedPressurePlate create(@Nonnull IModObject modObject) {
        ...
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new PressurePlatePainterTemplate(result, EnumPressurePlateType.WOOD.getMetaFromType(),
                        Blocks.WOODEN_PRESSURE_PLATE));
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new PressurePlatePainterTemplate(result, EnumPressurePlateType.STONE.getMetaFromType(),
                        Blocks.STONE_PRESSURE_PLATE));
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new PressurePlatePainterTemplate(result, EnumPressurePlateType.IRON.getMetaFromType(),
                        Blocks.HEAVY_WEIGHTED_PRESSURE_PLATE));
        MachineRecipeRegistry.instance.registerRecipe(MachineRecipeRegistry.PAINTER,
                new PressurePlatePainterTemplate(result, EnumPressurePlateType.GOLD.getMetaFromType(),
    ...
    private final @Nonnull NNEnumMap<EnumPressurePlateType, IBlockState> defaultPaints = new NNEnumMap<>(
            EnumPressurePlateType.class,
            Blocks.AIR.getDefaultState());

    private void init(@Nonnull IModObject modObject) {
        SmartModelAttacher.registerNoProps(this);
        PaintRegistry.registerModel(MODEL_UP, new ResourceLocation("minecraft:block/stone_pressure_plate_up"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
        PaintRegistry.registerModel(MODEL_DOWN, new ResourceLocation("minecraft:block/stone_pressure_plate_down"),
                PaintRegistry.PaintMode.ALL_TEXTURES);
        ...
        // we "hide" our textures for our variants in these blockstates. Our paint rendering will make sure they are
        // never actually rendered.
        defaultPaints.put(EnumPressurePlateType.DARKSTEEL,
                getDefaultState().withProperty(BlockPressurePlateWeighted.POWER, 1));
        defaultPaints.put(EnumPressurePlateType.SOULARIUM,
                getDefaultState().withProperty(BlockPressurePlateWeighted.POWER, 2));
        defaultPaints.put(EnumPressurePlateType.TUNED,
                getDefaultState().withProperty(BlockPressurePlateWeighted.POWER, 3));
    }
    ...
    protected int computeRedstoneStrength(@Nonnull World worldIn, @Nonnull BlockPos pos) {
        ...
        if (te instanceof TilePaintedPressurePlate) {
            ...
            return type.getCountingMode()
                    .count(worldIn.getEntitiesWithinAABB(type.getSearchClass(), PRESSURE_AABB.offset(pos),
                            type.getPredicate(getMobType(worldIn, pos))));
        } else {
    ...
    protected int getMetaForStack(@Nonnull IBlockAccess worldIn, @Nonnull BlockPos pos) {
        TileEntity te = BlockEnder.getAnyTileEntitySafe(worldIn, pos);
        if (te instanceof TilePaintedPressurePlate) {
            return EnumPressurePlateType.getMetaFromType(((TilePaintedPressurePlate) te).getType(),
                    ((TilePaintedPressurePlate) te).isSilent());
        }
        return 0;
    }
    ...
    @Override
    public @Nonnull IBlockState getStateForPlacement(@Nonnull World worldIn, @Nonnull BlockPos pos,
    ...
    protected @Nonnull ItemStack getDrop(@Nonnull IBlockAccess world, @Nonnull BlockPos pos) {
        ...
        ItemStack drop = mobType != null ? mobType.toStack(this, getMetaForStack(world, pos), 1) :
                new ItemStack(this, 1, getMetaForStack(world, pos));
    ...
    @Override
    public void setPaintSource(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
                               @Nullable IBlockState paintSource) {
    ...
    @Override
    public @Nonnull IBlockState getPaintSource(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
    ...
    public @Nonnull IBlockState getPaintSource(@Nonnull Block block, @Nonnull ItemStack stack) {
        ...
        return paintSource != null ? paintSource :
                defaultPaints.get(EnumPressurePlateType.getTypeFromMeta(stack.getMetadata()));
    }
    ...
    public List<IBakedModel> mapItemRender(@Nonnull Block block, @Nonnull ItemStack stack) {
        ...
        if (paintSource != defaultPaints.get(EnumPressurePlateType.getTypeFromMeta(stack.getMetadata()))) {
            IBlockState stdOverlay = ModObject.block_machine_base.getBlockNN().getDefaultState()
                    .withProperty(EnumRenderPart.SUB, EnumRenderPart.PAINT_OVERLAY);
            IBakedModel model2 = PaintRegistry.getModel(IBakedModel.class, MODEL_UP, stdOverlay,
                    PaintRegistry.OVERLAY_TRANSFORMATION);
            list.add(model2);
        }
        return list;
    }
    ...
    public List<IBlockState> mapBlockRender(@Nonnull IBlockStateWrapper state, @Nonnull IBlockAccess world,
                                            @Nonnull BlockPos pos,
                                            @Nullable BlockRenderLayer blockLayer,
        ...
        if (PaintUtil.canRenderInLayer(paintSource, blockLayer) && !isInvisible(paintSource)) {
            quadCollector.addFriendlybakedModel(blockLayer, mapRender(state, paintSource, getRotation(world, pos)),
