...
public class EnergyIngredientRenderer implements IIngredientRenderer<EnergyIngredient> {
    ...
    @Override
    public void render(@Nonnull Minecraft minecraft, int xPosition, int yPosition,
                       @Nullable EnergyIngredient ingredient) {
        if (ingredient != null) {
            ...
            if (ingredient.hasAmount()) {
                ...
                minecraft.getRenderItem().renderItemAndEffectIntoGUI(null, Material.POWDER_INFINITY.getStack(),
                        xPosition, yPosition);
    ...
    public @Nonnull String asString(@Nonnull EnergyIngredient ingredient) {
        return !ingredient.hasAmount() ? LangPower.RF() :
                ingredient.isPerTick() ? LangPower.RFt(ingredient.getAmount()) : LangPower.RF(ingredient.getAmount());
    }
    ...
    public @Nonnull List<String> getTooltip(@Nonnull Minecraft minecraft, @Nonnull EnergyIngredient ingredient,
        ...
        return Lists.newArrayList(Lang.POWER_NAME.get(asString(ingredient)),
                TextFormatting.GRAY + Lang.POWER_DETAILS.get());
    }
