        ...
        items.apply(new Callback<ItemStack>() {
            ...
            public void apply(@Nonnull ItemStack itemStack) {
                ...
                if (!allTooltips.isEmpty()) {
                    allTooltips.add(0, TextFormatting.WHITE.toString() + TextFormatting.BOLD +
                            itemStack.getDisplayName() + TextFormatting.RESET);
                    allTooltips.add(1, "");
        ...
        dsitems.forEach(item -> {
            if (item instanceof Item) { // keep null and casting warnings happy
                ...
                for (IDarkSteelUpgrade upgrade : UpgradeRegistry.getUpgrades()) {
                    if (upgrade.getRules().stream().filter(Rules::isStatic)
                            .allMatch(Rules.makeChecker(itemStack, item))) {
                    ...
                    for (String name : names) {
                        ...
                        if (upgrade != null) {
                            ...
                            if (upgrade instanceof IAdvancedTooltipProvider) {
                                ((IAdvancedTooltipProvider) upgrade).addDetailedEntries(itemStack, null, allTooltips,
                                        false);
                            } else {
                                allTooltips.add(Lang.DARK_STEEL_LEVELS1.get(TextFormatting.DARK_AQUA, name));
                                SpecialTooltipHandler.addDetailedTooltipFromResources(allTooltips,
                                        upgrade.getUnlocalizedName());
                            }
                            allTooltips.add(Lang.DARK_STEEL_LEVELS2.get(TextFormatting.DARK_AQUA, TextFormatting.ITALIC,
                                    UpgradeRegistry.getUpgradeItem(upgrade).getDisplayName(), upgrade.getLevelCost()) +
