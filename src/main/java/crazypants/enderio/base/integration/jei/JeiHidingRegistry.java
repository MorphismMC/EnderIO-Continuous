...
public class JeiHidingRegistry {
    ...
    private interface IConfiguredIngredient<V> {
        ...
        /**
         * "hide" forcefully hides this item. At least one "hide" configuration is enough to do so. This is intended for
         * pack makers and users.
         * <p>
         * "show" determines if the item is shown unless it's hidden. At least one "show" configuration is enough to not
         * hide it unless it is hidden. This is
    ...
    private static class ConfiguredItemstack implements IConfiguredIngredient<ItemStack> {
        ...
        // This is a bit of a hack. It works around the detail that JEI may still display some item variant, e.g. an
        // item filled with a fluid.
        ...
        private void addStack(@Nonnull NNList<ItemStack> list, @Nonnull ItemStack stack) {
            ...
            for (ItemStack tabStack : temp) {
                if (stack.getItemDamage() == OreDictionary.WILDCARD_VALUE ||
                        stack.getItemDamage() == tabStack.getItemDamage()) {
                    list.add(tabStack);
                }
            }
        }
        ...
        public void set(@Nonnull ItemStack v, boolean show, boolean hide) {
            if (v.getItem() != item) {
                throw new UnsupportedOperationException(
                        "Cannot configure item '" + v.getItem() + "' using configuration of item '" + item + "'");
            }
            if (show) {
    ...
    private static class ConfiguredFluidstack implements IConfiguredIngredient<FluidStack> {
        ...
        public void set(@Nonnull FluidStack v, boolean show, boolean hide) {
            if (v.getFluid() != fluid) {
                throw new UnsupportedOperationException(
                        "Cannot configure fluid '" + v.getFluid() + "' using configuration of fluid '" + fluid + "'");
            }
            shown |= show;
            hidden |= hide;
        }
