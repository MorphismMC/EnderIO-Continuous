    ...
    default List<? extends Target<?>> getGhostTargets() {
        if (!(this instanceof GuiContainerBase)) {
            throw new IllegalStateException(
                    "IHaveGhostTargets must be implemented by GuiContainerBase subclasses, or provide a custom implementation.");
        }
        GuiContainerBase gui = (GuiContainerBase) this;
        return gui.getGhostSlotHandler().getGhostSlots().stream().filter(this::isSlotTarget)
