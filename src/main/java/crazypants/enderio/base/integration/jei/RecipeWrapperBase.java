...
public abstract class RecipeWrapperBase implements IRecipeWrapper {
    ...
    public static void setLevelData(@Nonnull Class<? extends RecipeWrapperBase> clazz, @Nonnull IGuiHelper guiHelper,
                                    int x, int y, String textureSimple,
                                    String textureNormal) {
        IDrawable simpleFront = textureSimple != null ?
                guiHelper.createDrawable(new ResourceLocation(EnderIO.DOMAIN, textureSimple), 0, 0, 16, 16, 16, 16) :
                null;
        IDrawable normalFront = textureNormal != null ?
                guiHelper.createDrawable(new ResourceLocation(EnderIO.DOMAIN, textureNormal), 0, 0, 16, 16, 16, 16) :
                null;
    ...
    public List<String> getTooltipStrings(int mouseX, int mouseY) {
        Triple<IDrawable, IDrawable, Vector2i> level = LEVEL.get(this.getClass());
        if (level != null && mouseX >= level.getRight().x && mouseX <= level.getRight().x + 6 + 16 &&
                mouseY >= level.getRight().y - 6 && mouseY <= level.getRight().y + 16) {
            switch (getRecipeLevel()) {
                case ADVANCED:
