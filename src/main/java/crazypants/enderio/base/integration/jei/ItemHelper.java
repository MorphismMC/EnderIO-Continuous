        ...
        protected boolean set(Item item) {
            ...
            if (item instanceof ItemBlock) {
                ...
                if (NullHelper.untrust(block) == null) {
                    Log.error("ItemBlock " + item + " returned null from getBlock(). This is a major bug in the mod '" +
                ...
                if (block instanceof IFluidBlock) {
                    ...
                    if (fluid == null) {
                        Log.error(
                                "Block " + block + " returned null from getFluid(). This is a major bug in the mod '" +
                                        PaintUtil.block2Modname(block) + "'.");
                        return false;
                    }
                    final Block fblock = fluid.getBlock();
                    if (fblock != null && NullHelper.untrust(Block.REGISTRY.getNameForObject(fblock)) == null) {
                        Log.error("Fluid " + fluid + " (" + fluid.getName() + ", " + fluid.getClass() +
                                ") from block " + block +
                                " returned an unregistered block from getBlock(). This is a major bug in the mod '" +
                                PaintUtil.block2Modname(block) + "'.");
                        return false;
        ...
        public void apply(@Nonnull ItemStack stack) {
            if (Prep.isInvalid(stack)) {
                Log.error("The item " + item + " (" + item.getTranslationKey() +
                        ") produces empty itemstacks in getSubItems(). This is a major bug in the mod '" +
                        PaintUtil.block2Modname(item) + "'.");
                return;
            }
            if (stack.getItem() == Items.AIR) {
                Log.error("The item " + item + " (" + item.getTranslationKey() +
                        ") produces itemstacks without item in getSubItems(). This is a major bug in the mod '" +
                        PaintUtil.block2Modname(item) + "'.");
                return;
            }
            ...
            if (fluidStack != null && fluidStack.getFluid() != null) {
                ...
                if (block != null && NullHelper.untrust(Block.REGISTRY.getNameForObject(block)) == null) {
                    Log.error("Fluid " + fluidStack.getFluid() + " (" + fluidStack.getFluid().getName() + ", " +
                            fluidStack.getFluid().getClass() + ") from item " + stack +
