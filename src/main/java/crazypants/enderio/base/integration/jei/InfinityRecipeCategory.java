...
public class InfinityRecipeCategory implements IRecipeCategory<InfinityRecipeCategory.InfinityRecipeWrapper> {
    ...
    public static class InfinityRecipeWrapper implements IRecipeWrapper {
        ...
        public InfinityRecipeWrapper(@Nonnull Block block1, @Nonnull Block block2) {
            this.bsr = new BlockSceneRenderer(
                    new NNList<>(Pair.of(new BlockPos(0, 0, 0), block1.getDefaultState()),
                            Pair.of(new BlockPos(0, 1, 0), block2.getDefaultState())));
        }
        ...
        public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
            ...
            if (!InfinityConfig.enableInAllDimensions.get()) {
                if (InfinityConfig.enableInDimensions.get().length == 1 &&
                        InfinityConfig.enableInDimensions.get()[0] == 0) {
                    minecraft.fontRenderer.drawString(Lang.GUI_INFINTY_RECIPE_DIMENSIONS.get(), 45, 40,
                            ColorUtil.getRGB(Color.GRAY));
                } else {
                    minecraft.fontRenderer.drawString(Lang.GUI_INFINTY_RECIPE_DIMENSIONS_MULTI.get(), 45, 40,
                            ColorUtil.getRGB(Color.GRAY));
                }
            }
        }
        ...
        public @Nonnull List<String> getTooltipStrings(int mouseX, int mouseY) {
            if (mouseX >= 45 && mouseY >= 40 && mouseY < (40 + Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT) &&
                    !InfinityConfig.enableInAllDimensions.get() &&
                    !(InfinityConfig.enableInDimensions.get().length == 1 &&
                            InfinityConfig.enableInDimensions.get()[0] == 0) &&
                    mouseX < 45 + Minecraft.getMinecraft().fontRenderer
                            .getStringWidth(Lang.GUI_INFINTY_RECIPE_DIMENSIONS_MULTI.get())) {
                ...
                for (int dim : InfinityConfig.enableInDimensions.get()) {
                    if (Minecraft.getMinecraft().player.world.provider.getDimension() == dim) {
                        result.add(
                                Lang.GUI_INFINTY_RECIPE_DIMENSIONS_LIST_HERE.get(TelepadTarget.getDimenionName(dim)));
                        inCurrent = true;
                    } else {
    ...
    public static void registerExtras(IModRegistry registry) {
        ...
        Log.info(String.format("InfinityRecipeCategory: Added %d infinity recipe(s) to JEI in %.3f seconds.",
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull InfinityRecipeWrapper recipeWrapper,
