...
public class TOPCompatibility implements Function<ITheOneProbe, Void>, IProbeInfoProvider, IProbeConfigProvider {
        ...
        if (probeInfo != null && blockState != null) {
            ...
            if (world != null && hitData != null &&
                ...
                TileEntity tileEntity = blockState.getBlock() instanceof ITEProxy ?
                        ((ITEProxy) blockState.getBlock()).getParent(world,
                                NullHelper.notnull(hitData.getPos(), "hitData.getPos()"), blockState) :
                        BlockEnder.getAnyTileEntitySafe(world,
                                NullHelper.notnull(hitData.getPos(), "hitData.getPos()/2"));
                if (tileEntity != null) {
    ...
    private static class EioBox {
        ...
        public void finish() {
            if (eiobox != null) {
                ...
                addIcon(addIcon(probeinfo.vertical().horizontal(center()), IconEIO.TOP_NOICON_WIDE, 0),
                        IconEIO.TOP_MORE, 0);
            }
        }
    }
    ...
    private void mkSideConfigLine(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.hasIOMode) {
            if (mode != ProbeMode.NORMAL || TopConfig.showSideConfigByDefault.get()) {
                addIcon(eiobox.get().horizontal(eiobox.center()), IconEIO.IO_CONFIG_UP)
                        .vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1))
                        .text(TextFormatting.YELLOW + loc("top.machine.side",
                                TextFormatting.WHITE + loc("top.machine.side." + data.sideName)))
                        .text(TextFormatting.YELLOW +
                                loc("top.machine.ioMode", loc(data.ioMode.getColorerUnlocalizedName())));
            } else {
                eiobox.addMore();
            }
        }
                ...
                ChannelType.VALUES.apply(new Callback<ChannelType>() {
                    ...
                    public void apply(@Nonnull ChannelType type) {
                        if (!data.sendChannels.get(type).isEmpty() || !data.recvChannels.get(type).isEmpty()) {
                            final IProbeInfo lines = addIcon(eiobox.get().horizontal(eiobox.center()),
                                    type.getWidgetIcon())
                                            .vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1));
    ...
    private void mkRedstoneLine(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.hasRedstone) {
            if (mode != ProbeMode.NORMAL || TopConfig.showRedstoneByDefault.get()) {
                addIcon(eiobox.get().horizontal(eiobox.center()), data.redstoneIcon)
                        .vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1))
                        .text(loc(data.redstoneTooltip))
                        .text(loc("top.redstone.header", loc("top.redstone." + data.redstoneControlStatus)));
            } else {
                eiobox.addMore();
            }
        }
    ...
    private void mkPaint(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.isPainted) {
            ...
            // ItemStack.getDisplayName() should be run on the client, but I don't think we have a way to get the stack
            // there
    ...
    private void mkOwner(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (mode == ProbeMode.DEBUG && data.owner != null) {
            ...
            eiobox.get().horizontal(eiobox.center()).item(skull)
                    .vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1)).text(loc("top.owner.header"))
            ...
            if (mode != ProbeMode.NORMAL || TopConfig.showPowerByDefault.get()) {
                ...
                if (data.isPowered) {
                    rfLine.progress(data.rf, data.maxrf,
                            eiobox.getProbeinfo().defaultProgressStyle().suffix(EnderIO.lang.localize("top.suffix.rf"))
                ...
                if (data.hasRFIO) {
                    ...
                    rfLine.vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1))//
                            .text(loc("top.rf.header.avg")).text(loc("top.rf.header.maxin"))
                            .text(loc("top.rf.header.maxout"));

                    // LangPower.format should be run on the client, but we have no way to do that
                    String line1 = loc("top.rf.value",
                            (data.avgRF == 0 ? TextFormatting.WHITE :
                                    data.avgRF > 0 ? TextFormatting.GREEN + "+" : TextFormatting.RED) +
                    ...
                    rfLine = rfLine.vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1)).text(line1)
    ...
    private void mkXPLine(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.hasXP) {
            if (mode != ProbeMode.NORMAL || TopConfig.showXPByDefault.get()) {
                // We need to put the number of levels in as "current" value for it to be displayed as text. To make the
                // progress bar scale to the partial level, we set
                // the "max" value in a way that is in the same ratio to the number of levels as the xp needed for the
                // next level is to the current xp. If the bar
                // should be empty but we do have at least one level in, there will be a small error, as
                // (levels/Integer.MAX_VALUE) > 0.
                int scalemax = data.xpBarScaled > 0 ? data.experienceLevel * 100 / data.xpBarScaled : Integer.MAX_VALUE;
                eiobox.get().horizontal(eiobox.center()).item(new ItemStack(Items.EXPERIENCE_BOTTLE)).progress(
                        data.experienceLevel, scalemax,
                        eiobox.getProbeinfo().defaultProgressStyle().suffix(EnderIO.lang.localize("top.suffix.levels"))
                                .filledColor(0xff00FF0F)
    ...
    private void mkItemFillLevelLine(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.hasItemFillLevel) {
            if (mode != ProbeMode.NORMAL || TopConfig.showItemCountDefault.get()) {
                eiobox.get().horizontal(eiobox.center()).item(new ItemStack(Blocks.CHEST)).progress(data.fillCur,
                        data.fillMax,
                        eiobox.getProbeinfo().defaultProgressStyle().suffix(EnderIO.lang.localize("top.suffix.items"))
                                .filledColor(0xfff8f83c)
        ...
        if (data.tankData != null && !data.tankData.isEmpty()) {
            if (mode != ProbeMode.NORMAL || TopConfig.showTanksByDefault.get()) {
                for (ITankData tank : data.tankData) {
                    ...
                    eiobox.get().horizontal(eiobox.center()).item(stack)
                            .vertical(eiobox.getProbeinfo().defaultLayoutStyle().spacing(-1)).text(content1)
                            .text(content2);
    ...
    private void mkProgressLine(ProbeMode mode, EioBox eiobox, TOPData data) {
        if (data.progressResult != TOPData.ProgressResult.NONE) {
            if (mode != ProbeMode.NORMAL || TopConfig.showProgressByDefault.get() ||
                    data.progressResult == TOPData.ProgressResult.PROGRESS_NO_POWER) {
                final IProbeInfo progressLine = eiobox.get().horizontal(eiobox.center())
                        .item(new ItemStack(Items.CLOCK));
                switch (data.progressResult) {
                    case PROGRESS:
                        progressLine.progress((int) (data.progress * 100), 100,
                                eiobox.getProbeinfo().defaultProgressStyle()
                                        .suffix(EnderIO.lang.localize("top.suffix.percent")).filledColor(0xffffb600)
                                        .alternateFilledColor(0xffffb600));
                        break;
    ...
    private static IProbeInfo addIcon(IProbeInfo probeInfo, IWidgetIcon icon, int border) {
        ...
        return probeInfo.icon(texture, x, y, width, height,
                probeInfo.defaultIconStyle().width(width + border).height(height + border));
    }
}
