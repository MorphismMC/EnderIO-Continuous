...
public class IntegrationRegistry {
    ...
    public static void registerRegistry(@Nonnull RegistryEvent.NewRegistry event) {
        REGISTRY = new RegistryBuilder<IIntegration>().setName(new ResourceLocation(EnderIO.DOMAIN, "integration"))
                .setType(IIntegration.class)
    ...
    public static boolean isInSameTeam(@Nonnull UserIdent identA, @Nonnull UserIdent identB) {
        return new NNList<>(IntegrationRegistry.REGISTRY.getValuesCollection())
                .apply(new ShortCallback<IIntegration>() {
