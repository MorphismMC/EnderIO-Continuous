...
public class RailcraftUtil {
    ...
    public static void registerFuels() {
        ...
        if (rocket_fuel != null) {
            FMLInterModComms.sendMessage("railcraft", "boiler-fuel-liquid",
                    Fluids.ROCKET_FUEL.getName() + "@" +
                            (rocket_fuel.getPowerPerCycle() / 10 * rocket_fuel.getTotalBurningTime()));
        }
        IFluidFuel fire_water = FluidFuelRegister.instance.getFuel(Fluids.FIRE_WATER.getFluid());
        if (fire_water != null) {
            FMLInterModComms.sendMessage("railcraft", "boiler-fuel-liquid",
                    Fluids.FIRE_WATER.getName() + "@" +
                            (fire_water.getPowerPerCycle() / 10 * fire_water.getTotalBurningTime()));
        }
    }
