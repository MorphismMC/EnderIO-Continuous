...
public class BRProxy {
    ...
    public static void init(@Nonnull EnderIOLifecycleEvent.Init.Normal event) {
        if (Loader.isModLoaded("bigreactors") && IntegrationConfig.enableBigReactors.get()) {
            try {
                ReactorInterior = ReflectionHelper.getClass(BRProxy.class.getClassLoader(),
                        "erogenousbeef.bigreactors.api.registry.ReactorInterior");

                // public static ReactorInteriorData getBlockData(String oreDictName) {

                getBlockData = ReflectionHelper.findMethod(ReactorInterior, "getBlockData", "getBlockData",
                        String.class);

                // public static void registerBlock(String oreDictName, float absorption, float heatEfficiency, float
                // moderation, float heatConductivity)

                registerBlock = ReflectionHelper.findMethod(ReactorInterior, "registerBlock", "registerBlock",
                        String.class, float.class, float.class, float.class,
                        float.class);

                // public static ReactorInteriorData getFluidData(String fluidName) {

                getFluidData = ReflectionHelper.findMethod(ReactorInterior, "getFluidData", "getFluidData",
                        String.class);

                // public static void registerFluid(String fluidName, float absorption, float heatEfficiency, float
                // moderation, float heatConductivity)

                registerFluid = ReflectionHelper.findMethod(ReactorInterior, "registerFluid", "registerFluid",
                        String.class, float.class, float.class, float.class,
                        float.class);

                TurbineCoil = ReflectionHelper.getClass(BRProxy.class.getClassLoader(),
                        "erogenousbeef.bigreactors.api.registry.TurbineCoil");

                // public static CoilPartData getBlockData(String oreDictName)

                getTurbineBlockData = ReflectionHelper.findMethod(TurbineCoil, "getBlockData", "getBlockData",
                        String.class);

                // public static void registerBlock(String oreDictName, float efficiency, float bonus, float
                // extractionRate)

                registerTurbineBlock = ReflectionHelper.findMethod(TurbineCoil, "registerBlock", "registerBlock",
                        String.class, float.class, float.class, float.class);
