...
public class ThaumcraftUtil {
    ...
    public static void registerDarkSteelUpgrades(@Nonnull RegistryEvent.Register<IDarkSteelUpgrade> event) {
        if (Loader.isModLoaded(MODID_THAUMCRAFT)) {
            event.getRegistry().registerAll(ThaumaturgeRobesUpgrade.BOOTS, ThaumaturgeRobesUpgrade.LEGS,
                    ThaumaturgeRobesUpgrade.CHEST,
        ...
        if (Loader.isModLoaded(MODID_THAUMCRAFT)) {
            ...
            if (!blocks.isEmpty() && Prep.isValid(itemStack)) {
                event.getRegistry().register(
                        new ManaBeanFarmer(blocks.get(0), itemStack).setRegistryName(MODID_THAUMCRAFT, "manabean"));
