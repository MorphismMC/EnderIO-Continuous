...
public class EnderIO implements IEnderIOAddon {
    ...
    @SidedProxy(clientSide = "crazypants.enderio.base.init.ClientProxy",
                serverSide = "crazypants.enderio.base.init.CommonProxy")
    ...
    public void onImc(@Nonnull IMCEvent event) {
         ...
         * This is a mess.
         * 
         * Items are registered in the registry event between preInit and Init. OreDicts are registered in Init (because
         * Lex says so). MaterialRecipes.init() must
         * run after all oreDict registrations. RecipeLoader.addRecipes() should run in the registry event.
         * 
         * At the moment we can delay RecipeLoader.addRecipes(), but still PostInit is too late as there's code that
         * depends on it being done for all submods.
         */
    ...
    static void initCrashData() {
        ...
        CrashReport crashreport = CrashReport.makeCrashReport(new RuntimeException(),
        ...
        CrashReportCategory.addBlockInfo(crashreportcategory, new BlockPos(0, 0, 0),
                ModObject.block_machine_base.getBlockNN().getDefaultState());
        // the one failing usually is in net.minecraft.world.World.neighborChanged(BlockPos, Block, BlockPos). That
        // one's $2.
        try {
    ...
    private static void startupChecks() {
        ...
        if (com.mojang.authlib.minecraft.MinecraftProfileTexture.Type.ELYTRA.getClass().toString()
                .equals("force a classload real hard")) {
