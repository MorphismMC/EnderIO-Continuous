...
public class ItemRemoteInvAccess extends Item implements IAdvancedTooltipProvider, IOverlayRenderAware,
                                 IInternalPoweredItem, IHaveRenderers, IEioGuiHandler.WithPos {
    ...
    public void registerRenderers(@Nonnull IModObject modObject) {
        for (ItemRemoteInvAccessType type : ItemRemoteInvAccessType.values()) {
            ResourceLocation resourceLocation = new ResourceLocation(EnderIO.DOMAIN,
                    type.getUnlocalizedName(modObject.getUnlocalisedName()));
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World world, @Nonnull EntityPlayer player,
        ...
        if (!world.isRemote) {
            ...
            if (!REMOTE_X.hasTag(equipped) || !REMOTE_Y.hasTag(equipped) || !REMOTE_Z.hasTag(equipped) ||
                    !REMOTE_D.hasTag(equipped)) {
                player.sendStatusMessage(new TextComponentString(EnderIO.lang.localize("remoteinv.chat.notarget")),
                        true);
            ...
            if (!type.inRange(d, x, y, z, world.provider.getDimension(), (int) player.posX, (int) player.posY,
                    (int) player.posZ)) {
                player.sendStatusMessage(new TextComponentString(EnderIO.lang.localize("remoteinv.chat.outofrange")),
                        true);
                return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
            }
            ...
            if (world.provider.getDimension() != d) {
                ...
                if (targetWorld == null) {
                    player.sendStatusMessage(
                            new TextComponentString(EnderIO.lang.localize("remoteinv.chat.invalidtargetworld")), true);
                    return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
                }
            }
            ...
            if (!targetWorld.isBlockLoaded(pos)) {
                player.sendStatusMessage(new TextComponentString(EnderIO.lang.localize("remoteinv.chat.notloaded")),
                        true);
                return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
            }

            if (targetWorld.getBlockState(pos).getBlock() != InvpanelObject.blockInventoryPanel.getBlock()) {
                player.sendStatusMessage(new TextComponentString(EnderIO.lang.localize("remoteinv.chat.invalidtarget")),
                        true);
                return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
            }
            ...
            if (getEnergyStored(equipped) < type.getRfPerTick() * 10) {
                player.sendStatusMessage(new TextComponentString(EnderIO.lang.localize("remoteinv.chat.outofpower")),
                        true);
                return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
            }
            ...
            if (!drain(equipped, type.getMbPerOpen())) {
                ...
                player.sendStatusMessage(
                        new TextComponentString(
                                EnderIO.lang.localize("remoteinv.chat.outoffluid").replace("{FLUIDNAME}", fluidname)),
                        true);
                return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, equipped);
            }
            ...
            if (te instanceof TileInventoryPanel) {
                PacketHandler.INSTANCE.sendTo(new PacketPrimeInventoryPanelRemote((TileInventoryPanel) te),
                        (EntityPlayerMP) player);
            }
            ModObjectRegistry.getModObjectNN(this).openGui(player.world, pos, player, null, d);
            Ticker.create();
            // InvpanelObject.itemInventoryRemote.openGui(player.world, pos, player, null,
    ...
    @Nullable
    @Override
    public Object getGuiElement(boolean server, @Nonnull EntityPlayer player, @Nonnull World world,
                                @Nonnull BlockPos pos, @Nullable EnumFacing facing, int param1, int param2,
                                int param3) {
    ...
    public Container getServerGuiElement(@Nonnull EntityPlayer player, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        Log.warn("Unexpected failure to get tileentity at " + pos + " in dimension " + param1 +
                " for the Inventory Panel Remote. Got: " + te);
        return null;
    }
    ...
    private class CapabilityProvider implements IFluidHandlerItem {
        ...
        public int fill(@Nullable FluidStack resource, boolean doFill) {
            if (container.isEmpty() || !(container.getItem() == ItemRemoteInvAccess.this) || resource == null ||
                    resource.amount <= 0 || resource.getFluid() == null ||
