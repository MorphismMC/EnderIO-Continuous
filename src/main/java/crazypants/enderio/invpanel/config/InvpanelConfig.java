...
public final class InvpanelConfig {
    ...
    public static final IValue<Float> inventoryPanelScanCostPerSlot = F
            .make("inventoryPanelScanCostPerSlot", 0.1F, "Internal power used for scanning a slot")
            .sync().setMin(0);
    ...
    public static final IValue<Float> inventoryPanelExtractCostPerOperation = F
            .make("inventoryPanelExtractCostPerOperation", 32F,
    ...
    static {
        for (ItemRemoteInvAccessType type : ItemRemoteInvAccessType.values()) {
            int i = type.ordinal();
            remoteInventoryMBPerOpen
                    .add(F.make("remoteInventoryMBPerOpenTier" + i, DEF_MB_OPEN[i], "MB required to open the panel")
                            .sync().setMin(0));

            remoteInventoryRFPerTick.add(F
                    .make("remoteInventoryRFPerTickTier" + i, DEF_RF_TICK[i], "RF used per tick when the panel is open")
                    .sync().setMin(0));

            remoteInventoryMBCapacity.add(
                    F.make("remoteInventoryMBCapacityTier" + i, DEF_MB_CAP[i], "Capacity of the internal tank in MB")
                            .sync().setMin(0));

            remoteInventoryRFCapacity.add(F
                    .make("remoteInventoryRFCapacityTier" + i, DEF_RF_CAP[i], "Capacity of the internal energy storage")
                    .sync().setMin(0));

            remoteInventoryFluidTypes
                    .add(F.make("remoteInventoryFluidTypesTier" + i, NullHelper.notnull(DEF_FLUID[i], "DEF_FLUID"),
                            "The type of fluid required").sync());
        }
    }
