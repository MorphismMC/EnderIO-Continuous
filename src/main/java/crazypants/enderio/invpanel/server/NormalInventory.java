...
class NormalInventory extends AbstractInventory {
            ...
            if (stack.getCount() == 0) {
                ...
                if (extracted.isEmpty()) {
                    ...
                    if (extracted.getCount() < extracted.getMaxStackSize()) {
                        ...
                        // not sure why it would do that, but let's decide that the advertisement was wrong and the
    ...
    public int extractItem(@Nonnull IInventoryDatabaseServer db, IServerItemEntry entry, int slot, int count) {
        ...
        if (extracted.isEmpty()) {
            Log.error("Inventory at " + pos +
                    " implements IItemHandler wrong. extractItem() with and without simulation give different results.");
            // what else could be wrong with this inventory? It could dupe or void items left and right! We don't want
            // to touch something like that...
            setEmpty(db);
            errored = true;
            return 0;
        }
