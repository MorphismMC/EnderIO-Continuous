...
public class InventoryPanelContainer extends AbstractMachineContainer<TileInventoryPanel> implements IChangeLog {
    ...
    /*
     * Slots: slotCraftResult=0 firstSlotCraftingGrid=1 endSlotCraftingGrid=10 firstSlotReturn=11 endSlotReturn=21
     * startHotBarSlot=48 endHotBarSlot=57
    ...
    protected void addMachineSlots(@Nonnull InventoryPlayer playerInv) {
        ...
        addSlotToContainer(slotCraft = new SlotCraftingWrapper(playerInv.player,
                new InventoryCraftingWrapper(getInv(), this, 3, 3), getInv(),
    ...
    public boolean canInteractWith(@Nonnull EntityPlayer player) {
        ...
        if ((!player.getHeldItemMainhand().isEmpty() &&
                player.getHeldItemMainhand().getItem() instanceof ItemRemoteInvAccess)) {
            if (!player.getEntityWorld().isRemote) {
                return ((ItemRemoteInvAccess) player.getHeldItemMainhand().getItem())
                        .canInteractWith(player.getHeldItemMainhand(), player);
            } else {
                return true;
            }
        } else if ((!player.getHeldItemOffhand().isEmpty() &&
                player.getHeldItemOffhand().getItem() instanceof ItemRemoteInvAccess)) {
                    if (!player.getEntityWorld().isRemote) {
                        return ((ItemRemoteInvAccess) player.getHeldItemOffhand().getItem())
                                .canInteractWith(player.getHeldItemOffhand(), player);
                    } else {
                        return true;
                    }
                }
    ...
    public void tick(@Nonnull EntityPlayer player) {
        if (!super.canInteractWith(player)) {
            if ((!player.getHeldItemMainhand().isEmpty() &&
                    player.getHeldItemMainhand().getItem() instanceof ItemRemoteInvAccess)) {
                ((ItemRemoteInvAccess) player.getHeldItemMainhand().getItem()).tick(player.getHeldItemMainhand(),
                        player);
            } else if ((!player.getHeldItemOffhand().isEmpty() &&
                    player.getHeldItemOffhand().getItem() instanceof ItemRemoteInvAccess)) {
                        ((ItemRemoteInvAccess) player.getHeldItemOffhand().getItem()).tick(player.getHeldItemOffhand(),
                                player);
                    }
        }
    }
