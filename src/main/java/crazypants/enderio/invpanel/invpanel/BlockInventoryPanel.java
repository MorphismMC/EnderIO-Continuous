...
public class BlockInventoryPanel extends AbstractMachineBlock<TileInventoryPanel> implements ITintedBlock, ITintedItem {
    ...
    @Override
    public @Nonnull AxisAlignedBB getBoundingBox(@Nonnull IBlockState state, @Nonnull IBlockAccess world,
    ...
    @Override
    public int getBlockTint(@Nonnull IBlockState state, @Nullable IBlockAccess world, @Nullable BlockPos pos,
                            int tintIndex) {
        return (tintIndex > 0 && world != null && pos != null) ? FuncUtil.runIfOr(getTileEntitySafe(world, pos),
                te -> FuncUtil.runIfOr(te.getColor(),
                        color -> PersonalConfig.candyColors.get() ? color.getColorValue() :
                                MapColor.getBlockColor(color).colorValue,
                        -1),
                -1) : -1;
    }
    ...
    public void onBlockClicked(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull EntityPlayer player) {
        if (player.isSneaking()) {
            FuncUtil.doIf(DyeColor.getColorFromDye(player.getHeldItemMainhand()),
                    col -> FuncUtil.doIf(getTileEntitySafe(world, pos),
                            te -> te.setColor(EnumDyeColor.byDyeDamage(col.ordinal()))));
        }
