...
@Storable
public class TileInventoryPanel extends AbstractInventoryMachineEntity
                                implements IInventoryPanel, ITankAccess.IExtendedTankAccess, IHasNutrientTank {
    ...
    public TileInventoryPanel() {
        ...
        this.fuelTank = new SmartTank(Fluids.NUTRIENT_DISTILLATION.getFluid(),
                InvpanelConfig.inventoryPanelFree.get() ? 0 : 2000); // TODO fluid config
        ...
        addICap(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
                ICap.facedOnly(facingIn -> extractionDisabled ? ICap.DENY : ICap.NEXT));
    }
    ...
    public void setExtractionDisabled(boolean extractionDisabled) {
        ...
        if (NullHelper.untrust(world) != null && !world.isRemote) {
            PacketHandler.INSTANCE.sendToDimension(new PacketUpdateExtractionDisabled(this, extractionDisabled),
