...
public class GuiInventoryPanel extends GuiMachineBase<TileInventoryPanel> {
    ...
    private final @Nonnull BlockSceneRenderer bsr_active = new BlockSceneRenderer(new NNList<>( //
            Pair.of(new BlockPos(0, 0, 0), //
                    ModObject.blockEndermanSkull.getBlock().getDefaultState().withProperty(BlockEndermanSkull.VARIANT,
                            SkullType.REANIMATED_TORMENTED))));
    ...
    public GuiInventoryPanel(@Nonnull TileInventoryPanel te, @Nonnull Container container) {
        ...
        this.ghostSlotHandler = new GhostSlotHandler() {
            ...
            @Override
            protected void ghostSlotClicked(@Nonnull GuiContainerBase gui, @Nonnull GhostSlot slot, int x, int y,
                                            int button) {
        ...
        for (int i = 0; i < TileInventoryPanel.MAX_STORED_CRAFTING_RECIPES; i++) {
            ...
            addToolTip(new GuiToolTip(repButtons[index].getBounds(),
                    EnderIO.lang.localize("gui.inventorypanel.tooltip.recipe.store")) {
            ...
            if (recipe != null) {
                ...
                addToolTip(new GuiToolTip(repButtons[index].getBounds(), "") {
                    ...
                    protected void updateText() {
                        ...
                        text.add(RECIPE_ID + "invprecipe" + index + RECIPE_END + TextFormatting.GRAY +
                                EnderIO.lang.localize("gui.inventorypanel.tooltip.recipe.load"));
                        text.add(RECIPE + TextFormatting.GRAY +
                                EnderIO.lang.localize("gui.inventorypanel.tooltip.recipe.loadstacks"));
                        text.add(RECIPE + TextFormatting.GRAY +
                                EnderIO.lang.localize("gui.inventorypanel.tooltip.recipe.delete"));
                        text.add(RECIPE);
                        text.add(RECIPE);
                        text.add(RECIPE);
                    }
        ...
        if (JeiAccessor.isJeiRuntimeAvailable()) {
            ...
            btnSync.setSelectedToolTip(EnderIO.lang.localize("gui.enabled"),
                    EnderIO.lang.localize("gui.inventorypanel.tooltip.sync.jei.line1"),
        ...
        btnClear = new MultiIconButton(this, ID_CLEAR, 65, 60, EnderWidget.X_BUT, EnderWidget.X_BUT_PRESSED,
        ...
        addToolTip(new GuiToolTip(
                new Rectangle(InventoryPanelContainer.FILTER_SLOT_X, InventoryPanelContainer.FILTER_SLOT_Y, 16, 16),
                list) {
    ...
    public void syncSettingsChange() {
        ...
        String filterText;
        if (!btnSync.isSelected() || tfFilterExternalValue == null ||
                !tfFilterExternalValue.equals(tfFilter.getText())) {
        ...
        if (getTileEntity().getGuiSortMode() != sortMode || getTileEntity().getGuiSync() != btnSync.isSelected() ||
            ...
            PacketHandler.INSTANCE.sendToServer(
                    new PacketGuiSettings(getContainer().windowId, sortMode, filterText, btnSync.isSelected()));
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int mouseX, int mouseY) {
        ...
        if (!InvpanelConfig.inventoryPanelFree.get()) {
            ...
            if (fuelTank.getFluidAmount() > 0) {
                RenderUtil.renderGuiTank(fuelTank.getFluid(), fuelTank.getCapacity(), fuelTank.getFluidAmount(),
                        sx + 12, sy + 148, zLevel, 16, 47);
            }
        }
        ...
        final EnderWidget returnButton = te.isExtractionDisabled() ?
                ...
                btnReturnArea.contains(mouseX - sx, mouseY - sy) ? EnderWidget.RETURN_BUT_HOVER :
                        EnderWidget.RETURN_BUT;
     ...
     *
     * When the GUI is opened, any text stored in the invPanel will take precedence. If there's no stored text, the text
     * from the JEI field will be used.
     *
     * When text is entered into the search field, it is synced to JEI. When the field is cleared, that is synced, too.
     *
     * When text is entered into the JEI field, it is synced to the search field. But when it is cleared, that is not
     * synced. This in on purpose, to give the user
     * a way to quickly look up (or cheat in) something without having to disable syncing.
     *
     * When the GUI is closed, text will be remembered if it was entered into the search field. If it was entered into
     * the JEI field, it is not remembered. This
    ...
    public void drawHoveringText(@Nonnull List<String> list, int mouseX, int mouseY, @Nonnull FontRenderer font) {
        if (ghostSlotTooltipStacksize >= 1000) {
            list.add(TextFormatting.WHITE + EnderIO.lang.localize("gui.inventorypanel.tooltip.itemsstored",
    ...
    protected void mouseClicked(int x, int y, int button) throws IOException {
        ...
        if (btnReturnArea.contains(x, y)) {
            ...
            PacketHandler.INSTANCE
                    .sendToServer(new PacketSetExtractionDisabled(getContainer().windowId, !te.isExtractionDisabled()));
    ...
    public class ItemStackButton extends IconButton {
        ...
        public ItemStackButton(@Nonnull IGuiScreen gui, int id, int x, int y, @Nullable IWidgetIcon icon,
                               @Nullable ItemStack itm) {
