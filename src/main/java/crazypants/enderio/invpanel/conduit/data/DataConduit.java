...
public class DataConduit extends AbstractConduit implements IDataConduit {
    ...
    private Map<EnumFacing, InventoryDatabaseSource> sources = new EnumMap<EnumFacing, InventoryDatabaseSource>(
            EnumFacing.class);
    ...
    public boolean hasCapability(@Nonnull Capability<?> capability, @Nullable EnumFacing facing) {
        if (facing != null && capability == CapabilityDatabaseHandler.DATABASE_HANDLER_CAPABILITY &&
                externalConnections.contains(facing) && getConnectionMode(facing) != ConnectionMode.DISABLED) {
            return true;
        }
        return false;
    }
    ...
    public @Nullable IItemHandler getExternalInventory(@Nonnull EnumFacing dir) {
        if (getConnectionMode(dir).isActive()) {
            ...
            // Ghost items bug. Missing if check caused INV panel to scan it's own inventory, so placing items in the
            // return area causes it to show up instantly in
            // the inventory
            if (te != null && te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, dir.getOpposite()) &&
                    !(te instanceof TileInventoryPanel)) {
