...
public class SlotCraftingWrapper extends SlotCrafting {
    ...
    public SlotCraftingWrapper(@Nonnull EntityPlayer player, @Nonnull InventoryCrafting craftingInventory,
                               @Nonnull IInventory inventory, int slotIndex,
            ...
            if (!containeritemstack.isEmpty()) {
                if (this.craftMatrix.getStackInSlot(i).isEmpty()) {
                    ...
                    ItemStack remainder = ItemHandlerHelper.insertItem(
                            inventory.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null),
                            containeritemstack,
                            false);
