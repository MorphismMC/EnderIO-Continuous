...
public class CraftingHelper {
                ...
                if (candidate != null) {
                    ...
                    if (candidate.stack.isStackable() && maxStackSize > 1) {
                        targetAmount = Math.min(targetAmount,
                                current + Math.min(maxStackSize, candidate.getAvailable()));
                    }
                }
            }
            ...
            for (int idx = 0; idx < 9; idx++) {
                ...
                if (candidate != null) {
                    ...
                    for (Slot srcSlot : candidate.sourceSlots) {
                        ...
                        if (container.moveItems(srcSlot.slotNumber, slot.slotNumber, slot.slotNumber + 1,
    ...
    @Nullable
    private Candidate findAllCandidates(@Nonnull NNList<ItemStack> pstack, @Nonnull InventoryPanelContainer container,
                                        @Nonnull InventoryDatabaseClient db, @Nonnull Candidate[] candidates) {
    ...
    @Nonnull
    private Candidate findCandidates(@Nonnull ItemStack stack, @Nonnull InventoryPanelContainer container,
                                     @Nonnull InventoryDatabaseClient db, @Nonnull Candidate[] candidates) {
    ...
    private void findCandidates(@Nonnull Candidate candidates, @Nonnull ItemStack stack,
                                @Nonnull Collection<Slot> slots) {
        for (Slot slot : slots) {
