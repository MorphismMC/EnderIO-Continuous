...
class DyeColorTest {
    ...
    void testItemDye() {
        for (int i = 0; i < ItemDye.DYE_COLORS.length; i++) {
            ...
            System.out
                    .print(String.format("COLOR_%s(%d, %d),",
                            DyeColor.DYE_ORE_UNLOCAL_NAMES[i].replaceFirst("^.*\\.", "").toUpperCase(Locale.ENGLISH),
                            i * 16, 240));
            System.out.println(String.format("// %d: #%x #%x %s", i, col, fg,
                    DyeColor.DYE_ORE_UNLOCAL_NAMES[i].replaceFirst("^.*\\.", "")));
        }
        for (int i = 0; i < ItemDye.DYE_COLORS.length; i++) {
            System.out.print(String.format("EnderWidget.COLOR_%s,",
                    DyeColor.DYE_ORE_UNLOCAL_NAMES[i].replaceFirst("^.*\\.", "").toUpperCase(Locale.ENGLISH)));
        }
    }
}
